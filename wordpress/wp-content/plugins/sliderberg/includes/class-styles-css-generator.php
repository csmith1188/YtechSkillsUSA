<?php
/**
 * Styles CSS Generator Class
 * Handles CSS generation for blocks with support for borders, spacing, shadows, etc.
 *
 * @package Sliderberg
 * @since 1.1.0
 */
...
class Sliderberg_Styles_CSS_Generator {
    /**
     * Check if spacing value is presets or custom
     *
     * @param string $value Spacing value.
     * @return bool
     */
    public static function spacing_preset($value) {
     ...
     * @return string|null
     */
    public static function spacing_preset_css_var($value) {
        if (!$value) {
            return null;
        }
    ...
    public static function get_spacing_css($object) {
        ...
        foreach ($object as $key => $value) {
            if (self::spacing_preset($value)) {
                $css[$key] = self::spacing_preset_css_var($value);
            } else {
                $css[$key] = $value;
            }
        }
    ...
    public static function generate_css_string($styles) {
        ...
        foreach ($styles as $key => $value) {
            if (!self::is_undefined($value) && false !== $value && !empty($value) && 
     ...
     */
    public static function get_box_shadow_css($shadow) {
    ...
    /**
     * Get background color or gradient value
     *
