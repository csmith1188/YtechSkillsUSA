...
class UCRequestsLogView extends WP_List_Table{
	...
	public function get_columns(){
		...
		$columns = array(
			"url" => __("Request URL", "unlimited-elements"),
			"time" => __("Time", "unlimited-elements"),
			"user" => __("User", "unlimited-elements"),
		);
	...
	public function no_items(){
		...
		esc_attr_e("No requests found.", "unlimited-elements");
	}
	...
	private function prepareData(){
		...
		$changelogService = new UELM_UniteCreatorAddonChangelog();
	...
	private function displayHeader(){
		...
		$headerTitle = __("Requests Log", "unlimited-elements");
	...
	private function displayDisabledMessage(){
		...
		<div class="notice notice-warning">
			<p><?php esc_html_e("The changelog feature is disabled in the general settings. Please enable it to view the requests log.", "unlimited-elements"); ?></p>
		</div>
		<?php
	}
	...
	private function displayFooter(){
		...
		$urlChangelog = UELM_HelperUC::getViewUrl(UELM_GlobalsUnlimitedElements::VIEW_CHANGELOG);

		?>
		<div style="margin-top: 20px;">
			<a class="button" href="<?php echo esc_url($urlChangelog); ?>">
				<?php echo esc_html__("Back to Changelog", "unlimited-elements"); ?>
			</a>
		</div>
