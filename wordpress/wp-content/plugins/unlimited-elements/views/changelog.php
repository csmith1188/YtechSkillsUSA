...
class UELM_ChangelogView extends WP_List_Table{
	...
	public function __construct($args = array()){
		...
		$this->service = new UELM_UniteCreatorAddonChangelog();
	...
	public function get_columns(){
		...
		$columns = array(
			...
			"addon" => __("Widget", "unlimited-elements"),
			"type" => __("Type", "unlimited-elements"),
			"text" => __("Text", "unlimited-elements"),
			"version" => __("Version", "unlimited-elements"),
			"user" => __("Author", "unlimited-elements"),
	...
	public function no_items(){
		...
		esc_attr_e("No changelogs found.", "unlimited-elements");
	}
	...
	protected function get_bulk_actions(){
		...
		$actions = array(
			self::ACTION_DELETE => __("Delete Permanently", "unlimited-elements"),
		);
	...
	protected function column_addon($item){
		...
		$actions = array(
			self::ACTION_EDIT => $this->getActionLink(self::ACTION_EDIT, $id, __("Edit", "unlimited-elements")),
	...
	protected function extra_tablenav($which){
		...
		$urlViewImport = UELM_HelperUC::getViewUrl(UELM_GlobalsUnlimitedElements::VIEW_CHANGELOG_IMPORT);
		$isChangelogImportDisabled = UELM_HelperProviderUC::isAddonChangelogImportDisabled();
		
		$urlRequestsLog = UELM_HelperUC::getViewUrl("requests-log");
		...
		<div class="alignleft actions">
			<?php $this->displayFilterSelect(self::FILTER_ADDON, __("Filter by Widget", "unlimited-elements"), __("All Widgets", "unlimited-elements"), $addons, $selectedAddon); ?>
			<?php $this->displayFilterSelect(self::FILTER_VERSION, __("Filter by Version", "unlimited-elements"), __("All Versions", "unlimited-elements"), $versions, $selectedVersion); ?>
        ...
        <div class="alignright import-button" style="margin-left: 8px; display: none" >
            <a class="button button-primary" href="<?php echo esc_url($urlViewImport); ?>"><?php esc_attr_e("Import Changelog", "unlimited-elements"); ?></a>
        </div>
        <?php endif; ?>

        <div class="alignright export-button" style="margin-left: 8px; display: none" >
			<?php submit_button(__("Export Changelog", "unlimited-elements"), "primary", self::ACTION_EXPORT_JSON, false, array("id" => "export-json-submit")); ?>
        </div>

		<div class="alignright" style="margin-left: 8px;" >
			<a class="button" href="<?php echo esc_url($urlRequestsLog); ?>"><?php esc_attr_e("View Requests Log", "unlimited-elements"); ?></a>
		</div>

		<div class="alignright" style="margin-left: 8px;" >
			<?php submit_button(__("Export", "unlimited-elements"), "primary", self::ACTION_EXPORT, false, array("id" => "export-submit")); ?>
		</div>
	...
	private function processEditAction($ids){
		...
		$type = UELM_UniteFunctionsUC::getGetVar(self::EDIT_FIELD_TYPE, null, UELM_UniteFunctionsUC::SANITIZE_TEXT_FIELD);
		$text = UELM_UniteFunctionsUC::getGetVar(self::EDIT_FIELD_TEXT, null, UELM_UniteFunctionsUC::SANITIZE_TEXT_FIELD);
	...
	public function sortExportItems($item1, $item2){
		...
		$type1 = UELM_UniteFunctionsUC::getVal($item1, "type");
		$type2 = UELM_UniteFunctionsUC::getVal($item2, "type");

		$num1 = UELM_UniteFunctionsUC::getVal($arrNumbers, $type1, 4);
		$num2 = UELM_UniteFunctionsUC::getVal($arrNumbers, $type2, 4);
	...
	private function processExportAction(){
		...
		UELM_UniteFunctionsUC::downloadTxt($filename, $content);
	}
	...
	public function processExportJsonAction(){
        $export = new UELM_UniteCreatorImportExportChangelog();
	...
	private function processImportJsonAction(){
		$import = new UELM_UniteCreatorImportExportChangelog();
	...
	private function getFilter($key){
		...
		switch($key){
			...
			case self::FILTER_ADDON:
				$value = UELM_UniteFunctionsUC::getGetVar(self::FILTER_ADDON, null, UELM_UniteFunctionsUC::SANITIZE_ID);
			break;
			case self::FILTER_VERSION:
				$value = UELM_UniteFunctionsUC::getGetVar(self::FILTER_VERSION, null, UELM_UniteFunctionsUC::SANITIZE_NOTHING);
			break;
		}
	...
	private function getWhere(array $filters){
		...
		$where .= $wpdb->prepare(" AND type != %s", array(UELM_UniteCreatorAddonChangelog::TYPE_REQUEST));
		...
		$addon = UELM_UniteFunctionsUC::getVal($filters, self::FILTER_ADDON, null);

		if(empty($addon) === false)
			$where .= $wpdb->prepare(" AND addon_id = %d", array($addon));

		$version = UELM_UniteFunctionsUC::getVal($filters, self::FILTER_VERSION, null);
	...
	private function displayHeader(){
		...
		$headerTitle = __("Changelog", "unlimited-elements");
		...
		<template id="uc-inline-edit-template">
			<form class="uc-inline-edit-form" method="get">
				...
				<textarea name="<?php echo esc_attr(self::EDIT_FIELD_TEXT); ?>"
					placeholder="<?php esc_attr_e("Enter text", "unlimited-elements"); ?>"></textarea>
				<div class="uc-inline-edit-form-actions">
					...
					<button class="uc-inline-edit-form-cancel unite-button-secondary" type="button">
						<?php esc_html_e("Cancel", "unlimited-elements"); ?>
					</button>
				</div>
			</form>
		</template>
...
$changelog = new UELM_ChangelogView();
$changelog->display();
