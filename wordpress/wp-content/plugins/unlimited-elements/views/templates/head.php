...
$logoImage = ($isProVersion === true)
	? UELM_GlobalsUC::$urlPluginImages . "logo_unlimited-pro.svg"
	: UELM_GlobalsUC::$urlPluginImages . "logo_unlimited.svg";

$logoUrl = UELM_GlobalsUC::URL_SITE;
$logoTitle = UELM_GlobalsUC::URL_SITE;

$isBFMode = UELM_GlobalsUnlimitedElements::$blackFridayMode;
...
$buyButtonText = __("Go Pro", "unlimited-elements");


// determine versions of plugin activated
$verFlags = UELM_HelperUC::getActivePluginVersions();
if($verFlags[UELM_GlobalsUC::VERSION_ELEMENTOR] && $verFlags[UELM_GlobalsUC::VERSION_GUTENBERG]) {
	$plugin_ver_name = 'for Elementor and Gutenberg';
	$logoImage = UELM_GlobalsUC::$urlPluginImages . 'logo_unlimited' . ($isProVersion ? '-pro' : '') . '-elementor-gutenberg-new.svg';
} elseif($verFlags[UELM_GlobalsUC::VERSION_ELEMENTOR]) {
	$plugin_ver_name = 'for Elementor';
	$logoImage = UELM_GlobalsUC::$urlPluginImages . 'logo_unlimited' . ($isProVersion ? '-pro' : '') . '-elementor-new.svg';
} else {
	$plugin_ver_name = 'for Gutenberg'; 
	$logoImage = UELM_GlobalsUC::$urlPluginImages . 'logo_unlimited' . ($isProVersion ? '-pro' : '') . '-gutenberg-new.svg';
}

if($isBFMode == true){
	
	$headAddClass = "ue-header__bf";
	$logoImage = UELM_GlobalsUC::$urlPluginImages."logo_unlimited-white.svg";
	
	$buyButtonText = __("Get Deal Now!", "unlimited-elements");
	
	$urlHeaderImage = UELM_GlobalsUC::$urlPluginImages."banners/bf-banner-header.png";
}
...
<div class="ue-root ue-header <?php echo esc_attr($headAddClass) ?>">
	...
	<div class="ue-header-buttons">
		<a class="ue-btn ue-flex-center ue-view-demo-btn"
			href="<?php echo esc_url(UELM_GlobalsUC::URL_WIDGETS); ?>"
			target="_blank">
			<?php echo esc_html__("View Demos", "unlimited-elements"); ?>
		...
		<?php if($isProVersion === false): ?>
			<a class="ue-btn ue-flex-center ue-go-pro-btn" href="<?php echo esc_url(UELM_GlobalsUC::$url_buy_platform); ?>" target="_blank">
				<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20">
