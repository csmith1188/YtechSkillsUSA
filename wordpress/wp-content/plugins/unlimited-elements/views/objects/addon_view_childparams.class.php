...
class UELM_UniteCreatorAddonViewChildParams{
...
var json = {{ucfunc(\"csv_to_json\",yourattribute)}};
	...
	private function getStrCodeItemFlexibleContent($arrValues, $childKey){
		...
		foreach($arrValues as $key => $item){
			...
			$layoutType = UELM_UniteFunctionsUC::getVal($item, "acf_fc_layout");
			...
			$exampleField = UELM_UniteFunctionsUC::getFirstNotEmptyKey($item);
	...
	protected function addCustomFieldsParams($arrParams, $postID){
		...
		if($isPodsExists == true){
			$arrMetaKeys = UELM_UniteFunctionsWPUC::getPostMetaKeys_PODS($postID);
		...
		if($isAcfExists == true && $takeFromACF == true){
			...
			$arrMetaKeys = UELM_UniteFunctionsWPUC::getAcfFieldsKeys($postID);
			$title = "acf field";
			
			if(empty($arrMetaKeys))
				return($arrParams);
			
			$firstKey = UELM_UniteFunctionsUC::getFirstNotEmptyKey($arrMetaKeys);
			...
			if(empty($arrMetaKeys))
				$arrMetaKeys = UELM_UniteFunctionsWPUC::getPostMetaKeys($postID, "cf_");
	...
	private function checkAddWooPostParams($postID, $arrParams, $isForce = false){
		...
		if(empty($postID) && $isForce == true)
			$arrKeys = UELM_UniteCreatorWooIntegrate::getWooKeysNoPost();
		else
			$arrKeys = UELM_UniteCreatorWooIntegrate::getWooKeysByPostID($postID);
	...
	private function getArrImageThumbSizes(){
		...
		$arrSizesOutput["thumb"] = __("Thumb (max width 300)", "unlimited-elements");
		
		$arrSizes = UELM_UniteFunctionsWPUC::getArrThumbSizes();
		
		//add large
		$sizeLargeDesc = UELM_UniteFunctionsUC::getVal($arrSizes, "large");
		if(empty($sizeLargeDesc))
			$sizeLargeDesc = __("Large (max width 780)", "unlimited-elements");
	...
	public function getChildParams_post($postID = null, $arrAdditions = array()){
		...
		$arrParams[] = $this->createChildParam("title",UELM_UniteCreatorDialogParam::PARAM_EDITOR);
		$arrParams[] = $this->createChildParam("alias");
		$arrParams[] = $this->createChildParam("content", UELM_UniteCreatorDialogParam::PARAM_EDITOR);
		$arrParams[] = $this->createChildParam("content|wpautop", UELM_UniteCreatorDialogParam::PARAM_EDITOR);
		...
		foreach($arrAdditions as $addition){
			...
			switch($addition){
				...
				break;
				case UELM_GlobalsProviderUC::POST_ADDITION_WOO:
	...
	private function getUsersCode($itemName, $parentName){
		...
		$arrAvatarKeys = UELM_UniteFunctionsWPUC::getUserAvatarKeys();
		
		foreach($arrAvatarKeys as $key){
			$title = UELM_UniteFunctionsUC::convertHandleToTitle($key);
		...
		$arrMetaKeys = UELM_UniteFunctionsWPUC::getUserMetaKeys();
