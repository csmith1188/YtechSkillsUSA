...
class UELM_UniteCreatorObjectsListView extends UELM_UniteElementsBaseUC{
	...
	private function validateInited(){
		...
		if(empty($this->objectClass))
			UELM_UniteFunctionsUC::throwError("The object class should be inited");
		
		UELM_UniteFunctionsUC::validateNotEmpty($this->dbTable, "DB Table");
		UELM_UniteFunctionsUC::validateNotEmpty($this->urlListBase, "Url List Base");
		UELM_UniteFunctionsUC::validateNotEmpty($this->viewObject, "Single object view");
		UELM_UniteFunctionsUC::validateNotEmpty($this->viewObjectPreview, "Single Object Preview View");
		UELM_UniteFunctionsUC::validateNotEmpty($this->actionDelete, "Action Delete");
	...
	private function validateObjectMethods($object){
		...
		foreach($arrMethods as $method)
			UELM_UniteFunctionsUC::validateObjectMethod($object, $method, $this->objectClass);
	...
	protected function putHtml_beforeTable(){
		...
		$this->objTable->putSearchForm($textSearch, esc_html__("Clear","unlimited-elements"), $isPutSearchForm);
	...
	protected function putHtml_table(){
		...
		$txtDelete = esc_html__("Are you sure you want to delete this", "unlimited-elements")." ".$this->txtSingleName;
		
		$options = $this->getClientSideOptions();
		$strOptions = UELM_UniteFunctionsUC::jsonEncodeForHtmlData($options);
		...
		?>
			<div id="uc_table_objects_wrapper" data-options="<?php 
				...
				<div>
					<?php 
						$txtNotFound = esc_html__("No","unlimited-elements")." ".$this->txtMultipleName." ".esc_html__("Found","unlimited-elements");
				...
				<table id="uc_table_objects" class='unite_table_items' data-text-confirm-delete="<?php echo esc_attr($txtDelete)?>">
					<thead>
						<tr>
							<th width=''>
								<?php $this->objTable->putTableOrderHeader("title", $this->txtSingleName." ".esc_html__("Title", "unlimited-elements")) ?>
							</th>
							
							<?php if($this->enableCategories == true):?>
							<th width='200'><?php $objTable->putTableOrderHeader("catid", esc_html__("Category","unlimited-elements")) ?>
							<?php endif?>
						...
						<?php foreach($this->arrObjects as $key=>$object):
								...
								$editLink = UELM_HelperUC::getViewUrl($this->viewObject,"id=".$id);
								$previewLink = UELM_HelperUC::getViewUrl($this->viewObjectPreview,"id=".$id, true);
								
								$showTitle = UELM_HelperHtmlUC::getHtmlLink($editLink, $title);
								$rowClass = ($key%2==0)?"unite-row1":"unite-row2";
								
								if($this->enableCategories){
									uelm_dmp("check the getCategoryInfo function");
									$arrCategory = $object->getCategoryInfo();
									$catID = UELM_UniteFunctionsUC::getVal($arrCategory, "id");
									$catTitle = UELM_UniteFunctionsUC::getVal($arrCategory, "title");
						...
						<tr class="<?php echo esc_attr($rowClass)?>">
							...
							<td>
								<a href='<?php echo esc_attr($editLink)?>' class="unite-button-primary float_left mleft_15"><?php echo esc_html__("Edit", "unlimited-elements")." ".esc_attr($this->txtSingleName); ?></a>
								
								<a href='javascript:void(0)' data-action="delete" data-id="<?php echo esc_attr($id)?>" class="uc-button-action unite-button-secondary float_left mleft_15"><?php esc_html_e("Delete","unlimited-elements"); ?></a>
	...
	protected function putHtml_scripts(){
		...
		$script = 'jQuery(document).ready(function(){
				var objAdmin = new UELM_UniteCreatorAdmin_ObjectsList();
	...
	private function recordsToObjects($records){
		...
		foreach($records as $index=>$record){
			$objectID = UELM_UniteFunctionsUC::getVal($record, "id");
	...
	protected function putScriptIncludes(){
		...
		UELM_HelperUC::addScript("unitecreator_admin_objectslist", "unitecreator_admin_objectslist.js");
	}
