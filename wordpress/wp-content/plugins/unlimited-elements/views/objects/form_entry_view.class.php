...
class UELM_UCFormEntryView{
	...
	private function getEntry($id){
		...
		if(empty($entry) === true)
			UELM_UniteFunctionsUC::throwError("Entry with ID {$id} not found.");
	...
	private function displayHeader(){
		...
		$headerTitle = sprintf(__("Form Entry (ID %d)", "unlimited-elements"), $this->entry["id"]);
	...
	private function displayContent(){
		...
		$asides = array(
			__("Entry Information", "unlimited-elements") => array(
				__("Entry ID", "unlimited-elements") => $this->entry["id"],
				__("Form", "unlimited-elements") => $this->entry["form_name"],
				__("Page", "unlimited-elements") => $this->entry["post_title"],
				__("Date", "unlimited-elements") => $this->service->formatEntryDate($this->entry["created_at"]),
			),
			__("User Information", "unlimited-elements") => array(
				__("User ID", "unlimited-elements") => $this->entry["user_id"],
				__("User IP", "unlimited-elements") => $this->entry["user_ip"],
				__("User Agent", "unlimited-elements") => $this->entry["user_agent"],
			),
		);
				...
				<div id="post-body-content">
					<div class="postbox">
						<div class="postbox-header">
							<h2><?php echo esc_html__("Entry Fields", "unlimited-elements"); ?></h2>
						</div>
						<div class="inside">
									...
									<?php foreach($fields as $field): ?>
										<tr>
											...
											<td>
												...
												switch($field["type"]){
													case UELM_UniteCreatorForm::TYPE_FILES:
	...
	private function displayFooter(){
		...
		<div>
			<a class="button" href="<?php echo esc_url($url); ?>">
				<?php echo esc_html__("Back to Form Entries", "unlimited-elements"); ?>
			</a>
		</div>
		<?php
	}
