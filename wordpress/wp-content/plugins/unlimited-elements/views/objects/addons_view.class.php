...
class UELM_UniteCreatorAddonsView{
	...
	protected function initAddonType(){
		...
		if(empty($this->addonType)){
			$this->addonType = UELM_UniteFunctionsUC::getGetVar("addontype", null, UELM_UniteFunctionsUC::SANITIZE_KEY);
		}

		$this->objAddonType = UELM_UniteCreatorAddonType::getAddonTypeObject($this->addonType);
		
		UELM_UniteCreatorAdmin::setAdminGlobalsByAddonType($this->objAddonType);
	...
	protected function getHeaderText(){
		...
		if(!empty($this->objAddonType->managerHeaderPrefix))
			UELM_GlobalsUC::$alterViewHeaderPrefix = $this->objAddonType->managerHeaderPrefix;
		
		$headerTitle = esc_html__("Manage", "unlimited-elements")." ".$this->objAddonType->textPlural;
	...
	private function validateAddonType(){
		...
		if(empty($this->objAddonType))
			UELM_UniteFunctionsUC::throwError("This view should have an addon type");
	...
	protected function init(){
		...
		$this->objManager = new UELM_UniteCreatorManagerAddons();
	...
	protected function putHtml(){
		...
		$pluginName = UELM_GlobalsUC::PLUGIN_NAME;

		
		$replacingMessage = null;
		
		if($this->addonType == UELM_GlobalsUnlimitedElements::ADDONSTYPE_ELEMENTOR_TEMPLATE)
	...
	?>
		...
		<?php 
			if($this->showButtons == true)
				UELM_UniteProviderFunctionsUC::putAddonViewAddHtml()
		?>
			...
			<?php 
				...
				if(!empty($replacingMessage)){
					...
					<div class="unite-replacing-message">
						<?php echo wp_kses($replacingMessage, UELM_HelperUC::getKsesAllowedHTML()); ?>
					</div>
					<?php 
				}
				else
