...
class UELM_UniteCreatorActivationView extends UELM_UniteElementsBaseUC{
	...
	public function __construct(){
		...
		$this->urlPricing = UELM_GlobalsUC::$url_buy_platform;
		$this->urlSupport = UELM_GlobalsUC::URL_SUPPORT;

		$this->textActivate = esc_html__("Activate Blox Pro", "unlimited-elements");

		$this->textGoPro = esc_html__("GO PRO", "unlimited-elements");
		$this->textUnleash = esc_html__("Unleash access to +700 addons,", "unlimited-elements");

		$this->textAndTemplates = esc_html__("+100 page templates and +50 section designs", "unlimited-elements");

		$this->textPasteActivationKey = esc_html__("Paste your activation key here", "unlimited-elements");

		$this->textPlaceholder = "xxxx-xxxx-xxxx-xxxx";
		$this->textLinkToBuy = esc_html__("View our pricing plans", "unlimited-elements");

		$this->textDontHave = esc_html__("Don't have a pro activation key?", "unlimited-elements");

		$this->textDontHaveLogin = esc_html__("If you already purchased, get the key from my account?", "unlimited-elements");
		$this->textLinkToLogin = esc_html__("Go to My Account", "unlimited-elements");
		$this->urlLogin = "http://my.unitecms.net";

		$this->textActivationFailed = esc_html__("You probably got your activation code wrong", "unlimited-elements");

		$this->textYourProAccountLifetime = esc_html__("Your pro account is activated lifetime for this site", "unlimited-elements");
	              ...
	              <input id="uc_button_activate_pro" type="button" class='uc-button-activate' data-codetype="<?php echo esc_attr($this->codeType)?>" data-product="<?php echo esc_attr($this->product)?>" value="<?php echo esc_attr($this->textActivate)?>">
                   ...
                   <div id="uc_loader_activate_pro" class="uc-loader-activation" style='display:none'>
						...
						<span class='loader_text'>
	                                		<?php esc_html_e("Activating", "unlimited-elements")?>...
	                    </span>
		   ...
		   <div class="uc-activation-view">
	           ...
	           <div class="uc-popup-container uc-start">
	            ...
	            <div class="uc-popup-container uc-fail hidden">
	                <div class="uc-popup-content">
	                    <div class="uc-popup-holder">
	                        <div class="large-title"><?php esc_html_e("Ooops", "unlimited-elements")?>.... <br><?php esc_html_e("Activation Failed", "unlimited-elements")?> :(</div>
	                        ...
	                        <div class="bottom-text"><?php esc_html_e("or contact our","unlimited-elements")?> <a href="<?php echo esc_url($this->urlSupport)?>" target="_blank"><?php esc_html_e("support center", "unlimited-elements")?></a></div>
	                    </div>
	                </div>
	            </div>
	            ...
	            <div class="uc-popup-container uc-activated hidden">
	                <div class="uc-popup-content">
	                    <div class="uc-popup-holder">
	                        <div class="xlarge-title"><?php esc_html_e("Hi Five", "unlimited-elements")?>!</div>

	                        <?php if($this->isExpireEnabled == true):?>
	                        	<div class="popup-text small-padding"><?php echo esc_html($this->textYourProAccountLifetime)?></div>
		                        <div class="days"></div>
		                        <span><?php esc_html_e("DAYS", "unlimited-elements")?></span>
		                        <br><br>

		                        <?php if($this->writeRefreshPageMessage == true):?>
		                        <a href="javascript:location.reload()" class="btn"><?php esc_html_e("Refresh page to View Your Pro Catalog", "unlimited-elements")?></a>
		                        <?php endif?>

	                        <?php else:?>

	                        	<div class="popup-text small-padding"><?php esc_html_e("Your pro account is activated lifetime for this site","unlimited-elements")?>!</div>

	                        	<div class="popup-text small-padding"><?php esc_html_e("Thank you for purchasing from us and good luck", "unlimited-elements")?>!</div>
	...
	public function putHtmlDeactivate(){
		...
		?>
		<h2><?php esc_html_e("This pro version is active!", "unlimited-elements")?></h2>

		<a href="javascript:void(0)" class="uc-link-deactivate unite-button-primary" data-product="<?php echo esc_attr($this->product)?>"><?php esc_html_e("Deactivate Pro Version", "unlimited-elements")?></a>
	...
	public function putHtmlPopup(){
		...
		$title = esc_html__("Activate Your Pro Account", "unlimited-elements");
