...
if(UELM_GlobalsUC::$isProVersion){
	...
	require_once UELM_GlobalsUC::$pathPro."childparams_pro.class.php";
}

class UELM_UniteCreatorAddonView{
	...
	public function __construct(){
		...
		if(UELM_GlobalsUC::$isProVersion)
			$this->objChildParams = new UELM_UniteCreatorChildParamsPro();
		else
			$this->objChildParams = new UELM_UniteCreatorAddonViewChildParams();
	...
	public function initSkipPanelParams(){
		...
		$this->arrSkipPanelParams[UELM_UniteCreatorDialogParam::PARAM_TYPOGRAPHY] = true;
		$this->arrSkipPanelParams[UELM_UniteCreatorDialogParam::PARAM_MARGINS] = true;
		$this->arrSkipPanelParams[UELM_UniteCreatorDialogParam::PARAM_PADDING] = true;
		$this->arrSkipPanelParams[UELM_UniteCreatorDialogParam::PARAM_BACKGROUND] = true;
		$this->arrSkipPanelParams[UELM_UniteCreatorDialogParam::PARAM_BORDER] = true;
	...
	private function validateInitSettings(){
		...
		if($this->putAllTabs == false && empty($this->arrTabsToPut))
			UELM_UniteFunctionsUC::throwError("if all tabs setting turned off should be some tabs in arrTabsToPut array");
	...
	private function initSettingsItem(){
		...
		$settingsItem = new UELM_UniteCreatorSettings();
		$settingsItem->addRadioBoolean("enable_items", esc_html__("Enable Items", "unlimited-elements"), false);


		$params = array();
		$params["placeholder"] = __("Default is 'Items'","unlimited-elements");
		$params["class"] = "alias";

		$settingsItem->addTextBox("items_section_title", "", "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".esc_html__("Section Label", "unlimited-elements"), $params);
		...
		$settingsItem->addTextBox("items_section_heading", "", "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".esc_html__("Section Heading", "unlimited-elements"), $params);
		$settingsItem->addControl("enable_items", "items_section_heading", "show", "true");

		//title field
		$params["placeholder"] = "Default is: {{{title}}}";

		$settingsItem->addTextBox("items_title_field", "", "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".esc_html__("Title Field", "unlimited-elements"), $params);
		$settingsItem->addControl("enable_items", "items_title_field", "show", "true");

		//allow empty items
		$settingsItem->addRadioBoolean("items_allow_empty", "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".esc_html__("Allow Empty", "unlimited-elements"), false);
		$settingsItem->addControl("enable_items", "items_allow_empty", "show", "true");
		

		$settingsItem->setStoredValues($options);
		
		$this->settingsItemOutput = new UELM_UniteSettingsOutputInlineUC();
	...
	private function initSettingsJS(){
		...
		//items editor - settings
		$settingsJS = new UELM_UniteCreatorSettings();
		$settingsJS->addCheckbox("js_as_module", false, UELM_UniteSettingsUC::PARAM_NOTEXT, __("load as module", "unlimited-elements"));

		$settingsJS->setStoredValues($options);

		$this->settingsJSOutput = new UELM_UniteSettingsOutputInlineUC();
	...
	private function initByAddonType_general(){
		...
		if($this->objAddonType->typeName == UELM_GlobalsUC::ADDON_TYPE_BGADDON)
	...
	private function init(){
		...
		$this->objAddon->setOperationType(UELM_UniteCreatorAddon::OPERATION_EDIT);
		...
		UELM_UniteCreatorAdmin::setAdminGlobalsByAddonType($this->objAddonType, $this->objAddon);
	...
	protected function getHeaderTitle(){
		...
		$headerTitle = esc_html__("Edit ","unlimited-elements").$this->textSingle;
		$headerTitle .= " - " . $title;
	...
	private function initGeneralSettings(){
		...
		$filepathAddonSettings = UELM_GlobalsUC::$pathSettings."addon_fields.php";
	...
	private function putHtml_generalSettings(){
		...
		$settingsOutput = new UELM_UniteSettingsOutputWideUC();
		...
		<div class="uc-edit-addon-col uc-col-first">
			...
			<?php 
			uelm_echo( $this->textSingle.esc_html__(" Title", "unlimited-elements") ); 
			?>:
			...
			<?php 
			uelm_echo( $this->textSingle.esc_html__(" Name", "unlimited-elements") ); 
			?>:
			...
			<?php esc_html_e("Widget Type", "unlimited-elements");?>: <b> <?php echo esc_html($addonTypeTitle)?> </b>

			<?php if(!empty($catTitle)):?>
			<div class="vert_sap10"></div>

			<?php esc_html_e("Category", "unlimited-elements");?>:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<b> <?php 
			uelm_echo( $catTitle );
			?> </b>
			...
			<?php UELM_UniteProviderFunctionsUC::doAction(UELM_UniteCreatorFilters::ACTION_EDIT_ADDON_ADDSETTINGS, $arrOptions)?>
	...
	private function isPutRevisionsTab(){
		...
		$isRevisionsEnabled = UELM_HelperProviderUC::isAddonRevisionsEnabled();
	...
	private function putHtml_tabs(){
		...
		$htmlTabTitle = esc_html__("HTML", "unlimited-elements");
		if($this->isSVG == true)
			$htmlTabTitle = esc_html__("SVG Content","unlimited-elements");
		else{
	...
	private function putHtml_LibraryIncludes($arrJsLibIncludes){
		...
		$objLibrary = new UELM_UniteCreatorLibrary();
	...
	private function putHtml_Includes(){
		...
		$dataJs = UELM_UniteFunctionsUC::jsonEncodeForHtmlData($arrJsIncludes, "init");
		$dataCss = UELM_UniteFunctionsUC::jsonEncodeForHtmlData($arrCssIncludes, "init");
		...
		?>
				...
				<thead>
					<tr>
						<th class="uc-table-includes-left">
							<b>
							<?php esc_html_e("Choose From Browser", "unlimited-elements")?>
							</b>
						</th>
					...
					<tr>
						...
						<td valign="top">
							...
							<ul id="uc-js-libraries" class="unite-list-hor">
								<li class="pright_10">
									<span class="unite-title2"><?php esc_html_e("Libraries", "unlimited-elements")?>:</span> </b>
								</li>
			...
			<div id="uc_dialog_unclude_settings" title="<?php esc_html_e("Include Settings", "unlimited-elements")?>" class="unite-inputs" style="display:none">
				<div class="unite-dialog-inside">

					<?php esc_html_e("Include When:", "unlimited-elements")?>
					...
					<span id="uc_dialog_include_value_container" style="display:none">
						...
						<?php esc_html_e("equals", "unlimited-elements")?>
					...
					<label class="unite-inputs-label">
						<?php esc_attr_e("Handle","unlimited-elements")?>
					</label>

					<input type="text" name="include_handle" class="unite-input-alias">

					<i>* <?php esc_attr_e("leave empty for auto", "unlimited-elements")?></i>
					...
					<div class="uc-dialog-includes-js">
						...
						<label>
							...
							&nbsp;
							<?php esc_attr_e("Include after elementor-frontend.js", "unlimited-elements")?>
						...
						<label>
							...
							&nbsp;
							<?php esc_attr_e("Include as ES6 module", "unlimited-elements")?>
	...
	private function putHtml_assetsTab(){
		...
		$textNotSet = esc_html__("[not set]", "unlimited-elements");
		...
		?>
			<div class="uc-assets-folder-wrapper">
				<span class="uc-assets-folder-label"><?php 
					uelm_echo( $this->textSingle.esc_html__(" Assets Path: ", "unlimited-elements") );
				?></span>
				...
				<a id="uc_button_set_assets_folder" href="javascript:void(0)" class="unite-button-secondary"><?php esc_html_e("Set", "unlimited-elements")?></a>
				<a id="uc_button_set_assets_unset" href="javascript:void(0)" class="unite-button-secondary" <?php 
				uelm_echo( $unsetAddHtml );?>><?php esc_html_e("Unset", "unlimited-elements")?></a>
			</div>
		<?php
	...
	private function putHtml_changelogTab(){
		...
		<div class="uc-changelogs-wrapper">
			<div class="uc-changelogs-form">
				...
				<div class="uc-changelogs-form-editing">
					<div class="uc-changelogs-form-editing-title">
						<?php esc_html_e("Editing:", "unlimited-elements"); ?>
					</div>
				...
				<select name="type">
					<option value="" selected disabled>
						<?php esc_html_e("Select type", "unlimited-elements"); ?>
					</option>
					<?php foreach($types as $value => $label): ?>
						<option value="<?php echo esc_attr($value); ?>"><?php echo esc_html($label); ?></option>
					<?php endforeach; ?>
				</select>
				<textarea name="text" placeholder="<?php esc_attr_e("Enter text", "unlimited-elements"); ?>"></textarea>
			...
			<div class="uc-changelogs-loader">
				<?php esc_html_e("Loading...", "unlimited-elements"); ?>
			</div>
	...
	private function putLinkExpand($textareaID = ""){
		...
		?>
			<a class="uc-tabcontent-link-expand" href="javascript:void(0)" data-forid="<?php echo esc_attr($textareaID)?>"><?php esc_html_e("expand", "unlimited-elements");?></a>
		<?php
	}
	...
	private function putHtml_tabTableRow($textareaID, $title, $areaHtml, $paramsPanelID, $addVariableID = null, $isItemsRelated = false, $params = array()){
		...
		$isExpanded = UELM_UniteFunctionsUC::getVal($params, "expanded");
		$isExpanded = UELM_UniteFunctionsUC::strToBool($isExpanded);

		$mode = UELM_UniteFunctionsUC::getVal($params, "mode");
		...
		?>
					<tr <?php 
				uelm_echo( $rowClass );?> <?php 
				uelm_echo( $rowAddHtml );?>>
				...
				uelm_echo( $areaAddParams );?>><?php echo esc_html($areaHtml)?></textarea>
				...
				uelm_echo( $styleRight );?>>

							<?php if($isItemsRelated == true):?>
								<div class="uc-params-panel-filters">
									<a href="javascript:void(0)" class="uc-filter-active" data-filter="item" onfocus="this.blur()"><?php esc_html_e("Item", "unlimited-elements")?></a>
									<a href="javascript:void(0)" data-filter="main" onfocus="this.blur()"><?php esc_html_e("Main", "unlimited-elements")?></a>
								</div>
							<?php endif?>
						    ...
						    <a id="<?php echo esc_attr($addVariableID)?>" type="button" href="javascript:void(0)" class="unite-button-secondary mleft_20"><?php esc_html_e("Add Variable", "unlimited-elements")?></a>
							<?php endif?>
	...
	private function putHtml_overviewTab(){
		...
		if(!empty($link))
			$link = UELM_HelperHtmlUC::getHtmlLink($link, $link, "uc_overview_link","",true);
		...
		<div class="uc-tab-overview">
			<div class="uc-section-inline"><?php esc_html_e("Widget Title", "unlimited-elements")?>: <span id="uc_overview_title" class="unite-bold"><?php echo esc_html($title)?></span></div>
			<div class="uc-section-inline"><?php esc_html_e("Widget Name", "unlimited-elements")?>: <span id="uc_overview_name" class="unite-bold"><?php echo esc_html($name)?></span></div>
			<div class="uc-section">
				<div class="uc-section-title"><?php esc_html_e("Widget Description", "unlimited-elements")?>:</div>
				<div id="uc_overview_description" class="uc-section-content uc-desc-wrapper">
					<?php echo esc_html($description)?>
				</div>
				<div class="unite-clear"></div>
			</div>
			<div class="uc-section-inline"><?php esc_html_e("Link to resource", "unlimited-elements")?>: <?php echo esc_html($link)?></div>
			<div class="uc-section">
				<div class="uc-section-title uc-title-icon"><?php esc_html_e("Widget Icon", "unlimited-elements")?>:</div>
				<div id="uc_overview_icon" class="uc-section-content uc-addon-icon-small" style="background-image:url('<?php 
				uelm_echo( $addonIcon );?>')"></div>
			</div>
	...
	private function putHTMLPoweredByTwig(){
		?>
		 <div class="uc-edit-addon-poweredby">
		 	...
		 	<?php esc_attr_e("Powered by Twig Template Engine. ", "unlimited-elements")?>
		 		<a href="<?php echo esc_url(UELM_GlobalsUC::LINK_TWIG)?>" target="_blank"><?php esc_attr_e("show documentation", "unlimited-elements")?></a>.
		 		<?php esc_attr_e("To show code examples in right panel ","unlimited-elements")?>

		 		<a  href="javascript:void(0)" class="uc-link-code-examples" ><?php esc_attr_e("click here", "unlimited-elements")?></a>.
	...
	private function putHtml_content(){
		...
		$paramsEditorItems = new UELM_UniteCreatorParamsEditor();
		...
		<div id="uc_tab_contents" class="uc-tabs-content-wrapper uc-addon-props">
			...
			<div id="uc_tab_attr" class="uc-tab-content" style="display:none">
				...
				<?php
					$paramsEditorMain = new UELM_UniteCreatorParamsEditor();
			...
			<div id="uc_tab_html" class="uc-tab-content" style="display:none">
				...
				<table class="uc-tabcontent-table">
					...
					<?php
						...
						$rowTitle = $this->textSingle.esc_html__(" HTML","unlimited-elements");

						if($this->isSVG == true)
							$rowTitle = esc_html__("SVG Content","unlimited-elements");

						if(!empty($this->tabHtmlTitle))
							$rowTitle = $this->tabHtmlTitle.esc_html__(" Content","unlimited-elements");
						...
						$rowTitle = esc_html__("Item HTML","unlimited-elements");
						$areaHtml = $htmlItem;
						...
						$rowTitle = esc_html__("Item HTML 2","unlimited-elements");
						$areaHtml = $htmlItem2;
			...
			<div id="uc_tab_css" class="uc-tab-content" style="display:none">
				...
				<table class="uc-tabcontent-table">
					...
					<?php
						...
						$rowTitle = $this->textSingle.esc_html__(" CSS","unlimited-elements");
						$areaHtml = $css;
						...
						$rowTitle = esc_html__("Item CSS","unlimited-elements");
						$areaHtml = $cssItem;
			...
			<div id="uc_tab_js" class="uc-tab-content" style="display:none">
				...
				<table class="uc-tabcontent-table">
					...
					$rowTitle = $this->textSingle.esc_html__(" Javascript","unlimited-elements");
					$areaHtml = $js;
	...
	private function putHtml_actionButtons(){
		...
		$urlTestAddon = UELM_HelperUC::getViewUrl_TestAddon($addonID);

		$urlTestAddonNew = UELM_HelperUC::getViewUrl("testaddonnew", "id=$addonID");

		$urlPreviewAddon = UELM_HelperUC::getViewUrl_TestAddon($addonID,"preview=1");

		$urlAddonDefaults = UELM_HelperUC::getViewUrl_AddonDefaults($addonID);

		$textPreviewAddon = esc_html__("Preview ","unlimited-elements").$this->textSingle;
		$textTestAddon = esc_html__("Test ","unlimited-elements").$this->textSingle;
		$textTestAddonNew = esc_html__("Test ","unlimited-elements").$this->textSingle . " - New";
		$textBack = esc_html__("Back To ","unlimited-elements").$this->textPlural.esc_html__(" List","unlimited-elements");
		$textDefaults = $this->textSingle.esc_html__(" Defaults","unlimited-elements");

		$textExport = esc_html__("Export ", "unlimited-elements").$this->textSingle;

		$isExistsInCatalog = $this->objAddon->isExistsInCatalog();

		$urlBack = UELM_HelperUC::getViewUrl_Addons($addonType);
			...
			<div id="uc_buttons_panel" class="uc-edit-addon-buttons-panel">
				...
				<div class="unite-float-left">
					...
					<div class="uc-button-action-wrapper">
						<a id="button_update_addon" class="button_update_addon unite-button-primary" href="javascript:void(0)"><?php esc_html_e("Update", "unlimited-elements");?></a>

						<div style="padding-top:6px;">

							<span id="uc_loader_update" class="loader_text" style="display:none"><?php esc_html_e("Updating...", "unlimited-elements")?></span>
					...
					<?php if($isExistsInCatalog == true): ?>
						...
						<a id="uc_button_update_catalog" class="button_update_addon unite-button-secondary" href="javascript:void(0)"><?php esc_html_e("Update From Catalog", "unlimited-elements");?></a>
						<span id="uc_loader_update_catalog" class="loader_text" style="display:none"><?php esc_html__("Updating...", "unlimited-elements"); ?></span>
	...
	protected function getChildParams_instagramItem(){
		...
		$arrParams = array();
		$arrParams[] = $this->createChildParam("caption",UELM_UniteCreatorDialogParam::PARAM_EDITOR);
	...
	protected function getChildParams_instagramMain(){
		...
		$arrParams = array();
		$arrParams[] = $this->createChildParam("name", UELM_UniteCreatorDialogParam::PARAM_EDITOR);
		$arrParams[] = $this->createChildParam("username");
		$arrParams[] = $this->createChildParam("biography", UELM_UniteCreatorDialogParam::PARAM_EDITOR);
	...
	protected function getAddParams_dataset($paramDataset){
		...
		$arrItemHeaders = UELM_UniteProviderFunctionsUC::applyFilters(UELM_UniteCreatorFilters::FILTER_GET_DATASET_HEADERS, $arrItemHeaders, $datasetType, $datasetQuery);
	...
	private function getChildPostOptions(){
		...
		$paramPostList = $this->objAddon->getParamByType(UELM_UniteCreatorDialogParam::PARAM_POSTS_LIST);
		...
		if(!empty($paramPostList)){
			...
			$useCategory = UELM_UniteFunctionsUC::getVal($paramPostList, "use_category");
			$useCategory = UELM_UniteFunctionsUC::strToBool($useCategory);
			$output["use_category"] = $useCategory;

			$postExample = UELM_UniteFunctionsUC::getVal($paramPostList, "post_example");
			$useCustomFields = UELM_UniteFunctionsUC::getVal($paramPostList, "use_custom_fields");
			...
			$forWooProducts = UELM_UniteFunctionsUC::getVal($paramPostList, "for_woocommerce_products");
			$forWooProducts = UELM_UniteFunctionsUC::strToBool($forWooProducts);
	...
	protected function getParamChildKeys_getPostAdditions($postOptions){
		...
		$isAddWoo = UELM_UniteFunctionsUC::getVal($postOptions, "add_woo");

		$arrAdditions = array();

		if($isAddWoo == true)
			$arrAdditions[UELM_GlobalsProviderUC::POST_ADDITION_WOO] = UELM_GlobalsProviderUC::POST_ADDITION_WOO;

		if($enableCustomFields == true)
			$arrAdditions[UELM_GlobalsProviderUC::POST_ADDITION_CUSTOMFIELDS] = UELM_GlobalsProviderUC::POST_ADDITION_CUSTOMFIELDS;

		$params = $this->objAddon->getParams(UELM_UniteCreatorDialogParam::PARAM_POSTS_LIST);
		...
		$param = $params[0];
		$enableCustomFields = UELM_UniteFunctionsUC::getVal($param, "use_custom_fields");
		$enableCustomFields = UELM_UniteFunctionsUC::strToBool($enableCustomFields);

		$enableCategory = UELM_UniteFunctionsUC::getVal($param, "use_category");
		...
		if($enableCategory == true)
			$arrAdditions[UELM_GlobalsProviderUC::POST_ADDITION_CATEGORY] = UELM_GlobalsProviderUC::POST_ADDITION_CATEGORY;
	...
	protected function getParamChildKeys(){
		...
		$arrChildKeys = array();
		$arrChildKeys[UELM_UniteCreatorDialogParam::PARAM_POST] = $arrPostParams;
		$arrChildKeys["uc_current_post"] = $arrPostParams;

		$arrChildKeys[UELM_UniteCreatorDialogParam::PARAM_INSTAGRAM] = $this->getChildParams_instagramMain();
		...
		$arrChildKeys[UELM_UniteCreatorDialogParam::PARAM_IMAGE] = $this->objChildParams->getChildParams_image();


		//add dataset params
		$paramDataset = $this->objAddon->getParamByType(UELM_UniteCreatorDialogParam::PARAM_DATASET);
		if(!empty($paramDataset))
			$arrChildKeys[UELM_UniteCreatorDialogParam::PARAM_DATASET] = $this->getAddParams_dataset($paramDataset);
	...
	protected function getAddParamKeys(){
		...
		$arrAddKeys[UELM_UniteCreatorDialogParam::PARAM_MULTIPLE_SELECT] = $this->getAddParams_array();
		$arrAddKeys[UELM_UniteCreatorDialogParam::PARAM_NUMBER] = $this->getAddParams_responsive();

		$arrAddKeys[UELM_UniteCreatorDialogParam::PARAM_POST_TERMS] = $this->objChildParams->getAddParams_terms();
		$arrAddKeys[UELM_UniteCreatorDialogParam::PARAM_WOO_CATS] = $this->objChildParams->getAddParams_terms(true);

		$arrAddKeys[UELM_UniteCreatorDialogParam::PARAM_LISTING] = $this->objChildParams->getAddParams_listing();

		$arrAddKeys[UELM_UniteCreatorDialogParam::PARAM_USERS] = $this->objChildParams->getAddParams_users();

		$arrAddKeys[UELM_UniteCreatorDialogParam::PARAM_ICON_LIBRARY] = $this->objChildParams->getAddParams_iconLibrary();

		$arrAddKeys[UELM_UniteCreatorDialogParam::PARAM_LINK] = $this->objChildParams->getAddParams_link();
		$arrAddKeys[UELM_UniteCreatorDialogParam::PARAM_DATETIME] = $this->objChildParams->getAddParams_datetime();

		$arrAddKeys[UELM_UniteCreatorDialogParam::PARAM_SLIDER] = $this->objChildParams->getAddParams_slider();
		$arrAddKeys[UELM_UniteCreatorDialogParam::PARAM_DROPDOWN] = $this->objChildParams->getAddParams_dropdown();
		$arrAddKeys[UELM_UniteCreatorDialogParam::PARAM_RADIOBOOLEAN] = $this->objChildParams->getAddParams_dropdown();

		$arrAddKeys[UELM_UniteCreatorDialogParam::PARAM_MENU] = $this->objChildParams->getAddParams_menu();
		$arrAddKeys[UELM_UniteCreatorDialogParam::PARAM_TEMPLATE] = $this->objChildParams->getAddParams_template();
	...
	private function putConfig(){
		...
		if(UELM_GlobalsUnlimitedElements::$enableEditProOptions == true)
			$options["add_edit_pro"] = true;

		if(UELM_GlobalsUC::$isProVersion == true)
			$options["is_pro_version"] = true;
		
		if(UELM_GlobalsUnlimitedElements::$enableLimitProFunctionality == true)
			$options["enable_limit_pro_functionality"] = true;
					
		$dataOptions = UELM_UniteFunctionsUC::jsonEncodeForHtmlData($options, "options");

		$params = $this->objAddon->getParams();
		$dataParams = UELM_UniteFunctionsUC::jsonEncodeForHtmlData($params, "params");

		
		$paramsItems = $this->objAddon->getParamsItems();
		$dataParamsItems = UELM_UniteFunctionsUC::jsonEncodeForHtmlData($paramsItems, "params-items");
		...
		$dataVarItems = UELM_UniteFunctionsUC::jsonEncodeForHtmlData($variablesItems, "variables-items");
		$dataVarMain = UELM_UniteFunctionsUC::jsonEncodeForHtmlData($variablesMain, "variables-main");

		$dataParamsCats = UELM_UniteFunctionsUC::jsonEncodeForHtmlData($paramsCats, "params-cats");

		$objOutput = new UELM_UniteCreatorOutput();
		$objOutput->setProcessType(UELM_UniteCreatorParamsProcessor::PROCESS_TYPE_CONFIG);
		...
		$dataPanelKeys = UELM_UniteFunctionsUC::jsonEncodeForHtmlData($arrConstantData, "panel-keys");

		$arrItemConstantData = $objOutput->getItemConstantDataKeys();

		$dataItemPanelKeys = UELM_UniteFunctionsUC::jsonEncodeForHtmlData($arrItemConstantData, "panel-item-keys");

		//child keys of some fields
		$arrPanelChildKeys = $this->getParamChildKeys();

		$dataPanelChildKeys = UELM_UniteFunctionsUC::jsonEncodeForHtmlData($arrPanelChildKeys, "panel-child-keys");

		$arrPanelAddKeys = $this->getAddParamKeys();

		$dataPanelAddKeys = UELM_UniteFunctionsUC::jsonEncodeForHtmlData($arrPanelAddKeys, "panel-add-keys");

		$arrPanelTemplateCode = $this->getParamTemplateCodes();
		$dataPanelCode = UELM_UniteFunctionsUC::jsonEncodeForHtmlData($arrPanelTemplateCode, "panel-template-code");

		$dataSkipParams = UELM_UniteFunctionsUC::jsonEncodeForHtmlData($this->arrSkipPanelParams, "panel-skip-params");
		...
		<div id="uc_edit_item_config" style="display:none"
			<?php 
				uelm_echo( $dataParams )?>
			<?php 
				uelm_echo( $dataParamsItems )?>
			<?php 
				uelm_echo( $dataPanelKeys )?>
			<?php 
				uelm_echo( $dataPanelAddKeys )?>
			<?php 
				uelm_echo( $dataItemPanelKeys )?>
			<?php 
				uelm_echo( $dataVarItems )?>
			<?php 
				uelm_echo( $dataVarMain )?>
			<?php 
				uelm_echo( $dataParamsCats )?>
			<?php 
				uelm_echo( $dataOptions )?>
			<?php 
				uelm_echo( $dataPanelChildKeys )?>
			<?php 
				uelm_echo( $dataPanelCode )?>
			<?php 
				uelm_echo( $dataSkipParams )?>
		></div>
	...
	private function putBulkDialog(){
		?>
		<div id="uc_dialog_bulk" title="<?php esc_html_e("Bulk Operations", "unlimited-elements")?>" class="unite-inputs" style="display:none">
	...
	public function getBulkDialogContents($data){
		...
		$paramTitle = UELM_UniteFunctionsUC::getVal($paramData, "title");
		...
		<?php echo esc_html($paramType) ?> <?php esc_attr_e("attribute","unlimited-elements")?>: <b> <?php echo esc_html($paramTitle)?> ( <?php echo esc_html($paramName)?> ) </b>
		<span class="hor_sap40"></span>
		<?php esc_attr_e("Widget","unlimited-elements")?>: <b> <?php echo esc_html($addonTitle)?> </b>

		<br><br>

		<div class="unite-dialog-inner-constant">

		<div class="uc-dialog-loader loader_text" style="display:none"><?php esc_html_e("Updating Addons", "unlimited-elements")?>...</div>

		<table class="unite_table_items">

			<tr>
				<th class="">
					<input type='checkbox' title="<?php esc_html_e("Select All Addons", "unlimited-elements")?>" class="uc-check-all">
				</th>
				<th><b><?php esc_html_e("Widget Title", "unlimited-elements")?></b></th>
				<th><b><?php esc_html_e("Status", "unlimited-elements")?></b></th>
			</tr>
		...
		foreach($arrAddons as $index=>$catAddon){
			...
			$status = "<span class='unite-color-red'>".__("not exists","unlimited-elements")."</span>";
			if($isExists)
				$status = "<span class='unite-color-green'>".__("exists","unlimited-elements")."</span>";
			...
			<tr class="<?php echo esc_attr($rowClass)?>">
				...
				uelm_echo( $checked )?> class="uc-check-select">
				</td>
		...
		<span class='uc-section-selected'>
			<span id='uc_bulk_dialog_num_selected'><?php echo esc_html($numSelected)?></span> <?php esc_html_e("selected", "unlimited-elements")?>
		</span>

		<span class="hor_sap"></span>

		<a href="javascript:void(0)" data-action="update" class="uc-action-button unite-button-primary"><?php esc_html_e("Add / Update in Widgets", "unlimited-elements")?></a>

		<span class="hor_sap40"></span>

		<a href="javascript:void(0)" data-action="delete" class="uc-action-button unite-button-secondary"><?php esc_html_e("Delete From Widgets", "unlimited-elements")?></a>
	...
	public function getChangelogContents($data){
		...
		<?php if(empty($changelogs)): ?>
			<div class="uc-changelogs-empty">
				<?php esc_html_e("No changelogs yet.", "unlimited-elements"); ?>
			</div>
		<?php else: ?>
					...
					<li class="uc-changelog" data-id="<?php echo esc_attr($log["id"]); ?>" data-log="<?php echo esc_attr(json_encode($log)); ?>">
						<div class="uc-changelog-content">
							...
							<div class="uc-changelog-info" title="<?php echo esc_attr($log["created_date"]); ?>">
								...
								<?php esc_html_e("ago", "unlimited-elements"); ?>
								<?php esc_html_e("by", "unlimited-elements"); ?>
						...
						<div class="uc-changelog-actions">
							<button
								...
								title="<?php esc_attr_e("Edit", "unlimited-elements"); ?>"
							>
								<i class="fa fa-pen"></i>
							</button>
							<button
								class="uc-changelog-button uc-changelog-delete unite-button-secondary"
								title="<?php esc_attr_e("Delete", "unlimited-elements"); ?>"
							>
	...
	public function getRevisionsContents($data){
		...
		<?php if(empty($revisions)): ?>
			<div class="uc-revisions-empty">
				<?php esc_html_e("No revisions yet.", "unlimited-elements"); ?>
			</div>
		<?php else: ?>
				...
				<?php foreach($revisions as $revision): ?>
					...
					<?php if($previousRevision === null && $revision["is_today"] === true): ?>
						<li class="uc-revisions-label">
							<?php esc_html_e("Today", "unlimited-elements"); ?>
						</li>
					<?php elseif($previousRevision === null && $revision["is_today"] === false): ?>
						<li class="uc-revisions-label">
							<?php esc_html_e("Past", "unlimited-elements"); ?>
						</li>
	...
	private function putDialogs(){
		...
		//dialog param
		$objDialogParam = UELM_UniteCreatorDialogParam::getInstance($addonType);

		$objDialogParam->init(UELM_UniteCreatorDialogParam::TYPE_MAIN, $this->objAddon);
		$objDialogParam->outputHtml();
		
		
		//dialog variable item

		$objDialogVariableItem = UELM_UniteCreatorDialogParam::getInstance($addonType);
		$objDialogVariableItem->init(UELM_UniteCreatorDialogParam::TYPE_ITEM_VARIABLE, $this->objAddon);
		$objDialogVariableItem->outputHtml();

		//dialog variable main
		$objDialogVariableMain = UELM_UniteCreatorDialogParam::getInstance($addonType);
		$objDialogVariableMain->init(UELM_UniteCreatorDialogParam::TYPE_MAIN_VARIABLE, $this->objAddon);
	...
	private function putProInvitePopup() {
		...
		<div class="uelm-pro-message-popup">
			<div class="content">
				<h2>
				  <?php echo esc_html__("The edit block functionality", "unlimited-elements"); ?><br>
				  <?php echo esc_html__("available in PRO Version Only", "unlimited-elements"); ?>
				</h2>
				<p><?php echo esc_html__('Unlock Access To All Pro Blocks and Features.', "unlimited-elements"); ?></p>
				<p>
                    <a href="<?php echo UELM_GlobalsUC::$url_buy_platform?>" 
                    target="_blank" 
                    class="unite-button-primary"><?php echo esc_html__('Upgrade Now', "unlimited-elements"); ?></a>
                </p>
			</div>
		</div>
		<?php
