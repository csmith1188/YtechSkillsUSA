...
class UELM_FormEntriesView extends WP_List_Table{
	...
	public function __construct($args = array()){
		...
		$this->service = new UELM_UCFormEntryService();
	...
	public function get_columns(){
		...
		$columns = array(
			...
			"main" => __("Main", "unlimited-elements"),
			"form" => __("Form", "unlimited-elements"),
			"page" => __("Page", "unlimited-elements"),
			"id" => __("ID", "unlimited-elements"),
			"date" => __("Date", "unlimited-elements"),
		);
	...
	public function no_items(){
		...
		esc_attr_e("No entries found.", "unlimited-elements");
	}
	...
	protected function get_views(){
		...
		$statuses = array(
			self::STATUS_ALL => __("All", "unlimited-elements"),
			self::STATUS_UNREAD => __("Unread", "unlimited-elements"),
			self::STATUS_READ => __("Read", "unlimited-elements"),
			self::STATUS_TRASH => __("Trash", "unlimited-elements"),
		);
	...
	protected function get_bulk_actions(){
		...
		$actions = array(
			self::ACTION_READ => __("Mark as Read", "unlimited-elements"),
			self::ACTION_UNREAD => __("Mark as Unread", "unlimited-elements"),
			self::ACTION_TRASH => __("Move to Trash", "unlimited-elements"),
		);
	...
	protected function column_main($item){
		...
		$id = $item["id"];
		$label = $item["main"]["text"] ?: $item["main"]["value"];
		...
		foreach($item["fields"] as $field){
			if($field["type"] === UELM_UniteCreatorForm::TYPE_FILES && empty($field["value"]) === false){
		...
		if($status === self::STATUS_TRASH){
			...
			$actions = array(
				self::ACTION_UNTRASH => $this->getActionLink(self::ACTION_UNTRASH, $id, __("Restore", "unlimited-elements")),
				self::ACTION_DELETE => $this->getActionLink(self::ACTION_DELETE, $id, __("Delete Permanently", "unlimited-elements")),
			);
		}else{
			$content = $this->getActionLink(self::ACTION_VIEW, $id, $label);

			$actions = array(
				self::ACTION_VIEW => $this->getActionLink(self::ACTION_VIEW, $id, __("View", "unlimited-elements")),
				self::ACTION_TRASH => $this->getActionLink(self::ACTION_TRASH, $id, __("Trash", "unlimited-elements")),
			);

			if($item["is_read"] === true)
				$actions[self::ACTION_UNREAD] = $this->getActionLink(self::ACTION_UNREAD, $id, __("Mark as Unread", "unlimited-elements"));
			else
				$actions[self::ACTION_READ] = $this->getActionLink(self::ACTION_READ, $id, __("Mark as Read", "unlimited-elements"));
		}
	...
	protected function extra_tablenav($which){
		...
		<div class="alignleft actions">
			...
			<label class="screen-reader-text" for="filter-form">
				<?php echo esc_html__("Filter by form", "unlimited-elements"); ?>
			</label>
			<select id="filter-form" name="<?php echo esc_attr(self::FILTER_FORM); ?>">
				<option value=""><?php echo esc_html__("All Forms", "unlimited-elements"); ?></option>
			...
			<label class="screen-reader-text" for="filter-page">
				<?php echo esc_html__("Filter by page", "unlimited-elements"); ?>
			</label>
			<select id="filter-page" name="<?php echo esc_attr(self::FILTER_PAGE); ?>">
				<option value=""><?php echo esc_html__("All Pages", "unlimited-elements"); ?></option>
			...
			<label class="screen-reader-text" for="filter-date">
				<?php echo esc_html__("Filter by date", "unlimited-elements"); ?>
			</label>
			<select id="filter-date" name="<?php echo esc_attr(self::FILTER_DATE); ?>">
				<option value=""><?php echo esc_html__("All Time", "unlimited-elements"); ?></option>
			...
			<?php submit_button(__("Filter", "unlimited-elements"), "", "", false, array("id" => "filter-submit")); ?>

		</div>
		<div class="alignright" style="margin-left: 8px;">
			<?php submit_button(__("Export to CSV", "unlimited-elements"), "primary", self::ACTION_EXPORT, false, array("id" => "export-submit")); ?>
		</div>
		<?php
	}
	...
	private function processExportAction(){
		...
		$entryHeaders = array(
			"id" => __("Entry ID", "unlimited-elements"),
			"form_name" => __("Form", "unlimited-elements"),
			"post_title" => __("Page", "unlimited-elements"),
			"post_url" => __("Page URL", "unlimited-elements"),
			"created_at" => __("Date", "unlimited-elements"),
			"user_id" => __("User ID", "unlimited-elements"),
			"user_ip" => __("User IP", "unlimited-elements"),
			"user_agent" => __("User Agent", "unlimited-elements"),
		);
		...
		UELM_UniteFunctionsUC::downloadCsv($filename, $headers, $rows);
	}
	...
	private function getDatesFilter(){
		...
		$items = array(
			self::DATE_TODAY => __("Today", "unlimited-elements"),
			self::DATE_YESTERDAY => __("Yesterday", "unlimited-elements"),
			self::DATE_LAST_7 => __("Last 7 days", "unlimited-elements"),
			self::DATE_LAST_30 => __("Last 30 days", "unlimited-elements"),
		);
	...
	private function getFilter($key){
		...
		switch($key){
			case self::FILTER_ID:
				$value = UELM_UniteFunctionsUC::getGetVar(self::FILTER_ID, null, UELM_UniteFunctionsUC::SANITIZE_NOTHING);
			break;
			case self::FILTER_STATUS:
				$value = UELM_UniteFunctionsUC::getGetVar(self::FILTER_STATUS, self::STATUS_ALL, UELM_UniteFunctionsUC::SANITIZE_KEY);
			break;
			case self::FILTER_SEARCH:
				$value = UELM_UniteFunctionsUC::getGetVar(self::FILTER_SEARCH, null, UELM_UniteFunctionsUC::SANITIZE_TEXT_FIELD);
			break;
			case self::FILTER_FORM:
				$value = UELM_UniteFunctionsUC::getGetVar(self::FILTER_FORM, null, UELM_UniteFunctionsUC::SANITIZE_NOTHING);
			break;
			case self::FILTER_PAGE:
				$value = UELM_UniteFunctionsUC::getGetVar(self::FILTER_PAGE, null, UELM_UniteFunctionsUC::SANITIZE_NOTHING);
			break;
			case self::FILTER_DATE:
				$value = UELM_UniteFunctionsUC::getGetVar(self::FILTER_DATE, null, UELM_UniteFunctionsUC::SANITIZE_KEY);
			break;
		}
	...
	private function getWhere(array $filters){
		...
		$id = UELM_UniteFunctionsUC::getVal($filters, self::FILTER_ID, null);

		if(empty($id) === false){
			$ids = is_array($id) ? $id : array($id);
			$placeholders = UELM_UniteFunctionsWPUC::getDBPlaceholders($ids, "%d");
			$where .= $wpdb->prepare(" AND id IN($placeholders)", $ids);
		}

		$status = UELM_UniteFunctionsUC::getVal($filters, self::FILTER_STATUS, null);

		if(empty($status) === false)
			$where .= " AND {$this->getStatusWhere($status)}";

		$search = UELM_UniteFunctionsUC::getVal($filters, self::FILTER_SEARCH, null);
		...
		$form = UELM_UniteFunctionsUC::getVal($filters, self::FILTER_FORM, null);

		if(empty($form) === false)
			$where .= $wpdb->prepare(" AND form_name = %s", array($form));

		$post = UELM_UniteFunctionsUC::getVal($filters, self::FILTER_PAGE, null);

		if(empty($post) === false)
			$where .= $wpdb->prepare(" AND post_title = %s", array($post));

		$date = UELM_UniteFunctionsUC::getVal($filters, self::FILTER_DATE, null);
	...
	private function getSortingField(){
		...
		$field = UELM_UniteFunctionsUC::getGetVar("orderby", null, UELM_UniteFunctionsUC::SANITIZE_KEY);
		$field = UELM_UniteFunctionsUC::getVal($fields, $field, "id");
	...
	private function getSortingOrder(){
		...
		$order = UELM_UniteFunctionsUC::getGetVar("order", null, UELM_UniteFunctionsUC::SANITIZE_KEY);
		$order = UELM_UniteFunctionsUC::getVal($orders, $order, "DESC");
	...
	private function displayHeader(){
		...
		$headerTitle = __("Form Entries", "unlimited-elements");
	...
	private function displaySearchBox(){
		...
		<p class="search-box">
			<label class="screen-reader-text" for="filter-search">
				<?php echo esc_html__("Search", "unlimited-elements"); ?>
			</label>
			<input
				id="filter-search"
				type="search"
				name="<?php echo esc_attr(self::FILTER_SEARCH); ?>"
				placeholder="<?php echo esc_attr__("Search...", "unlimited-elements"); ?>"
				value="<?php echo esc_attr($this->getFilter(self::FILTER_SEARCH)); ?>"
			/>
			<?php submit_button(__("Search", "unlimited-elements"), "", "", false, array("id" => "search-submit")); ?>
		</p>
		<?php
	}
