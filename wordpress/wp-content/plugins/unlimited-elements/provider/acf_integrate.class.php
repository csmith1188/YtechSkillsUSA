...
class UELM_UniteCreatorAcfIntegrate{
	...
	private $arrFieldsTypes = array();
		...
		private function getImageFieldTitle($field){
			...
			$filename = UELM_UniteFunctionsUC::getVal($field, "filename");
		...
		private function getImageFieldData($field, $key=null){
			...
			$imageID = UELM_UniteFunctionsUC::getVal($field, "id");

			
			$title = $this->getImageFieldTitle($field);
			
			$caption = UELM_UniteFunctionsUC::getVal($field, "caption");
			if(!empty($caption))
				$title = $caption;
			
			$description = UELM_UniteFunctionsUC::getVal($field, "description");
			$alt = UELM_UniteFunctionsUC::getVal($field, "alt");
			
			if(empty($alt))
				$alt = $title;
			
			$urlImage = UELM_UniteFunctionsUC::getVal($field, "url");
			$arrSizes = UELM_UniteFunctionsUC::getVal($field, "sizes");
			
			$width = UELM_UniteFunctionsUC::getVal($field, "width");
			$height = UELM_UniteFunctionsUC::getVal($field, "height");
			
			if(!empty($outputImageSize)){
				
				$urlImageSize = UELM_UniteFunctionsUC::getVal($arrSizes, $outputImageSize);
				
				if(!empty($urlImageSize)){
					
					$urlImage = $urlImageSize;
					$width = UELM_UniteFunctionsUC::getVal($arrSizes, $outputImageSize."-width");
					$height = UELM_UniteFunctionsUC::getVal($arrSizes, $outputImageSize."-height");
				}
			...
			$thumbMedium = UELM_UniteFunctionsUC::getVal($arrSizes, "medium");
			$arrValues[$keyprefix."thumb"] = $thumbMedium;
			
			$thumbMediumWidth = UELM_UniteFunctionsUC::getVal($arrSizes, "medium-width");
			$thumbMediumHeight = UELM_UniteFunctionsUC::getVal($arrSizes, "medium-width");
			...
			foreach($arrSizes as $size => $value){
				...
				$thumbWidth = UELM_UniteFunctionsUC::getVal($arrSizes, $size."-width");
				$thumbHeight = UELM_UniteFunctionsUC::getVal($arrSizes, $size."-height");
		...
		private function getPostData($objPost, $key, $addID = false){
			...
			$arrValues[$key."_title"] = UELM_UniteFunctionsUC::getVal($arrPost, "post_title");
			$arrValues[$key."_alias"] = UELM_UniteFunctionsUC::getVal($arrPost, "post_name");
			$arrValues[$key."_content"] = UELM_UniteFunctionsUC::getVal($arrPost, "post_content");
			$arrValues[$key."_link"] = UELM_UniteFunctionsWPUC::getPermalink($objPost);
		...
		private function getFileFieldData($data, $key){
			...
			$url = UELM_UniteFunctionsUC::getVal($data, "url");
			$filesize = UELM_UniteFunctionsUC::getVal($data, "filesize");
		...
		private function getDataType($data, $key){
			...
			$fieldType = UELM_UniteFunctionsUC::getVal($this->arrFieldsTypes, $key);
			...
			if(is_array($data)){
				...
				if(isset($data[0])){
					...
					if($itemType == "array"){
						...
						$itemType = UELM_UniteFunctionsUC::getVal($item, "type");
		...
		private function getUserData($arrUser, $key){
			...
			$arrValues = array();
			$arrValues[$key."_firstname"] = UELM_UniteFunctionsUC::getVal($arrUser, "user_firstname");
			$arrValues[$key."_lastname"] = UELM_UniteFunctionsUC::getVal($arrUser, "user_lastname");
			$arrValues[$key."_displayname"] = UELM_UniteFunctionsUC::getVal($arrUser, "display_name");
			$arrValues[$key."_email"] = UELM_UniteFunctionsUC::getVal($arrUser, "user_email");
			$arrValues[$key."_avatar"] = UELM_UniteFunctionsUC::getVal($arrUser, "user_avatar");
		...
		private function getLinkAttributes($data){
			...
			$title = UELM_UniteFunctionsUC::getVal($data, "title");
			$target = UELM_UniteFunctionsUC::getVal($data, "target");
		...
		private function getTaxonomyField($data) {
			...
			$isIDsArray = UELM_UniteFunctionsUC::isValidIDsArray($data);
		...
		private function addAcfValues($arrValues, $key, $data){
			...
			switch($type){
				...
				default:		
					...
					//uelm_dmp("assoc");uelm_dmp($data); exit();
		...
		private function getFieldsObjects($postID, $objName, $addPrefix = true){
			...
			switch($objName){
				...
				break;
				default:
					UELM_UniteFunctionsUC::throwError("get acf fields objects function works only for post right now");
				break;
			}
		...
		private function getFieldsTypes($postID, $objName, $addPrefix = true){
			...
			foreach($arrObjects as $key => $arrObject){
				...
				$type = UELM_UniteFunctionsUC::getVal($arrObject, "type");
		...
		public function modifyFieldsData($arrData){
			...
			foreach($arrData as $key => $item){
				...
				$isAssocArray = UELM_UniteFunctionsUC::isAssocArray($item);
				
				$firstItem = UELM_UniteFunctionsUC::getArrFirstValue($item);
		...
		public function getAcfFieldValue($fieldName, $objID, $objName = "post"){
			...
			switch($objName){
				...
				break;
				default:
					UELM_UniteFunctionsUC::throwError("get acf fields function works only for post and term right now");
				break;
			}
			...
			$value = UELM_UniteFunctionsUC::getVal($arrDataOutput, $fieldName);
		...
		private function getAcfFieldsData($postID, $objName = "post"){
			...
			switch($objName){
				...
				break;
				default:
					UELM_UniteFunctionsUC::throwError("get acf fields function works only for post and term and users right now");
				break;
			}
		...
		public function getAcfFieldsImageIDs($postID, $objName = "post"){
			...
			foreach($arrData as $name=>$item){
				$type = UELM_UniteFunctionsUC::getVal($item, "type");
				
				if($type != "image")
					continue;
				
				$imageID = UELM_UniteFunctionsUC::getVal($item, "id");
		...
		public function getAcfFieldsKeys($postID, $objName = "post", $addPrefix = true){
			...
			foreach($arrFields as $key=>$value){
				...
				$fieldType = UELM_UniteFunctionsUC::getVal($arrTypes, $key);
				...
				if(is_array($value)){
					...
					$item = UELM_UniteFunctionsUC::getArrFirstValue($value);
		...
		public function getOptionsPages(){
			...
			$noPageMessage = __("[No Page Selected]","unlimited-elements");
			...
			foreach($optionPages as $page){
				...
				$pageTitle = UELM_UniteFunctionsUC::getVal($page, "page_title");
				$pageID = UELM_UniteFunctionsUC::getVal($page, "ID");
