...
class UELM_UniteCreatorLayoutsExporter extends UELM_UniteCreatorLayoutsExporterWork{
	...
	protected function insertAttachmentByImage($arrImage){
		...
		$type = UELM_UniteFunctionsUC::getVal($arrType, "type");
		if(empty($type))
			$type = "image/jpeg";
		...
		$extension = UELM_UniteFunctionsUC::getVal($name_parts, "extension");
		
		//get full url
		$urlFull = UELM_HelperUC::URLtoFull($url);
				
		//check for existing image id
		$imageID = UELM_UniteFunctionsWPUC::getAttachmentIDFromImageUrl($urlFull);
				
		if(!empty($imageID)){
		
			$urlExistingImage = UELM_UniteFunctionsWPUC::getUrlAttachmentImage($imageID);
