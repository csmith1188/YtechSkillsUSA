...
class UELM_UniteCreatorPluginIntegrations{
	...
	public function WPP_getPopularPosts($args, $addDebug = false){
		...
		$limit = UELM_UniteFunctionsUC::getVal($args, "limit", 5);
		$range = UELM_UniteFunctionsUC::getVal($args, "range", "last7days");
		$cat = UELM_UniteFunctionsUC::getVal($args, "cat", "");
		...
		$arrPosts = UELM_UniteFunctionsUC::convertStdClassToArray($arrPosts);
		...
		foreach($arrPosts as $index => $post){
			...
			$id = UELM_UniteFunctionsUC::getVal($post, "id");
			$title = UELM_UniteFunctionsUC::getVal($post, "title");
			$pageviews = UELM_UniteFunctionsUC::getVal($post, "pageviews");
	...
	public static function checkAddStickyPosts($arrPosts, $args){
        ...
        $arrStickyAssoc = UELM_UniteFunctionsUC::arrayToAssoc($arrStickyPostIDs);
	...
	public static function getArrContactForm7(){
		...
		$arrPosts = UELM_UniteFunctionsWPUC::getPostsByType("wpcf7_contact_form");
		...
		$arrForms["[ Select From ]"] = __("Please Select Contact From 7","unlimited-elements");
	...
	public static function putJetWooWishlistButton(){
		...
		$isAddDefault = UELM_UniteFunctionsUC::strToBool($isAddDefault);
	...
	public function saboxGetUserData($arrData){
		...
		$userID = UELM_UniteFunctionsUC::getVal($arrData, "id");
		
		if(empty($userID))
			return($arrData);
			
		$arrMeta = UELM_UniteFunctionsWPUC::getAllUserMeta($userID);
		
		if(empty($arrMeta))
			return($arrData);
			
		$urlProfileImage = UELM_UniteFunctionsUC::getVal($arrMeta, "sabox-profile-image");
	...
	private function initSABoxIntegration(){
		...
		add_filter("uelm_get_user_data",array($this,"saboxGetUserData"));
	...
	public function fvplayerModifyPostsIncludeby($includeBy){
		...
		$includeBy["fvplayers_user_watched"] = __("FVPlayer - User Watched Posts", "unlimited-elements");
		...
		switch($includeBY){
			case "fvplayers_user_watched":
				...
				if(UELM_GlobalsProviderUC::$showPostsQueryDebug == true){
					uelm_dmp("FVPlayer - get user recently watched posts by function: fv_player_get_user_watched_post_ids");
					uelm_dmp($arrIDs);
				}
	...
	public function favoritesModifyPostsIncludeby($includeBy){
		...
		$includeBy["favorites_get_user_posts"] = __("Favorites Plugin - Get User Posts", "unlimited-elements");
	...
	public function addRelevanssiIntegrationSetting($arrAjaxSettings, $paramName){
		...
		$arrAjaxSettings[] = array(
			...
			"label"        => __( 'Enable Relevanssi Plugin Integration', "unlimited-elements" ),
			"default"      => "",
			...
			'description'  => __('When searching using search filter, if enable the search using relevancy plugin', 'unlimited-elements'),
		);
	...
	public function relevanssiModifyQueryArgs($args, $value, $name){
		...
		$search = UELM_UniteFunctionsUC::getVal($args, "s");
		
		if(empty($search))
			return($args);
		*/
		
		$relevanssiIntegration = UELM_UniteFunctionsUC::getVal($value, "{$name}_relevanssi_integration");
		$relevanssiIntegration = UELM_UniteFunctionsUC::strToBool($relevanssiIntegration);
	...
	public function languagesPostQueryAddLang($args){
		...
		$args["lang"] = UELM_UniteFunctionsWPUC::getLanguage();
	...
	private function initLanguagesIntegration(){
		...
		if(function_exists('pll_current_language') == false && UELM_UniteCreatorWpmlIntegrate::isWpmlExists() == false)
			return(false);
		...
		switch($includeBY){
			case "favorites_get_user_posts":
				...
				if(!empty($response)){
					...
					$arrIDs = UELM_UniteFunctionsUC::getVal($arrRespones, "posts");
				}
				
				//show debug
				
				if(UELM_GlobalsProviderUC::$showPostsQueryDebug == true){
					uelm_dmp("Favorites plugin - get usre favorites");
					uelm_dmp($arrIDs);
				}
