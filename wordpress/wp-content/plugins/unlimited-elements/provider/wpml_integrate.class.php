...
class UELM_UniteCreatorWpmlIntegrate{
	...
	public function init(){
		...
		$this->arrShortPrefix["__none__"] = __("Not Selected","unlimited-elements");
		...
		foreach($this->arrLanguages as $language){
			...
			$code = UELM_UniteFunctionsUC::getVal($language, "code");
			$isActive = UELM_UniteFunctionsUC::getVal($language, "active");
			
			if($isActive == true && empty($this->activeLanguage)){
				$this->activeLanguage = $code;
			}
			
			$langName = UELM_UniteFunctionsUC::getVal($language, "native_name");
			if(empty($langName))
				$langName = UELM_UniteFunctionsUC::getVal($language, "translated_name");
		...
		if(empty($this->activeLanguage))
			$this->activeLanguage = UELM_UniteFunctionsUC::getArrFirstValue($this->arrShortPrefix);
	...
	public function getLanguagesShort($addPrefix = false, $noActive = false){
		...
		if($noActive == true){
			...
			if(empty($defaultLanguage))
				$defaultLanguage = UELM_UniteFunctionsUC::getArrFirstValue($arrLang);
	...
	public static function getTranslatedAttachmentID($thumbID){
		...
		if(empty(self::$objWpmlSingleton)){
			...
			self::$objWpmlSingleton = new UELM_UniteCreatorWpmlIntegrate();
	...
	private function getTranslatableParamsFields($widgetTitle, $params, $isItems = false){
		...
		foreach($params as $param){
			...
			$fieldName = UELM_UniteFunctionsUC::getVal($param, "name");
