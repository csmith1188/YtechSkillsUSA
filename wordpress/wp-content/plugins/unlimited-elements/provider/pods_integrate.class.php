...
class UELM_UniteCreatorPodsIntegrate{
	...
	public static function getObjPodsIntegrate(){
		...
		if(empty(self::$objPodsIntegrate))
			self::$objPodsIntegrate = new UELM_UniteCreatorPodsIntegrate();
	...
	private function getImageData($post, $fieldName, $arrValues){
		...
		$imageID = UELM_UniteFunctionsUC::getVal($post, "ID");
		if(empty($imageID))
			return(null);
		...
		$urlThumbMedium = UELM_UniteFunctionsWPUC::getUrlAttachmentImage($imageID, UELM_UniteFunctionsWPUC::THUMB_MEDIUM);
		$urlThumbLarge = UELM_UniteFunctionsWPUC::getUrlAttachmentImage($imageID, UELM_UniteFunctionsWPUC::THUMB_LARGE);
	...
	private function getFileData($post, $fieldName, $arrValues){
		...
		$filepath = UELM_UniteFunctionsUC::getVal($post, "guid");
		$title = $this->getImageFieldTitle($post);
	...
	private function getFileFieldData($post, $fieldName, $arrValues){
		...
		if(self::DEBUG_FIELDS == true){
			uelm_dmp("file data $postID is image: ".$isImage);
			uelm_dmp($arrValues[$fieldName]);
		}
	...
	public function getPodsFields($postID, $addPrefix = true){
		...
		foreach($arrFields as $name => $field){
			...
			if($type == self::TYPE_FILE){
				$format = UELM_UniteFunctionsUC::getVal($field, "file_format_type");
				if($format == "multi")
