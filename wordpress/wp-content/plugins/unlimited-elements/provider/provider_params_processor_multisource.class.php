...
class UELM_ParamsProcessorMultisource{
	...
	private function validateImageSizeExists(){
		...
		foreach($params as $param){
			...
			if($type == UELM_UniteCreatorDialogParam::PARAM_IMAGE){
				$imageTitle = UELM_UniteFunctionsUC::getVal($param, "title");
			}
		...
		UELM_HelperHtmlUC::outputErrorMessage("Multisource Error: Missing <b>image size attribute</b> for: <b>$imageTitle</b> image attribute. in widget: <b>$addonTitle</b>. Please add it to attributes list. Special Attribute -> Image Size");
	...
	private function getData_posts($forWooProducts = false){
		...
		$filteringAttributes = UELM_UniteFunctionsUC::getVal($dataResponse, "uc_filtering_attributes");
		$filteringAddClass = UELM_UniteFunctionsUC::getVal($dataResponse, "uc_filtering_addclass");
		...
		$arrPosts = UELM_UniteFunctionsUC::getVal($dataResponse, $this->name . "_items");

		//debug meta - show meta fields
		if($this->showDebugMeta === true)
			UELM_HelperUC::$operations->putPostsCustomFieldsDebug($arrPosts, true);
	...
	private function getData_terms(){
		...
		if($this->showDebugMeta == true)
			UELM_HelperUC::$operations->putTermsCustomFieldsDebug($arrTerms, true);
	...
	private function getData_menu(){
		...
		$menuID = UELM_UniteFunctionsUC::getVal($this->arrValues, $this->nameParam."_id");
		$depth = UELM_UniteFunctionsUC::getVal($this->arrValues, $this->nameParam."_depth");
		...
		$arrItems = UELM_UniteFunctionsWPUC::getMenuItems($menuID, $isOnlyParents);

		if($this->showDebugMeta == true)
			UELM_HelperUC::$operations->putMenuCustomFieldsDebug($arrItems, true);
	...
	private function getData_repeater(){
		...
		$arrRepeaterItems = UELM_HelperProviderUC::getRepeaterItems($this->arrValues, $this->name, $this->showDebugData, $this->showDebugMeta);
	...
	private function getData_jsonCsv(){
		...
		if($this->showDebugData == true){
			uelm_dmp("---- the debug is ON, please turn it off before release --- ");
		}

		$arrData = UELM_HelperProviderUC::getRepeaterItems_json($this->arrValues, $this->name, $this->showDebugData);
	...
	private function getData_instagram(){
		...
		foreach($items as $key=>$item){
			...
			$isVideo = UELM_UniteFunctionsUC::getVal($item, "isvideo");

			$item["type"] = $isVideo?"video":"image";

			$item["caption_text"] = UELM_UniteFunctionsUC::getVal($item, "caption");
	...
	private function getData_api(){
		...
		// get api data
		$type = UELM_UniteFunctionsUC::getVal($params, "type");
		$data = UELM_UniteCreatorAPIIntegrations::getInstance()->getDataForMultisource($type, $params);
	...
	private function getData_gallery(){
		...
		$arrImages = UELM_UniteFunctionsUC::getVal($this->arrValues, $this->name."_gallery");
		...
		if(!empty($arrCacheIDs))
			UELM_UniteFunctionsWPUC::cachePostMetaQueries($arrCacheIDs);
    ...
    private function getData_rss(){
        ...
        $dataRss = UELM_UniteFunctionsUC::getVal($data, $this->name);
	...
	private function getData($source){
		...
		switch($source){
			...
			default:
				UELM_UniteFunctionsUC::throwError("getData error, Wrong items source: $source");
		}
	}
	...
	private function checkDebugBeforeData($itemsSource){
		...
		switch($itemsSource){
			case self::SOURCE_JSONCSV:
				...
				$isDebugJsonCsv = UELM_UniteFunctionsUC::getVal($this->arrValues, $this->name."_debug_jsoncsv_data");
				$isDebugJsonCsv = UELM_UniteFunctionsUC::strToBool($isDebugJsonCsv);
				...
				$isShowExamples = UELM_UniteFunctionsUC::getVal($this->arrValues, $this->name."_show_example_jsoncsv");
				$isShowExamples = UELM_UniteFunctionsUC::strToBool($isShowExamples);
	...
	private function printJsonCsvExamples(){
		...
		uelm_dmp("----- Show JSON CSV examples is ON. Please turn it OFF before release");

		//-------- show the json

		uelm_dmp("JSON content example:");
		...
		uelm_dmp("CSV content example:");

		$csv = UELM_UniteFunctionsUC::arrayToCsv($arrExample);
	...
	* get image data by id
	...
	private function getImageData($id){
		...
		$itemParam = array();
		$itemParam["type"] = UELM_UniteCreatorDialogParam::PARAM_IMAGE;
		$itemParam["name"] = "image";

		$item = array();
		$item = $this->objProcessor->getProcessedParamData($item, $id, $itemParam, UELM_UniteCreatorParamsProcessorWork::PROCESS_TYPE_OUTPUT);
	...
	private function getTaxonomyValue($dataItem, $taxonomy){
		...
		$postID = UELM_UniteFunctionsUC::getVal($dataItem, "id");
		...
		if(!empty($taxonomy)){
			...
			$arrTitles = UELM_UniteFunctionsWPUC::getPostSingleTermsTitles($postID, $taxonomy);
		}else{

			//select withtout given taxonomy

			$post = get_post($postID);
			$arrTitles = UELM_UniteFunctionsWPUC::getPostTermsTitles($post, false);
		}
	...
	private function getMetaValue($dataItem, $metaKey){
		...
		switch($this->itemsType){
			...
			case self::SOURCE_PRODUCTS:
				...
				$arrCustomFieldValues = UELM_UniteFunctionsWPUC::getPostCustomFields($postID, false);

				$value = UELM_UniteFunctionsUC::getVal($arrCustomFieldValues, $metaKey);

				//show debug
				if($this->showDebugData == true && $this->showDataType == "input"){

					$title = UELM_UniteFunctionsUC::getVal($dataItem, "title");
			...
			case self::SOURCE_TERMS:
				...
				$termID = UELM_UniteFunctionsUC::getVal($dataItem, "id");

				if(empty($termID))
					return("");

				$arrFields = UELM_UniteFunctionsWPUC::getTermCustomFields($termID, false);

				$value = UELM_UniteFunctionsUC::getVal($arrFields, $metaKey);


			break;
			case self::SOURCE_USERS:

				$userID = UELM_UniteFunctionsUC::getVal($dataItem, "id");

				$arrMeta = UELM_UniteFunctionsWPUC::getUserMeta($userID, array($metaKey));

				$value = UELM_UniteFunctionsUC::getVal($arrMeta, $metaKey);

			break;
			default:

				UELM_UniteFunctionsUC::throwError("getMetaValue error - wrong source: ".$this->itemsType);
			break;
		}
	...
	private function getUserAvatarImage($dataItem, $defaultValue){
		...
		$arrImage = UELM_UniteFunctionsWPUC::getUserAvatarData($userID);

		$urlAvatar = UELM_UniteFunctionsUC::getVal($arrImage, "avatar_url");
	...
	private function getFieldValue($item, $paramName, $source, $dataItem, $param){
		...
		if($source == "default" || empty($source)){
			...
			$item = $this->objProcessor->getProcessedParamData($item, $defaultValue, $param, UELM_UniteCreatorParamsProcessorWork::PROCESS_TYPE_OUTPUT);
			...
			foreach($source as $index => $singleSource){
				...
				switch($singleSource){
					case "text_before":
						...
						$textBefore = UELM_UniteFunctionsUC::getVal($emptyItem, $paramName);
					break;
					case "text_after":
						$emptyItem = $this->getFieldValue($emptyItem, $paramName, $singleSource, $dataItem, $param);
						$textAfter = UELM_UniteFunctionsUC::getVal($emptyItem, $paramName);
					break;
					case "separator":
						$emptyItem = $this->getFieldValue($emptyItem, $paramName, $singleSource, $dataItem, $param);
						$sap = UELM_UniteFunctionsUC::getVal($emptyItem, $paramName);
					...
					case "truncate":
						...
						$truncateChars = UELM_UniteFunctionsUC::getVal($arrTruncate, $paramName,100);
		...
		if(is_array($source)){
			...
			foreach($source as $singleSource){
				...
				$value = UELM_UniteFunctionsUC::getVal($item, $paramName);

				$item = $this->getFieldValue($item, $paramName, $singleSource, $dataItem, $param);

				$valueAfter = UELM_UniteFunctionsUC::getVal($item, $paramName);
			...
			if($isTruncate == true){
				...
				$text = UELM_UniteFunctionsUC::truncateString($text, $truncateChars, true, "");
		...
		switch($source){
			...
			case "truncate":
				...
				$value = UELM_UniteFunctionsUC::getVal($this->arrValues, $textBeforeKey);
			...
			case "static_value":
				...
				$value = UELM_UniteFunctionsUC::getVal($this->arrValues, $staticValueKey);
			...
			case "term_field":
				...
				$taxonomy = UELM_UniteFunctionsUC::getVal($this->arrValues, $taxonomyField);
			...
			case "meta_field":
				...
				$metaKey = UELM_UniteFunctionsUC::getVal($this->arrValues, $metaField);
		...
		if($isProcessReturn == true){
			...
			$item = $this->objProcessor->getProcessedParamData($item, $value, $param, UELM_UniteCreatorParamsProcessorWork::PROCESS_TYPE_OUTPUT);
		...
		if($source == "field")
			$source = UELM_UniteFunctionsUC::getVal($this->arrValues, $this->nameParam."_field_name_".$paramName);
		
		//post values source

		$isFound = false;
		
		$value = UELM_UniteFunctionsUC::getArrayValueByPath($dataItem, $source);
    ...
    */
        ...
        if($source == self::SOURCE_DEMO){
            uelm_dmp("Switching to demo data source in editor only.");
        }
        ...
        uelm_dmp("Input data from: <b>$source</b>, found: $numItems");
	...
	private function getItems($itemsSource, $arrData){
		...
		if(empty($arrFields) === true && UELM_GlobalsProviderUC::$isUnderAjax === false)
			UELM_UniteFunctionsUC::throwError("multisource getItems error: $itemsSource fields not found");

		//get items params
		$arrItemParams = $this->arrParamsItems;
		$arrItemParams = UELM_UniteFunctionsUC::arrayToAssoc($arrItemParams, "name");
		...
		foreach($arrData as $dataItem){
			...
			if($itemsSource !== self::SOURCE_DEMO){
				foreach($arrFields as $fieldKey => $source){
					...
					$param = UELM_UniteFunctionsUC::getVal($arrItemParams, $paramName);
			...
			foreach($arrItemParams as $itemParam){
				$paramName = UELM_UniteFunctionsUC::getVal($itemParam, "name");
			
				if(isset($arrUsedParams[$paramName]) === true)
					continue;

				$value = UELM_UniteFunctionsUC::getVal($itemParam, "default_value");
				$paramType = UELM_UniteFunctionsUC::getVal($itemParam, "type");
				...
				switch($this->itemsType){
					...
					case self::SOURCE_POSTS:
						if($paramName === "title")
							$value = UELM_UniteFunctionsUC::getVal($dataItem, "title");

						if($paramType === UELM_UniteCreatorDialogParam::PARAM_IMAGE)
							$value = UELM_UniteFunctionsUC::getVal($dataItem, "image");

						$objectID = UELM_UniteFunctionsUC::getVal($dataItem, "id");
						
					break;
					case self::SOURCE_TERMS:
					case self::SOURCE_USERS:
						if($paramName === "title")
							$value = UELM_UniteFunctionsUC::getVal($dataItem, "name");
					...
					case self::SOURCE_INSTAGRAM:
						if($paramName === "title")
							$value = UELM_UniteFunctionsUC::getVal($dataItem, "caption_text");
				...
				$item = $this->objProcessor->getProcessedParamData($item, $value, $itemParam, UELM_UniteCreatorParamsProcessorWork::PROCESS_TYPE_OUTPUT);
			...
			//modify demo fields
			if($itemsSource === self::SOURCE_DEMO){
				$item["title"] = UELM_UniteFunctionsUC::getVal($dataItem, "title");
			}
	...
	private function getAttributeDefaults($param){
		...
		$strDefaults = UELM_UniteFunctionsUC::getVal($param, "multisource_attributes_defaults");
	...
	public function getMultisourceSettingsData($value, $name, $processType, $param, $data){
		...
		$this->isInsideEditor = UELM_HelperUC::isEditMode();

		$itemsSource = UELM_UniteFunctionsUC::getVal($value, $name . "_source");
		...
		//debug
		$isShowInputData = UELM_UniteFunctionsUC::getVal($this->arrValues, $this->name . "_show_input_data");
		$isShowInputData = UELM_UniteFunctionsUC::strToBool($isShowInputData);

		$this->showDebugData = $isShowInputData;
		$this->showDataType = UELM_UniteFunctionsUC::getVal($this->arrValues, $this->name . "_input_data_type");

		if(empty($this->showDataType))
			$this->showDataType = "input";

		$isShowMeta = UELM_UniteFunctionsUC::getVal($this->arrValues, $this->name . "_show_metafields");
        ...
        if(UELM_GlobalsUC::$hideDebug !== true) {
	...
	public static function checkShowItemsDebug($arrItemData){
		...
		uelm_dmp("Getting data from the settings items repeater");
