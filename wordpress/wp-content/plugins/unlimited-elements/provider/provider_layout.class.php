...
class UELM_UniteCreatorLayout extends UELM_UniteCreatorLayoutWork{
	...
	public function initByPost($post){
		...
		UELM_UniteFunctionsUC::validateNotEmpty($post," post object");
		...
		if($postType == UELM_GlobalsProviderUC::POST_TYPE_LAYOUT){
			
			$layoutType = UELM_GlobalsUnlimitedElements::ADDONSTYPE_ELEMENTOR_TEMPLATE;
			
			/*
			$layoutType = get_post_meta($postID, UELM_GlobalsProviderUC::META_KEY_LAYOUT_TYPE, true);
		...
		/*
		$layoutData = get_post_meta($postID, UELM_GlobalsProviderUC::META_KEY_LAYOUT_DATA);
		...
		*/
		...
		//get params
		$params = get_post_meta($postID, UELM_GlobalsProviderUC::META_KEY_LAYOUT_PARAMS, true);
		
		$layoutData = "";
		
		$catID = get_post_meta($postID, UELM_GlobalsProviderUC::META_KEY_CATID, true);		
		if(empty($catID))
			$catID = 0;
	...
	public function initByID($id){
		...
		if($id === "current_post"){
			...
			$post = get_post();
			if(empty($post))
				UELM_UniteFunctionsUC::throwError("Current post not found");
			...
			$post = @get_post($id);
			if(empty($post))
				UELM_UniteFunctionsUC::throwError("layout with id: $id not found");
	...
	public function createLayoutInDB($arrInsert, $arrParams = array()){
		...
		UELM_UniteFunctionsUC::validateNotEmpty($this->objLayoutType, "layout type object");
		...
		if(empty($postType)){
			...
			if($this->objLayoutType->isBasicType == true)
				$postType = UELM_UniteFunctionsUC::getVal($arrParams, "post_type");
			else
				$postType = UELM_GlobalsProviderUC::POST_TYPE_LAYOUT;		//basic type - false (blox type)
		} 
				
					
		if(empty($postType))
			UELM_UniteFunctionsUC::throwError("You must specify post type for import layout");
		
					
		$title = UELM_UniteFunctionsUC::getVal($arrInsert, "title");
		$name = UELM_UniteFunctionsUC::getVal($arrInsert, "name");
		
		//for non blox layouts - take name from post title
		if($postType != UELM_GlobalsProviderUC::POST_TYPE_LAYOUT)
			$name = sanitize_title($title);
		
		$layoutData = UELM_UniteFunctionsUC::getVal($arrInsert, "layout_data");
		
		$catID = UELM_UniteFunctionsUC::getVal($arrInsert, "catid");
		$parentID = UELM_UniteFunctionsUC::getVal($arrParams, "parent_id");
		...
		$maxOrder = UELM_UniteFunctionsWPUC::getMaxMenuOrder($postType, $parentID);
		...
		if(!empty($catID))
		   add_post_meta($postID, UELM_GlobalsProviderUC::META_KEY_CATID, $catID);				
		...
		add_post_meta($postID, UELM_GlobalsProviderUC::META_KEY_BLOX_PAGE, "true");
		
		if(!empty($layoutData))
			add_post_meta($postID, UELM_GlobalsProviderUC::META_KEY_LAYOUT_DATA, $layoutData);
		
		//add layout type meta
		if($postType == UELM_GlobalsProviderUC::POST_TYPE_LAYOUT){
			
			add_post_meta($postID, UELM_GlobalsProviderUC::META_KEY_LAYOUT_TYPE, $this->objLayoutType->typeNameDistinct);
		...
		if($this->objLayoutType->defaultBlankTemplate == true)
			UELM_UniteFunctionsWPUC::updatePageTemplateAttribute($postID, UELM_GlobalsProviderUC::PAGE_TEMPLATE_LANDING_PAGE);
	...
	public function updateLayoutInDB($arrUpdate){
		...
		//update post
		$title = UELM_UniteFunctionsUC::getVal($arrUpdate, "title");
		if($title == "Auto Draft")
			UELM_UniteFunctionsUC::throwError("Please change the title, 'Auto Draft' is a bad title :), <br><br> The page will be published when you save");
		...
		if($postStatus == "auto-draft"){
			...
			//set import title
			$importTitle = UELM_UniteFunctionsUC::getVal($arrUpdate, "import_title");
		...
		if(!empty($arrUpdatePost)){
			...
			if($success == 0)
				UELM_UniteFunctionsUC::throwError("Unable to update page");
		}
		...
		if(array_key_exists("params", $arrUpdate)){
			...
			$params = UELM_UniteFunctionsUC::getVal($arrUpdate, "params");
			update_post_meta($postID, UELM_GlobalsProviderUC::META_KEY_LAYOUT_PARAMS, $params);			
		}		
		...
		if(array_key_exists("layout_data", $arrUpdate)){
			...
			update_post_meta($postID, UELM_GlobalsProviderUC::META_KEY_BLOX_PAGE, "true");
			$updated = update_post_meta($postID, UELM_GlobalsProviderUC::META_KEY_LAYOUT_DATA, $layoutData);
			
			//small validation
			if($updated == false){
				$oldData = $this->getRawLayoutData();
				if($oldData != $layoutData)
					UELM_UniteFunctionsUC::throwError("Unable to update page layout data");
			}
			
			//update post content with stripped html
			$layoutHtml = UELM_HelperUC::outputLayout($postID, true);
			$layoutHtml = UELM_UniteFunctionsUC::getPrettyHtml($layoutHtml);
	...
	public function delete(){
		...
		delete_metadata("post", $this->id, UELM_GlobalsProviderUC::META_KEY_BLOX_PAGE);
		delete_metadata("post", $this->id, UELM_GlobalsProviderUC::META_KEY_LAYOUT_DATA);
		delete_metadata("post", $this->id, UELM_GlobalsProviderUC::META_KEY_CATID);
		delete_metadata("post", $this->id, UELM_GlobalsProviderUC::META_KEY_LAYOUT_TYPE);
	...
	public function getLastRevision(){
		...
		if(empty($firstRevision))
			UELM_UniteFunctionsUC::throwError("Failed to get template revision");
	...
	private function getPostBaseName($postID){
		...
		$baseName = UELM_HelperUC::convertTitleToAlias($postTitle);
	...
	protected function getNewLayoutName($title, $importParams){
		...
		$parentID = UELM_UniteFunctionsUC::getVal($importParams, "parent_id");
	...
	public function getExportLayoutName(){
		...
		$layoutName = UELM_HelperUC::convertTitleToHandle($layoutName);
	...
	protected function isLayoutExistsByName($name, $layoutType){
		...
		$isExists = UELM_UniteFunctionsWPUC::isPostNameExists($name);
	...
	public function isGroup(){
		...
		$arrChildren = UELM_UniteFunctionsWPUC::getPostChildren($this->post);
	...
	public function getPreviewImage($getThumb = false){
		...
		$attachmentID = UELM_UniteFunctionsWPUC::getFeaturedImageID($this->id);
		
		$urlPreview = null;
		
		if(!empty($attachmentID)){
			if($getThumb == true)
				$urlPreview = UELM_UniteFunctionsWPUC::getUrlAttachmentImage($attachmentID, UELM_UniteFunctionsWPUC::THUMB_MEDIUM);
			else
				$urlPreview = UELM_UniteFunctionsWPUC::getUrlAttachmentImage($attachmentID);
		}
	...
	public function getDefaultPreviewImage(){
		...
		$urlPreview = UELM_HelperUC::getDefaultPreviewImage($typeName);
