...
class UELM_UniteProviderAdminUC extends UELM_UniteCreatorAdmin{
	...
	public function __construct($uelm_mainFilepath){
		...
		$this->uelm_mainFilepath = $uelm_mainFilepath;

		$mainFilename = basename($uelm_mainFilepath);

		$pathPlugin = str_replace('\\', "/", UELM_GlobalsUC::$pathPlugin);
		...
		UELM_UniteFunctionsUC::validateNotEmpty($this->defaultView, "default view");
		UELM_UniteFunctionsUC::validateNotEmpty($this->pluginTitle, "plugin title");

		//update globals
		UELM_GlobalsUC::$view_default = $this->defaultView;
		UELM_GlobalsUC::$defaultAddonType = $this->defaultAddonType;
	...
	public function createTables($isForce = false){
		...
		$response1 = $this->createTable(UELM_GlobalsUC::TABLE_ADDONS_NAME, $isForce);

		$response2 = $this->createTable(UELM_GlobalsUC::TABLE_CATEGORIES_NAME, $isForce);

		$isAddonChangelogEnabled = UELM_HelperProviderUC::isAddonChangelogEnabled();

		$response3 = null;

		if($isAddonChangelogEnabled === true)
			$response3 = $this->createTable(UELM_GlobalsUC::TABLE_CHANGELOG_NAME, $isForce);
		...
		if($isFormEntriesEnabled === true){
			$response4 = $this->createTable(UELM_GlobalsUC::TABLE_FORM_ENTRIES_NAME, $isForce);
			$response5 = $this->createTable(UELM_GlobalsUC::TABLE_FORM_ENTRY_FIELDS_NAME, $isForce);
		}
	...
	public function createTable($tableName, $isForce = false){
		...
		$tableRealName = UELM_UniteFunctionsWPUC::prefixDBTable($tableName);
		if($isForce == false && UELM_UniteFunctionsWPUC::isDBTableExists($tableRealName))
			return;

		$charset_collate = $wpdb->get_charset_collate();

		switch($tableName){
			case UELM_GlobalsUC::TABLE_LAYOUTS_NAME:
			...
			case UELM_GlobalsUC::TABLE_CATEGORIES_NAME:
				$sql = "CREATE TABLE " . $tableRealName . " (
					id INT(9) NOT NULL AUTO_INCREMENT,
					title VARCHAR(255) NOT NULL,
					alias VARCHAR(255),
			...
			case UELM_GlobalsUC::TABLE_ADDONS_NAME:
				$sql = "CREATE TABLE " . $tableRealName . " (
			...
			case UELM_GlobalsUC::TABLE_CHANGELOG_NAME:
				$isAddonChangelogEnabled = UELM_HelperProviderUC::isAddonChangelogEnabled();
			...
			case UELM_GlobalsUC::TABLE_FORM_ENTRIES_NAME:
				$isFormEntriesEnabled = UELM_HelperProviderUC::isFormEntriesEnabled();
			...
			case UELM_GlobalsUC::TABLE_FORM_ENTRY_FIELDS_NAME:
				$isFormEntriesEnabled = UELM_HelperProviderUC::isFormEntriesEnabled();
			...
			default:
				UELM_UniteFunctionsUC::throwError("table: $tableName not found");
			break;
		}
	...
	private function checkDBUpgrade(){
		...
		$optionDBVersion = "uelm_unitecreator_db_version";
	...
	protected function addActionAjax($ajaxAction, $eventFunction){
		...
		$this->addAction('wp_ajax_' . UELM_GlobalsUC::PLUGIN_NAME . "_" . $ajaxAction, $eventFunction, true);
		$this->addAction('wp_ajax_nopriv_' . UELM_GlobalsUC::PLUGIN_NAME . "_" . $ajaxAction, $eventFunction, true);
	}
	...
	protected function addEvent_onActivate($eventFunc = "onActivate"){
		...
		register_activation_hook($this->uelm_mainFilepath, array($this, $eventFunc));
	}
	...
	protected function isInsidePlugin(){
		...
		$page = UELM_UniteFunctionsUC::getGetVar("page", "", UELM_UniteFunctionsUC::SANITIZE_KEY);
	...
	protected function validateAdminPermissions(){
		...
		if(UELM_UniteFunctionsWPUC::isAdminPermissions() == false){
	...
	public function adminPages(){
		...
		UELM_GlobalsUC::$alterViewHeaderPrefix = $this->pluginTitle;

		if(!empty($this->arrAllowedViews) && in_array(self::$view, $this->arrAllowedViews) == false){
			echo esc_html__("this view not allowed in the plugin", "unlimited-elements");
	...
	public function addScriptsToAllAdminPages(){
		...
		UELM_HelperUC::addStyleAbsoluteUrl(UELM_GlobalsUC::$url_provider . "assets/provider_admin.css", "uc_provider_admin");
	}
	...
	public function onAddOutsideScripts(){
		...
		try{
			...
			$isPostsPage = UELM_UniteFunctionsWPUC::isAdminPostsPage();
	...
	public function onPrintFooterScripts(){
		...
		UELM_HelperProviderUC::onPrintFooterScripts();
	}
	...
	public function addAdminMenu_putActually($arrMenuPages, $arrSubMenuPages){
		...
		foreach($arrMenuPages as $mainMenu){
			...
			$pluginName = UELM_UniteFunctionsUC::getVal($mainMenu, "plugin_name");

			$menuSlug = UELM_UniteFunctionsUC::getVal($mainMenu, "slug");
		...
		foreach($arrSubMenuPages as $key => $submenuMenu){
			...
			$pluginName = UELM_UniteFunctionsUC::getVal($submenuMenu, "plugin_name");
	...
	public function addAdminMenu(){
		...
		//add plugins menu
		$arrMenuPages = UELM_UniteCreatorAdminWPPluginBase::getArrMenuPages();
		$arrSubMenuPages = UELM_UniteCreatorAdminWPPluginBase::getArrSubmenuPages();
	...
	private function importCurrentThemeAddons(){
		...
		$dirAddons = apply_filters("uelm_path_theme_addons", UELM_GlobalsUC::DIR_THEME_ADDONS);
	...
	protected function importPackageAddons(){
		...
		$pathAddons = UELM_GlobalsUC::$pathPlugin . self::DIR_INSTALL_ADDONS . "/";
	...
	private function onAfterUpdatePlugin(){
		...
		$urlRedirect = UELM_HelperUC::getViewUrl_Default();

		uelm_dmp("addons installed, redirecting...");
	...
	private function runProviderAction(){
		...
		$action = UELM_UniteFunctionsUC::getGetVar("provider_action", "", UELM_UniteFunctionsUC::SANITIZE_KEY);
		if(empty($action))
			return (false);
	...
	public function plugin_action_links($links){
		...
		$settings_link = sprintf('<a href="%s">%s</a>', admin_url('admin.php?page=' . Settings::PAGE_ID), __('Settings', "unlimited-elements"));

		array_unshift($links, $settings_link);

		$links['go_pro'] = sprintf('<a href="%s" target="_blank" class="elementor-plugins-gopro">%s</a>', Utils::get_pro_link('https://elementor.com/pro/?utm_source=wp-plugins&utm_campaign=gopro&utm_medium=wp-dash'), __('Go Pro', "unlimited-elements"));
	...
	private function loadBlankWindowAdminPage($superClear = false){
		...
		$title = UELM_UniteFunctionsWPUC::getAdminTitle(self::$adminTitle);

		UELM_HelperUC::addStyle("blank_page_preview", "uc_blank_page_preview");

		$arrCustomStyles = UELM_UniteProviderFunctionsUC::getCustomStyles();
		$htmlCustomCssStyles = UELM_HelperHtmlUC::getHtmlCustomStyles($arrCustomStyles);
	...
	private function checkMigrateFromAddonLibrary(){
		...
		$isMigrated = UELM_UniteProviderFunctionsUC::getOption($keyMigrate);
		...
		UELM_UniteProviderFunctionsUC::updateOption($keyMigrate, true);
	}
	...
	protected function modifyPluginVariables(){
		...
		if($this->pluginName == UELM_GlobalsUC::PLUGIN_NAME)
			return (false);
	...
	private function setUserCapability(){
		...
		$currentCapability = UELM_UniteFunctionsUC::getVal($allCaps, $this->capability);
	...
	public function checkRankmathAjaxCollision(){
		...
		$arrKeys = UELM_UniteFunctionsWPUC::getAllWPActionKeys("admin_init");
	...
	public function onAdminInit(){
		...
		UELM_GlobalsUC::initAfterAdminInit();
		...
		// run blank mode
		if(UELM_GlobalsUC::$blankWindowMode == true){

			$isSuperClear = UELM_UniteFunctionsUC::getGetVar("superclear", "", UELM_UniteFunctionsUC::SANITIZE_KEY);
			$isSuperClear = UELM_UniteFunctionsUC::strToBool($isSuperClear);
	...
	public function initAddonRevisioner(){
		...
		UELM_UniteCreatorAddonRevisioner::init();
	}
	...
	protected function validateInitValues(){
		...
		UELM_UniteFunctionsUC::validateNotEmpty($this->pluginName, "plugin name");
	}
	...
	protected function init(){
		...
		UELM_UniteProviderFunctionsUC::doAction(UELM_UniteCreatorFilters::ACTION_BEFORE_ADMIN_INIT);
		...
		if($this->isInsidePlugin() == true){
			...
			UELM_GlobalsUC::$isInsidePlugin = true;
		...
		UELM_UniteFunctionsWPUC::onAdminInit();
