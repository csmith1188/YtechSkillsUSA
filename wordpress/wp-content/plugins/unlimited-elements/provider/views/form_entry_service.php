...
class UELM_UCFormEntryService{
	...
	public function getTable(){
		...
		$table = UELM_UniteFunctionsWPUC::prefixDBTable(UELM_GlobalsUC::TABLE_FORM_ENTRIES_NAME);
	...
	public function getFieldsTable(){
		...
		$table = UELM_UniteFunctionsWPUC::prefixDBTable(UELM_GlobalsUC::TABLE_FORM_ENTRY_FIELDS_NAME);
	...
	private function getEntryMainField($fields){
		...
		if (empty($fields) || !is_array($fields)) {
		        return array(
		            "value" => __("(empty)", "unlimited-elements")
		        );
		}
		...
		foreach($fields as $field){
			...
			$isValidEmail = UELM_UniteFunctionsUC::isEmailValid($field["value"]);
		...
		if($main["value"] === "")
			$main["value"] = __("(empty)", "unlimited-elements");
