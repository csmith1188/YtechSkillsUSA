...
class UELM_BloxViewLayoutOuterProvider extends UELM_BloxViewLayoutOuter{
	...
	public function setPageTitle(){
		...
		if(!$this->layoutID)
			$title = esc_html__("New Page", "unlimited-elements");
		else{
			$title = $this->objLayout->getTitle(true);
			$title .= " - ".esc_html__("Edit Page", "unlimited-elements")."";
		}
		
		UELM_UniteProviderFunctionsUC::setAdminTitle($title);
	...
	private function checkModifyNewPage(){
		...
		if($status == "auto-draft"){
			...
			//update title
			$title = UELM_UniteFunctionsUC::getGetVar("title", "", UELM_UniteFunctionsUC::SANITIZE_TEXT_FIELD);
			if(empty($title)){
