...
class UELM_UniteCreatorWooIntegrate{
	...
	public static function initActions(){
		...
		add_action("ue_woocommerce_product_integrations", array("UELM_UniteCreatorWooIntegrate", "onInsideEditorWooProduct"), 10, 1);
		
		add_action("ue_woocommerce_product_integrations_bottom", array("UELM_UniteCreatorWooIntegrate", "onInsideEditorWooProductBottom"), 10, 1);
	...
	private function init(){
		...
		$this->urlCurrentPage = UELM_UniteFunctionsWPUC::getUrlCurrentPage();
	...
	public static function getInstance(){
		...
		if(empty(self::$instance))
			self::$instance = new UELM_UniteCreatorWooIntegrate();
	...
	private function addAddToCartForVariation($arrVariation){
		...
		$variationID = UELM_UniteFunctionsUC::getVal($arrVariation, "variation_id");
		$sku = UELM_UniteFunctionsUC::getVal($arrVariation, "sku");
		
		$params = "add-to-cart={$variationID}";
		
		$urlAddCart = UELM_UniteFunctionsUC::addUrlParams($this->urlCurrentPage, $params);
		
    	$arrVariation["link_addcart_cart"] = UELM_UniteFunctionsUC::addUrlParams($this->urlCart, $params);
    	$arrVariation["link_addcart_checkout"] = UELM_UniteFunctionsUC::addUrlParams($this->urlCheckout, $params);
	...
	private function addAddToCartData($arrProduct, $productID, $productSku){
		...
		$urlAddCart = UELM_UniteFunctionsUC::addUrlParams($this->urlCurrentPage, $params);
    	$type = UELM_UniteFunctionsUC::getVal($arrProduct, "woo_type");
		
    	$arrProduct["woo_link_addcart_cart"] = UELM_UniteFunctionsUC::addUrlParams($this->urlCart, $params);
    	$arrProduct["woo_link_addcart_checkout"] = UELM_UniteFunctionsUC::addUrlParams($this->urlCheckout, $params);
	...
	private function addPricesFromTo($arrProduct, $arrPrices, $objProduct){
		...
		$regularPriceFrom = UELM_UniteFunctionsUC::getVal($arrProduct, "woo_regular_price_from");
		$regularPriceTo = UELM_UniteFunctionsUC::getVal($arrProduct, "woo_regular_price_to");
		
		$salePriceFrom = UELM_UniteFunctionsUC::getVal($arrProduct, "woo_sale_price_from");
	...
	private function getVariationAttributeTitle($slug, $value, $arrTitles){
		...
		$title = UELM_UniteFunctionsUC::getVal($arrTitles, $key);
		
		if(empty($title))
			$title = UELM_UniteFunctionsUC::getVal($arrTitles, $slug);
	...
	private function addTitlesForVariation($arrVariation, $arrAttributeTitles){
		...
		$arrAttributes = UELM_UniteFunctionsUC::getVal($arrVariation, "attributes");
		...
		foreach($arrAttributes as $slug=>$value){
			...
			$attrTitle = UELM_UniteFunctionsUC::getVal($arrTitle, "attr");
			$valueTitle = UELM_UniteFunctionsUC::getVal($arrTitle, "title");
	...
	private function getAttributeTitles_tax($arrOutput, $objAttribute, $product){
			...
			$attributeLabel = UELM_UniteFunctionsUC::getVal($attribute_taxonomy, "attribute_label");
	...
	private function combineAttributesByName($arrAttributes){
		...
		foreach($arrAttributes as $name=>$arr){
			...
			$name = UELM_UniteFunctionsUC::getVal($arr, "attr");
			$value = UELM_UniteFunctionsUC::getVal($arr, "title");
			
			$arrAttribute = UELM_UniteFunctionsUC::getVal($arrCombined, $name);
	...
	public function getProductGallery($productID){
		...
		foreach($arrAttachmentIDs as $id){
			...
			$data = UELM_UniteFunctionsWPUC::getAttachmentData($id);
	...
	private function getProductData($productID){
    	...
    	$productSku = UELM_UniteFunctionsUC::getVal($arrData, "sku");
    	
    	$salePrice = UELM_UniteFunctionsUC::getVal($arrData, "sale_price");
    	$regularPrice = UELM_UniteFunctionsUC::getVal($arrData, "regular_price");
    	$price = UELM_UniteFunctionsUC::getVal($arrData, "price");
    	...
    	foreach($arrProperties as $propertyName){
    		...
    		$value = UELM_UniteFunctionsUC::getVal($arrData, $propertyName);
    	...
    	$arrWooStars = array();
    	$rating = UELM_UniteFunctionsUC::getVal($arrData, "average_rating");
    	$rating = floatval($rating);
    	
    	$arrWooStars = UELM_HelperHtmlUC::getRatingArray($rating);
    	...
    	$finalPrice = UELM_UniteFunctionsUC::getVal($arrProduct, "woo_price");
    	$regularPrice = UELM_UniteFunctionsUC::getVal($arrProduct, "woo_regular_price");
	...
	public function getProductIDsFromCurrentPostContent(){
		...
		$arrLinks = UELM_UniteFunctionsUC::parseHTMLGetLinks($content);
	...
	public function checkModifyQueryClauses($args, $excludeOutofStockVariation, $showDebug){
		...
		$orderby = UELM_UniteFunctionsWPUC::SORTBY_SALES;
				
		switch($orderby){
			case UELM_UniteFunctionsWPUC::SORTBY_SALES:
			case UELM_UniteFunctionsWPUC::SORTBY_RATING:

				if($showDebug == true)
					uelm_dmp("modify post query for orderby:".$orderby);
		...
		if($excludeOutofStockVariation == true){
			...
			if(!empty($termsArray) && UELM_GlobalsProviderUC::$isUnderAjax == true
				&& count($termsArray) <= 2)
	...
	public function modifyWCSortbyQuery($arrClauses){
		...
		if(empty(UELM_GlobalsProviderUC::$lastQueryArgs))
			return($arrClauses);
		...
		$orderBY = UELM_UniteFunctionsUC::getVal($args, "orderby");
		$dir = UELM_UniteFunctionsUC::getVal($args, "order", "DESC");
		...
		switch($orderBY){
			...
			case 'rating':
				...
				if($dir == "ASC"){
					$orderby = UELM_UniteFunctionsUC::getVal($arrClauses, "orderby");
	...
	public static function onInsideEditorWooProductBottom($productID){
		...
		UELM_UniteCreatorPluginIntegrations::putJetWooWishlistButton();
	...
	public static function getWooEndpoint($type){
		...
		switch($type){
			...
			default:
				$type = esc_html($type);
				UELM_UniteFunctionsUC::throwError("get_woo_endpoints error: wrong endpoint type: $type, 
	...
	public static function getCurrentProductVariationImageItems(){
		...
		foreach($arrVariations as $variation){
			...
			$imageID = UELM_UniteFunctionsUC::getVal($variation, "image_id");
	...
	public function getVariationTermsFromQueryArgs($args) {
		...
		$taxQuery = UELM_UniteFunctionsUC::getVal($args, "tax_query");
		if (empty($taxQuery)) {
			return array();
		}
	...
	public function getCartProductIDs(){
		...
		foreach($arrCartItems as $item){
			...
			$productID = UELM_UniteFunctionsUC::getVal($item, "product_id");
	...
	public function getCartNumProducts(){
		...
		foreach($arrItems as $item){
			...
			$quantity = UELM_UniteFunctionsUC::getVal($item, "quantity");
	...
	public function removeFromCartFromData(){
		...
		try{
			...
			if(self::isWooActive() == false)
				UELM_UniteFunctionsUC::throwError("woocommerce not active");
			
			$itemKey = UELM_UniteFunctionsUC::getGetVar("key","",UELM_UniteFunctionsUC::SANITIZE_KEY);
			
			UELM_UniteFunctionsUC::validateNotEmpty($itemKey,"cart item key");
			...
			if(empty($item))
				UELM_UniteFunctionsUC::throwError("Cart Item not found");
			
			
			$success = $cart->remove_cart_item($itemKey);
	
			if($success == false)
				UELM_UniteFunctionsUC::throwError("Cart item not exists.");
	...
	public function updateCartQuantityFromData(){
		...
		try{
			...
			$quantity = UELM_UniteFunctionsUC::getGetVar("quantity","",UELM_UniteFunctionsUC::SANITIZE_ID);
			
			UELM_UniteFunctionsUC::validateNotEmpty($itemKey,"cart quantity");
			
			if(empty($quantity))
				$quantity = 0;
			
			if(is_numeric($quantity) == false)
				UELM_UniteFunctionsUC::throwError("quantiy needs to be numeric");
			...
			if($success == false)
				UELM_UniteFunctionsUC::throwError("Cart item not exists, or unable to update quantity.");
	...
	public function getCartItemsHtml(){
		...
		foreach($arrCartItems as $cart_item_key=>$cart_item){
		    ...
		    $urlImage = null;
		    if(!empty($imageID))
		    	$urlImage = wp_get_attachment_image_url( $imageID, UELM_UniteFunctionsWPUC::THUMB_SMALL );
...
";
	...
	public function getCartTotalsHtml($isSecond = false){
		...
		if(empty(WC()->cart)){
			...
			$subtotal = UELM_UniteFunctionsUC::getVal($arrTotals, "subtotal");
			$subtotalTax = UELM_UniteFunctionsUC::getVal($arrTotals, "subtotal_tax");
...
";
   ...
   return($html);
	...
	public function putCartHtml($type){
		...
		switch($type){
			...
			default:
				...
				$html = UELM_HelperHtmlUC::getErrorMessageHtml($message,"",true);
			break;
		}
		uelm_echo($html);
	}
	...
	public static function initMiniCartIntegration(){
		...
		add_filter( 'woocommerce_add_to_cart_fragments', array("UELM_UniteCreatorWooIntegrate", "onAddCartFragment") );
