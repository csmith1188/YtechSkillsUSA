...
class UELM_UniteProviderFunctionsUC{
	...
	public static function initGlobalsBase(){
		...
		UELM_GlobalsUC::$table_prefix = $tablePrefix;

		self::$tablePosts = $tablePrefix."posts";
		self::$tablePostMeta = $tablePrefix."postmeta";

		UELM_GlobalsUC::$table_addons = $tablePrefix.UELM_GlobalsUC::TABLE_ADDONS_NAME;
		UELM_GlobalsUC::$table_categories = $tablePrefix.UELM_GlobalsUC::TABLE_CATEGORIES_NAME;

		$pluginUrlAdminBase = UELM_GlobalsUC::PLUGIN_NAME;

		UELM_GlobalsUC::$pathPlugin = realpath(dirname(__FILE__)."/../")."/";

		$pluginName = basename(UELM_GlobalsUC::$pathPlugin);

		UELM_GlobalsUC::$path_base = ABSPATH;

		UELM_GlobalsUC::$pathPlugin = UELM_UniteFunctionsUC::pathToUnix(UELM_GlobalsUC::$pathPlugin);
		UELM_GlobalsUC::$path_base = UELM_UniteFunctionsUC::pathToUnix(UELM_GlobalsUC::$path_base);

		//protection against wrong base path (happends at some hostings subdomain)
		if(strpos(UELM_GlobalsUC::$path_base, UELM_GlobalsUC::$pathPlugin) === false){
			UELM_GlobalsUC::$path_base = realpath(UELM_GlobalsUC::$pathPlugin."../../../")."/";
			UELM_GlobalsUC::$path_base = UELM_UniteFunctionsUC::pathToUnix(UELM_GlobalsUC::$path_base);
		}
		...
		UELM_GlobalsUC::$path_images = $arrUploadDir["basedir"]."/";

		//set cache folder

		try{

			UELM_GlobalsUC::$path_cache = UELM_GlobalsUC::$path_images."uelm_cache/";
			UELM_UniteFunctionsUC::mkdirValidate(UELM_GlobalsUC::$path_cache, "cache folder");

			//create index.html
			UELM_UniteFunctionsUC::writeFile("", UELM_GlobalsUC::$path_cache."index.html");

		}catch(Exception $e){

			UELM_GlobalsUC::$path_cache = UELM_GlobalsUC::$pathPlugin."cache/";
		}

		UELM_GlobalsUC::$url_base = site_url()."/";
		UELM_GlobalsUC::$urlPlugin = plugins_url($pluginName)."/";

		UELM_GlobalsUC::$url_component_admin = admin_url()."admin.php?page=$pluginUrlAdminBase";
		...
		UELM_GlobalsUC::$url_images = $arrUploadDir["baseurl"]."/";

		UELM_GlobalsUC::$url_ajax = admin_url("admin-ajax.php","relative");
		UELM_GlobalsUC::$url_ajax_full = admin_url("admin-ajax.php");

		UELM_GlobalsUC::$url_ajax_front = UELM_GlobalsUC::$url_ajax;

		UELM_GlobalsUC::$is_admin = self::isAdmin();

		UELM_GlobalsUC::$url_provider = UELM_GlobalsUC::$urlPlugin."provider/";

		UELM_GlobalsUC::$url_default_addon_icon = UELM_GlobalsUC::$url_provider."assets/images/icon_default_addon.png";

		UELM_GlobalsUC::$is_ssl = is_ssl();

		self::setAssetsPath();

		UELM_GlobalsUC::$url_assets_libraries = UELM_GlobalsUC::$urlPlugin."assets_libraries/";

		//UELM_GlobalsUC::$view_default set in admin class

		UELM_GlobalsUC::$url_assets_internal = UELM_GlobalsUC::$urlPlugin."assets_internal/";

		UELM_GlobalsUC::$layoutShortcodeName = "blox_layout";

		UELM_GlobalsUC::$enableWebCatalog = true;

		$window = UELM_UniteFunctionsUC::getGetVar("ucwindow","",UELM_UniteFunctionsUC::SANITIZE_KEY);
		if($window === "blank")
			UELM_GlobalsUC::$blankWindowMode = true;
	...
	*/
	public static function setAssetsPath($dirAssets = null, $returnValues = false){
		...
		$uploadsBaseDir = UELM_UniteFunctionsUC::getVal($arrUploads, "basedir");
		$uploadsBaseUrl = UELM_UniteFunctionsUC::getVal($arrUploads, "baseurl");
		...
		if(is_dir($uploadsBaseDir)){
			$pathBase = UELM_UniteFunctionsUC::addPathEndingSlash($uploadsBaseDir);
			$urlBase = UELM_UniteFunctionsUC::addPathEndingSlash($uploadsBaseUrl);
		}
		...
		UELM_UniteFunctionsUC::mkdir($pathAssets);

		if(is_dir($pathAssets) == false)
			UELM_UniteFunctionsUC::throwError("Can't create folder: {$pathAssets}");
		...
		if(empty($pathAssets))
			UELM_UniteFunctionsUC::throwError("Cannot set assets path");

		if(empty($urlAssets))
			UELM_UniteFunctionsUC::throwError("Cannot set assets url");
		...
		if($returnValues == true){
			...
			UELM_GlobalsUC::$url_assets = $urlAssets;
		}
	...
	public static function addScriptsFramework($specialSettings = ""){
		...
		UELM_UniteFunctionsWPUC::addMediaUploadIncludes();
		...
		if($specialSettings != "nojqueryui"){
			UELM_HelperUC::addStyle("jquery-ui.structure.min","jui-smoothness-structure","css/jui/new");
			UELM_HelperUC::addStyle("jquery-ui.theme.min","jui-smoothness-theme","css/jui/new");
		}
	...
	public static function addScript($handle, $url, $inFooter = false, $deps = array()){
		...
		if(empty($url))
			UELM_UniteFunctionsUC::throwError("empty script url, handle: $handle");

		$version = UELM_UNLIMITED_ELEMENTS_VERSION;
		if(UELM_GlobalsUC::$inDev == true)	//add script
			$version = time();
	...
	public static function addStyle($handle, $url, $deps = array()){
		...
		if(empty($url))
			UELM_UniteFunctionsUC::throwError("empty style url, handle: $handle");
	...
	public static function printCustomScript($script, $hardCoded = false, $isModule = false, $handle = null, $isPutOnce = false){
		...
		if(isset(self::$arrScripts[$handle])){
			...
			$handle .= "_". UELM_UniteFunctionsUC::getRandomString(5, true);
		}
	...
	*/
	...
	public static function getUrlJQueryInclude(){
		...
		$url = UELM_GlobalsUC::$url_base."wp-includes/js/jquery/jquery".".js";
	...
	public static function getUrlJQueryMigrateInclude(){
		...
		$url = UELM_GlobalsUC::$url_base."wp-includes/js/jquery/jquery-migrate".".js";
	...
	public static function sanitizeVar($var, $type){
		...
		switch($type){
			case UELM_UniteFunctionsUC::SANITIZE_ID:
			...
			break;
			case UELM_UniteFunctionsUC::SANITIZE_KEY:
			...
			break;
			case UELM_UniteFunctionsUC::SANITIZE_TEXT_FIELD:
				$var = sanitize_text_field($var);
			break;
			case UELM_UniteFunctionsUC::SANITIZE_NOTHING:
			break;
			default:
				UELM_UniteFunctionsUC::throwError("Wrong sanitize type: " . $type);
			break;
		}
	 ...
	 */
	public static function getImageUrlFromImageID($imageID, $size = UELM_UniteFunctionsWPUC::THUMB_FULL){

		$urlImage = UELM_UniteFunctionsWPUC::getUrlAttachmentImage($imageID, $size);
	 ...
	 * get image url from image id
	 */
	public static function getThumbUrlFromImageID($imageID, $size = UELM_UniteFunctionsWPUC::THUMB_MEDIUM){
		...
		switch($size){
			case UELM_GlobalsUC::THUMB_SIZE_NORMAL:
				$size = UELM_UniteFunctionsWPUC::THUMB_MEDIUM;
			break;
			case UELM_GlobalsUC::THUMB_SIZE_LARGE:
				$size = UELM_UniteFunctionsWPUC::THUMB_LARGE;
			break;
		}
	...
	public static function getImageIDFromUrl($urlImage){
		...
		$imageID = UELM_UniteFunctionsWPUC::getAttachmentIDFromImageUrl($urlImage);
	...
	public static function putFooterTextLine(){
		?>
			&copy; <?php esc_html_e("All rights reserved","unlimited-elements")?>, <a href="https://unlimited-elements.com" target="_blank"><?php echo esc_attr(UELM_GlobalsUnlimitedElements::$pluginTitleCurrent) ?></a>. &nbsp;&nbsp;
		<?php
	}
	...
	public static function addSystemConstantData($data){
		...
		$data["uc_url_blog"] = UELM_UniteFunctionsWPUC::getUrlBlog();
		...
		if($isWPMLExists == true){
			...
			$data["uc_lang"] = UELM_UniteFunctionsWPUC::getLanguage();
		}
	...
	public static function getNonceName(){
		...
		$name = UELM_GlobalsUC::PLUGIN_NAME."_actions_{$userID}";
	...
	public static function verifyNonce($nonce){
		...
		if(function_exists("wp_verify_nonce") == false){
			...
			uelm_dmp("verify nonce function not found. some other plugin interrupting this call");
			uelm_dmp("please find it in this trace by follow 'wp-content/plugins'");
		...
		if($verified == false)
			UELM_UniteFunctionsUC::throwError("Action security failed, please refresh the page and try again.");
	...
	public static function sendEmail($emailTo, $subject, $message){
		...
		if($isSent == false)
			UELM_UniteFunctionsUC::throwError("The mail is not sent");
	...
	public static function setAdminTitle($title){
		...
		if(UELM_GlobalsUC::$is_admin == false)
			UELM_UniteFunctionsUC::throwError("The function works only in admin area");

		UELM_UniteProviderAdminUC::$adminTitle = $title;
	}
