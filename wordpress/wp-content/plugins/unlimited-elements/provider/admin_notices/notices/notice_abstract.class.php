...
abstract class UELM_UCAdminNoticeAbstract{
	...
	public function isDebug(){
		...
		if(UELM_GlobalsUnlimitedElements::$debugAdminNotices === true)
			return true;
	...
	protected function createBuilder(){
		...
		$builder = new UELM_UCAdminNoticeBuilder($id);
	...
	protected function createBannerBuilder(){
		...
		$builder = new UELM_UCAdminNoticeBannerBuilder($id);
		$builder = $this->initBuilder($builder);
	...
	private function getDebugData(){
		...
		if($currentTime < $startTime)
			return "Notice <b>{$id}</b> hidden - scheduled (will be visible on " . uelm_date($dateFormat, $startTime) . ")";
			
		$finishTime = $this->getFinishTime();

		if($currentTime <= $finishTime)
			return "Notice <b>{$id}</b> visible (will be hidden on ". uelm_date($dateFormat, $finishTime). ")";
	...
	private function isDismissed(){
		...
		$isDismissed = UELM_UniteFunctionsUC::strToBool($isDismissed);
	...
	private function isFreeAllowed(){
		...
		if($this->freeOnly === true && UELM_GlobalsUC::$isProVersion === true)
			return false;
	...
	private function isLocationAllowed(){
		...
		switch($this->location){
			...
			case self::LOCATION_PLUGIN:
				...
				if($page === UELM_GlobalsUnlimitedElements::PLUGIN_NAME)
					return true;
			break;
		}
	...
	private function getStartTime(){
		...
		$installTime = intval(UELM_UCAdminNoticesOptions::getOption('install_time'));
	...
	private function getOption($key, $fallback = null){
		...
		$value = UELM_UCAdminNoticesOptions::getNoticeOption($this->getId(), $key, $fallback);
	...
	private function setOption($key, $value){
		...
		UELM_UCAdminNoticesOptions::setNoticeOption($this->getId(), $key, $value);
	}
	...
	private function deleteOption($key){
		...
		UELM_UCAdminNoticesOptions::deleteNoticeOption($this->getId(), $key);
	}
