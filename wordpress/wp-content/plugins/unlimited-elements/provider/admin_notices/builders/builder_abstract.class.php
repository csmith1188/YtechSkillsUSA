...
abstract class UELM_UCAdminNoticeBuilderAbstract{
	...
	protected function getDismissHtml(){
		...
		$text = __('Dismiss', 'unlimited-elements');
		$title = __('Dismiss Notice', 'unlimited-elements');
	...
	protected function getDismissAjaxUrl(){
		...
		$ajaxUrl = UELM_HelperUC::getUrlAjax('dismiss_notice');
		$ajaxUrl = UELM_UniteFunctionsUC::addUrlParams($ajaxUrl, array('id' => $this->id));
	...
	protected function getPostponeAjaxUrl($duration){
		...
		$ajaxUrl = UELM_HelperUC::getUrlAjax('postpone_notice');
		$ajaxUrl = UELM_UniteFunctionsUC::addUrlParams($ajaxUrl, array('id' => $this->id, 'duration' => $duration));
