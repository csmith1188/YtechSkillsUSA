...
class UELM_ProviderOperationsUC extends UELM_UCOperations{
	...
	private function getSearchFromData($data){
		...
		$type = UELM_UniteFunctionsUC::getVal($data, "_type");

		if($type != "query")
			return(null);

		$searchTerm = UELM_UniteFunctionsUC::getVal($data, "q");
	...
	public function getSelect2TermsTitles($data){
		...
		$arrIDs = UELM_UniteFunctionsUC::getVal($data, "post_ids");
	...
	public function getSelect2PostTitles($data){
		...
		$arrTypesAssoc = UELM_UniteFunctionsWPUC::getPostTypesAssoc(array(), true);
		
		if(empty($arrIDs))
			return(null);

		$response = UELM_UniteFunctionsWPUC::getPostTitlesByIDs($arrIDs);
		...
		foreach($response as $record){
			...
			$id = UELM_UniteFunctionsUC::getVal($record, "id");
			$title = UELM_UniteFunctionsUC::getVal($record, "title");
			$postType = UELM_UniteFunctionsUC::getVal($record, "type");

			$typeTitle = UELM_UniteFunctionsUC::getVal($arrTypesAssoc, $postType);
	...
	public function getTermsListForSelectFromData($data){
		...
		$taxonomy = UELM_UniteFunctionsUC::getVal($data, "taxonomy");
	...
	private function getUserResult($user){
		...
		$cap = UELM_UniteFunctionsUC::getFirstNotEmptyKey($caps);

		$name = UELM_UniteFunctionsUC::getVal($data, "display_name");

		if(empty($name))
			$name = UELM_UniteFunctionsUC::getVal($data, "user_nicename");

		if(empty($name))
			$name = UELM_UniteFunctionsUC::getVal($data, "user_login");

		if(empty($name))
			$name = UELM_UniteFunctionsUC::getVal($data, "user_email");

		$userID = UELM_UniteFunctionsUC::getVal($data, "ID");
	...
	public function getPostListForSelectFromData($data, $addNotSelected = false, $limit = 10){
		...
		$filterPostType = UELM_UniteFunctionsUC::getVal($data, "post_type");

		switch($filterPostType){
			case "product":
				$arrTypesAssoc = array("product" => __("Product","unlimited-elements"));
			break;
			case "elementor_template":
				$arrTypesAssoc = array("elementor_library" => __("Template","unlimited-elements"));
			break;
			default:
				$arrTypesAssoc = UELM_UniteFunctionsWPUC::getPostTypesAssoc(array(), true);
			break;
		...
		if($addNotSelected == true){
			$arr = array();
			$arr["id"] = 0;
			$arr["text"] = __("[please select post]", "unlimited-elements");
			$arrResult[] = $arr;
		}
		...
		foreach($response as $post){
			...
			$postTypeTitle = UELM_UniteFunctionsUC::getVal($arrTypesAssoc, $postType);
	...
	public function getLinkAutocompleteFromData($data){
		...
		$query = UELM_UniteFunctionsUC::getVal($data, "query");
		$limit = 20;
		$results = array();
