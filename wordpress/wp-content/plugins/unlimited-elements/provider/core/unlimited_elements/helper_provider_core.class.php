...
class UELM_HelperProviderCoreUC_EL{
	...
	public static function registerPostType_UnlimitedLibrary(){
		...
		$arrLabels = array(
						'name' => __( 'Unlimited Elements Library' ,"unlimited-elements"),
						'singular_name' => __( 'Unlimited Elements Library' ,"unlimited-elements"),
						'add_new_item' => __( 'Add New Template' ,"unlimited-elements"),
						'edit_item' => __( 'Edit Template' ,"unlimited-elements"),
						'new_item' => __( 'New Template' ,"unlimited-elements"),
						'view_item' => __( 'View Template' ,"unlimited-elements"),
						'view_items' => __( 'View Template' ,"unlimited-elements"),
						'search_items' => __( 'Search Template' ,"unlimited-elements"),
						'not_found' => __( 'No Template Found' ,"unlimited-elements"),
						'not_found_in_trash' => __( 'No Template found in trash' ,"unlimited-elements"),
						'all_items' => __( 'All Templates' ,"unlimited-elements")
				);
		...
		$arrPostType =	array(
							...
							'hierarchical' => true,
							'description' => __("Unlimited Elements Template", "unlimited-elements"),
							'supports' => $arrSupports,
							//'show_in_admin_bar' => true
					);


		register_post_type( UELM_GlobalsUnlimitedElements::POSTTYPE_UNLIMITED_ELEMENS_LIBRARY, $arrPostType);

		add_post_type_support( UELM_GlobalsUnlimitedElements::POSTTYPE_UNLIMITED_ELEMENS_LIBRARY, 'elementor' );
	...
	public static function removeElementorPostCacheFile($postID){
		...
		$pathFiles = UELM_GlobalsUC::$path_images."elementor/css/";
	...
	public static function processParamValueByType($value, $type, $param){
    		...
    		switch($type){
    			...
    			case UELM_UniteCreatorDialogParam::PARAM_RADIOBOOLEAN:
    				...
    				$trueValue = UELM_UniteFunctionsUC::getVal($param, "true_value");
    				$falseValue = UELM_UniteFunctionsUC::getVal($param, "false_value");
	...
	public static function getGeneralSettingsValues(){
		...
		$arrValues = self::$operations->getCustomSettingsObjectValues(self::$filepathGeneralSettings, UELM_GlobalsUnlimitedElements::GENERAL_SETTINGS_KEY);
	...
	public static function isInstagramSetUp(){
		...
		$settings = UELM_HelperProviderCoreUC_EL::getGeneralSettingsValues();

		$token = UELM_UniteFunctionsUC::getVal($settings, "instagram_access_token");
	...
	public static function getGeneralSetting($name){
		...
		if(isset($arrSettings[$name]) == false)
			UELM_UniteFunctionsUC::throwError("Setting: $name does not exists in unlimited elements");
	...
	public static function registerWidgetByName($name){
		...
		$isAlphaNumeric = UELM_UniteFunctionsUC::isAlphaNumeric($name);
		...
		// class_alias('UELM_UniteCreatorElementorWidget', $className);
		$code = "class {$className} extends UELM_UniteCreatorElementorWidget{}";
		// phpcs:ignore Generic.PHP.ForbiddenFunctions.Found
		eval($code);

		$widget = new $className();
        \UELM_Elementor\Plugin::instance()->widgets_manager->register($widget);
	...
	public static function getImportedElementorTemplates($paramName = null){
		...
		$args = array();
		$args["post_type"] = UELM_GlobalsUnlimitedElements::POSTTYPE_ELEMENTOR_LIBRARY;
		$args["posts_per_page"] = 1000;
		$args["meta_key"] = UELM_GlobalsUnlimitedElements::META_TEMPLATE_SOURCE;
		...
		foreach($arrPosts as $post){
			...
			$sourceName = get_post_meta($postID, UELM_GlobalsUnlimitedElements::META_TEMPLATE_SOURCE_NAME, true);
	...
	public static function getArrElementorTemplatesShort(){
		...
		foreach($arrPosts as $post){
			...
			$postID = UELM_UniteFunctionsUC::getVal($post, "ID");
			$title = UELM_UniteFunctionsUC::getVal($post, "post_title");
			$slug = UELM_UniteFunctionsUC::getVal($post, "post_name");
			$postType = UELM_UniteFunctionsUC::getVal($post, "post_type");
	...
	public static function getImportedElementorTemplateID($name){
		...
		$templateID = UELM_UniteFunctionsUC::getVal($arrTemplates, $name);
	...
	private static function getWidgetNamesFromElementorContent_iterate($arrContent, $withBG){
		...
		foreach($arrContent as $item){
			...
			$type = UELM_UniteFunctionsUC::getVal($item, "elType");
			
			if($type == "widget"){

				$widgetName = UELM_UniteFunctionsUC::getVal($item, "widgetType");
			...
			if($withBG == true){
				...
				$backgroundType = UELM_UniteFunctionsUC::getVal($item, "uc_background_type");
	...
	public static function includeHoverAnimationsStyles($value = ""){
		...
		if(class_exists("\UELM_Elementor\Control_Hover_Animation") == false)
			return(false);
		...
		UELM_UniteProviderFunctionsUC::addStyle("e-animations", $urlAnimationsCss);
	}
	...
	public static function getHoverAnimations(){
		...
		$animations = array(
			"grow" => __("Grow", "unlimited-elements"),
			"shrink" => __("Shrink", "unlimited-elements"),
			"pulse" => __("Pulse", "unlimited-elements"),
			"pulse-grow" => __("Pulse Grow", "unlimited-elements"),
			"pulse-shrink" => __("Pulse Shrink", "unlimited-elements"),
			"push" => __("Push", "unlimited-elements"),
			"pop" => __("Pop", "unlimited-elements"),
			"bounce-in" => __("Bounce In", "unlimited-elements"),
			"bounce-out" => __("Bounce Out", "unlimited-elements"),
			"rotate" => __("Rotate", "unlimited-elements"),
			"grow-rotate" => __("Grow Rotate", "unlimited-elements"),
			"float" => __("Float", "unlimited-elements"),
			"sink" => __("Sink", "unlimited-elements"),
			"bob" => __("Bob", "unlimited-elements"),
			"hang" => __("Hang", "unlimited-elements"),
			"skew" => __("Skew", "unlimited-elements"),
			"skew-forward" => __("Skew Forward", "unlimited-elements"),
			"skew-backward" => __("Skew Backward", "unlimited-elements"),
			"wobble-vertical" => __("Wobble Vertical", "unlimited-elements"),
			"wobble-horizontal" => __("Wobble Horizontal", "unlimited-elements"),
			"wobble-to-bottom-right" => __("Wobble To Bottom Right", "unlimited-elements"),
			"wobble-to-top-right" => __("Wobble To Top Right", "unlimited-elements"),
			"wobble-top" => __("Wobble Top", "unlimited-elements"),
			"wobble-bottom" => __("Wobble Bottom", "unlimited-elements"),
			"wobble-skew" => __("Wobble Skew", "unlimited-elements"),
			"buzz" => __("Buzz", "unlimited-elements"),
			"buzz-out" => __("Buzz Out", "unlimited-elements"),
		);
	...
	public static function getHoverAnimationClasses($addNotChosen = false){
		...
		if(class_exists("\UELM_Elementor\Control_Hover_Animation") === true)
			$animations = \UELM_Elementor\Control_Hover_Animation::get_animations();
		else
		...
		if($addNotChosen === true)
			$animationClasses[""] = __("Not Chosen", "unlimited-elements");
	...
	public static function getBreakPoints($onlyCustom = true){
		...
		$arrBreakpoints = UELM_Elementor\Plugin::$instance->breakpoints->get_breakpoints();
	...
	public static function getElementorContentByPostID($postID){
		...
		$arrData = UELM_UniteFunctionsUC::jsonDecode($strData);
	...
	public static function getAddonWithDataFromContent($arrContent, $elementID){
		...
		if(empty($arrElement)){
			UELM_UniteFunctionsUC::throwError("Elementor Widget with id: $elementID not found");
		}
		...
		if(strpos($widgetType, "ucaddon_") === false){
			...
			if($widgetType == "global")
				UELM_UniteFunctionsUC::throwError("The widget can't be global widget. Please change the grid to regular widget.");
    ...
    private static function collectPostsWidgetsByRecord($record){
    	...
    	$config = UELM_UniteFunctionsUC::getVal($record, "config");
    	if(empty($config))
    		return(false);

    	$arrConfig = UELM_UniteFunctionsUC::jsonDecode($config);

    	$params = UELM_UniteFunctionsUC::getVal($arrConfig, "params");
    	if(empty($params))
    		return(false);
    	...
    	foreach($params as $param){
    		...
    		if($type != UELM_UniteCreatorDialogParam::PARAM_POSTS_LIST && $type != UELM_UniteCreatorDialogParam::PARAM_LISTING)
    			continue;

    		//post list only
    		$widgetName = UELM_UniteFunctionsUC::getVal($record, "alias");
	...
	public static function getPreloadDBData($isOutputPage = false){
    	...
    	$tableCats = UELM_GlobalsUC::$table_categories;
    	$tableAddons = UELM_GlobalsUC::$table_addons;
    	$addonType = UELM_GlobalsUnlimitedElements::ADDONSTYPE_ELEMENTOR;
    	$addonTypeBG = UELM_GlobalsUC::ADDON_TYPE_BGADDON;
    	...
    	foreach($arrRecords as $record){
    		...
    		$recordAddonType = UELM_UniteFunctionsUC::getVal($record, "addontype");
    		...
    		$catID = UELM_UniteFunctionsUC::getVal($record, "catid");
    		$catTitle = UELM_UniteFunctionsUC::getVal($record, "cat_title");
    		$catAlias = UELM_UniteFunctionsUC::getVal($record, "cat_alias");
    		$catOrdering = UELM_UniteFunctionsUC::getVal($record, "cat_ordering");
	...
	public static function getCachedFrameContent($strTemplateIDs){
		...
		UELM_UniteFunctionsUC::validateIDsList($strTemplateIDs,"template ids");
	...
	public static function putElementorTemplate($templateID, $mode = null){
		...
		if(self::$arrTemplatesCounter[$templateID] >= $numTemplates){
			...
			$text = sprintf(__("Infinite Template Loop Found with id: %s","unlimited-elements"), $templateID);
		...
		if($mode == "no_ue_widgets")	//in dynamic popup for example
			UELM_GlobalsProviderUC::$isUnderNoWidgetsToDisplay = true;

		$output = self::getElementorTemplate($templateID);


		if($mode == "no_ue_widgets")
			UELM_GlobalsProviderUC::$isUnderNoWidgetsToDisplay = false;
	...
	public static function getElementorTemplate($templateID, $withCss = false){
		...
		$output = \UELM_Elementor\Plugin::instance()->frontend->get_builder_content_for_display( $templateID, $withCss);
	...
	private static function checkHandleElementorStyles($arrStyles){
		...
		foreach($filteredStyles as $handle){
			...
			$filepathLoop = UELM_HelperUC::urlToPath($urlStyleLoop);
			...
			UELM_HelperUC::addStyleAbsoluteUrl($urlStyleLoop, $handleLoop);
			
			//if file not exists - return null
			$filepathStyle = UELM_HelperUC::urlToPath($urlStyle);
	...
	public static function putListingItemTemplate_post($post, $templateID, $widgetID, $listingType = "elementor", $withCss = false){
		...
		$arrElementorFilter = UELM_UniteFunctionsUC::getVal($wp_filter, $keyElementorFilter);
		...
		UELM_GlobalsProviderUC::$isUnderDynamicTemplateLoop = true;
		
		//set author data

		UELM_UniteFunctionsWPUC::setGlobalAuthorData($post);
		
		//fix for jet engine

		$isJetExists = UELM_UniteCreatorPluginIntegrations::isJetEngineExists();
		...
		if(UELM_GlobalsProviderUC::$isUnderAjax == true){
			UELM_GlobalsProviderUC::$isUnderAjaxDynamicTemplate = true;
		}

		//set elementor document

		$isElementorProActive = UELM_HelperUC::isElementorProActive();
		...
		if($isElementorProActive == true){
			...
			$documentToChange = \UELM_Elementor\Plugin::$instance->documents->get( $postID );
		...
		UELM_GlobalsUnlimitedElements::$renderingDynamicData = array(
			"post_id"=>$postID,
		...
		if(UELM_GlobalsProviderUC::$isUnderAjax == true){
			...
			foreach($arrDiff as $handleToAdd){
				...
				$objStyle = UELM_UniteFunctionsUC::getVal($objStyles2->registered, $handleToAdd);
		...
		UELM_GlobalsUnlimitedElements::$renderingDynamicData = null;

		UELM_GlobalsProviderUC::$isUnderAjaxDynamicTemplate = false;
		...
		UELM_GlobalsProviderUC::$isUnderDynamicTemplateLoop = false;
	...
	public static function putDynamicLoopElementStyle($element){
		...
		if(empty(UELM_GlobalsUnlimitedElements::$renderingDynamicData))
			return(false);

		$postID = UELM_UniteFunctionsUC::getVal(UELM_GlobalsUnlimitedElements::$renderingDynamicData, "post_id");
		$templateID = UELM_UniteFunctionsUC::getVal(UELM_GlobalsUnlimitedElements::$renderingDynamicData, "template_id");
		$widgetID = UELM_UniteFunctionsUC::getVal(UELM_GlobalsUnlimitedElements::$renderingDynamicData, "widget_id");
		...
		$elementID = $element->get_ID();
 		...
 		$arrControls = $element->get_controls();
  		...
  		foreach($dynamicSettings as $key => $setting){
  			...
  			$arrControl = UELM_UniteFunctionsUC::getVal($arrControls, $key);

  			$type = UELM_UniteFunctionsUC::getVal($arrControl, "type");
  		...
  		foreach($arrControls as $controlName => $control){
  			...
  			$arrValues = UELM_UniteFunctionsUC::getVal($settings, $controlName);
  			...
  			if(is_string($arrValues))
  				...
  				$value = UELM_UniteFunctionsUC::getVal($arrValues, "url");
  			}

  			if(empty($value))
  				continue;

  			$arrSelectors = UELM_UniteFunctionsUC::getVal($control, "selectors");
  			if(empty($arrSelectors))
  				continue;

  			$responsive = UELM_UniteFunctionsUC::getVal($control, "responsive");

  			$maxRes = UELM_UniteFunctionsUC::getVal($responsive, "max");
  			...
  			foreach($arrSelectors as $cssKey=>$cssValue){
  				...
  				if($maxRes == "tablet")
  					$styleToAdd = UELM_HelperHtmlUC::wrapCssMobile($styleToAdd, true);
  				else
  				if($maxRes == "mobile")
  					$styleToAdd = UELM_HelperHtmlUC::wrapCssMobile($styleToAdd);
	...
	public static function putListingItemTemplate($item, $templateID, $widgetID){
		...
		if(empty($type)){
			uelm_dmp("wrong listing type, can't output");
			return(false);
		}
		...
		if(empty($templateID)){
			...
			uelm_dmp("$type - $title - no template id");
			return(false);
		}
	...
	public static function savePostForDynamic($postID){
		...
		if(empty($post))
			UELM_UniteFunctionsUC::throwError("Post not found: $post");
	...
	public static function restorePostForDynamic(){
		...
		if(empty(self::$originalPost))
			UELM_UniteFunctionsUC::throwError("restorePostForDynamic error - need to save first");
	...
	public static function isElementorCachingActive(){
		...
		if(isset(\UELM_Elementor\Plugin::$instance->experiments) == false)
			return(false);
		
		$featureActive = \UELM_Elementor\Plugin::$instance->experiments->is_feature_active( 'e_element_cache' );
	...
	public static function globalInit(){
		...
		self::$operations = new UELM_UCOperations();

		//set path and url
		self::$pathCore = dirname(__FILE__)."/";

		self::$pathCore = UELM_UniteFunctionsUC::pathToUnix(self::$pathCore);

		$pathRelative = str_replace(UELM_GlobalsUC::$pathPlugin, "", self::$pathCore);

		self::$urlCore = UELM_GlobalsUC::$urlPlugin.$pathRelative;
