...
require UELM_HelperUC::getPathViewObject("activation_view.class");


class UELM_UnlimitedElementsLicenceView extends UELM_UniteCreatorActivationView{
	...
	private function initByUpress(){
		...
		$this->textUnleash = esc_html__("Activate Unlimited Elements Plugin UPress version", "unlimited-elements");
		
		$this->textGoPro = __("Activate In UPress","unlimited-elements");
	...
	private function initByEnvato(){
		...
		$this->textPasteActivationKey = esc_html__("Paste your envato activation key here", "unlimited-elements");

		$urlActivation = UELM_HelperUC::getViewUrl(UELM_GlobalsUnlimitedElements::VIEW_LICENSE_ELEMENTOR);
		
		//$this->textSwitchTo = "For back to regular activation <br> 
			//<a href='$urlActivation'>Click Here</a>
		//";
		
		$this->textDontHave = __("The activation key (product key) is located in <br> downloads section in CodeCanyon.","unlimited-elements");
	...
	protected function putPopupForm(){
		...
		?>
		<span class="activate-license uelm_for_elementor">
	...
	private function initByFreemius(){
		...
		$urlCCodecanyon = UELM_HelperUC::getViewUrl(UELM_GlobalsUnlimitedElements::VIEW_LICENSE_ELEMENTOR, "envato=true");
	...
	public function __construct(){
		...
		//$isEnvato = UELM_UniteFunctionsUC::getGetVar("envato", "", UELM_UniteFunctionsUC::SANITIZE_TEXT_FIELD);
		//$isEnvato = UELM_UniteFunctionsUC::strToBool($isEnvato);

		$this->textActivate = esc_html__("Activate Unlimited Elements", "unlimited-elements");
		
		$this->urlPricing = UELM_GlobalsUnlimitedElements::LINK_BUY;
		
		$this->textPlaceholder = esc_html__("xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx","unlimited-elements");
		
		$this->textUnleash = esc_html__("Unleash access to +700 widgets for Elementor", "unlimited-elements");
		...
		switch($type){
			...
			break;
			default:
				UELM_UniteFunctionsUC::throwError("Wrong type");
			break;
		}
		...
		$this->textYourProAccountLifetime = esc_html__("Unlimited Elements is activated lifetime for this site!", "unlimited-elements");
	...
	private function putFsScripts(){
		 ...
		 $moduleID = UELM_GlobalsUnlimitedElements::FREEMIUS_PLUGIN_ID;
	...
	private function putUpressMessage($isActive){
		...
		if($isActive == true){
			$message = __("The Unlimited Elements UPress license is active","unlimited-elements");
			$class = "uc-license-message-active";
		}
		else{
			$class = "uc-license-message-notactive";
			$message = __("The Unlimited Elements UPress license is not active","unlimited-elements");
		}
		?>
	...
	public function putHtmlDeactivate(){
		...
		$isActiveByFreemius = UELM_HelperProviderUC::isActivatedByFreemius();
		...
		<span class="activate-license uelm_for_elementor">
	         ...
	         <a href="javascript:void(0)" class="unite-button-primary"><?php esc_html_e("Change License", "unlimited-elements")?></a>
		</span>             
             <br>
             <br>
		<?php
...
$objLicense = new UELM_UnlimitedElementsLicenceView();
$objLicense->display();
