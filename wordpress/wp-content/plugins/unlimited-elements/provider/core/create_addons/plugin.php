...
class UELM_AddonLibraryCreatorPluginUC extends UELM_UniteCreatorPluginBase{
	...
	public function addItems_managerMenuSingle($arrMenu){
		...
		$arrNewItems[] = array("key"=>"duplicate_item",
							   "text"=>esc_html__("Duplicate","unlimited-elements"),
							   "insert_after"=>"remove_item");
		
		$arrNewItems[] = array("key"=>"export_addon",
							   "text"=>esc_html__("Export ","unlimited-elements").$this->textSingle,
							   "insert_after"=>"test_addon_blank");
		
		$arrMenu = UELM_UniteFunctionsUC::insertToAssocArray($arrMenu, $arrNewItems);
	...
	public function addItems_managerMenuField($arrMenu){
		...
		$arrNewItems[] = array("key"=>"add_addon",
				"text"=>esc_html__("Add Widget","unlimited-elements"),
				"insert_after"=>"top");
	...
	public function addItems_managerMenuCategory($arrMenu){
		...
		$arrNewItems[] = array("key"=>"export_cat_addons",
				"text"=>esc_html__("Export Category ","unlimited-elements").$this->textPlural,
				"insert_after"=>"bottom");
	...
	public function drawItemButtons2(){
		?>
	 			...
	 			<a data-action="duplicate_item" type="button" class="unite-button-secondary button-disabled uc-button-item"><?php esc_html_e("Duplicate","unlimited-elements")?></a>
	...
	*/
	public function editGlobals(){
		...
		UELM_GlobalsUC::$permisison_add = true;
...
if(file_exists($filepathProvider)){
	...
	$objPlugin = new UELM_AddonLibraryCreatorPluginUC();
}
