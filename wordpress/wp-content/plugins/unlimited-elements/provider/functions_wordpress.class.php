...
if ( ! defined( 'ABSPATH' ) ) exit;
	...
	class UELM_UniteFunctionsWPUC{
		...
		public static function isDBTableExists($tableName){
			...
			if(empty($tableName))
				UELM_UniteFunctionsUC::throwError("Empty table name!!!");
		...
		public static function getObjAcfIntegrate(){
			...
			if(empty(self::$objAcfIntegrate))
				self::$objAcfIntegrate = new UELM_UniteCreatorAcfIntegrate();
		...
		public static function getPostTypesWithCats($arrFilterTypes = null){
			...
			foreach($arrPostTypes as $name => $arrPostType){
				...
				$arrTax = UELM_UniteFunctionsUC::getVal($arrPostType, "taxonomies");
		...
		public static function getPostTypesWithCatIDs(){
			...
			foreach($arrTypes as $typeName => $arrType){
				...
				foreach($arrType as $arr){
					$cats = UELM_UniteFunctionsUC::getVal($arr, "cats");
		...
		public static function getTerms_filterBySlugs($arrTermObjects, $arrSlugs){
			...
			$arrSlugsAssoc = UELM_UniteFunctionsUC::arrayToAssoc($arrSlugs);
			$arrTermsNew = array();
		...
		public static function getTermsArgs($taxonomy, $orderBy = null, $orderDir = null, $hideEmpty = false, $addArgs = null){
			...
			$hideEmpty = UELM_UniteFunctionsUC::strToBool($hideEmpty);
		...
		public static function getSpecificTerms($slugs, $taxonomy){
			...
			UELM_HelperUC::addDebug("Terms Args", $args);
		...
		public static function getPostSingleTerms($postID, $taxonomyName){
			...
			//check from cache
			if(isset(UELM_GlobalsProviderUC::$arrPostTermsCache[$postID][$taxonomyName])){

				$arrTerms = UELM_GlobalsProviderUC::$arrPostTermsCache[$postID][$taxonomyName];
		...
		public static function getPostSingleTermsTitles($postID, $taxonomyName){
			...
			$output = UELM_UniteFunctionsUC::assocToArrayNames($arrTerms, "name");
		...
		public static function getTermsTitles($arrTerms, $withTax = true, $withID = false){
			...
			foreach($arrTerms as $term){
				...
				if($withTax == true){
					...
					$taxanomy = UELM_UniteFunctionsUC::getVal($term, "taxonomy");
		...
		public static function updateTermsIndexes(){
			...
			$tableTerms = UELM_GlobalsUC::$table_prefix."term_taxonomy";
	...
	public static function mergeArgsTaxQuery($args, $arrTaxQuery){
		...
		public static function getTaxQueryFromTerms($arrTerms){
			...
			foreach($arrTerms as $term){
				...
				$arrTaxTerms = UELM_UniteFunctionsUC::getVal($arrByTax, $taxonomy);
		...
		public static function filterTermsLastLevel($arrTerms, $taxonomy){
			...
			foreach($arrTerms as $term){
				...
				$parentID = UELM_UniteFunctionsUC::getVal($term, "parent_id");
			...
			$mainTerm = UELM_UniteFunctionsUC::getVal($arrParents, "term_0");
			...
			foreach($arrTerms as $term){
				...
				$level = UELM_UniteFunctionsUC::getVal($term, "level");
		...
		private static function getTermLevel($termID, $arrParentIDs){
			...
			do{
				$termID = UELM_UniteFunctionsUC::getVal($arrParentIDs, $termID);
		...
		public static function getTermsIDsWithParentIDs($taxonomy){
			...
			$arrParentsCache = UELM_UniteFunctionsUC::getVal(self::$arrTermParentsCache, $taxonomy);
		...
		public static function addCategory($catName){
			...
			if($catID == false)
				UELM_UniteFunctionsUC::throwError("category: $catName don't created");
		...
		public static function getCategoriesAssoc($taxonomy = "category", $addNotSelected = false, $forPostType = null){
			...
			if($addNotSelected == true)
				$arrCats["all"] = esc_html__("[All Categories]", "unlimited-elements");
		...
		public static function getCategoriesByIDs($arrIDs,$strTax = null){
			...
			if(!empty($arrCats))
				$arrCats = UELM_UniteFunctionsUC::convertStdClassToArray($arrCats);
		...
		public static function getCatIDBySlug($slug, $type = "slug"){
			...
			foreach($arrCats as $cat){
				...
				switch($type){
					...
					break;
					default:
						UELM_UniteFunctionsUC::throwError("Wrong cat name");
					break;
				}
		...
		public static function getArrSortBy($isForWoo = false, $forFilter = false){
			...
			$arr = array();
			$arr["default"] = __("Default", "unlimited-elements");

			if($forFilter == true){
				$arr["meta"] = __("Meta Field", "unlimited-elements");
			}
			...
			$arr[$postid] = __("Post ID", "unlimited-elements");

			$arr[self::SORTBY_DATE] = __("Date", "unlimited-elements");
			$arr[self::SORTBY_TITLE] = __("Title", "unlimited-elements");

			if($isForWoo == true){
				$arr[self::SORTBY_PRICE] = __("Price (WooCommerce)", "unlimited-elements");
				$arr[self::SORTBY_SALE_PRICE] = __("Sale Price (WooCommerce)", "unlimited-elements");
				$arr[self::SORTBY_SALES] = __("Number of Sales (WooCommerce)", "unlimited-elements");
				$arr[self::SORTBY_RATING] = __("Rating (WooCommerce)", "unlimited-elements");
			}

			$arr[self::SORTBY_SLUG] = __("Slug", "unlimited-elements");
			$arr[self::SORTBY_AUTHOR] = __("Author", "unlimited-elements");
			$arr[self::SORTBY_LAST_MODIFIED] = __("Last Modified", "unlimited-elements");
			$arr[self::SORTBY_COMMENT_COUNT] = __("Number Of Comments", "unlimited-elements");
			$arr[self::SORTBY_RAND] = __("Random", "unlimited-elements");
			$arr[self::SORTBY_NONE] = __("Unsorted", "unlimited-elements");
			$arr[self::SORTBY_MENU_ORDER] = __("Menu Order", "unlimited-elements");
			$arr[self::SORTBY_PARENT] = __("Parent Post", "unlimited-elements");

			if($forFilter !== true){
				
				$arr["post__in"] = __("Preserve Posts In Order", "unlimited-elements");

				$arr[self::SORTBY_META_VALUE] = __("Meta Field Value", "unlimited-elements");
				$arr[self::SORTBY_META_VALUE_NUM] = __("Meta Field Value (numeric)", "unlimited-elements");
			}
		...
		public static function getArrSortDirection(){
			...
			$arr = array();
			$arr["default"] = __("Default", "unlimited-elements");
			$arr[self::ORDER_DIRECTION_DESC] = __("Descending", "unlimited-elements");
			$arr[self::ORDER_DIRECTION_ASC] = __("Ascending", "unlimited-elements");
		...
		public static function getArrTermSortBy(){
			...
			$arr = array();
			$arr["default"] = __("Default", "unlimited-elements");
			$arr["name"] = __("Name", "unlimited-elements");
			$arr["slug"] = __("Slug", "unlimited-elements");
			$arr["term_group"] = __("Term Group", "unlimited-elements");
			$arr["term_id"] = __("Term ID", "unlimited-elements");
			$arr["description"] = __("Description", "unlimited-elements");
			$arr["parent"] = __("Parent", "unlimited-elements");
			$arr["parent_children"] = __("Parent and Children", "unlimited-elements");
			$arr["count"] = __("Count - (number of posts associated)", "unlimited-elements");
			$arr["rand"] = __("Random", "unlimited-elements");
		...
		public static function getTermCustomFields($termID, $addPrefixes = true){
			...
			$isAcfActive = UELM_UniteCreatorAcfIntegrate::isAcfActive();
		...
		public static function getPostCustomFields($postID, $addPrefixes = true, $imageSize = null){
			...
			if($isAcfActive){
				...
				$isPodsExists = UELM_UniteCreatorPodsIntegrate::isPodsExists();
				if($isPodsExists){
					$objPods = UELM_UniteCreatorPodsIntegrate::getObjPodsIntegrate();
					$arrCustomFields = $objPods->getPodsFields($postID, $addPrefixes);
				}

				//handle toolset
				$isToolsetActive = UELM_UniteCreatorToolsetIntegrate::isToolsetExists();

				if($isToolsetActive == true && empty($arrCustomFields)){
					$objToolset = new UELM_UniteCreatorToolsetIntegrate();
		...
		public static function getTermMetaImageID($termID){
			...
			if($isAcfActive){
				...
				$imageID = UELM_UniteFunctionsUC::getArrFirstValue($arrImageIDs);
			...
			foreach($arrNames as $name){
				...
				if(is_numeric($imageID) == false){
					...
					$arrItem = UELM_UniteFunctionsUC::maybeUnserialize($imageID);
					
					if(is_array($arrItem))
						$imageID = UELM_UniteFunctionsUC::getVal($arrItem, "id");
				}
		...
		public static function getTermImageID($termID, $metaKey){
			...
			if(!empty($attachmentID) && is_numeric($attachmentID) == false){
				...
				$arrItem = UELM_UniteFunctionsUC::maybeUnserialize($attachmentID);
				
				if(is_array($arrItem))
					$attachmentID = UELM_UniteFunctionsUC::getVal($arrItem, "id");
			}
	...
	public static function getPostMetaKeys_TOOLSET($postID){
		...
		$isToolsetExists = UELM_UniteCreatorToolsetIntegrate::isToolsetExists();
	...
	public static function getPost($postID, $addAttachmentImage = false, $getMeta = false){
		...
		$post = get_post($postID);
		if(empty($post))
			UELM_UniteFunctionsUC::throwError("Post with id: $postID not found");
	...
	public static function getPostIDByPostName($postName){
		...
		$tablePosts = UELM_UniteProviderFunctionsUC::$tablePosts;
	...
	public static function getPosts_whereFilter($where, $wp_query){
		...
		$titleFilter = UELM_UniteFunctionsUC::getVal($arrQuery, "title_filter");
	...
	public static function getPostsByType($postType, $sortBy = self::SORTBY_TITLE, $addParams = array(), $returnPure = false){
		...
		$titleFilter = UELM_UniteFunctionsUC::getVal($query, "title_filter");
		if(!empty($titleFilter)){
			$query["suppress_filters"] = false;  //no limit
			add_filter('posts_where', array("UELM_UniteFunctionsWPUC", "getPosts_whereFilter"), 10, 2);
		}

		$arrPosts = get_posts($query);
		
		if(!empty($titleFilter))
			remove_filter("posts_where", array("UELM_UniteFunctionsWPUC", "getPosts_whereFilter"));
	...
	public static function groupTaxQuery($arrQuery){
		...
		foreach($arrQuery as $term){
			...
			$arrTerms = UELM_UniteFunctionsUC::getVal($arrTermsByTax, $taxonomy);
			if(empty($arrTerms))
				$arrTerms = array();
	...
	public static function getPostsArgs($filters, $isTaxonly = false){
		...
		$category = UELM_UniteFunctionsUC::getVal($filters, "category");
		$categoryRelation = UELM_UniteFunctionsUC::getVal($filters, "category_relation");
		$categoryIncludeChildren = UELM_UniteFunctionsUC::getVal($filters, "category_include_children");

		$excludeCategory = UELM_UniteFunctionsUC::getVal($filters, "exclude_category");

		$categoryExcludeChildren = UELM_UniteFunctionsUC::getVal($filters, "category_exclude_children");
		$categoryExcludeChildren = UELM_UniteFunctionsUC::strToBool($categoryExcludeChildren);
		...
		$search = UELM_UniteFunctionsUC::getVal($filters, "search");
		if(!empty($search))
			$args["s"] = $search;

		$postType = UELM_UniteFunctionsUC::getVal($filters, "posttype");
		...
		if($orderby == self::SORTBY_META_VALUE || $orderby == self::SORTBY_META_VALUE_NUM)
			$args["meta_key"] = UELM_UniteFunctionsUC::getVal($filters, "meta_key");
		...
		$orderDir = UELM_UniteFunctionsUC::getVal($filters, "orderdir");

		if(!empty($orderDir))
			$args["order"] = $orderDir;

		$args["posts_per_page"] = UELM_UniteFunctionsUC::getVal($filters, "limit");

		$postStatus = UELM_UniteFunctionsUC::getVal($filters, "status");
		if(!empty($postStatus))
			$args["post_status"] = $postStatus;

		//get exlude posts
		$excludeCurrentPost = UELM_UniteFunctionsUC::getVal($filters, "exclude_current_post");
		$excludeCurrentPost = UELM_UniteFunctionsUC::strToBool($excludeCurrentPost);
	...
	public static function orderPostsByIDs($arrPosts, $arrPostIDs){
		...
		foreach($arrPostIDs as $postID){
			$post = UELM_UniteFunctionsUC::getVal($arrPostsAssoc, $postID);
			if(empty($post))
				continue;
	...
	public static function getPostPageTemplate($post){
		...
		$arrPost = $post->to_array();
		$pageTemplate = UELM_UniteFunctionsUC::getVal($arrPost, "page_template");
	...
	public static function getPostTitlesByIDs($arrIDs){
		...
		UELM_UniteFunctionsUC::validateIDsList($strIDs,"post ids");
		...
		$response = UELM_UniteFunctionsUC::arrayToAssoc($response, "id");

		$output = array();
		foreach($arrIDs as $id){
			$item = UELM_UniteFunctionsUC::getVal($response, $id);
			if(empty($item))
				continue;
	...
	public static function getPostContent($post){
		...
		if(UELM_GlobalsProviderUC::$disablePostContentFiltering !== true)
	...
	public static function updatePost($postID, $arrUpdate){
		...
		if(empty($arrUpdate))
			UELM_UniteFunctionsUC::throwError("nothing to update post");
		...
		if(is_wp_error($wpError)){
			UELM_UniteFunctionsUC::throwError("Error updating post: $postID");
		}
	}
	...
	public static function insertPost($title, $alias, $params = array()){
		...
		$catSlug = UELM_UniteFunctionsUC::getVal($params, "cat_slug");
		$content = UELM_UniteFunctionsUC::getVal($params, "content");
		$isPage = UELM_UniteFunctionsUC::getVal($params, "ispage");
		$isPage = UELM_UniteFunctionsUC::strToBool($isPage);

		$catID = null;
		if(!empty($catSlug)){
			$catID = self::getCatIDBySlug($catSlug);
			if(empty($catID))
				UELM_UniteFunctionsUC::throwError("Category id not found by slug: $catSlug");
		}

		$isPostExists = self::isPostNameExists($alias);

		if($isPostExists == true)
			UELM_UniteFunctionsUC::throwError("Post with name: <b> {$alias} </b> already exists");
		...
		$postType = UELM_UniteFunctionsUC::getVal($params, "post_type");
		if(!empty($postType))
	...
	public static function deletePostMetadata($postID){
		...
		$tablePostMeta = UELM_UniteProviderFunctionsUC::$tablePostMeta;
	...
	public static function cachePostsTermsQueries($arrPosts){
		...
		foreach($arrTerms as $term){
			...
			if(isset(UELM_GlobalsProviderUC::$arrPostTermsCache[$postID]) == false)
			...
			UELM_GlobalsProviderUC::$arrPostTermsCache[$postID][$taxonomy][$termID] = $term;
		}
	}
	...
	public static function getUrlAttachmentImage($thumbID, $size = self::THUMB_FULL){
		...
		if($isWPML)
			$thumbID = UELM_UniteCreatorWpmlIntegrate::getTranslatedAttachmentID($thumbID);
		
		$arrImage = wp_get_attachment_image_src($thumbID, $size);
		if(empty($arrImage))
			return (false);

		$url = UELM_UniteFunctionsUC::getVal($arrImage, 0);
	...
	public static function getImageDataByUrl($urlImage){
		...
		$title = UELM_HelperUC::getTitleFromUrl($urlImage, "image");
	...
	public static function getProductCatImage($productCatID){
		...
		$urlImage = self::getUrlAttachmentImage($imageID, UELM_UniteFunctionsWPUC::THUMB_LARGE);
	...
	public static function getArrThumbSizes(){
		...
		foreach($arrWPSizes as $size){
			...
			$title = UELM_UniteFunctionsUC::convertHandleToTitle($size);
			...
			$arrSize = UELM_UniteFunctionsUC::getVal($_wp_additional_image_sizes, $size);
			
			if(!empty($arrSize)){
				$maxWidth = UELM_UniteFunctionsUC::getVal($arrSize, "width");
				$maxHeight = UELM_UniteFunctionsUC::getVal($arrSize, "height");
				
				$crop = UELM_UniteFunctionsUC::getVal($arrSize, "crop");
				$crop = UELM_UniteFunctionsUC::strToBool($crop);
			}
		...
		$arrSizes["full"] = __("Full", "unlimited-elements");
	...
	public static function getAttachmentPostTitle($post){
		...
		$filename = UELM_UniteFunctionsUC::getVal($post, "guid");

		if(empty($title))
			$title = $filename;

		$info = pathinfo($title);
		$name = UELM_UniteFunctionsUC::getVal($info, "filename");
	...
	public static function getUserMeta($userID, $arrMetaKeys = null, $addPrefixed = false){
		...
		$arrMetaKeys = UELM_UniteFunctionsUC::arrayToAssoc($arrMetaKeys);
				
		$arrOutput = array();
		foreach($arrKeys as $key){
			$metaValue = UELM_UniteFunctionsUC::getVal($arrMeta, $key);
	...
	public static function getUserAvatarData($userID, $urlDefaultImage = ""){
		...
		$hasAvatar = UELM_UniteFunctionsUC::getVal($arrAvatar, "found_avatar");
		$size = UELM_UniteFunctionsUC::getVal($arrAvatar, "size");
		$url = UELM_UniteFunctionsUC::getVal($arrAvatar, "url");
	...
	public static function getUserData($objUser, $getMeta = false, $getAvatar = false, $arrMetaKeys = null, $getNumPosts = false){
		...
		$userData = UELM_UniteFunctionsUC::convertStdClassToArray($userData);

		$arrData = array();
		$arrData["id"] = UELM_UniteFunctionsUC::getVal($userData, "ID");

		$username = UELM_UniteFunctionsUC::getVal($userData, "user_nicename");
		...
		if(empty($name))
			$name = UELM_UniteFunctionsUC::getVal($userData, "user_login");

		$arrData["name"] = $name;

		$arrData["user_login"] = UELM_UniteFunctionsUC::getVal($userData, "user_login");

		$arrData["email"] = UELM_UniteFunctionsUC::getVal($userData, "user_email");
		...
		$urlWebsite = UELM_UniteFunctionsUC::getVal($userData, "user_url");
	...
	public static function getRolesShort($addAll = false){
		...
		if($addAll == true){
			$arrAll["__all__"] = __("[All Roles]", "unlimited-elements");
	...
	public static function getArrAuthorsShort($addCurrentUser = false){
		...
		if(!empty(self::$cacheAuthorsShort)){
			if($addCurrentUser){
				$arrUsers = UELM_UniteFunctionsUC::addArrFirstValue(self::$cacheAuthorsShort, "-- Logged In User --", "uc_loggedin_user");
	...
	public static function mergeQueryVars($args1, $args2){
		...
		foreach($args1 as $key=>$value){
			...
			$arrValue2 = UELM_UniteFunctionsUC::getVal($args2, $key);
	...
	public static function clearFiltersFunctions($tag, $numCallback = null){
		...
		$arrTags = UELM_UniteFunctionsUC::getVal($wp_filter, $tag);
	...
	public static function printRegisteredIncludes($type = "js"){
		...
		if($type == "js")
			uelm_dmp("Registered scripts: ");
		else
			uelm_dmp("Registered styles: ");
		
		foreach( $objScripts->queue as $scriptName ){

			$objScript = UELM_UniteFunctionsUC::getVal($objScripts->registered, $scriptName);
			...
			uelm_dmp("$scriptName | $url | $strDeps");
		}
	...
	public static function isArchiveLocation(){
		...
		if(class_exists("UELM_UniteCreatorElementorIntegrate")){
			$templateType = UELM_UniteCreatorElementorIntegrate::getCurrentTemplateType();
	...
	*/
	...
	public static function getAdminTitle($customTitle){
		...
		if(is_network_admin()){
			...
			$admin_title = sprintf(__('Network Admin: %s',"unlimited-elements"), esc_html(get_network()->site_name));
		}elseif(is_user_admin()){
			/* translators: User dashboard screen title. 1: Network name */
			$admin_title = sprintf(__('User Dashboard: %s',"unlimited-elements"), esc_html(get_network()->site_name));
		}else{
		...
		if($admin_title == $title){
			...
			$admin_title = sprintf(__('%1$s &#8212; WordPress',"unlimited-elements"), $title);
		}else{
			/* translators: Admin screen title. 1: Admin screen name, 2: Network or site name */
			$admin_title = sprintf(__('%1$s &lsaquo; %2$s &#8212; WordPress',"unlimited-elements"), $title, $admin_title);
		}
	...
	public static function getPermalink($post){
		...
		if($debugMultisite)
			uelm_dmp("multisite: $isMultisite");
		
		if($isMultisite){
			
			$arrPost = (array)$post;
			
			$blogID = UELM_UniteFunctionsUC::getVal($arrPost, "blog_id");
			...
			if($debugMultisite){
				uelm_dmp("multisite");
				uelm_dmp("current: $currentBlogID");
				uelm_dmp("blogid: $blogID");
				uelm_dmp("url: $url");
			}			
	...
	public static function preventCachingPage(){
		...
		$arrNotCacheTags = array("UELM_DONOTCACHEPAGE", "UELM_DONOTCACHEDB", "UELM_DONOTMINIFY", "UELM_DONOTCDN");
	...
	public static function getAllWPActionKeys($action){
		...
		$initFuncs = UELM_UniteFunctionsUC::getVal($wp_filter, "admin_init");
	...
	public static function onAdminInit(){
		...
		add_action('admin_print_scripts', array('UELM_UniteFunctionsWPUC', 'onStylesAndScriptsDeregister'), PHP_INT_MAX);
		add_action('wp_print_scripts', array('UELM_UniteFunctionsWPUC', 'onStylesAndScriptsDeregister'), PHP_INT_MAX);
	...
	public static function onFrontInit(){
        ...
        add_action('wp_after_insert_post', function ($post_id, $post, $update) { UELM_GlobalsUC::$hideDebug = true; }, 10, 3);
...
UELM_UniteFunctionsWPUC::initStaticVars();
