...
class UELM_UniteCreatorTemplateEngine extends UELM_UniteCreatorTemplateEngineWork{
	...
	public function printPostMeta($postID){
		...
		$postMeta = UELM_UniteFunctionsWPUC::getPostMeta($postID);
		
		if(empty($postMeta))
			uelm_dmp("no meta for this post");
		else{
			echo "<pre>";
	...
	public function getTermMeta($termID, $key = ""){
		...
		$arrMeta = UELM_UniteFunctionsWPUC::getTermMeta($termID);
	...
	public function getPostTerms($postID, $taxonomy, $addCustomFields = false, $type = "", $maxTerms = null){
		...
		if($taxonomy == "all" || $type == "all")
			$arrTerms = UELM_UniteFunctionsWPUC::getPostAllSingleTerms($postID);
		else
			$arrTerms = UELM_UniteFunctionsWPUC::getPostSingleTerms($postID, $taxonomy);
		
		if($type == "last_level")
			$arrTerms = UELM_UniteFunctionsWPUC::filterTermsLastLevel($arrTerms, $taxonomy);
	...
	public function getUserMeta($userID, $key = null){
		...
		if(empty($user))
			UELM_UniteFunctionsUC::throwError("user with id: $userID not found");

		if(empty($key)){
			
			uelm_dmp("getUserMeta Error. no key given. please select meta key from this keys: ");
	...
	public function putPostTags($postID){
		...
		$htmlTags = UELM_UniteFunctionsWPUC::getTagsHtmlList($postID);
