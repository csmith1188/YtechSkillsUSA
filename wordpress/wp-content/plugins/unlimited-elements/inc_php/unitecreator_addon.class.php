...
class UELM_UniteCreatorAddonWork extends UELM_UniteElementsBaseUC{
	...
	public function __construct(){
		...
		$this->operations = new UELM_UCOperations();

		$this->objProcessor = new UELM_UniteCreatorParamsProcessor();
	...
	public function validateInited(){
		...
		if($this->isInited == false)
			UELM_UniteFunctionsUC::throwError("The addon is not inited!");
	}
	...
	private function validateTitle($title){
		...
		UELM_UniteFunctionsUC::validateNotEmpty($title, "Widget Title");
	}
	...
	private function validateName($name){
		...
		$fieldName = esc_html__("Widget Name", "unlimited-elements");

		UELM_UniteFunctionsUC::validateNotEmpty($name, $fieldName);
		UELM_UniteFunctionsUC::validateAlphaNumeric($name, $fieldName);
	...
	private function validateTestSlot($num){
		...
		UELM_UniteFunctionsUC::validateNumeric($num,"test slot");
		
		$num = (int)$num;	//Security Update 3
		
		if($num < 0 || $num > 3)
			UELM_UniteFunctionsUC::throwError("Wrong test slot number: $num");
	}
	...
	private function validateParams($arrParams, $type = "main"){
		...
		foreach($arrParams as $param){
			...
			if($type == UELM_UniteCreatorDialogParam::PARAM_POSTS_LIST && isset($arrTypes[$type]))
				UELM_UniteFunctionsUC::throwError("There should be only one post list in attributes");

			$arrTypes[$type] = true;

			$name = UELM_UniteFunctionsUC::getVal($param, "name");
			if(empty($name))
				UELM_UniteFunctionsUC::throwError("Empty param name found");
	...
	private function validateNameNotExists($name){
		...
		if($isExists == true)
			UELM_UniteFunctionsUC::throwError("The widget with name: $name already exists");
	}
	...
	public function initByID($id){
		...
		UELM_UniteFunctionsUC::validateNotEmpty($id, "widget id");
		
		UELM_UniteFunctionsUC::validateNumeric($id,"widget id");
		...
		$addonName = UELM_UniteFunctionsUC::getVal(self::$arrCacheNameByID, "addon_".$id);
				
		if(!empty($addonName)){
			$record = UELM_UniteFunctionsUC::getVal(self::$arrCacheRecords, $addonName);
		}

		if(empty($record)){
			
			try{
				$record = $this->db->fetchSingle(UELM_GlobalsUC::$table_addons, "id={$id}");
			}catch(Exception $e){
				UELM_UniteFunctionsUC::throwError("Widget with ID: {$id} not found");
			}
		}
	...
	public function initByName($name, $checkCache = true){
		...
		try{
			...
			if($checkCache == true && array_key_exists($name, self::$arrCacheRecords) == true)
				...
				$record = $this->db->fetchSingle(UELM_GlobalsUC::$table_addons, array("name" => $name));

			$this->initByDBRecord($record);
			
		}catch(Exception $e){
			UELM_UniteFunctionsUC::throwError("Widget with name:<b> {$name} </b> not found");
		}
	}
	...
	public function initByAlias($alias, $type, $checkCache = true){
		...
		if($type == UELM_GlobalsUC::ADDON_TYPE_REGULAR_ADDON)
			$type = "";
		...
		try{
			...
			if($checkCache == true && array_key_exists($name, self::$arrCacheRecords) == true)
				...
				$record = $this->db->fetchSingle(UELM_GlobalsUC::$table_addons, array("alias" => $alias, "addontype" => $type));
			}
			...
			UELM_UniteFunctionsUC::throwError("Widget with alias:<b> {$alias} </b> not found");
		}
	}
	...
	public function initByBlockName($blockName, $addonType){
		...
		UELM_UniteFunctionsUC::validateNotEmpty($blockName,"block name");

		$prefix = UELM_GlobalsUnlimitedElements::PLUGIN_NAME."/"."ue-";
		
		if(strpos($blockName, $prefix) === false)
			UELM_UniteFunctionsUC::throwError("Wrong block found: $blockName");
	...
	private function initParamsFindDoubles($arrParams){
		...
		foreach($arrParams as $key => $param){
			...
			if(array_key_exists($name, $arrNames) == true)
				$arrParams[$key]["param_error"] = esc_html__("Double Name, please remove", "unlimited-elements");
	...
	private function parseJsonFromRecord($record, $name){
		...
		$data = UELM_UniteFunctionsUC::getVal($record, $name);
		...
		if(is_object($data))
			return UELM_UniteFunctionsUC::convertStdClassToArray($data);
	...
	private function initAssetsPath(){
		...
		$pathAbsolute = UELM_UniteFunctionsUC::joinPaths($this->pathAssetsBase, $path);

		$isUnderAssets = UELM_HelperUC::isPathUnderAssetsPath($pathAbsolute, $this->objAddonType);
	...
	private function arrIncludesToFullUrl($arrIncludes){
		...
		foreach($arrIncludes as $key => $include){
			if(is_string($include))
				$include = UELM_HelperUC::URLtoFull($include, $this->urlAssetsBase);
			else{
				$url = UELM_UniteFunctionsUC::getVal($include, "url");
				if(!empty($url))
					$include["url"] = UELM_HelperUC::URLtoFull($url, $this->urlAssetsBase);
			}
	...
	public function initByDBRecord($record){
		...
		//cache db record
		$addonName = UELM_UniteFunctionsUC::getVal($record, "name");
		
		if(!isset(self::$arrCacheRecords[$addonName]))
			self::$arrCacheRecords[$addonName] = $record;
		
		UELM_UniteFunctionsUC::validateNotEmpty($record, "The Widget not exists");

		$this->isInited = true;

		$this->data = $record;

		$this->id = UELM_UniteFunctionsUC::getVal($record, "id");
		
		self::$arrCacheNameByID["addon_".$this->id] = $addonName; 
		
		$this->title = UELM_UniteFunctionsUC::getVal($record, "title");
		$this->name = UELM_UniteFunctionsUC::getVal($record, "name");
		$this->alias = UELM_UniteFunctionsUC::getVal($record, "alias");
		$this->catid = UELM_UniteFunctionsUC::getVal($record, "catid");
		$this->ordering = (int)UELM_UniteFunctionsUC::getVal($record, "ordering");
		$this->isActive = (int)UELM_UniteFunctionsUC::getVal($record, "is_active");
		$this->type = UELM_UniteFunctionsUC::getVal($record, "addontype");
		$this->updateHash = UELM_UniteFunctionsUC::getVal($record, "test_slot1");
		...
		if(!empty($this->arrTemplates)){
			$this->html = UELM_UniteFunctionsUC::getVal($this->arrTemplates, "html");
			$this->htmlItem = UELM_UniteFunctionsUC::getVal($this->arrTemplates, "html_item");
			$this->htmlItem2 = UELM_UniteFunctionsUC::getVal($this->arrTemplates, "html_item2");

			$this->css = UELM_UniteFunctionsUC::getVal($this->arrTemplates, "css");
			$this->cssItem = UELM_UniteFunctionsUC::getVal($this->arrTemplates, "css_item");
			$this->js = UELM_UniteFunctionsUC::getVal($this->arrTemplates, "js");
		}else{    //get data the old way

			$this->html = UELM_UniteFunctionsUC::getVal($record, "html");
			$this->htmlItem = UELM_UniteFunctionsUC::getVal($record, "html_item");
			$this->css = UELM_UniteFunctionsUC::getVal($record, "css");
			$this->js = UELM_UniteFunctionsUC::getVal($record, "js");
		...
		if(!empty($arrConfig)){
			...
			$this->options = UELM_UniteFunctionsUC::getVal($arrConfig, "options");

			$arrIncludes = UELM_UniteFunctionsUC::getVal($arrConfig, "includes");

			$this->variablesItems = UELM_UniteFunctionsUC::getVal($arrConfig, "variables_item");
			if(empty($this->variablesItems))
				$this->variablesItems = array();

			$this->variablesMain = UELM_UniteFunctionsUC::getVal($arrConfig, "variables_main");
			if(empty($this->variablesMain))
				$this->variablesMain = array();
			
			$this->paramsCats = UELM_UniteFunctionsUC::getVal($arrConfig, "params_cats");
			...
			$jsonIncludes = UELM_UniteFunctionsUC::getVal($record, "includes");
		...
		$this->pathAssetsBase = UELM_HelperUC::getAssetsPath($objAddonType);
		$this->urlAssetsBase = UELM_HelperUC::getAssetsUrl($objAddonType);
		...
		$this->hasItems = UELM_UniteFunctionsUC::strToBool($enableItems);
		...
		if(!empty($arrIncludes)){
			...
			$this->includesJS = UELM_UniteFunctionsUC::getVal($arrIncludes, "js", array());
			$this->includesJSLib = UELM_UniteFunctionsUC::getVal($arrIncludes, "jslib", array());
			$this->includesCSS = UELM_UniteFunctionsUC::getVal($arrIncludes, "css", array());
	...
	public function getObjAddonType(){
		...
		$this->objAddonType = UELM_UniteCreatorAddonType::getAddonTypeObject($this->type);
	...
	public function getBlockName(){
		...
		if($isBG == true)		
			$blockName = UELM_GlobalsUnlimitedElements::PLUGIN_NAME . '/ue-' .$name."--bg";
		else
			$blockName = UELM_GlobalsUnlimitedElements::PLUGIN_NAME . '/ue-' .$name;
	...
	public function getUrlIcon(){
		...
		$showIcon = UELM_UniteFunctionsUC::strToBool($showIcon);

		if($showIcon == false)
			return (null);

		$urlIcon = UELM_GlobalsUC::$url_default_addon_icon;
	...
	public function getDefaultUrlPreview(){
		...
		$filepathDefaultPreview = UELM_GlobalsUC::$pathPlugin . "images/" . $filenameDefaultPreview;

		$urlPreview = null;
		if(file_exists($filepathDefaultPreview))
			$urlPreview = UELM_GlobalsUC::$urlPlugin . "images/" . $filenameDefaultPreview;
	...
	public function getPreviewIconContents(){
		...
		$iconContents = UELM_UniteFunctionsUC::fileGetContents($iconPath);
	...
	public function getParamsValues(){
		...
		$arrValues = $this->getProcessedMainParamsValues(UELM_UniteCreatorParamsProcessor::PROCESS_TYPE_CONFIG);
	...
	public function getListingTypes(){
		...
		$paramsDynamic = $this->getParams(UELM_UniteCreatorDialogParam::PARAM_LISTING);
	...
	public function getOption($name){
		...
		$value = UELM_UniteFunctionsUC::getVal($this->options, $name);
	...
	private function getArrCatsFull(){
		...
		self::$arrCacheCatsFull = UELM_UniteFunctionsUC::arrayToAssoc($arrCats, "id");
	...
	public function getCatTitle(){
		...
		$catTitle = UELM_UniteFunctionsUC::getVal($arrCats, $catID);
	...
	public function getCatIcon(){
		...
		$arrCat = UELM_UniteFunctionsUC::getVal($arrCats, $catID);
		if(empty($arrCat))
			return ("");

		$params = UELM_UniteFunctionsUC::getVal($arrCat, "params");

		$icon = UELM_UniteFunctionsUC::getVal($params, "icon");
	...
	public function getPathAssetsFull(){
		...
		$path = UELM_UniteFunctionsUC::joinPaths($pathAssetsGlobals, $this->pathAssets);

		$path = UELM_UniteFunctionsUC::addPathEndingSlash($path);
	...
	public function getAdminLabels(){
		...
		foreach($params as $param){
			$isAdminLabel = UELM_UniteFunctionsUC::getVal($param, "admin_label");
			$isAdminLabel = UELM_UniteFunctionsUC::strToBool($isAdminLabel);
		...
		if($this->isHasItems()){
			$this->arrAdminLabels[] = array("uc_num_items", esc_html__("Items", "unlimited-elements"));
		}
	...
	private function isAttributeTypeExists($arrParams, $type){
		...
		foreach($arrParams as $param){
			$paramType = UELM_UniteFunctionsUC::getVal($param, "type");
			if($paramType == $type)
				return (true);
		}
	...
	public function getListingParamForOutput(){
		...
		foreach($arrParams as $param){
			$paramType = UELM_UniteFunctionsUC::getVal($param, "type");
			if($paramType != UELM_UniteCreatorDialogParam::PARAM_LISTING)
				continue;
	...
	public function getMultisourceParam(){
		...
		foreach($arrParams as $param){
			...
			$type = UELM_UniteFunctionsUC::getVal($param, "type");
			if($type != UELM_UniteCreatorDialogParam::PARAM_LISTING)
				continue;
	...
	public function isEditorMainAttributeExists(){
		...
		$isExists = $this->isAttributeTypeExists($this->params, UELM_UniteCreatorDialogParam::PARAM_EDITOR);
	...
	public function isEditorItemsAttributeExists(){
		...
		$isExists = $this->isAttributeTypeExists($this->paramsItems, UELM_UniteCreatorDialogParam::PARAM_EDITOR);
	...
	public function getParamPosition($paramName, $isMain){
		...
		UELM_UniteFunctionsUC::throwError("Param: {$paramName} don't exist in $paramType params");
	}
	...
	public function getStoredData($key){
		...
		$data = UELM_UniteFunctionsUC::getVal($this->arrStoredData, $key);
	...
	private function hasListingProperty($property){
		...
		$arrParams = $this->getParams(UELM_UniteCreatorDialogParam::PARAM_LISTING);
		...
		foreach($arrParams as $param){
			...
			$userFor = UELM_UniteFunctionsUC::getVal($param, "use_for");
			...
			switch($property){
				...
				case "remote_parent":
					...
					$remoteType = UELM_UniteFunctionsUC::getVal($param, "remote_type");

					if($remoteType == "parent")
						return(true);
				break;
				default:
					UELM_UniteFunctionsUC::throwError("wrong property: ". $property);
				break;
			}
	...
	public function hasSequenceAnimation(){
		...
		$arrParams = $this->getParams(UELM_UniteCreatorDialogParam::PARAM_SPECIAL);
		...
		foreach($arrParams as $param){
			...
			$type = UELM_UniteFunctionsUC::getVal($param, "attribute_type");
	...
	public function isHTMLEscaped(){
		...
		$arrParams = $this->getParams(UELM_UniteCreatorDialogParam::PARAM_TEXTFIELD);
		...
		foreach($arrParams as $param) {
			...
			$html = UELM_UniteFunctionsUC::getVal($this->arrTemplates, "html");
	...
	public function isAjaxEnabled(){
		...
		$postList = $this->getParamByType(UELM_UniteCreatorDialogParam::PARAM_POSTS_LIST);
		...
		$isAjaxEnabled = UELM_UniteFunctionsUC::getVal($postList, "enable_ajax");
		$isAjaxEnabled = UELM_UniteFunctionsUC::strToBool($isAjaxEnabled);
	...
	public function isAjaxSearch(){
		...
		$special = UELM_UniteFunctionsUC::getVal($options, "special");
	...
	public function getArrLibraryIncludesUrls($processProvider){
		...
		$operations = new UELM_UCOperations();
		...
		foreach($this->includesJSLib as $libName){
			...
			if($libName == "font-awsome"){
				$optionDisable = UELM_HelperProviderCoreUC_EL::getGeneralSetting("force_disable_font_awesome");
	...
	public function getAllRegularIncludesUrls(){
		...
		foreach($arrMerged as $arrInclude){
			if(is_string($arrInclude))
				$arrUrls[] = $arrInclude;
			$url = UELM_UniteFunctionsUC::getVal($arrInclude, "url");
			$arrUrls[] = $url;
		}
	...
	private function isBGForGutenbergEditor(){
		...
		if(UELM_GlobalsProviderUC::$isInsideEditor == false)
			return(false);
	...
	public function getHtmlConfig($putMode = false, $isOutputSidebar = false, $options = array()){
		...
		UELM_GlobalsProviderUC::$activeAddonForSettings = $this;
		...
		$source = UELM_UniteFunctionsUC::getVal($this->arrHtmlConfigOptions, "source");
		$objSettings->setCurrentAddon($this);
		
		if($source == "addon"){
			$objSettings->addGlobalParam("source", "addon", UELM_UniteSettingsUC::TYPE_IMAGE);
		}
		...
		if(!empty($this->params) || $this->hasItems){
			...
			if(empty($this->paramsCats))
				$objSettings->addSap(esc_html__("General", "unlimited-elements"), "config", "config");
			
			if($this->hasItems == true){
				if($this->itemsType == self::ITEMS_TYPE_IMAGE && $isOutputSidebar == true){
					$objSettings->addGallery("uc_items", "", __("Select Images", "unlimited-elements"));
				}else{
		...
		if($this->hasItems == true && $isOutputSidebar == true){
			$objSettings->addSap(esc_html__("Items", "unlimited-elements"), "items");
		...
		$isDisableFonts = UELM_UniteFunctionsUC::getVal($this->arrHtmlConfigOptions, "disable_fonts");
		$isDisableFonts = UELM_UniteFunctionsUC::strToBool($isDisableFonts);
		if($isDisableFonts == true)
			$isFontsPanelEnabled = false;

		if($isFontsPanelEnabled == true){
			$objSettings->addSap(esc_html__("Fonts", "unlimited-elements"), "fonts");
		...
		if($numSettings == 0){
			$textEmpty = esc_html__("no settings for this widget", "unlimited-elements");
		...
		if($isOutputSidebar == false){
			...
			$objOutput = new UELM_UniteSettingsOutputSidebarUC();
		}
	...
	public function getArrFontsParams(){
		...
		$settingsOutput = new UELM_UniteCreatorSettingsOutput();
	...
	public function getSettingsItemsObject(){
		...
		if(empty($this->paramsItems)){
			UELM_UniteFunctionsUC::throwError("Item params not found");
		}
	...
	public function convertFromUrlAssets($value){
		...
		if(!empty($urlAssets))
			$value = UELM_HelperUC::convertFromUrlAssets($value, $urlAssets);
	...
	private function convertToUrlAssets($val){
		...
		$urlAssetsFull = UELM_HelperUC::URLtoFull($this->urlAssets);
		$urlAssetsRelative = UELM_HelperUC::URLtoRelative($this->urlAssets);
	...
	public function modifyDataConvertToUrlAssets($arrData){
		...
		if(is_string($arrData)){
			$arrData = UELM_HelperUC::URLtoRelative($arrData);
		...
		foreach($arrData as $key => $val){
			$val = UELM_HelperUC::URLtoRelative($val);
	...
	public function getProcessedMainParamsValues($processType = null){
		...
		if(empty($processType))
			$processType = UELM_UniteCreatorParamsProcessor::PROCESS_TYPE_OUTPUT;
	...
	public function getProcessedMainParamsImages(){
		...
		$arrParamsImages = $this->objProcessor->getProcessedParamsValues($objParams, UELM_UniteCreatorParamsProcessor::PROCESS_TYPE_SAVE, "uc_image");
	...
	public function getArrItemsForConfig(){
		...
		$arrItems = $this->getProcessedItemsData(UELM_UniteCreatorParamsProcessor::PROCESS_TYPE_CONFIG, false);
	...
	public function getDataForLayoutGrid(){
		...
		$arrAddon["config"] = $this->getProcessedMainParamsValues(UELM_UniteCreatorParamsProcessor::PROCESS_TYPE_CONFIG);
	...
	private function setResponsiveParamValues($param, $name, $arrValues){
		...
		if(isset($arrValues[$name . "_tablet"])){
			...
			$param["value_tablet"] = UELM_UniteFunctionsUC::getVal($arrValues, $name . "_tablet", $defaultValueTablet);
		}

		if(isset($arrValues[$name . "_mobile"])){
			$defaultValueMobile = UELM_UniteFunctionsUC::getVal($param, "default_value_mobile");
			$param["value_mobile"] = UELM_UniteFunctionsUC::getVal($arrValues, $name . "_mobile", $defaultValueMobile);
		}
	...
	private function setParamsValuesWork($arrValues, $arrParams, $type){
		...
		if(!is_array($arrValues))
			UELM_UniteFunctionsUC::throwError("The values shoud be array");
		...
		foreach($arrParams as $key => $param){
			...
			$value = UELM_UniteFunctionsUC::getVal($arrValues, $name, $defaultValue);
	...
	public function setArrItems($arrItems){
		...
		if($arrItems === UELM_GlobalsUC::VALUE_EMPTY_ARRAY)
			$arrItems = array();
		...
		if(UELM_UniteFunctionsUC::isAssocArray($arrItems) == true){
			uelm_dmp($arrItems);
			$errorMessage = "the items should not be assoc array";
			uelm_dmp("Error: " . $errorMessage);
			UELM_UniteFunctionsUC::throwError($errorMessage);
		}
	...
	private function updateInDB($arrUpdate){
		...
		$this->db->update(UELM_GlobalsUC::$table_addons, $arrUpdate, array("id" => $this->id));
	...
	private function getCreateUpdateData($data){
		...
		$html = UELM_UniteFunctionsUC::getVal($data, "html");
		$htmlItem = UELM_UniteFunctionsUC::getVal($data, "html_item");
		$htmlItem2 = UELM_UniteFunctionsUC::getVal($data, "html_item2");
		$css = UELM_UniteFunctionsUC::getVal($data, "css");
		$cssItem = UELM_UniteFunctionsUC::getVal($data, "css_item");
		$js = UELM_UniteFunctionsUC::getVal($data, "js");
		...
		$paramsItems = UELM_UniteFunctionsUC::getVal($data, "params_items");
		$options = UELM_UniteFunctionsUC::getVal($data, "options");
		$variablesItem = UELM_UniteFunctionsUC::getVal($data, "variables_item");
		$variablesMain = UELM_UniteFunctionsUC::getVal($data, "variables_main");

		$includes = UELM_UniteFunctionsUC::getVal($data, "includes");

		$paramsCats = UELM_UniteFunctionsUC::getVal($data, "params_cats");
		
		if(empty($includes)){
			$arrJsIncludes = UELM_UniteFunctionsUC::getVal($data, "includes_js");
			$arrJsLib = UELM_UniteFunctionsUC::getVal($data, "includes_jslib");
			$arrCssIncludes = UELM_UniteFunctionsUC::getVal($data, "includes_css");
			...
			$arrJsIncludes = UELM_HelperUC::arrUrlsToRelative($arrJsIncludes, true);
			$arrCssIncludes = UELM_HelperUC::arrUrlsToRelative($arrCssIncludes, true);
	...
	private function insertNewAddonToDB($arrInsert){
		...
		$catID = UELM_UniteFunctionsUC::getVal($arrInsert, "catid");
		...
		$newID = $this->db->insert(UELM_GlobalsUC::$table_addons, $arrInsert);

		if($newID === 0){    //in case that the table is not auto incriment

			$maxID = $this->db->getMaxOrder(UELM_GlobalsUC::$table_addons, "id");
			$newID = $maxID + 1;

			$arrUpdate = array();
			$arrUpdate["id"] = $newID;

			$this->db->update(UELM_GlobalsUC::$table_addons, $arrUpdate, "id=0");
		}
	...
	public function duplicate(){
		...
		$sqlSelect = "select " . self::FIELDS_ADDONS . " from " . UELM_GlobalsUC::$table_addons . " where id={$this->id}";
		$sqlInsert = "insert into " . UELM_GlobalsUC::$table_addons . " (" . self::FIELDS_ADDONS . ") ($sqlSelect)";

		$this->db->runSql($sqlInsert);
		$lastID = $this->db->getLastInsertID();
		UELM_UniteFunctionsUC::validateNotEmpty($lastID);
		...
		$this->db->update(UELM_GlobalsUC::$table_addons, $arrUpdate, array("id" => $lastID));
	...
	public function convertToType($addonType){
		...
		$objNewType = UELM_UniteCreatorAddonType::getAddonTypeObject($addonType);
	...
	public function convertToBloxType($addonType, $newName = null, $newTitle = null){
		...
		if($newType != UELM_GlobalsUC::ADDON_TYPE_REGULAR_ADDON)
			UELM_UniteFunctionsUC::throwError("Convert addon type works for regular addons only for now.");
	...
	public function copyAssetsPathToBlox(){
		...
		$pathAssetsOld = str_replace("/blox_assets", "/ac_assets", UELM_GlobalsUC::$pathAssets);

		$pathOldAbsolute = UELM_UniteFunctionsUC::joinPaths($pathAssetsOld, $path);
		$pathOldAbsolute = UELM_UniteFunctionsUC::addPathEndingSlash($pathOldAbsolute);

		$isExists = is_dir($pathOldAbsolute);

		$pathNewAbsolute = UELM_UniteFunctionsUC::joinPaths(UELM_GlobalsUC::$pathAssets, $path);
		$pathNewAbsolute = UELM_UniteFunctionsUC::addPathEndingSlash($pathNewAbsolute);

		UELM_UniteFunctionsUC::copyDir($pathOldAbsolute, $pathNewAbsolute);
	}
	...
	private function triggerAfterUpdateAction(){
		...
		UELM_UniteProviderFunctionsUC::doAction(UELM_UniteCreatorFilters::ACTION_ADDON_AFTER_UPDATE, $this);
	}
	...
	public function triggerAfterDeleteAction(){
		...
		UELM_UniteProviderFunctionsUC::doAction(UELM_UniteCreatorFilters::ACTION_ADDON_AFTER_DELETE, $this);
	}
	...
	public function getTestData($num){
		...
		$jsonData = UELM_UniteFunctionsUC::getVal($this->data, $fieldName);
	...
	public function modifyAddonDataConvertToUrlAssets($addonData){
		...
		$arrConfig = UELM_UniteFunctionsUC::getVal($addonData, "config");
	...
	public function setValuesFromTestData($slotNum){
		...
		$config = UELM_UniteFunctionsUC::getVal($arrData, "config");

		$items = UELM_UniteFunctionsUC::getVal($arrData, "items");
		$fonts = UELM_UniteFunctionsUC::getVal($arrData, "fonts");
