...
class UELM_UniteCreatorAddonValidator{
	...
	protected function validateInited(){
		...
		if(empty($this->objAddon))
			UELM_UniteFunctionsUC::throwError("Widget is not inited");
	}
	...
	protected function isAddonMissingDefaults(){
		...
		$items = UELM_UniteFunctionsUC::getVal($arrDefaults, "items");
	...
	protected function isAddonMissingImageUrl(){
		...
		$arrImagesParams = $this->objAddon->getParams(UELM_UniteCreatorDialogParam::PARAM_IMAGE);
		$arrImagesParamsItems = $this->objAddon->getParamsItems(UELM_UniteCreatorDialogParam::PARAM_IMAGE);
	...
	protected function isHasNonExistingIncludes(){
		...
		foreach($arrIncludes as $urlInclude){
			...
			$isUnderAssets = UELM_HelperUC::isUrlUnderAssets($urlInclude, $objAddonType);
			if($isUnderAssets == false)
				continue;
			
			$pathInclude = UELM_HelperUC::urlToPath($urlInclude);
	...
	private function runValidation($validation){
		...
		switch($validation){
			case self::VALIDATION_PREVIEW_EXISTS:
				...
				if($isExists == false)
					$this->arrMessages[] = __("Missing Preview Link","unlimited-elements");
				
			break;
			default:
				UELM_UniteFunctionsUC::throwError("wrong validation: ".$validation);
			break;
		}
	...
	protected function isHasNonValidHtml(){
		...
		$arrErrors = UELM_UniteFunctionsUC::validateHTML($html);
	...
	protected function validateRegularAddon(){
		...
		if($hasNonExistingIncludes == true)
			$this->arrMessages[] = __("Not existing include file!","unlimited-elements");
	...
	protected function validateShapeDevider(){
		...
		if($pos === false)
			$this->arrMessages[] = __("Not valid fill color!", "unlimited-elements");
	}
	...
	protected function runValidations(){
		...
		switch($type){
			...
			case UELM_GlobalsUC::ADDON_TYPE_ELEMENTOR:
	 ...
	 */
	public function isValid(UELM_UniteCreatorAddon $objAddon){
