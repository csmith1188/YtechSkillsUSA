...
class UELM_UniteCreatorAssets{
	...
	protected function setStartPath($startPath){
		...
		$startPath = UELM_UniteFunctionsUC::pathToUnix($startPath);
	...
	protected function validateInited(){
		if(empty($this->pathKey))
			UELM_UniteFunctionsUC::throwError("assets not inited");
	}
	...
	private function validateStartPath(){
		...
		if(empty($this->startPath))
			UELM_UniteFunctionsUC::throwError("Path not inited");
	...
	private function validatePathUnderStartPath($path){
		...
		if($searchPos !== 0)
			UELM_UniteFunctionsUC::throwError("Wrong path, should be under start path only");
	...
	private function validateFolderName($folderName){
		...
		if(strpbrk($folderName, "\\/?%*:|\"<>") !== FALSE)
			UELM_UniteFunctionsUC::throwError("The folder name: $folderName is not valid");
	...
	private function validateFilename($filename){
		...
		if(strpbrk($filename, "\\/?%*:|\"<>") !== FALSE)
			UELM_UniteFunctionsUC::throwError("The file name: $filename is not valid");
	...
	private function isFilenameAllowed($filename){
		...
		if(empty($ext))
			UELM_UniteFunctionsUC::throwError("Files without extension of: $filename don't allowed in assets");
	...
	private function validateAllowedFiletype($filename){
		...
		if($isAllowed == false)
			UELM_UniteFunctionsUC::throwError("File $filename not allowed in assets");
	}
	...
	public function deleteFilesInExtracted($path){
		...
		$arrDirs = UELM_UniteFunctionsUC::getDirList($path);
		...
		foreach($arrDelete as $dir){
			...
			UELM_UniteFunctionsUC::deleteDir($pathDir);
		}
	...
	public function validateAllowedFilesInExtracted($path, $isDelete = true){
		...
		$arrFiles = UELM_UniteFunctionsUC::getFileListTree($path);
		...
		foreach ($arrFiles as $pathFile) {
			...
			try {
				...
				if ($isDelete === true) {
					UELM_UniteFunctionsUC::deleteDir($path, false);
				}
	...
	private function checkDeleteNotAllowedFiles($path){
		...
		UELM_UniteFunctionsUC::deleteListOfFiles($arrFilesToDelete);
		
		UELM_UniteFunctionsUC::throwError("Found some not allowed files in the zip like: $fileExample , please check this file.");
	...
	private function validateFileAllowedForEdit($filename){
		...
		if(empty($ext))
			UELM_UniteFunctionsUC::throwError("Can't edit files without extension");
		...
		switch($fileType){
			...
			break;
			default:
				UELM_UniteFunctionsUC::throwError("File <b>$filename</b> type not allowed in to edit");
			break;
		}
	...
	protected function validateEditFileData($path, $filename){
		...
		if(file_exists($filepath) == false)
			UELM_UniteFunctionsUC::throwError("The file: $filename not exists.");
	...
	private function validateCreateNewFileFolder($path, $filename, $isFile = true){
		...
		if(is_file($filepath) == true)
			UELM_UniteFunctionsUC::throwError("file with name <b>$filename</b> already exists");
	
		if(is_dir($filepath) == true)
			UELM_UniteFunctionsUC::throwError("folder <b>$filename</b> already exists");
	...
	private function getActivePath($inputPath = ""){
		...
		$path = UELM_HelperUC::pathToRelative($path, false);
	...
	private function getStartPathRelative(){
		...
		return UELM_HelperUC::pathToRelative($this->startPath, false);
	...
	protected function getUrlDir($pathDir){
		...
		$urlDir = UELM_HelperUC::pathToRelativeUrl($pathDir);
	...
	protected function getHtmlDir($pathDir = null, $addWrapper = false){
		...
		$isDisableCheckboxes = UELM_UniteFunctionsUC::strToBool($isDisableCheckboxes);
		...
		foreach($arrFiles as $file){
			...
			if($file == ".." && $isStartPath == true){
				if($isNoFiles == true){
					$emptyText = esc_html__("empty folder", "unlimited-elements");
			...
			if($isThumbsView == true && $filetype == self::FILETYPE_IMAGE){
				$urlThumb = UELM_HelperUC::$operations->createThumbs($fileUrl);
				$urlThumb = UELM_HelperUC::URLtoFull($urlThumb);
	...
	private function getArrExistsFiles($path, $arrFiles){
		...
		if(!is_array($arrFiles))
			UELM_UniteFunctionsUC::throwError("getArrExistsFiles error - arrFiles should be array");
	...
	public function sanitizePath($path){
		...
		if(is_dir($path) == false){
			$path = UELM_HelperUC::pathToAbsolute($path);
		}
	
		$path = UELM_UniteFunctionsUC::realpath($path);
	
		if(empty($path) || !is_dir($path))
			UELM_UniteFunctionsUC::throwError("Wrong path given");
	...
	protected function deleteFiles($path, $arrFiles){
		...
		foreach($arrFiles as $filename){
			...
			if(is_dir($filepath))
				UELM_UniteFunctionsUC::deleteDir($filepath);
			else
				unlink($filepath);
	...
	protected function createFolder($path, $folderName){
		...
		UELM_UniteFunctionsUC::mkdir($pathCreate);

		if(is_dir($pathCreate) == false)
			UELM_UniteFunctionsUC::throwError("Can't create folder <b>{$folderName}</b>, please check parent folder permissions");
	...
	protected function createFile($path, $filename){
		...
		UELM_UniteFunctionsUC::writeFile("", $filepath);
		
		if(is_file($filepath) == false)
			UELM_UniteFunctionsUC::throwError("file <b>$filename</b> didn't created. Please check folder permissions");
	...
	protected function renameFile($path, $filename, $newFilename){
		...
		$success = UELM_UniteFunctionsUC::move($filepathCurrent, $filepathNew);
		
		if($success == false)
			UELM_UniteFunctionsUC::throwError("The file didn't renamed");
	...
	protected function getFileContent($path, $filename){
		...
		$content = UELM_UniteFunctionsUC::fileGetContents($filepath);
	...
	protected function moveFiles($pathSource, $arrFiles, $pathTarget, $actionOnExists){
		...
		if(empty($arrFiles))
			UELM_UniteFunctionsUC::throwError("No files to move");
		
		if(!is_array($arrFiles))
			UELM_UniteFunctionsUC::throwError("arrFiles should be array");
		...
		foreach($arrFiles as $file){
			...
			if($isTargetExists == true){
				...
				switch($actionOnExists){
					case "overwrite":
						$success = UELM_UniteFunctionsUC::move($filepathSource, $filepathTarget);
					break;
					case "skip":
						$success = true;
					break;
					default:
						UELM_UniteFunctionsUC::throwError("Action on file exists not given");
					break;
				}
			...
			if($success == false){
				if(is_dir($filepathSource) == true)
					UELM_UniteFunctionsUC::throwError("Can't move directory: <b> {$file} </b>");
				else
					UELM_UniteFunctionsUC::throwError("Can't move file <b> {$file} </b>");
			}
		}
	...
	protected function unzipFile($path, $filename){
		...
		UELM_UniteFunctionsUC::validateFilepath($filepath);
	...
	protected function handleUploadFile($uploadPath, $arrFile){
		try{
			...
			$filename = UELM_UniteFunctionsUC::getVal($arrFile, "name");
			
			$this->validateAllowedFiletype($filename);
			
			$tempFilepath = UELM_UniteFunctionsUC::getVal($arrFile, "tmp_name");
	
			$destFilepath = $uploadPath . '/' . sanitize_file_name($filename);
			
			if(is_file($tempFilepath) == false)
				UELM_UniteFunctionsUC::throwError("wrong upload filepath!");
			...
			if ( isset( $uploaded_file['file'] ) ) {
				UELM_UniteFunctionsUC::move( $uploaded_file['file'], $destFilepath, true );
				$success = true;
			}
			*/

			UELM_UniteFunctionsUC::moveUploadedFile($arrFile['tmp_name'], $destFilepath);
			$success = UELM_UniteFunctionsUC::fileExists($destFilepath);

			if($success == false)
				UELM_UniteFunctionsUC::throwError("Upload Failed to: $destFilepath\n" . json_encode($uploaded_file) . "\n" . json_encode($arrFile));
	...
	private function putUploadFileDialog(){
		...
		?>
			<div id="uc_dialog_upload_files" title="<?php esc_html_e("Upload Files", "unlimited-elements")?>" style="display:none">
				
				<div class="uc-dialog-upload-inner">
					
					<div class="uc-assets-activepath">
					<?php esc_html_e("Upload to folder: ", "unlimited-elements") ?> 
						<b>
						<span id="uc_dialogupload_activepath">some path</span>
						</b>
					</div>
					
					<form id="uc_form_dropzone" action="<?php echo esc_url(UELM_GlobalsUC::$url_ajax); ?>" class="dropzone">
						<input type="hidden" name="action" value="<?php echo esc_html(UELM_GlobalsUC::PLUGIN_NAME); ?>_ajax_action">
	...
	private function putDialogCreateFolder(){
		...
		<div id="uc_dialog_create_folder" title="<?php esc_html_e("Create Folder", "unlimited-elements")?>" style="display:none" class="unite-inputs">
			
			<div class="unite-dialog-top"></div>
			<div class="unite-inputs-label"><?php esc_html_e("Folder Name", "unlimited-elements")?></div>
			
			<input id="uc_dialog_create_folder_name" type="text" class="unite-input-regular" value="">
			
			<?php 
				$prefix = "uc_dialog_create_folder";
				$buttonTitle = esc_html__("Create Folder", "unlimited-elements");
				$loaderTitle = esc_html__("Creating Folder...", "unlimited-elements");
				$successTitle = esc_html__("Folder Created", "unlimited-elements");
	...
	private function putDialogCreateFile(){
		...
		<div id="uc_dialog_create_file" title="<?php esc_html_e("Create File", "unlimited-elements")?>" style="display:none" class="unite-inputs">
			
			<div class="unite-dialog-top"></div>
			<div class="unite-inputs-label"><?php esc_html_e("File Name", "unlimited-elements")?></div>
			
			<input id="uc_dialog_create_file_name" type="text" class="unite-input-regular" value="">
			
			<?php 
				$prefix = "uc_dialog_create_file";
				$buttonTitle = esc_html__("Create File", "unlimited-elements");
				$loaderTitle = esc_html__("Creating File...", "unlimited-elements");
				$successTitle = esc_html__("File Created", "unlimited-elements");
				UELM_HelperHtmlUC::putDialogActions($prefix, $buttonTitle, $loaderTitle, $successTitle);
			?>			
	...
	private function putDialogRenameFile(){
		...
		<div id="uc_dialog_rename_file" title="<?php esc_html_e("Rename File / Folder", "unlimited-elements")?>" style="display:none" class="unite-inputs uc-dialog-rename-file">
			<div class="unite-dialog-top"></div>
			<div class="unite-inputs-label"><?php esc_html_e("Rename this file/folder to", "unlimited-elements")?>:</div>
			<input id="uc_dialog_rename_file_input" type="text" class="unite-input-regular">
	
			<?php
			$prefix = "uc_dialog_rename_file";
			$buttonTitle = esc_html__("Rename", "unlimited-elements");
			$loaderTitle = esc_html__("Renaming...", "unlimited-elements");
			$successTitle = esc_html__("File/Dir Renamed", "unlimited-elements");
			UELM_HelperHtmlUC::putDialogActions($prefix, $buttonTitle, $loaderTitle, $successTitle);
			?>
	...
	private function putDialogEditFile(){
		...
		?>
		<div id="uc_dialog_edit_file" title="<?php esc_html_e("Edit File", "unlimited-elements")?>" style="display:none" class="unite-inputs uc-dialog-edit-file">
			
			<div class="uc-dialog-inner">
				<div id="uc_dialog_edit_file_loader" class="uc-loader-wrapper">
					<span class="loader_text"><?php esc_html_e("Loading...", "unlimited-elements")?></span>
				</div>
				
				<textarea id="uc_dialog_edit_file_textarea" style="display:none"></textarea>

				<div id="uc_dialog_edit_file_loadersaving" class="unite-dialog-loader" style="display:none"><?php esc_html_e("Saving File...", "unlimited-elements")?></div>
				<div id="uc_dialog_edit_file_success" class="unite-dialog-success" style="display:none"><?php esc_html_e("File Saved...", "unlimited-elements")?></div>
	...
	private function putDialogMoveFiles(){
		...
		$objAssets->setOption(UELM_UniteCreatorAssets::OPTION_ID, "uc_movefile_browser");
		
		?>
		<div id="uc_dialog_move_files" title="<?php esc_html_e("Move Files", "unlimited-elements")?>" style="display:none" class="unite-inputs uc-dialog-move-file">
			
			<div class="unite-dialog-top"></div>
			
			<div id="uc_dialog_move_label" class="unite-inputs-label" data-text="<?php esc_html_e("Move %1 files to", "unlimited-elements")?>">:</div>
			...
			<?php 
				...
				$buttonTitle = esc_html__("Move Files", "unlimited-elements");
				$loaderTitle = esc_html__("Moving Files...", "unlimited-elements");
				$successTitle = esc_html__("Files Moved", "unlimited-elements");
				UELM_HelperHtmlUC::putDialogActions($prefix, $buttonTitle, $loaderTitle, $successTitle);
			?>
			...
			<div id="uc_dialog_move_message" class="uc-dialog-move-message" style="display:none">
				...
				<a href="javascript:void(0)" class="unite-button-secondary" data-action="overwrite"><?php esc_html_e("Overwrite", "unlimited-elements")?></a>
				<a href="javascript:void(0)" class="unite-button-secondary" data-action="skip"><?php esc_html_e("Skip", "unlimited-elements")?></a>
				<a href="javascript:void(0)" class="unite-button-secondary" data-action="cancel"><?php esc_html_e("Cancel", "unlimited-elements")?></a>
	...
	private function putActionsPanel(){
		...
		?>
				<div class="uc-assets-buttons-panel">
					<a class="uc-button-upload-file uc-panel-button unite-button-secondary" data-action="upload" href="javascript:void(0)" ><?php esc_html_e("Upload", "unlimited-elements")?></a>
					<a class="uc-button-select-all uc-panel-button unite-button-secondary button-disabled" data-action="select_all" href="javascript:void(0)"  data-textselect="<?php esc_html_e("Select All", "unlimited-elements")?>" data-textunselect="<?php esc_html_e("Unselect All", "unlimited-elements")?>" ><?php esc_html_e("Select All", "unlimited-elements")?></a>
					<a class="uc-button-create-folder uc-panel-button unite-button-secondary" data-action="create_folder" href="javascript:void(0)"><?php esc_html_e("Create Folder", "unlimited-elements")?></a>
					<a class="uc-button-create-file uc-panel-button unite-button-secondary" data-action="create_file" href="javascript:void(0)"><?php esc_html_e("Create File", "unlimited-elements")?></a>
					
					<a class="uc-panel-button unite-button-secondary button-disabled uc-relate-multiple" data-action="delete" href="javascript:void(0)"><?php esc_html_e("Delete", "unlimited-elements")?></a>
					<span class="uc-preloader-deleting loader_text mleft_5" style="display:none"><?php esc_html_e("deleting...", "unlimited-elements")?></span>
										
					<a class="uc-panel-button unite-button-secondary button-disabled uc-relate-single uc-relate-file" data-action="edit" href="javascript:void(0)"><?php esc_html_e("Edit", "unlimited-elements")?></a>
					
					<a class="uc-panel-button unite-button-secondary button-disabled uc-relate-single" data-action="rename" href="javascript:void(0)"><?php esc_html_e("Rename", "unlimited-elements")?></a>
					
					<a class="uc-panel-button unite-button-secondary uc-relate-multiple button-disabled" data-action="move" href="javascript:void(0)"><?php esc_html_e("Move", "unlimited-elements")?></a>
					
					<a class="uc-panel-button unite-button-secondary button-disabled uc-relate-special uc-relate-type-zip" data-action="unzip" href="javascript:void(0)"><?php esc_html_e("Unzip", "unlimited-elements")?></a>
					<span class="uc-preloader-unzip loader_round mleft_5" style="display:none"><?php esc_html_e("unzipping...", "unlimited-elements")?></span>
					
					<a class="uc-panel-button unite-button-secondary button-disabled uc-relate-single" data-action="view" href="javascript:void(0)"><?php esc_html_e("View", "unlimited-elements")?></a>
		...
		?>
				<div class="uc-assets-activepath">
					...
					<span class="uc-assets-activepath-inner">
						<?php esc_html_e("Active Path", "unlimited-elements")?>:
	...
	public function putHTML($path = null, $wrapperOnly = false){
		...
		try{
			...
			$jsonOptions = htmlspecialchars($jsonOptions);
				...
				$htmlError = UELM_HelperUC::getHtmlErrorMessage($message,$trace, "Assets Manager Error: ");
				?>
				<div 
			...
			?>
				...
				uelm_echo($wrapperStyle);?> data-isbrowser="<?php echo esc_attr($this->isBrowerMode);?>" data-path="<?php echo esc_attr($activePathData);?>" data-startpath="<?php echo esc_attr($startPathData); ?>" data-options="<?php echo esc_attr($jsonOptions);?>">
					...
					<div class="uc-preloader-filelist loader_text mtop_25" style="display:none">
						...
						<?php esc_html_e("Loading File list...", "unlimited-elements")?>
