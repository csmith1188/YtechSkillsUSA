...
class UELM_UniteCreatorCategory extends UELM_UniteElementsBaseUC{
	...
	protected function validateInited(){
		...
		if(empty($this->id))
			UELM_UniteFunctionsUC::throwError("The category is not inited");
	...
	public function initByID($catID, $useCache = false){
		...
		//get from DB
		if(empty($record))
			$record = $this->db->fetchSingle(UELM_GlobalsUC::$table_categories,"id=$catID");
	...
	public function initByRecord($record){
		...
		$this->type = UELM_UniteFunctionsUC::getVal($record, "type");
		
		$params = UELM_UniteFunctionsUC::getVal($record, "params");
		$params = UELM_UniteFunctionsUC::jsonDecode($params);
	...
	private function validateTypeExists(){
		...
		if($this->type === null)
			UELM_UniteFunctionsUC::throwError("The type should be set");
	...
	private function validateAliasForUpdate($newAlias){
		...
		UELM_UniteFunctionsUC::validateNotEmpty($newAlias);
		UELM_UniteFunctionsUC::validateAlphaNumeric($newAlias);
		if(is_string($newAlias) == false)
			UELM_UniteFunctionsUC::throwError("The alias must be string");
		
		if(is_numeric($newAlias) == true)
			UELM_UniteFunctionsUC::throwError("The alias must be a words, not only numbers");
	}
	...
	public function getParam($key){
		...
		$value = UELM_UniteFunctionsUC::getVal($this->params, $key);
	...
	public function getExportCatData($exportType = null){
		...
		$exportTitle = UELM_UniteProviderFunctionsUC::applyFilters(UELM_UniteCreatorFilters::FILTER_EXPORT_CAT_TITLE, $this->title, $exportType, $this->type);
	...
	private function createAliasFromTitle($title, $existingAlias = null){
		...
		if(is_array($existingAlias))
			UELM_UniteFunctionsUC::throwError("The alias can't be array");
		
		$alias = $existingAlias;
		
		if(empty($alias))
			$alias = UELM_HelperUC::convertTitleToHandle($title);
	...
	private function updateValueInDB($key, $value){
		...
		$this->db->update(UELM_GlobalsUC::$table_categories, $arrUpdate, "id=".$this->id);
	}
	...
	private function update($title, $alias=null, $params=null){
		...
		$this->db->update(UELM_GlobalsUC::$table_categories, $arrUpdate, array("id"=>$this->id));
	}
	 ...
	 */
	public function addDefaultByAddonType(UELM_UniteCreatorAddonType $objType){
				
		$title = $objType->defaultCatTitle;
		
		if(empty($title))
			UELM_UniteFunctionsUC::throwError("There should be default cat title");
	...
	public function add($title, $type, $catData = null){
		...
		$maxOrder = $this->db->getMaxOrder(UELM_GlobalsUC::$table_categories);
		
		$existingAlias = UELM_UniteFunctionsUC::getVal($catData, "alias");
		...
		//set params
		$arrParams = UELM_UniteFunctionsUC::getVal($catData, "params");
		if(!empty($arrParams)){
		...
		//insert the category
		$catID = $this->db->insert(UELM_GlobalsUC::$table_categories, $arrInsert);
	...
	*/
	public function updateFromData($data){
		...
		if(empty($title))
			$title = UELM_UniteFunctionsUC::getVal($data, "category_title");
		
		$alias = UELM_UniteFunctionsUC::getVal($data, "category_alias");
		if(empty($alias))
			$alias = null;
