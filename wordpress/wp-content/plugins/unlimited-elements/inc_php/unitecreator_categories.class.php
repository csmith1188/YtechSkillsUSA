...
class UELM_UniteCreatorCategoriesWork extends UELM_UniteElementsBaseUC{
	...
	private function validateCatTitleNotExists($title, $type, $catID=null){
		...
		if($isExists == true)
			UELM_UniteFunctionsUC::throwError("Category with title: $title already exists");
	...
	private function validateTitle($title){
		...
		UELM_UniteFunctionsUC::validateNotEmpty($title, "Category Title");
		
		UELM_UniteFunctionsUC::validateNoTags($title, "Category Title");
	 ...
	 */
	private function getFirstCats(UELM_UniteCreatorAddonType $objAddonType, $showAll){
		
		$typeName = $objAddonType->typeName;
		
		$filterActive = UELM_UniteCreatorManagerAddons::getStateFilterActive($typeName);
		...
		$arrCatAll = array();
		$arrCatAll["id"] = "all";
		$arrCatAll["title"] = UELM_HelperUC::getText("all_addons");
		$arrCatAll["alias"] = "";
		...
		$arrCatZero = array();
		$arrCatZero["id"] = 0;
		$arrCatZero["title"] = UELM_HelperUC::getText("uncategorized");
		$arrCatZero["alias"] = "";
	...
	private function getListSimple($addonType = ""){
		...
		$response = $this->db->fetch(UELM_GlobalsUC::$table_categories, $where);
	...
	private function getListExtra_WhereEnding($type, $filterTitle="", $ordering="", $params = array()){
		...
		$whereFilterActive = UELM_UniteFunctionsUC::getVal($params, "where_filter_active");
		
		$filterSearchAddons = UELM_UniteFunctionsUC::getVal($params, "filter_search_addons");
		...
		if($type != UELM_GlobalsUnlimitedElements::ADDONSTYPE_ELEMENTOR_TEMPLATE && !empty($filterSearchAddons)){
	...
	private function getListExtraLayouts($filterTitle="", $ordering=""){
		...
		$arrCatsNew[] = array(
				"id" => 0,
				"title" => esc_html__("Uncategorized", "unlimited-elements"),
				"num_layouts" => 0
		);
	 ...
	 */
	protected function getListExtraQuery(UELM_UniteCreatorAddonType $objAddonType, $filterTitle="", $ordering="", $params = null){
		...
		if($isLayoutsType == false)
			$whereFilterActive = UELM_UniteCreatorAddons::getFilterActiveWhere(null, " and addons", $addonType);
		...
		if($isLayoutsType == false)
			...
			$tableAddons = UELM_GlobalsUC::$table_layouts;
	 ...
	 */
	public function getListExtra(UELM_UniteCreatorAddonType $objAddonType, $filterTitle="", $ordering="", $showAll = false, $params = null){
	...
	public function getCatRecords($addonType){
		...
		$arrCats = $this->db->fetch(UELM_GlobalsUC::$table_categories, $where, "ordering");
	...
	private function getFirstCatByAddType($addType){
		...
		switch($addType){
			case "empty":
				$arrCatsOutput[""] = esc_html__("[Not Selected]", "unlimited-elements");
			break;
			case "new":
				$arrCatsOutput["new"] = esc_html__("[Add New Category]", "unlimited-elements");
			break;
			case "component":
				$arrCatsOutput[""] = esc_html__("[From Gallery Settings]", "unlimited-elements");
			break;
			case "all_uncat":
				$arrCatsOutput["all"] = UELM_HelperUC::getText("all_addons");
				$arrCatsOutput[0] = UELM_HelperUC::getText("uncategorized");
			break;
			case "uncategorized":
				$arrCatsOutput[0] = UELM_HelperUC::getText("uncategorized");
			break;
			case "all_uncat_layouts":
				$arrCatsOutput["all"] = UELM_HelperUC::getText("all_layouts");
				$arrCatsOutput[0] = UELM_HelperUC::getText("uncategorized");
			break;
	...
	public function getCatsShort($addType = "", $type="", $addTypeToTitle = false){
		...
		foreach($arrCats as $cat){
			...
			$type = UELM_UniteFunctionsUC::getVal($cat, "type");
			...
			try{
				...
				$objType = UELM_UniteCreatorAddonType::getAddonTypeObject($type);
	...
	public function getArrCats($type){
		...
		foreach($records as $record){
			...
			$cat["params"] = UELM_UniteFunctionsUC::jsonDecode($params, true);
			$arrCats[] = $cat;
		}
	...
	private function getMaxOrder($type=""){
		...
		$query = "select MAX(ordering) as maxorder from ".UELM_GlobalsUC::$table_categories." $where";
	...
	public function isCatExists($catID){
		...
		try{
			...
			$arrCat = $this->db->fetchSingle(UELM_GlobalsUC::$table_categories,"id=$catID");
	...
	private function isCatExistsByTitle($title, $type = "", $catID = null){
		...
		if(empty($type))
			$type = UELM_UniteCreatorDB::ISNULL;
		
		$arrWhere = array();
		$arrWhere["title"] = $title;
		$arrWhere["type"] = $type;
		
		$response = $this->db->fetch(UELM_GlobalsUC::$table_categories, $arrWhere);
	...
	public function getCat($catID){
		...
		try{
			...
			UELM_UniteFunctionsUC::throwError("Category with id: $catID not found");
	...
	public function getCatType($catID){
		...
		$type = UELM_UniteFunctionsUC::getVal($arrCat, "type");
	...
	public function getCatByTitle($title, $type=""){
		...
		try{
			$arrCat = $this->db->fetchSingle(UELM_GlobalsUC::$table_categories, $arrWhere);
	...
	private function getCatHTML($cat, $class = ""){
		...
		$title = $cat["title"];
		$numAddons = UELM_UniteFunctionsUC::getVal($cat, "num_addons", 0); 
		...
		if($verFlags[UELM_GlobalsUC::VERSION_GUTENBERG] && !$verFlags[UELM_GlobalsUC::VERSION_ELEMENTOR]) { 
		...
		if($isweb == true && $isWebCatalogMode == false){
			...
			$urlWebIcon = UELM_GlobalsUC::$urlPluginImages."icon_cloud.svg";
			
			$textCloudCat = __("This category not installed yet in your catalog", "unlimited-elements");
		...
		if($isweb == false && $isWebCatalogMode == true){
			...
			$urlPinIcon = UELM_GlobalsUC::$urlPluginImages."icon_pin.svg";
			
			$textCloudCat = __("This category is installed", "unlimited-elements");
	...
	public function getLayoutsCatsListFromData($data){
		...
		$orderParam = UELM_UniteFunctionsUC::getVal($data, "sort");
		$filterWord = UELM_UniteFunctionsUC::getVal($data, "filter_word");
	...
	private function remove($catID){
		...
		//remove category
		$this->db->delete(UELM_GlobalsUC::$table_categories,"id=".$catID);
				
		//do action by type after remove
		switch($type){
			case self::TYPE_LAYOUT:	
				$this->db->runSql("UPDATE ".UELM_GlobalsUC::$table_layouts." SET catid='0' WHERE catid='".$catID."'");
			break;
			default:	//remove addons
				$this->db->delete(UELM_GlobalsUC::$table_addons,"catid=".$catID);
			break;
		}
	...
	public function updateType($catID, $type){
		...
		$arrUpdate = array();
		$arrUpdate["type"] = $type;
		$this->db->update(UELM_GlobalsUC::$table_categories,$arrUpdate,array("id"=>$catID));
	}
	...
	private function updateOrder($arrCatIDs){
		...
		foreach($arrCatIDs as $index=>$catID){
			...
			$this->db->update(UELM_GlobalsUC::$table_categories,$arrUpdate,$where);
		}
	}
	...
	*/
	private function getObjCatFromData($data){
		...
		$catID = UELM_UniteFunctionsUC::getVal($data, "cat_id");
		if(empty($catID))
			$catID = UELM_UniteFunctionsUC::getVal($data, "id");
	...
	public function updateOrderFromData($data){
		$arrCatIDs = UELM_UniteFunctionsUC::getVal($data, "cat_order");
		if(is_array($arrCatIDs) == false)
			UELM_UniteFunctionsUC::throwError("Wrong categories array");
	...
	public function addFromData($data){
		...
		$title = UELM_UniteFunctionsUC::getVal($data, "catname");
		$type = UELM_UniteFunctionsUC::getVal($data, "type");
		...
		$response["message"] = esc_html__("Category Added", "unlimited-elements");
		$response["htmlSelectCats"] = $this->getHtmlSelectCats($type);
	...
	private function convertArrCatToType($cat, $typeDest){
		...
		$this->db->update(UELM_GlobalsUC::$table_categories, $arrUpdate, "id=".$id);
