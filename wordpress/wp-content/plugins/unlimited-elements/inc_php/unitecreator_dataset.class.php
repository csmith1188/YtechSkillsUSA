...
class UELM_UniteCreatorDataset{
	...
	private function getArrQueryShort($arrQueries){
		...
		foreach($arrQueries as $name => $arrQuery){
			...
			$title = UELM_UniteFunctionsUC::getVal($arrQuery, "title");
			$arrShort[$name] = $title;
		}
	...
	public function registerDataset($type, $title, $arrQueries){
		...
		if(isset(self::$arrDatasetTypes[$type]))
			UELM_UniteFunctionsUC::throwError("Dataset type is already exists");
	...
	public function getDataset($type){
		...
		if(!isset(self::$arrDatasetTypes[$type]))
			UELM_UniteFunctionsUC::throwError("The dataset type: $type not exists");
