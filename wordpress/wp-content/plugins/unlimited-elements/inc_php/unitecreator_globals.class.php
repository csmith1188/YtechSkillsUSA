...
if ( ! defined( 'ABSPATH' ) ) exit;
	...
	class UELM_GlobalsUC{
		...
		const URL_RATE = "https://wordpress.org/support/plugin/unlimited-elements/reviews/?filter=5";
		...
		public static function initGlobals(){
			...
			UELM_UniteProviderFunctionsUC::initGlobalsBase();

			self::$current_protocol = "http://";
			if(self::$is_ssl == true)
				self::$current_protocol = "https://";

			self::$current_host = UELM_UniteFunctionsUC::getVal($_SERVER, "HTTP_HOST");
			...
			self::$current_page_url = self::$current_host.UELM_UniteFunctionsUC::getVal($_SERVER, "REQUEST_URI");
			...
			UELM_UniteFunctionsUC::validateNotEmpty(UELM_GlobalsUC::$url_assets_internal, "assets internal");

			self::$isLocal = UELM_UniteFunctionsUC::isLocal();
		
			UELM_UniteProviderFunctionsUC::doAction(UELM_UniteCreatorFilters::ACTION_AFTER_INIT_GLOBALS);

			if(self::$is_admin){

				$isDebugMode = UELM_UniteFunctionsUC::getGetVar("debug", "", UELM_UniteFunctionsUC::SANITIZE_KEY);
				self::$is_admin_debug_mode = UELM_UniteFunctionsUC::strToBool($isDebugMode);
			}
		...
		public static function initAfterIncludes(){
			...
			$product = UELM_HelperUC::getProductFromRequest();
			if(empty($product))
			...
			$isShowTrace = UELM_HelperUC::hasPermissionsFromQuery("uctrace");
			
			if($isShowTrace == true){
				UELM_GlobalsUC::$SHOW_TRACE_FRONT = true;
				UELM_GlobalsUC::$SHOW_TRACE = true;
			}
		...
		public static function initAfterInitTrigger(){
			...
			load_plugin_textdomain("unlimited-elements", false, UELM_GlobalsUC::$pathWPLanguages);
		...
		public static function initAfterPluginsLoaded(){
			...
			//show errors by url
			$showAjaxErrors = UELM_HelperUC::hasPermissionsFromQuery("showajaxerrors");
			...
			$showQueryDebugByUrl = UELM_HelperUC::hasPermissionsFromQuery("ucquerydebug");
			...
			$isTestFree = UELM_HelperUC::hasPermissionsFromQuery("testfreeversion");
			if($isTestFree == true)
				UELM_GlobalsUC::$isProVersion = false;
			
			//general debug
			$isGeneralDebug = UELM_HelperUC::hasPermissionsFromQuery("ucdebug");
			if($isGeneralDebug == true)
				UELM_GlobalsUC::$isDebug = true;
		...
		public static function initAfterAdminInit(){
			...
			if(!empty($wp_locale)){
				UELM_GlobalsUC::$isAdminRTL = $wp_locale->is_rtl();
			}
		...
		public static function printVars(){
			...
			UELM_UniteFunctionsUC::showClassStaticVariables("UELM_GlobalsUC");
