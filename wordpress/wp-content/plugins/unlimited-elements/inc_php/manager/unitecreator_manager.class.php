...
class UELM_UniteCreatorManager{
	...
	public static function getObjManagerByAddonType($addonType, $data = array()){
		...
		$manager = UELM_UniteProviderFunctionsUC::applyFilters(UELM_UniteCreatorFilters::FILTER_GET_MANAGER_OBJECT_BYDATA, null, $data);
				
		if(empty($manager)){
			if($objAddonType->isLayout == true)
				$manager = new UELM_UniteCreatorManagerLayouts();
			else
				$manager = new UELM_UniteCreatorManagerAddons();
		}
	...
	protected function getMenuSingleItem(){
		...
		$arrMenuItem = array();
		$arrMenuItem["no_action"] = esc_html__("No Action","unlimited-elements");
	...
	protected function getMenuField(){
		...
		$arrMenuField = array();
		$arrMenuField["no_action"] = esc_html__("No Action","unlimited-elements");
	...
	protected function putAddHtml(){
		uelm_dmp("put add html here by child class");
	}
	...
	protected function getNoItemsText(){
		...
		$text = esc_html__("No Items", "unlimited-elements");
	...
	protected function getCatList(){
		...
		uelm_dmp("getCatList - function for override!!!");
	...
	private function putHtmlCats($htmlCatList = null){
		...
		<div id="categories_wrapper" class="categories_wrapper unselectable">
			...
			<div class="manager-cats-buttons">
					<span class="manager-cats-title"><?php esc_html_e("Categories","unlimited-elements")?></span>
	...
	protected function putDialogEditCategory(){
		?>
			<div id="uc_dialog_edit_category"  title="<?php esc_html_e("Edit Category","unlimited-elements")?>" style="display:none;" >
				
				<div class="unite-dialog-top"></div>
					
					<?php esc_html_e("Category ID", "unlimited-elements")?>: <b><span id="span_catdialog_id"></span></b>
					
					<br><br>
					
					<?php esc_html_e("Edit Title", "unlimited-elements")?>:
	...
	protected function putDialogAddCategory(){
		?>
			...
			<div id="uc_dialog_add_category"  title="<?php esc_html_e("Add New Category","unlimited-elements")?>" style="display:none;" class="unite-inputs">
			
				<div class="unite-dialog-top"></div>
				<div class="unite-inputs-label"><?php esc_html_e("Enter Category Name", "unlimited-elements")?></div>
			
				<input id="uc_dialog_add_category_catname" type="text" class="unite-input-regular" value="">
				
			<?php 
				$prefix = "uc_dialog_add_category";
				$buttonTitle = esc_html__("Create Category", "unlimited-elements");
				$loaderTitle = esc_html__("Adding Category...", "unlimited-elements");
				$successTitle = esc_html__("Category Added", "unlimited-elements");
				UELM_HelperHtmlUC::putDialogActions($prefix, $buttonTitle, $loaderTitle, $successTitle);
			?>			
	...
	protected function putDialogDeleteCategory(){
		?>
			<div id="uc_dialog_delete_category"  title="<?php esc_html_e("Delete Category","unlimited-elements")?>" style="display:none;" class="unite-inputs">
			
				<div class="unite-dialog-top"></div>
			
				<?php esc_html_e("Are you sure you want to delete the: ", "unlimited-elements")?>
				
				<b><span id="uc_dialog_delete_category_catname"></span></b>
				
				<?php esc_html_e(" category and all it's widgets?", "unlimited-elements")?>
				
			<?php 
				$prefix = "uc_dialog_delete_category";
				$buttonTitle = esc_html__("Delete Category", "unlimited-elements");
				$loaderTitle = esc_html__("Deleting Category...", "unlimited-elements");
				$successTitle = esc_html__("Category and it's addons Deleted", "unlimited-elements");
				UELM_HelperHtmlUC::putDialogActions($prefix, $buttonTitle, $loaderTitle, $successTitle);
			?>			
		...
		?>
			...
			<?php foreach($arrMenu as $operation=>$text):
				...
				if(is_array($text)){
					$arr = $text;
					$text = $arr["text"];
					$class = UELM_UniteFunctionsUC::getVal($arr, "class");
					$disabled = UELM_UniteFunctionsUC::getVal($arr, "disabled");
					$title = UELM_UniteFunctionsUC::getVal($arr, "title");
				}
	...
	private function putMenuCatField(){
		...
		$arrMenuCatField["add_category"] = __("Add Category","unlimited-elements");
		
		if($this->enableCatsActions == false){
			$arrMenuCatField = array();
			$arrMenuCatField["no_action"] = __("No Action","unlimited-elements");
		}
	...
	private function validateInited(){
		...
		if(empty($this->type))
			UELM_UniteFunctionsUC::throwError("The manager is not inited");
	}
		...
		?>
							...
							<div class="status_operations">
								...
								<div id="item_operations_wrapper" class="item_operations_wrapper unite-disabled">
									...
									<?php esc_html_e("Move To", "unlimited-elements")?>
	...
	private function putStatusLine(){
		...
		?>
			...
			?>
						...
						<a href="javascript:void(0)" class="manager-button-debug-dialog" title="<?php esc_attr_e("Show Debug Data","unlimited-elements")?>">
							<i class="fas fa-question"></i>
						</a>
						<?php endif?>
						
						<?php if($this->putUpdateCatalogButton == true):?>
						<a href="javascript:void(0)" class="manager-button-update-catalog" title="<?php esc_attr_e("Update Catalog","unlimited-elements")?>">
	...
	private function putMenuCopyMove(){
		?>
			<ul id="menu_copymove" class="unite-context-menu" style="display:none">
				<li>
					<a href="javascript:void(0)" data-operation="copymove_move"><?php esc_html_e("Move Here","unlimited-elements")?></a>
				</li>
			</ul>
		<?php
	}
	...
	protected function getMenuMulitipleItems(){
		...
		$arrMenuItemMultiple["no_action"] = __("No Action","unlimited-elements");
	...
	* put scripts according manager type
	...
	public static function putScriptsIncludes($type){
		...
		UELM_HelperUC::addScript("unitecreator_manager_items","unitecreator_manager_items","js/manager");
		UELM_HelperUC::addScript("unitecreator_manager","unitecreator_manager","js/manager");
		UELM_HelperUC::addStyle("unitecreator_manager","unitecreator_manager_css");
		
		switch($type){
			case self::TYPE_PAGES:
			case self::TYPE_ADDONS:
				UELM_HelperUC::addScript("unitecreator_manager_cats","unitecreator_manager_cats","js/manager");
				UELM_HelperUC::addScript("unitecreator_manager_actions_addons","unitecreator_manager_actions_addons","js/manager");
			...
			case self::TYPE_ITEMS_INLINE:
				...
				UELM_HelperUC::addScript("unitecreator_manager_actions_inline","unitecreator_manager_actions_inline","js/manager");
			break;
		}
	...
	public function outputHtml(){
		...
		$htmlPassData = "";
		if(!empty($this->arrPassData))
			$htmlPassData = UELM_UniteFunctionsUC::jsonEncodeForHtmlData($this->arrPassData,"passdata");
		
		//add text
		if(!empty($this->arrText)){
			$optionText = UELM_UniteFunctionsUC::jsonEncodeForHtmlData($this->arrText, "text");
			if(!empty($optionText))
				$this->managerAddHtml .= " ".$optionText;
		}
		
		if(!empty($this->arrOptions)){
			$optionOptions = UELM_UniteFunctionsUC::jsonEncodeForHtmlData($this->arrOptions, "options");
	...
	public function init($type = ""){
		...
		$this->itemsLoaderText = __("Getting Items", "unlimited-elements");
		$this->textItemsSelected = __("items selected","unlimited-elements");
		
		if($this->hasCats){
			$this->objCats = new UELM_UniteCreatorCategories();
