...
class UELM_UniteCreatorVariablesOutput{
	...
	private function validateInited(){
		if($this->isInited == false)
			UELM_UniteFunctionsUC::throwError("UELM_UniteCreatorVariablesOutput not inited");
	}
	...
	private function getVarValue($name){
		$value = UELM_UniteFunctionsUC::getVal($this->var, $name);
		return($value);
	}
	...
	private function getContent_varItemSimple(){
		...
		if($this->isFirstItem){
			...
			$enableFirstItem = UELM_UniteFunctionsUC::strToBool($enableFirstItem);
			...
			$enableLastItem = UELM_UniteFunctionsUC::strToBool($enableLastItem);
	...
	public function getContent_paramRelated($isItem = false){
		...
		$paramValue = UELM_UniteFunctionsUC::getVal($params, $paramName);
	...
	public function getItemVarContent($var, $item, $index, $numItems){
		...
		$type = UELM_UniteFunctionsUC::getVal($var, "type");
		...
		switch($type){
			...
			break;
			default:
				UELM_UniteFunctionsUC::throwError("Wrong item variable type: <b>{$type}</b>");
			break;
		}
	...
	public function getContent_GETParam(){
		...
		$name = UELM_UniteFunctionsUC::getVal($this->var, "name");
		$defaultValue = UELM_UniteFunctionsUC::getVal($this->var, "default_value");
		$sanitizeType = UELM_UniteFunctionsUC::getVal($this->var, "sanitize_type");
		
		$content = UELM_UniteFunctionsUC::getGetVar($name, $defaultValue, $sanitizeType);
	...
	private function getContent_filter(){
		...
		$filterName = UELM_UniteFunctionsUC::getVal($this->var, "filter_name");
		
		$filterParam = UELM_UniteFunctionsUC::getVal($this->var, "filter_param");
	...
	public function getMainVarContent($var){
		...
		switch($type){
			case self::TYPE_PARAM_RELATED:
				$content = $this->getContent_paramRelated();
			break;
			case UELM_UniteCreatorDialogParam::PARAM_VAR_GET:
				$content = $this->getContent_GETParam();
			break;
			case UELM_UniteCreatorDialogParam::PARAM_VAR_FILTER:
			...
			default:
				UELM_UniteFunctionsUC::throwError("Wrong main variable type: <b>{$type}</b>");
			break;
		}
