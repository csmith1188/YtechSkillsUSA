...
class UELM_UniteCreatorForm{

	const ANTISPAM_BLOCKS_OPTIONS_KEY = "uelm_form_antispam_blocks";
	const ANTISPAM_SUBMISSIONS_OPTIONS_KEY = "uelm_form_antispam_submissions";

	const LOGS_OPTIONS_KEY = "uelm_form_logs";
	const LOGS_MAX_COUNT = 10;

	const FOLDER_NAME = "uelm_form";
	...
	public static function getConditionsRepeaterSettings(){
		...
		$settings->addSelect("operator", $arrOptions, __("Operator", "unlimited-elements"), "and", $params);
		...
		$settings->addTextBox("field_name", "", __("Field Name", "unlimited-elements"), $params);
		...
		$settings->addSelect("condition", $arrOptions, __("Condition", "unlimited-elements"), "=", $params);
		...
		$settings->addTextBox("field_value", "", __("Field Value", "unlimited-elements"), $params);
	...
	public function addFormIncludes(){
		...
		if(self::$isFormIncluded == false){
			$urlFormJS = UELM_GlobalsUC::$url_assets_libraries . "form/uc_form.js";

			UELM_UniteProviderFunctionsUC::addAdminJQueryInclude();
			UELM_HelperUC::addScriptAbsoluteUrl_widget($urlFormJS, "uc_form");
		}
	...
	public function getVisibilityConditionsParamsData($data, $visibilityParam){
		...
		$name = UELM_UniteFunctionsUC::getVal($visibilityParam, "name");

		$arrValue = UELM_UniteFunctionsUC::getVal($visibilityParam, "value");

		if(empty($arrValue))
			return ($data);

		$arrValue = UELM_UniteFunctionsUC::getVal($arrValue, "{$name}_conditions");
	...
	private function getFieldsData($arrContent, $arrFields, $arrFiles){
		...
		foreach($arrFields as $arrField){
			// get field input
			$fieldId = UELM_UniteFunctionsUC::getVal($arrField, "id");
			$fieldType = UELM_UniteFunctionsUC::getVal($arrField, "type");
			$fieldValue = UELM_UniteFunctionsUC::getVal($arrField, "value");
			$fieldParams = array();

			// get saved settings from layout
			$fieldSettings = UELM_HelperProviderCoreUC_EL::getAddonValuesWithDataFromContent($arrContent, $fieldId);
			
			// @TODO: get array of the uc_items and extract text of the selected value
			$fieldText = "";

			if($fieldType === self::TYPE_FILES){
				$fieldValue = UELM_UniteFunctionsUC::getVal($arrFiles, $fieldId, array());
			...
			$name = UELM_UniteFunctionsUC::getVal($fieldSettings, "field_name");
			$title = UELM_UniteFunctionsUC::getVal($fieldSettings, "label");
			$required = UELM_UniteFunctionsUC::getVal($fieldSettings, "required");
			$required = UELM_UniteFunctionsUC::strToBool($required);
	...
	private function mailPoetService()
	{
		...
		$mailPoetLists  = explode( ',', UELM_UniteFunctionsUC::getVal( $this->formSettings, "mailpoet_list_field" ) );
		$emailField     = UELM_UniteFunctionsUC::getVal( $this->formSettings, "name_email_field" );
		$firstNameField = UELM_UniteFunctionsUC::getVal( $this->formSettings, "name_first_name_field" );
		$lastNameField  = UELM_UniteFunctionsUC::getVal( $this->formSettings, "name_last_name_field" );
		...
		$subscriber_email = UELM_UniteFunctionsUC::getVal($subscriber, 'email');
	...
	private function isEmailValid($fieldValue){
		...
		return UELM_UniteFunctionsUC::isEmailValid($fieldValue);
	}
	...
	public function validateFormSettings($formSettings){
		...
		$formActions = UELM_UniteFunctionsUC::getVal($formSettings, "form_actions");
		...
		foreach($formValidations as $validation){
			foreach($validation["actions"] as $actionKey){
				...
				foreach($validation["rules"] as $fieldName => $rules){
					...
					$fieldTitle = UELM_UniteFunctionsUC::getVal($validation["titles"], $fieldName, $fieldKey);
					$fieldValue = UELM_UniteFunctionsUC::getVal($formSettings, $fieldKey);
						...
						switch($ruleName){
							case "required":
								if($fieldValue === "")
									...
									$errors[] = sprintf(esc_html__("%s field is empty.", "unlimited-elements"), $errorTitle);
							break;

							case "required_if":
								foreach($ruleParams as $depFieldName => $depFieldRequiredValue){
									$depFieldKey = $this->getFieldKey($depFieldName, $actionKey);
									$depFieldValue = UELM_UniteFunctionsUC::getVal($formSettings, $depFieldKey);
							...
							case "email":
								...
								if($fieldValue !== "" && $validEmail === false)
									// translators: %s is field name
									$errors[] = sprintf(esc_html__("%s field has an invalid email address:", "unlimited-elements"), $errorTitle) . ' ' . $fieldValue . '.';
							break;
							...
							case "email_recipients":
								...
								foreach($emails as $email){
									...
									if($validEmail === false)
										...
										$errors[] = sprintf(esc_html__("%1\$s field has an invalid email address: %2\$s.", "unlimited-elements"), $errorTitle, $email);
								}
							break;

							case "google_connect":
								$services = new UELM_UniteServicesUC();
								...
								try{
									...
									$errors[] = sprintf(__("%s Google access token is missing or expired. Please connect to Google in the \"General Settings > Integrations\".", "unlimited-elements"), $errorTitle);
								}
							break;

							case "url":
								$validUrl = UELM_UniteFunctionsUC::isUrlValid($fieldValue);

								if($fieldValue !== "" && $validUrl === false)
									// translators: %s is page url
									$errors[] = sprintf(esc_html__("%s field has an invalid URL.", "unlimited-elements"), $errorTitle);
							break;

							default:
								UELM_UniteFunctionsUC::throwError("Validation rule \"$ruleName\" is not implemented.");
						}
					}
				}
			}
	...
	private function getFormSettingsValidations(){
		...
		$validations = array(
			...
			array(
				...
				"titles" => array(
					"to" => __("To", "unlimited-elements"),
					"custom_to" => __("Custom To", "unlimited-elements"),
					"subject" => __("Subject", "unlimited-elements"),
					"message" => __("Message", "unlimited-elements"),
					"from" => __("From Address", "unlimited-elements"),
					"reply_to" => __("Reply To", "unlimited-elements"),
					"cc" => __("Cc", "unlimited-elements"),
					"bcc" => __("Bcc", "unlimited-elements"),
				),
			),
			...
			array(
				...
				"titles" => array(
					"credentials" => "",
					"id" => __("Spreadsheet ID", "unlimited-elements"),
				),
			),
			...
			array(
				...
				"titles" => array(
					"name" => __("Name", "unlimited-elements"),
				),
			),
	...
	private function createFormEntry(){
		...
		$isFormEntriesEnabled = UELM_HelperProviderUC::isFormEntriesEnabled();

		if($isFormEntriesEnabled === false)
			return;

		try{
			UELM_UniteFunctionsWPUC::processDBTransaction(function(){

				global $wpdb;

				$entriesTable = UELM_UniteFunctionsWPUC::prefixDBTable(UELM_GlobalsUC::TABLE_FORM_ENTRIES_NAME);
				...
				if($isEntryCreated === false)
					UELM_UniteFunctionsUC::throwError($wpdb->last_error);

				$entryId = $wpdb->insert_id;

				$entryFieldsTable = UELM_UniteFunctionsWPUC::prefixDBTable(UELM_GlobalsUC::TABLE_FORM_ENTRY_FIELDS_NAME);
			...
			UELM_UniteFunctionsUC::throwError("Unable to create form entry: {$e->getMessage()}");
		}
	}
	...
	private function createFormLog($messages){
		...
		$isFormLogsSavingEnabled = UELM_HelperProviderUC::isFormLogsSavingEnabled();
	...
	private function uploadFormFiles(){
		...
		$folderPath = UELM_GlobalsUC::$path_images . $folderName;

		$created = wp_mkdir_p($folderPath);

		if($created === false)
			UELM_UniteFunctionsUC::throwError("Unable to create upload folder: $folderPath");
		...
		foreach($this->formFields as &$field){
			...
			$fieldValue = UELM_UniteFunctionsUC::getVal($field, "value");
			...
			foreach($fieldValue as $file){
				...
				//error handling
				$error = UELM_UniteFunctionsUC::getVal($uploaded_file, "error");
				...
				$uploadedFile = UELM_UniteFunctionsUC::getVal($uploaded_file, "file");
				
				if ( !empty($uploadedFile)) {
					UELM_UniteFunctionsUC::move( $uploadedFile, $filePath, true );
					$moved = true;
				}
				...
				$chmoded = UELM_UniteFunctionsUC::chmod($filePath, 0644);
				...
				$urlFile = UELM_GlobalsUC::$url_images . $folderName . $fileName;
	...
	private function sendEmail($emailFields){
		...
		try {
			...
			if($isSent === false){
				...
				UELM_UniteFunctionsUC::throwError("Unable to send email to $emails.");
			}
		
		} catch (Exception $e) {
    		UELM_UniteFunctionsUC::throwError($e->getMessage());
		}		
	}
	...
	private function sendWebhook($webhookFields){
		...
		$response = UELM_Http::make()->post($webhookFields["url"], $webhookFields["body"]);

		if($response->status() !== 200)
			UELM_UniteFunctionsUC::throwError("Unable to send webhook to {$webhookFields["url"]}.");
	}
	...
	private function getWebhookFields($action){
		...
		$url = UELM_UniteFunctionsUC::getVal($this->formSettings, $this->getFieldKey("url", $action));
		$mode = UELM_UniteFunctionsUC::getVal($this->formSettings, $this->getFieldKey("mode", $action));
		$body = array();
	...
	private function getRedirectFields(){
		...
		$url = UELM_UniteFunctionsUC::getVal($this->formSettings, "redirect_url");
	...
	private function getGoogleSheetsFields(){
		...
		$spreadsheetId = UELM_UniteFunctionsUC::getVal($this->formSettings, "google_sheets_id");
		$sheetId = UELM_UniteFunctionsUC::getVal($this->formSettings, "google_sheets_sheet_id", 0);
	...
	private function getHookFields(){
		...
		$name = UELM_UniteFunctionsUC::getVal($this->formSettings, "hook_name");
	...
	private function prettifyFieldValue($fieldType, $fieldValue){
		...
		$decodedValue = UELM_UniteFunctionsUC::maybeJsonDecode($fieldValue);
	...
	public function submitFormFront(){
		...
		$formData = UELM_UniteFunctionsUC::getPostGetVariable("formData", null, UELM_UniteFunctionsUC::SANITIZE_NOTHING);
		$formFiles = UELM_UniteFunctionsUC::getFilesVariable("formFiles");
		$formId = UELM_UniteFunctionsUC::getPostGetVariable("formId", null, UELM_UniteFunctionsUC::SANITIZE_KEY);
		$postId = UELM_UniteFunctionsUC::getPostGetVariable("postId", null, UELM_UniteFunctionsUC::SANITIZE_ID);
		$templateId = UELM_UniteFunctionsUC::getPostGetVariable("templateId", null, UELM_UniteFunctionsUC::SANITIZE_ID);
		
		$recaptchaToken = UELM_UniteFunctionsUC::getPostGetVariable("recaptcha_token", "", UELM_UniteFunctionsUC::SANITIZE_NOTHING);
		$honeyPotVal = UELM_UniteFunctionsUC::getPostGetVariable("honeypot_val", "", UELM_UniteFunctionsUC::SANITIZE_NOTHING);
				
		UELM_UniteFunctionsUC::validateNotEmpty($formId, "form id");
		UELM_UniteFunctionsUC::validateNumeric($postId, "post id");

		if(empty($formData) === true)
			UELM_UniteFunctionsUC::throwError("No form data found.");
		
		$postContent = UELM_HelperProviderCoreUC_EL::getElementorContentByPostID($postId);
		
		if(empty($postContent))
			UELM_UniteFunctionsUC::throwError("Form elementor content not found.");

		$templateContent = null;

		if(empty($templateId) === false){
			$templateContent = UELM_HelperProviderCoreUC_EL::getElementorContentByPostID($templateId);

			if(empty($templateContent) === true)
				UELM_UniteFunctionsUC::throwError("Template elementor content not found.");
		}
		
		$addonForm = UELM_HelperProviderCoreUC_EL::getAddonWithDataFromContent($postContent, $formId);
	...
	private function doSubmitActions($formSettings, $formFields){
		...
		try{
			$this->addDebugMessage(__("Form has been received.","unlimited-elements"));
			...
			if(empty($formErrors) === false){
				...
				UELM_UniteFunctionsUC::throwError("Form settings validation failed ($formErrors).");
			}
			...
			if($isSpam === true){
				...
				UELM_UniteFunctionsUC::throwError($spamError, self::ERROR_CODE_SPAM);
			}
			...
			if(empty($fieldsErrors) === false){
				...
				UELM_UniteFunctionsUC::throwError($validationError, self::ERROR_CODE_VALIDATION);
			}
			...
			if(empty($filesErrors) === false){
				...
				UELM_UniteFunctionsUC::throwError("Form upload failed.");
			}
			
			// Process form actions
			$formActions = UELM_UniteFunctionsUC::getVal($this->formSettings, "form_actions");
			$actionsErrors = array();
				...
				try{
					...
					switch($action){
						case self::ACTION_SAVE:
							...
							$this->addDebugMessage(__("Form entry has been successfully created.","unlimited-elements"));
						break;
						...
						case self::ACTION_EMAIL2:
							...
							$this->addDebugMessage(__("Email has been successfully sent to .","unlimited-elements").$emails);
						break;
						...
						default:
							UELM_UniteFunctionsUC::throwError("Form action \"$action\" is not implemented.");
					}

					$this->executeFormAction("after_{$action}_action");
				}catch(Exception $exception){
					$errorMessage = "{$this->getActionTitle($action)}: {$exception->getMessage()}";
					$debugType = UELM_UniteFunctionsUC::getVal($this->formSettings, "debug_type");
			...
			if(empty($actionsErrors) === false){
				...
				UELM_UniteFunctionsUC::throwError("Form actions failed ($actionsErrors).");
			}
		...
		$isDebug = UELM_UniteFunctionsUC::getVal($this->formSettings, "debug_mode");
		...
		UELM_HelperUC::ajaxResponse($success, $message, $data);
	}
	...
	private function detectSpamFromAntispam(){
		...
		$userIp = UELM_UniteFunctionsUC::getUserIp();
		$currentTime = current_time("timestamp");

		// get blocks
		$antispamBlocks = $this->getAntispamBlocks();
		$userBlockTime = UELM_UniteFunctionsUC::getVal($antispamBlocks, $userIp, 0);
		...
		$userSubmissions = UELM_UniteFunctionsUC::getVal($antispamSubmissions, $userIp, array());
	...
	private function detectSpamFromHoneypot(){
		...
		$enableHoneypot = UELM_UniteFunctionsUC::getVal($this->formSettings, "enable_honeypot");
		$enableHoneypot = UELM_UniteFunctionsUC::strToBool($enableHoneypot);
		
		if($enableHoneypot == false)
			return(false);
		
		$honeyPotValue = UELM_UniteFunctionsUC::getVal($this->formSettings, "honeypot_value");
		
		if(empty($honeyPotValue))
			return(false);
		
		$this->lastSpamError = __("Honey Pot Value Detected", "unlimited-elements");
	...
	private function detectSpamRecaptcha(){
		...
		$recaptchaExists = UELM_UniteFunctionsUC::getVal($this->formSettings, "add_recaptcha_protection");
		$recaptchaExists = UELM_UniteFunctionsUC::strToBool($recaptchaExists);
		
		if($recaptchaExists == false)
			return(false);
		
	    $siteKey = UELM_HelperProviderCoreUC_EL::getGeneralSetting("recaptcha_site_key");
	    $secretKey = UELM_HelperProviderCoreUC_EL::getGeneralSetting("recaptcha_secret_key");
	
	    if (empty($siteKey) || empty($secretKey))
	        return false;
		
	    $token = UELM_UniteFunctionsUC::getVal($this->formSettings, "recaptcha_token");
	    
	    //get the trashold
	    $threshold = UELM_UniteFunctionsUC::getVal($this->formSettings, "recaptcha_threshold",0.5);
	    ...
	    if (empty($token)) {
	    	...
	    	$this->lastSpamError = __("No recaptcha token recieved","unlimited-elements");
	    ...
	    $args = array(
	        "body" => array(
	            "secret" => $secretKey,
	            "response" => $token,
	            "remoteip" => UELM_UniteFunctionsUC::getUserIp(),
	        ),
	        "timeout" => 20,
	    );
	    ...
	    $errorPrefix = __("reCAPTCHA error(s):","unlimited-elements");
	    ...
	    if (empty($result["success"]) || $result["success"] !== true) {
		    ...
		    if (!empty($result["error-codes"])) {
	        	...
	        	$this->lastSpamError =  __("reCAPTCHA verification failed.","unlimited-elements");
	    	}
    	    
	        return true; // Verification failed
	    }
	
	    $score = floatval(UELM_UniteFunctionsUC::getVal($result, "score", 0));
		
	    if ($score < $threshold) {
	       	
	    	$this->lastSpamError = $errorPrefix.__("score too low: $score, the threshold is: $threshold","unlimited-elements");
	...
	private function getFormName(){
		...
		return $this->formSettings["form_name"] ?: __("Unnamed", "unlimited-elements");
	}
	...
	private function getFormMeta(){
		...
		if($this->formMeta === null)
			$this->formMeta = array(
				...
				"user_ip" => UELM_UniteFunctionsUC::getUserIp(),
				"user_agent" => UELM_UniteFunctionsUC::getUserAgent(),
	...
	private function getFormMessage($key, $fallback){
		...
		$message = UELM_UniteFunctionsUC::getVal($this->formSettings, $key);
	...
	private function getFormSuccessMessage(){
		...
		$fallback = __("Your submission has been received!", "unlimited-elements");
	...
	private function getFormErrorMessage(){
		...
		$fallback = __("Oops! Something went wrong, please try again later.", "unlimited-elements");
	...
	private function getSpamErrorMessage(){
		...
		$fallback = __("Something went wrong, please try again later.", "unlimited-elements");
	...
	private function getValidationErrorMessage($errors){
		...
		$fallback = __("Please correct the following errors:", "unlimited-elements");
	...
	private function getFieldEmptyErrorMessage(){
		...
		$fallback = __("The field is empty.", "unlimited-elements");
	...
	private function getFileUploadErrorMessage(){
		...
		$fallback = __("The file upload failed.", "unlimited-elements");
	...
	private function getFileTypeErrorMessage($extensions){
		...
		$fallback = __("The file must be of type: %s.", "unlimited-elements");
	...
	private function getFieldTitle($field){
		...
		return $field["title"] ?: __("Untitled", "unlimited-elements");
	}
	...
	private function getActionTitle($key){
		...
		$titles = array(
			self::ACTION_EMAIL => __("Email", "unlimited-elements"),
			self::ACTION_EMAIL2 => __("Email 2", "unlimited-elements"),
			self::ACTION_WEBHOOK => __("Webhook", "unlimited-elements"),
			self::ACTION_WEBHOOK2 => __("Webhook 2", "unlimited-elements"),
			self::ACTION_REDIRECT => __("Redirect", "unlimited-elements"),
			self::ACTION_GOOGLE_SHEETS => __("Google Sheets", "unlimited-elements"),
			self::ACTION_HOOK => __("WordPress Hook", "unlimited-elements"),
		);

		return UELM_UniteFunctionsUC::getVal($titles, $key, $key);
	}
	...
	private function getPlaceholderReplace($placeholder){
		...
		switch($placeholder){
			...
			case self::PLACEHOLDER_EMAIL_FIELD:
				foreach($this->formFields as $field){
					$validEmail = UELM_UniteFunctionsUC::isEmailValid($field["value"]);
			...
			case self::PLACEHOLDER_PAGE_URL:
				...
				$urlPage = UELM_UniteFunctionsWPUC::getUrlCurrentPage();
	...
	private function getEmailFields($action){
		...
		$from = UELM_UniteFunctionsUC::getVal($this->formSettings, $this->getFieldKey("from", $action));
		$from = $this->replacePlaceholders($from, array(self::PLACEHOLDER_ADMIN_EMAIL));
		
		$fromName = UELM_UniteFunctionsUC::getVal($this->formSettings, $this->getFieldKey("from_name", $action));
		$fromName = $this->replaceTitlePlaceholders($fromName);
		
		$replyTo = UELM_UniteFunctionsUC::getVal($this->formSettings, $this->getFieldKey("reply_to", $action));
		$replyTo = $this->replacePlaceholders($replyTo, array(self::PLACEHOLDER_ADMIN_EMAIL, self::PLACEHOLDER_EMAIL_FIELD));
		
		$to = UELM_UniteFunctionsUC::getVal($this->formSettings, $this->getFieldKey("to", $action));

		if($to === "custom")
			$to = UELM_UniteFunctionsUC::getVal($this->formSettings, $this->getFieldKey("custom_to", $action));
		...
		$cc = UELM_UniteFunctionsUC::getVal($this->formSettings, $this->getFieldKey("cc", $action));
		...
		$bcc = UELM_UniteFunctionsUC::getVal($this->formSettings, $this->getFieldKey("bcc", $action));
		...
		$subject = UELM_UniteFunctionsUC::getVal($this->formSettings, $this->getFieldKey("subject", $action));
		$subject = $this->replaceTitlePlaceholders($subject);
		
		$subject = html_entity_decode($subject, ENT_QUOTES | ENT_HTML5);
		
		$message = UELM_UniteFunctionsUC::getVal($this->formSettings, $this->getFieldKey("message", $action));
	...
	private function prepareFilesFieldAllowedTypes($fieldSettings){
		...
		$allowedTypes = UELM_UniteFunctionsUC::getVal($fieldSettings, "allowed_types", array());
		$customAllowedTypes = UELM_UniteFunctionsUC::getVal($fieldSettings, "custom_allowed_types");
		...
		*/
		...
		foreach($allowedTypes as $type){
			if(isset($typesMap[$type]) === false)
				UELM_UniteFunctionsUC::throwError("File type \"$type\" is not implemented.");
	...
	private function getAntispamSettings(){
		...
		if(UELM_GlobalsUC::$isLocal == true)
			return(false);
		
		$enabled = UELM_HelperProviderCoreUC_EL::getGeneralSetting("form_antispam_enabled");
		$enabled = UELM_UniteFunctionsUC::strToBool($enabled);

		$submissionsLimit = UELM_HelperProviderCoreUC_EL::getGeneralSetting("form_antispam_submissions_limit");
		...
		$submissionsPeriod = UELM_HelperProviderCoreUC_EL::getGeneralSetting("form_antispam_submissions_period");
		...
		$blockPeriod = UELM_HelperProviderCoreUC_EL::getGeneralSetting("form_antispam_block_period");
