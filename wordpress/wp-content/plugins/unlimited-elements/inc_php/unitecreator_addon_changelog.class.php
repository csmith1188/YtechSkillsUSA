...
class UELM_UniteCreatorAddonChangelog{
	...
	public function getTable(){
		...
		$table = UELM_UniteFunctionsWPUC::prefixDBTable(UELM_GlobalsUC::TABLE_CHANGELOG_NAME);
	...
	public function getAddonTitle($addonId, $fallback){
		...
		try{
			...
			return sprintf(__("%s (deleted)", "unlimited-elements"), $fallback);
		}
	}
	...
	public function getAddonVersion($addonId){
		...
		try{
			...
			$isFree = UELM_UniteFunctionsUC::getVal($options, "is_free_addon");
			$isFree = UELM_UniteFunctionsUC::strToBool($isFree);

			if($isFree === true)
				return __("Free", "unlimited-elements");

			return __("Pro", "unlimited-elements");
		}catch(Exception $exception){
			return null;
		}
	}
	...
	public function getAddonChangelogs($addonId, $filters = array()){
		...
		$limit = UELM_UniteFunctionsUC::getVal($filters, "limit", null);
	...
	public function findChangelog($id){
		...
		$idPlaceholders = UELM_UniteFunctionsWPUC::getDBPlaceholders($ids, "%d");
	...
	public function addChangelog($addonId, $type, $text){
		...
		$data = array(
			...
			"type" => $type,
			"text" => $text,
			"plugin_version" => UELM_UNLIMITED_ELEMENTS_VERSION,
	...
	public function updateChangelog($id, $data){
		...
		$result = UELM_UniteFunctionsWPUC::processDBTransaction(function() use ($id, $data){
	...
	public function deleteChangelog($id){
		...
		$result = UELM_UniteFunctionsWPUC::processDBTransaction(function() use ($id){
	...
	private function getTypeTitle($type){
		...
		$titles = array(
			self::TYPE_RELEASE => __("Release", "unlimited-elements"),
			self::TYPE_CHANGE => __("Change", "unlimited-elements"),
			self::TYPE_FEATURE => __("Feature", "unlimited-elements"),
			self::TYPE_FIX => __("Fix", "unlimited-elements"),
			self::TYPE_OTHER => __("Other", "unlimited-elements"),
			self::TYPE_REQUEST => __("Request", "unlimited-elements"),
		);

		return UELM_UniteFunctionsUC::getVal($titles, $type, $type);
	}
	...
	private function prepareChangelogs($results){
		...
		foreach($results as $result){
			...
			$changelogs[] = array_merge($result, array(
				"user_username" => $user ? $user->user_login : __("(deleted)", "unlimited-elements"),
