...
class UELM_UniteCreatorLayoutsExporterWork extends UELM_UniteCreatorExporterBase{
	...
	protected function validateInited(){
		...
		if(empty($this->objLayout))
			UELM_UniteFunctionsUC::throwError("The layout object is not inited");
	}
	
	/**
	 * init by layout
	 */
	public function initByLayout(UELM_UniteCreatorLayout $objLayout){
	...
	protected function putLayoutFile($strLayout = null, $filename = null){
		...
		UELM_UniteFunctionsUC::writeFile($strLayout, $filepath);
	}
	...
	private function createCategoryCopyPath(){
		...
		$catID = UELM_UniteFunctionsUC::getVal($arrCat, "id");
		
		if(empty($catName))
			UELM_UniteFunctionsUC::throwError("Category should not be empty");
		
		$catID = UELM_UniteFunctionsUC::getVal($arrCat, "id");
		if(empty($catID))
			UELM_UniteFunctionsUC::throwError("Category should have id");
				
		$path = $this->pathExportLayouts.$catName."/";
		
		UELM_UniteFunctionsUC::mkdirValidate($path, "Copy Layout Cat Folders");
	...
	protected function prepareExportFolders_layout(){
		...
		UELM_UniteFunctionsUC::validateDir($this->pathExportLayouts, "Export Layouts");
		
		//make layout folder
		$this->pathExportLayout = $this->pathExportLayouts."template_".UELM_UniteFunctionsUC::getRandomString(10)."/";
		UELM_UniteFunctionsUC::mkdirValidate($this->pathExportLayout, "Export Layout");
		
		//make inner paths
		$this->pathExportLayoutAddons = $this->pathExportLayout."addons/";
		UELM_UniteFunctionsUC::mkdirValidate($this->pathExportLayoutAddons, "Layout Addons");

		$this->pathExportLayoutBGAddons = $this->pathExportLayout."bg_addons/";
		
		$this->pathExportLayoutImages = $this->pathExportLayout."images/";
		UELM_UniteFunctionsUC::mkdirValidate($this->pathExportLayoutImages, "Layout Images");
	...
	protected function prepareExportFolders_layouts(){
		...
		UELM_UniteFunctionsUC::mkdirValidate($this->pathExportLayouts, "Layouts");
		
		//clean folder
		if($this->isLayoutsFolderInited == false)
			UELM_UniteFunctionsUC::deleteDir($this->pathExportLayouts, false);
		
		//create index.html
		UELM_UniteFunctionsUC::writeFile("", $this->pathExportLayouts."index.html");
	...
	protected function putLayoutAddons($arrAddons = null, $isBGAddons = false){
		...
		if($isBGAddons == true && !empty($arrAddons)){
			...
			UELM_UniteFunctionsUC::mkdirValidate($this->pathExportLayoutBGAddons, "Layout Background Addons");
			
			$pathExport = $this->pathExportLayoutBGAddons;
			$addonsType = UELM_GlobalsUC::ADDON_TYPE_BGADDON;
		}
		...
		foreach($arrAddons as $addon){
			...
			$objAddonExporter = new UELM_UniteCreatorExporter();
	...
	protected function prepareExportZip($title = null){
		...
		$handle = UELM_HelperUC::convertTitleToHandle($title);
		
		
		UELM_UniteFunctionsUC::validateDir($this->pathCopyLayout,"copy layout path");
		...
		if(file_exists($this->pathExportLayout) == false)
			UELM_UniteFunctionsUC::throwError("zip file {$filepath} could not be created");
	...
	protected function downloadExportFile(){
		...
		UELM_UniteFunctionsUC::downloadFile($this->pathExportZip);
	}
	...
	protected function getExportedFileData(){
		...
		$urlFile = UELM_HelperUC::pathToFullUrl($filepath);
	...
	public function deleteExportLayoutFolder(){
		...
		if(is_dir($this->pathExportLayout))
			UELM_UniteFunctionsUC::deleteDir($this->pathExportLayout, true);
	...
	private function processConfigImage_getSaveFilename($pathImage, $forceSaveFilename = null){
		...
		$counter = 0;
		$textPortion = UELM_UniteFunctionsUC::getStringTextPortion($basename);
	...
	protected function processConfigImage($urlImage, $forceSaveFilename = null){
		...
		if(is_numeric($urlImage))
			$urlImage = UELM_UniteProviderFunctionsUC::getImageUrlFromImageID($urlImage);
		...
		$urlImage = UELM_HelperUC::URLtoFull($urlImage);
		$pathImage = UELM_HelperUC::urlToPath($urlImage);
		...
		$handlePath = UELM_HelperUC::convertTitleToHandle($pathImage, false);
	...
	private function modifyAddons_exportImport($addonData, $modifyType){
		...
		foreach($arrConfigImages as $key=>$urlImage){
			...
			switch($modifyType){
				...
				break;
				default:
					UELM_UniteFunctionsUC::throwError("Wrong modify type: $modifyType");
				break;
			}
		...
		//process items
		$arrItemsImages = $objAddon->getProcessedItemsData(UELM_UniteCreatorParamsProcessor::PROCESS_TYPE_SAVE, false, "uc_image");
	...
	public function modifySettinsImagesForExport($arrSettings, $arrImageKeys){
		...
		foreach($arrImageKeys as $key){
			...
			$urlImage = UELM_UniteFunctionsUC::getVal($arrSettings, $key);
			if(empty($urlImage))
				continue;
	...
	protected function addLayoutPreviewImage(){
		...
		$layoutNew = new UELM_UniteCreatorLayout();
	...
	private function putLayoutImages(){
		...
		foreach($this->arrExportImages as $arrImage){
			...
			if(is_file($sourceFilepath) == false)
				UELM_UniteFunctionsUC::throwError("Image file: $sourceFilepath not found!");
	...
	private function getImportLayoutCatID($arrLayout){
		...
		//force category
		$forceCatID = UELM_UniteFunctionsUC::getVal($this->importParams, "force_to_cat");
		if(!empty($forceCatID)){
		...
		$catTitle = UELM_UniteFunctionsUC::getVal($arrLayout, "catname");
		if(empty($catTitle))
			return($catID);
	...
	private function importLayoutByContent($content, $layoutID = null){
		...
		if(empty($arrLayout))
			UELM_UniteFunctionsUC::throwError("Wrong file format");
		
		$arrLayout = UELM_UniteFunctionsUC::convertStdClassToArray($arrLayout);
		
		$arrLayoutOriginal = $arrLayout;
		
		//set layout type
		$layoutType = UELM_UniteFunctionsUC::getVal($arrLayout, "type");
		$this->objLayout->setLayoutType($layoutType);
		
		$objType = UELM_UniteCreatorAddonType::getAddonTypeObject($layoutType, true);
		
		if(empty($layoutID)){	//new layout
			
			$title = UELM_UniteFunctionsUC::getVal($arrLayout, "title");
			$title = $objLayouts->getUniqueTitle($title);
			
			$name = null;	//will generate
			
			if($objType->isBasicType == false)
				$name = UELM_UniteFunctionsUC::getVal($arrLayout, "name");
			
			$params = UELM_UniteFunctionsUC::getVal($arrLayout, "params");
			...
			$arrUpdate = array();
			$arrUpdate["layout_data"] = UELM_UniteFunctionsUC::getVal($arrLayout, "layout_data");
			$this->objLayout->updateLayoutInDB($arrUpdate);
			
			$arrParams = UELM_UniteFunctionsUC::getVal($arrLayout, "params");
			...
			$arrUpdate = array();
			$arrUpdate["import_title"] = UELM_UniteFunctionsUC::getVal($arrLayout, "title");
			$arrUpdate["layout_data"] = UELM_UniteFunctionsUC::getVal($arrLayout, "layout_data");
		...
		UELM_UniteProviderFunctionsUC::doAction(UELM_UniteCreatorFilters::ACTION_AFTER_IMPORT_LAYOUT_FILE, $arrLayoutOriginal, $this->lastImportID);
	...
	private function importTxtFile($filepath, $layoutID = null){
		...
		if(empty($content))
			UELM_UniteFunctionsUC::throwError("layout file content don't found");
	...
	protected function prepareImportFolders(){
		...
		UELM_UniteFunctionsUC::mkdirValidate($pathImportBase, "import layout base");
		...
		$this->pathImportLayout = $pathImportBase."layout_".self::$serial."_".UELM_UniteFunctionsUC::getRandomString(10)."/";
		
		UELM_UniteFunctionsUC::mkdirValidate($this->pathImportLayout, "import layout");
	...
	protected function extractImportLayoutFile($filepath){
		...
		if($extracted == false)
			UELM_UniteFunctionsUC::throwError("The import layout zip didn't extracted");
	...
	protected function importLayoutTxtFromZip($layoutID = null){
		...
		UELM_UniteFunctionsUC::validateFilepath($filepathLayout,"layout.txt");
		
		$content = UELM_UniteFunctionsUC::fileGetContents($filepathLayout);
	...
	public function importLayoutAddons($overwriteAddons = false){
		...
		$exporterAddons = new UELM_UniteCreatorExporter();
	...
	public function importLayoutBGAddons($overwriteAddons = false){
		...
		$exporterAddons = new UELM_UniteCreatorExporter();
		$exporterAddons->setMustImportAddonType(UELM_GlobalsUC::ADDON_TYPE_BGADDON);
	...
	public function import($arrFile, $layoutID=null, $overwriteAddons = false, $params = array()){
		...
		if(is_string($arrFile))
			...
			$filepath = UELM_UniteFunctionsUC::getVal($arrFile, "tmp_name");
		
		if(empty($filepath))
			UELM_UniteFunctionsUC::throwError("layout filepath not found");
		...
		switch($ext){
			...
			case "zip":
				...
				UELM_UniteFunctionsUC::throwError("Import layout zip file is not supported anymore");
				
				$this->importZipFile($filepath, $layoutID, $overwriteAddons);
				break;
			default:
				UELM_UniteFunctionsUC::throwError("Wrong file: $filename");
			break;
		}
	...
	private function importLayoutImages_getDestFilepath($path, $filename, $filepathSource){
		...
		$newFilename = UELM_UniteFunctionsUC::findFreeFilepath($path, $filename, $filepathSource);
	...
	protected function importLayoutImages_copyImages(){
		...
		$pathDest = UELM_GlobalsUC::$path_images.self::IMAGES_IMPORT_FOLDER."/";		
		UELM_UniteFunctionsUC::checkCreateDir($pathDest);
		...
		foreach($this->arrImportImages as $key => $arrImage){
			...
			if(!file_exists($destFilepath)){
				...
				if($success == false)
					UELM_UniteFunctionsUC::throwError("file: $filename could not be copied to path: $copyPathDest");
			}
			
			$arrImage["dest"] = $destFilepath;
			
			$arrImage["url"] = UELM_HelperUC::pathToRelativeUrl($destFilepath);
			$arrImage["urlfull"] = UELM_HelperUC::pathToFullUrl($destFilepath);
	...
	protected function getImportedImageData($urlImage){
		...
		$arrImage = UELM_UniteFunctionsUC::getVal($this->arrImportImages, $filename);
	...
	protected function processConfigImage_import($urlImage){
		...
		if(empty($arrImage))
			UELM_UniteFunctionsUC::throwError("Local image: $filename not found");
		
		//check if exists image id
		$imageID = UELM_UniteFunctionsUC::getVal($arrImage, "imageid");
		
		if(!empty($imageID))
			return($imageID);
		
		$url = UELM_UniteFunctionsUC::getVal($arrImage, "url");
		
		UELM_UniteFunctionsUC::validateNotEmpty($url,"url for image: $filename");
	...
	protected function importLayoutImages(){
		...
		UELM_UniteProviderFunctionsUC::doAction(UELM_UniteCreatorFilters::ACTION_AFTER_IMPORT_LAYOUT_FILE_IMAGES, $this->lastImportID, $this->arrImportImages);
