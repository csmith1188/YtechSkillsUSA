...
class UELM_UniteCreatorLayoutWork extends UELM_UniteElementsBaseUC{
	...
	public function setLayoutType($type){
		...
		$this->objLayoutType = UELM_UniteCreatorAddonType::getAddonTypeObject($type, true);
	...
	public function validateInited(){
		...
		if(empty($this->id))
			UELM_UniteFunctionsUC::throwError("The layout is not inited");
	}
	...
	private function validateTitle($title){
		...
		UELM_UniteFunctionsUC::validateNotEmpty($title, "Title");
	...
	private function validateNameNotExists($name, $layoutType){
		...
		if($isExists == true)
			UELM_UniteFunctionsUC::throwError("The layout with name: $name already exists");
	...
	private function validateName($name){
		...
		$fieldName = $this->objLayoutType->textSingle.esc_html__(" Name", "unlimited-elements");
		
		UELM_UniteFunctionsUC::validateNotEmpty($name, $fieldName);
		UELM_UniteFunctionsUC::validateUrlAlias($name, $fieldName);
	...
	private function validateLayoutNotExistsByTitle($title, $layoutType){
		...
		if($isExists == true)
			UELM_UniteFunctionsUC::throwError("The layout with title: $title already exists");
	...
	public static function getGridGlobalStoredValues(){
		...
		$arrValues = UELM_UniteProviderFunctionsUC::getOption(self::LAYOUTS_GLOBAL_SETTINGS_OPTION, array());
	...
	public static function getGlobalSettingsObject($includeGlobalOptions = true){
		...
		$filepathSettings = UELM_GlobalsUC::$pathSettings."layouts_global_settings.xml";
	...
	public static function getGridSettingsObject(){
		...
		$filepathSettings = UELM_GlobalsUC::$pathSettings."layouts_grid_settings.xml";
	...
	public function getPageParamsSettingsObject(){
		...
		switch($this->objLayoutType->paramsSettingsType){
			case UELM_UniteCreatorAddonType_Layout::LAYOUT_PARAMS_TYPE_SCREENSHOT:
				$filepathSettings = UELM_GlobalsUC::$pathSettings."layout_grid_settings_screenshot.xml";
			break;
			default:
				UELM_UniteFunctionsUC::throwError("Wrong page params type: ".$this->objLayoutType->paramsSettingsType);
			break;
		}
		...
		$objSettings = UELM_UniteProviderFunctionsUC::applyFilters(UELM_UniteCreatorFilters::FILTER_LAYOUT_PROPERTIES_SETTINGS, $objSettings);
	...
	public static function updateLayoutGlobalSettingsFromData($data){
		...
		UELM_UniteProviderFunctionsUC::updateOption(self::LAYOUTS_GLOBAL_SETTINGS_OPTION, $arrValues);
	}
	...
	public function initByName($name, $type){
		...
		//validate
		$objType = UELM_UniteCreatorAddonType::getAddonTypeObject($type, true);
		...
		try{
			$record = $this->db->fetchSingle(UELM_GlobalsUC::$table_layouts, $where);
		}catch(Exception $e){
			UELM_UniteFunctionsUC::throwError("Layout with name: $name not found");
		}
		UELM_UniteFunctionsUC::validateNotEmpty($record, "layout record");
	...
	public function initByID($id){
		...
		$id = (int)$id;
		if(empty($id))
			UELM_UniteFunctionsUC::throwError("Empty layout ID");
		
		$options = array();
		$options["tableName"] = UELM_GlobalsUC::$table_layouts;
		$options["where"] = "id=".$id;
	...
	public function getAllGridOptions(){
		...
		$layoutOptions = UELM_UniteFunctionsUC::getVal($this->gridData, "options", array());
	...
	protected function modifyInitLayoutData($gridData){
		...
		$rows = UELM_UniteFunctionsUC::getVal($gridData, "rows");
		if(empty($rows))
			return($gridData);
		...
		foreach($rows as $keyRow => $row){
			...
			$cols = UELM_UniteFunctionsUC::getVal($row, "cols");
			if(empty($cols))
				continue;
	...
	protected function generateName($title = "", $layoutType = null, $baseName = null){
		...
		UELM_UniteFunctionsUC::validateNotEmpty($title);
		...
		$alias = UELM_HelperUC::convertTitleToAlias($title);
		...
		if($isExists == true)
			$alias .= "-".UELM_UniteFunctionsUC::getRandomString(5);
	...
	public function initByRecord($record){
		...
		$this->ordering = UELM_UniteFunctionsUC::getVal($record, "ordering");
		$this->layoutType = UELM_UniteFunctionsUC::getVal($record, "layout_type");
		
		if(empty($this->name))
			$this->name = $this->generateName(null, $this->layoutType);
		
		$this->objLayoutType = UELM_UniteCreatorAddonType::getAddonTypeObject($this->layoutType, true);
		
		$this->isTemplate = $this->objLayoutType->isTemplate;
		
		$data = UELM_UniteFunctionsUC::getVal($record, "layout_data");
		$data = UELM_UniteFunctionsUC::maybeUnserialize($data);
		
		$this->record = $record;
		$this->data = $data;
		
		$params = UELM_UniteFunctionsUC::getVal($record, "params");
		if(!empty($params))
			$this->params = UELM_UniteFunctionsUC::jsonDecode($params);
		
		if(empty($this->params))
			$this->params = array();
			
		$gridData = UELM_UniteFunctionsUC::getVal($data, "grid_data");
		...
		//uelm_dmp("print layout"); uelm_dmp($this->gridData);exit();
	...
	private function modifyAddonDataForEditor($addonData){
		...
		try{
			...
			//merge config
			$arrConfig = $objAddon->getProcessedMainParamsValues(UELM_UniteCreatorParamsProcessor::PROCESS_TYPE_CONFIG);
			
			$origConfig = UELM_UniteFunctionsUC::getVal($addonData, "config");
			if(empty($origConfig))
				$origConfig = array();
			...
			if($objAddon->isHasItems()){
				$arrItems = $objAddon->getProcessedItemsData(UELM_UniteCreatorParamsProcessor::PROCESS_TYPE_CONFIG, false);
	...
	protected function createGridDataFull_modifySettingsImage($settings, $key){
		...
		$value = UELM_UniteFunctionsUC::getVal($settings, $key);
		if(empty($value))
			return($settings);
		...
		//url is ID
		$urlImage = UELM_UniteProviderFunctionsUC::getImageUrlFromImageID($value);
		if(empty($urlImage))
			return($settings);
	...
	protected function createGridDataFull_modifySettingsBGAddons($settings, $addParams){
		...
		$bgAddonName = UELM_UniteFunctionsUC::getVal($settings, $settingsKey);
		
		if(empty($bgAddonName))
			return($settings);	
	
		$addonData = UELM_UniteFunctionsUC::getVal($settings,  $settingsKey."_data");
		if(empty($addonData)){
		    
		    $addonData = array();
		    $addonData["name"] = $bgAddonName;
		    $addonData["addontype"] = UELM_GlobalsUC::ADDON_TYPE_BGADDON;
	...
	private function createGridDataFull_modifyAddonData($addonData, $addParams = null){
		...
		$isAddContent = UELM_UniteFunctionsUC::getVal($addParams, "add_addon_content");
		$isAddContent = UELM_UniteFunctionsUC::strToBool($isAddContent);
	...
	public function modifyAddonDataForSave($data){
		...
		try{
			...
			$config = UELM_UniteFunctionsUC::getVal($data, "config");
			...
			if(!empty($items) && is_array($items)){
				...
				$arrItemsImages = $addon->getProcessedItemsData(UELM_UniteCreatorParamsProcessor::PROCESS_TYPE_SAVE, false, "uc_image");
	...
	private function prepareDataForSave($gridData){
		...
		if(is_string($gridData)){
			$gridData = UELM_UniteFunctionsUC::decodeContent($gridData);
	...
	private function mapModifyLayoutDataSettings_columns($parent, $modifyFunc, $addParams = null){
			...
			$cols = UELM_UniteFunctionsUC::getVal($parent, "cols");
			if(empty($cols))
				return($parent);
			
			foreach($cols as $colIndex=>$col){
				
				$colSettings = UELM_UniteFunctionsUC::getVal($col, "settings");
				...
				$addonData = UELM_UniteFunctionsUC::getVal($col, "addon_data");
				if(empty($addonData))
					continue;
				
				foreach($addonData as $addonIndex=>$addon){
					
					$addonOptions = UELM_UniteFunctionsUC::getVal($addon, "options");
	...
	private function checkSettingsAddData($arrElement, $keyOptions){
		...
		/*
		$settings = UELM_UniteFunctionsUC::getVal($arrElement, $keyOptions);
	...
	public function mapModifyLayoutDataSettings($arrData, $modifyFunc, $addParams = null){
		...
		//modify grid options
		$gridSettings = UELM_UniteFunctionsUC::getVal($arrData, "options");
		...
		//modify rows options
		$rows = UELM_UniteFunctionsUC::getVal($arrData, "rows");
		...
		foreach($rows as $rowIndex => $row){
			...
			$rowSettings = UELM_UniteFunctionsUC::getVal($row, "settings");
			...
			$arrContainers = UELM_UniteFunctionsUC::getVal($row, "containers");
			...
			if(!empty($arrContainers)){
				...
				foreach($arrContainers as $containerIndex=>$container){
					...
					$containerSettings = UELM_UniteFunctionsUC::getVal($container, "settings");
	...
	private function mapModifyLayoutDataAddons_columns($arrParent, $modifyFunc, $addParams = null){
			...
			$cols = UELM_UniteFunctionsUC::getVal($arrParent, "cols");
	...
	public function mapModifyLayoutDataAddons($arrData, $modifyFunc, $addParams = null){
		...
		foreach($rows as $keyRow=>$row){
			...
			$containers = UELM_UniteFunctionsUC::getVal($row, "containers");
			if(!empty($containers)){
	...
	public function getRowsFront(){
		...
		$rows = UELM_UniteFunctionsUC::getVal($this->gridData, "rows");
		
		if(empty($rows))
		    $rows = array();
		
		//UELM_UniteFunctionsUC::validateNotEmpty($rows, "Layout Rows");		
		//uelm_dmp($this->gridData); exit();
	...
	public function getGridOptionsDiff(){
		...
		$arrDiffOptions = UELM_UniteFunctionsUC::getDiffArrItems($allOptions, $defaultOptions);
	...
	public function getDescription($specialChars = false){
		...
		$description = UELM_UniteFunctionsUC::getVal($this->params, "description");
		if($specialChars == true)
	...
	public function getIcon(){
		...
		$icon = UELM_UniteFunctionsUC::getVal($this->params, "page_icon");
	...
	public function getPreviewImage($getThumb = false){
		...
		//$isAutoGenerate = UELM_UniteFunctionsUC::getVal($params, "auto_generate_thumb");
		//$isAutoGenerate = UELM_UniteFunctionsUC::strToBool($isAutoGenerate);

		$arrImageKeys = array("preview_image","custom_preview_image","page_image");
		
		foreach($arrImageKeys as $key){
		    $urlPreview = UELM_UniteFunctionsUC::getVal($params, $key);
		   ...
		   if(is_numeric($urlPreview))
		         $urlPreview = UELM_UniteProviderFunctionsUC::getImageUrlFromImageID($urlPreview);
		           
		   $urlPreview = UELM_HelperUC::URLtoFull($urlPreview);
	...
	public function getPreviewImageFilepath(){
		...
		$pathImage = UELM_HelperUC::urlToPath($urlPreview);
		if(empty($pathImage))
			return(null);
	...
	public function getParentID(){
		...
		uelm_dmp("getParentID - function for override");
		exit();
	}
	...
	public function getObjLayoutType(){
		...
		if($this->isInited() == true)
			UELM_UniteFunctionsUC::throwError("The layout type should be inited");
		
		$this->objLayoutType = UELM_UniteCreatorAddonType::getAddonTypeObject(UELM_GlobalsUC::ADDON_TYPE_REGULAR_LAYOUT, true);
	...
	public function getCatNameForExport(){
		...
		$catName = UELM_UniteFunctionsUC::getVal($arrCat, "name");
		if(empty($catName))
			return(false);
		...
		$prefix = "";
		if($typeName != UELM_GlobalsUC::ADDON_TYPE_REGULAR_LAYOUT)
    ...
    public function getCategory(){
        ...
        $objCats = new UELM_UniteCreatorCategories();
       	...
       	$catID = UELM_UniteFunctionsUC::getVal($this->record, "catid");
	...
	public function getShortcode($shortcodeName = ""){
		...
		if(empty($shortcodeName))
			$shortcodeName = UELM_GlobalsUC::$layoutShortcodeName;
		
		$shortcode = $shortcodeName." id={$id} title=\"{$title}\"";
		
		$shortcode = UELM_UniteProviderFunctionsUC::wrapShortcode($shortcode);
	...
	protected function isLayoutExistsByTitle($title, $layoutType){
		...
		$response = $this->db->fetch(UELM_GlobalsUC::$table_layouts, "title='{$title}' and {$sqlType}");
	...
	protected function isLayoutExistsByName($name, $layoutType){
		...
		$response = $this->db->fetch(UELM_GlobalsUC::$table_layouts, $where);
	...
	private function collectAddonSpecialAddons_params($paramsShapes, $config, $addonType){
		...
		foreach($paramsShapes as $param){
			...
			$shapeName = UELM_UniteFunctionsUC::getVal($config, $paramName);
			if(empty($shapeName))
				continue;
	...
	private function getParamsShapesFromAddon($objAddon, $isItems = false){
		...
		if($isItems == false)
			$paramsShapes = $objAddon->getParams(UELM_UniteCreatorDialogParam::PARAM_SHAPE);
		else
			$paramsShapes = $objAddon->getParamsItems(UELM_UniteCreatorDialogParam::PARAM_SHAPE);
		...
		if($isItems == false)
			$paramsAddonPickers = $objAddon->getParams(UELM_UniteCreatorDialogParam::PARAM_ADDONPICKER);
		else
			$paramsAddonPickers = $objAddon->getParamsItems(UELM_UniteCreatorDialogParam::PARAM_ADDONPICKER);
		...
		foreach($paramsAddonPickers as $param){
			$addonType = UELM_UniteFunctionsUC::getVal($param, "addon_type");
			if($addonType == UELM_GlobalsUC::ADDON_TYPE_SHAPES)
				$paramsShapes[] = $param;
		}
	...
	private function collectAddonSpecialAddons($addonName, $addonData){
		...
		if(!empty($paramsShapes)){
			$config = UELM_UniteFunctionsUC::getVal($addonData, "config");
			$this->collectAddonSpecialAddons_params($paramsShapes, $config, UELM_GlobalsUC::ADDON_TYPE_SHAPES);
		}
		...
		foreach($arrItems as $item)
			$this->collectAddonSpecialAddons_params($paramsItemsShapes, $item, UELM_GlobalsUC::ADDON_TYPE_SHAPES);
	...
	private function modifyAddons_collectAddonNames($addonData){
		...
		$name = UELM_UniteFunctionsUC::getVal($addonData, "name");
	...
	public function getRowSettingsCombined($row){
		...
		$settings = UELM_UniteFunctionsUC::getVal($row, "settings");
		...
		foreach($settings as $key=>$value){
			...
			$globalValue = UELM_UniteFunctionsUC::getVal($allOptions, $key);
			if($globalValue !== "")
		...
		foreach($allOptions as $key=>$value){
			...
			$localValue = UELM_UniteFunctionsUC::getVal($settings, $key);
			if($localValue !== "")
				$value = $localValue;
	...
	public function getInfoText(){
		...
		$textEmpty = esc_html__("empty layout", "unlimited-elements");
		...
		$text = esc_html__("layout with","unlimited-elements")." ". $numRows. " " . __("sections", "unlimited-elements");
	...
	public function getParam($paramName){
		...
		$value = UELM_UniteFunctionsUC::getVal($this->params, $paramName);
	...
	public function getUrlEditPost(){
		...
		uelm_dmp("getUrlEditPost function for override");
		exit();
	}
	...
	public function getUrlViewPost(){
		...
		uelm_dmp("getUrlViewPost function for override");
		exit();
	}
	...
	public function modifyLayoutElementsSettings($modifyFunc){
		...
		if(is_array($modifyFunc) == false)
			UELM_UniteFunctionsUC::throwError("Wrong modify func");
	...
	* get background addon name from settings
	...
	private function getBGAddonNameFromSettings($settings){
		...
		$enableBackground = UELM_UniteFunctionsUC::getVal($settings, "bg_enable");
		$enableBackground = UELM_UniteFunctionsUC::strToBool($enableBackground);
		...
		$enableBGAddon = UELM_UniteFunctionsUC::getVal($settings, $bgAddonEnableKey);
		$enableBGAddon = UELM_UniteFunctionsUC::strToBool($enableBGAddon);
		if($enableBGAddon == false)
			return(null);
		
		$bgAddonName = UELM_UniteFunctionsUC::getVal($settings, $bgAddonKey);
		if(empty($bgAddonName))
			return(null);
	...
	public function modifySettings_collectSpecialAddonNames($settings, $elementType){
		...
		if(!empty($bgAddonName))
			$this->arrAddonNames[$bgAddonName] = UELM_GlobalsUC::ADDON_TYPE_BGADDON;
	...
	private function getErrorMessagePrefix(){
		...
		$prefix = esc_html__("Error in ","unlimited-elements").$title." :";
	...
	public function getArrAddons(){
		...
		foreach($this->arrAddonNames as $name=>$type){
			...
			try{
				...
				if($type != UELM_GlobalsUC::ADDON_TYPE_SHAPE_DEVIDER){
	...
	public function modifyGridDataForSave_containers($gridData){
		...
		$arrRows = UELM_UniteFunctionsUC::getVal($gridData, "rows");
		...
		foreach($arrRows as $index=>$row){
			...
			$settings = UELM_UniteFunctionsUC::getVal($container, "settings");
			if(!empty($settings))
				continue;
			
			$cols = UELM_UniteFunctionsUC::getVal($container, "cols");
			$row["cols"] = $cols;
	...
	public function modifyLayoutDataForSave($arrSettings, $type){
		...
		$bgAddonSingleData = UELM_UniteFunctionsUC::getVal($arrSettings, "bg_addon_single_data");
	...
	* modify layout params for save
	...
	public function createLayoutInDB($arrInsert, $arrParams = array()){
		...
		$id = $this->db->insert(UELM_GlobalsUC::$table_layouts, $arrInsert);
	...
	public function createSmall($title, $name, $description, $catID, $importParams = array()){
		...
		if($isNameExists == true){		//update
			...
			$objLayoutExisting = new UELM_UniteCreatorLayout();
	...
	public function create($data){
		...
		unset($data["title"]);
		unset($data["layoutid"]);
		UELM_UniteFunctionsUC::validateNotEmpty($title, UELM_HelperUC::getText("layout_title"));

		$layoutType = UELM_UniteFunctionsUC::getVal($data, "layout_type");
		$objLayoutType = UELM_UniteCreatorAddonType::getAddonTypeObject($layoutType, true);
		
		$this->objLayoutType = $objLayoutType;
		
		$pageName = UELM_UniteFunctionsUC::getVal($data, "name");
		if(!empty($pageName)){
			$pageName = UELM_HelperUC::convertTitleToAlias($pageName);
	...
	public function updateLayoutInDB($arrUpdate){
		...
		$this->db->update(UELM_GlobalsUC::$table_layouts, $arrUpdate, $where);
	}
	...
	public function delete(){
		...
		$this->db->delete(UELM_GlobalsUC::$table_layouts, "id=".$this->id);
	...
	public function duplicate(){
		...
		$sqlSelect = "select ".self::FIELDS_LAYOUTS." from ".UELM_GlobalsUC::$table_layouts." where id={$this->id}";
		$sqlInsert = "insert into ".UELM_GlobalsUC::$table_layouts." (".self::FIELDS_LAYOUTS.") ($sqlSelect)";
		...
		$this->db->update(UELM_GlobalsUC::$table_layouts, $arrUpdate, array("id"=>$lastID));
