 ...
 *
 */
	class UELM_HelperUC extends UELM_UniteHelperBaseUC{
		...
		public static function isDebug(){
			...
			$debug = UELM_UniteFunctionsUC::getGetVar("ucdebug","",UELM_UniteFunctionsUC::SANITIZE_KEY);

			if(empty($debug))
				return(false);

			$debug = UELM_UniteFunctionsUC::strToBool($debug);
		...
		public static function validatePluginStartup(){
			...
			if(function_exists("simplexml_load_file") == false)
				UELM_UniteFunctionsUC::throwError("Your php missing SimpleXML Extension. The plugin can't work without this extension because it has many xml files to load. Please enable this extension in php.ini");
		...
		public static function getDB(){
			...
			if(empty(self::$db))
				self::$db = new UELM_UniteCreatorDB();
		...
		public static function includeAllPlugins(){
			...
			$objUCPlugins = new UELM_UniteCreatorPlugins();
		...
		public static function runProviderFunc($func){
			...
			$exists = method_exists("UELM_UniteProviderFunctionsUC",$func);

			if(!$exists)
				return(false);

			call_user_func_array(array("UELM_UniteProviderFunctionsUC",$func), $args);
		...
		public static function getFontPanelData(){
			...
			self::$arrFontPanelData = require UELM_GlobalsUC::$pathSettings."font_panel_data.php";
		...
		public static function getText($textKey){
			...
			if(array_key_exists($searchKey, UELM_GlobalsUC::$arrServerSideText))
				return(UELM_GlobalsUC::$arrServerSideText[$textKey]);

			if(array_key_exists($searchKey, UELM_GlobalsUC::$arrClientSideText))
				return(UELM_GlobalsUC::$arrClientSideText[$textKey]);
		...
		public static function getAdminView(){
			...
			if(UELM_UniteProviderFunctionsUC::isAdmin() == false)
				return(null);

			$view = UELM_UniteCreatorAdmin::getView();
		...
		public static function getCssMobileSize($arrSizeCss, $cssPrefix = ""){
			...
			foreach($arrSizeCss as $size=>$cssSize){
				...
				$cssSize = UELM_UniteFunctionsUC::addTabsToText($cssSize,"    ");
				$cssSize = UELM_HelperHtmlUC::wrapCssMobile($cssSize, $size);
		...
		public static function isSpecialAddonType($type){
			...
			switch($type){
				case UELM_GlobalsUC::ADDON_TYPE_SHAPE_DEVIDER:
				case UELM_GlobalsUC::ADDON_TYPE_SHAPES:
		...
		public static function getProductFromRequest(){
			...
			$action = UELM_UniteFunctionsUC::getPostGetVariable("client_action", "", UELM_UniteFunctionsUC::SANITIZE_TEXT_FIELD);
			if(empty($action))
				return(null);
		...
		public static function getPHPInfo(){
			...
			foreach($arrKeys as $key){
				$arrValues = UELM_UniteFunctionsUC::getVal($arrInfo, $key);
				$localValue = UELM_UniteFunctionsUC::getVal($arrValues, "local_value");
		...
		public static function isRunCodeOnce($key){
			...
			$isAlreadyRun = UELM_UniteFunctionsUC::getVal(self::$arrRunOnceCache, $key);
		...
		public static function getFromSql($query, $arg1 = null, $arg2 = null){
			...
			if(strpos($lower, "select") !== 0)
				UELM_UniteFunctionsUC::throwError("get from sql error - the query should start with the word 'select'");
		...
		public static function hasPermissionsFromQuery($getvar){
			...
			$isEnabled = UELM_UniteFunctionsUC::getGetVar($getvar,"",UELM_UniteFunctionsUC::SANITIZE_TEXT_FIELD);
			$isEnabled = UELM_UniteFunctionsUC::strToBool($isEnabled);

			if($isEnabled == false) {
				$data = UELM_UniteFunctionsUC::getPostGetVariable("data", "", UELM_UniteFunctionsUC::SANITIZE_NOTHING);
				if(empty($data))
					return(false);
				$isEnabled = UELM_UniteFunctionsUC::getVal($data, $getvar);
				$isEnabled = UELM_UniteFunctionsUC::strToBool($isEnabled);
			}
			...
			if(UELM_UniteFunctionsWPUC::isCurrentUserHasPermissions() == true)
				return(true);
		...
		public static function getQueryVarWithPermission($getvar){
			...
			$value = UELM_UniteFunctionsUC::getGetVar($getvar,"",UELM_UniteFunctionsUC::SANITIZE_TEXT_FIELD);
		...
		public static function addDebug($title, $content = null){
			...
			$name = UELM_HelperUC::convertTitleToHandle($title);
		...
		public static function showDebug($type = null){
			...
			if(empty($arrDebug)){
				uelm_dmp("no debug content found");
				return(false);
			}

			foreach($arrDebug as $item){

				$name = UELM_UniteFunctionsUC::getVal($item, "name");
				...
				$content = UELM_UniteFunctionsUC::getVal($item, "content");

				$titleOutput = $title;
				if(!empty($content))
					$titleOutput = "<b>$title:</b>";

				uelm_dmp($titleOutput);
				uelm_dmp($content);
		...
		public static function logMemoryUsage($operation, $isUpdateOption = false){
			...
			//log the page
			if(empty(self::$arrLogMemory)){
				$arrLogItem["current_page"] = UELM_GlobalsUC::$current_page_url;
			}

			self::$arrLogMemory[] = $arrLogItem;

			if($isUpdateOption == true)
				UELM_UniteProviderFunctionsUC::updateOption("uelm_unite_creator_memory_usage_log", self::$arrLogMemory);
		...
		public static function getLastMemoryUsage(){
			...
			$arrLog = UELM_UniteProviderFunctionsUC::getOption("uelm_unite_creator_memory_usage_log");
		...
		public static function setState($name, $value){
			...
			$arrState = UELM_UniteProviderFunctionsUC::getOption($optionName);
			if(empty($arrState) || is_array($arrState) == false)
				$arrState = array();

			$arrState[$name] = $value;
			UELM_UniteProviderFunctionsUC::updateOption($optionName, $arrState);
		}
		...
		public static function getState($name){
			...
			$arrState = UELM_UniteProviderFunctionsUC::getOption($optionName);
			$value = UELM_UniteFunctionsUC::getVal($arrState, $name, null);
		...
		public static function URLtoFull($url, $urlBase = null){
			...
			if(getType($urlBase) == "boolean")
				UELM_UniteFunctionsUC::throwError("the url base should be null or string");

			if(is_array($url))
				UELM_UniteFunctionsUC::throwError("url can't be array");
			...
			if(empty($urlBase))
				$url = UELM_GlobalsUC::$url_base.$url;
			else{

				$convertUrl = UELM_GlobalsUC::$url_base;
			...
			$url = UELM_UniteFunctionsUC::cleanUrl($url);
		...
		public static function URLtoRelative($url, $isAssets = false){
			...
			$replaceString = UELM_GlobalsUC::$url_base;
			if($isAssets == true)
				$replaceString = UELM_GlobalsUC::$url_assets;

			//in case of array take "url" from the array
			if(is_array($url)){

				$strUrl = UELM_UniteFunctionsUC::getVal($url, "url");
				if(empty($strUrl))
					return($url);
		...
		public static function URLtoAssetsRelative($url){
			...
			$url = str_replace(UELM_GlobalsUC::$url_assets, "", $url);
		...
		public static function urlToPath($url){
			...
			$path = UELM_GlobalsUC::$path_base.$urlRelative;
			
			$path = UELM_UniteFunctionsUC::cleanPath($path);
		...
		$len = strlen($realpath);
		$realBase = realpath(UELM_GlobalsUC::$path_base);
		...
		$relativePath = UELM_UniteFunctionsUC::pathToUnix($relativePath);
		...
		//add slash to end
		if($isDir == true)
			$relativePath = UELM_UniteFunctionsUC::addPathEndingSlash($relativePath);
	...
	public static function pathToAbsolute($path){
		...
		$basePath = UELM_GlobalsUC::$path_base;
		$basePath = UELM_UniteFunctionsUC::pathToUnix($basePath);

		$path = UELM_UniteFunctionsUC::pathToUnix($path);

		$realPath = UELM_UniteFunctionsUC::realpath($path, false);

		if(!empty($realPath))
			return ($path);

		if(UELM_UniteFunctionsUC::isPathUnderBase($path, $basePath)){
			$path = UELM_UniteFunctionsUC::pathToUnix($path);
	...
	public static function getImageDetails($urlImage){
		...
		$info = UELM_UniteFunctionsUC::getPathInfo($urlImage);
		$urlDir = UELM_UniteFunctionsUC::getVal($info, "dirname");
		if(!empty($urlDir))
			$urlDir = $urlDir . "/";

		$arrInfo = array();
		$arrInfo["url_full"] = UELM_GlobalsUC::$url_base . $urlImage;
		$arrInfo["url_dir_image"] = $urlDir;
		$arrInfo["url_dir_thumbs"] = $urlDir . UELM_GlobalsUC::DIR_THUMBS . "/";

		$filepath = UELM_GlobalsUC::$path_base . urldecode($urlImage);
		$filepath = realpath($filepath);

		$path = dirname($filepath) . "/";
		$pathThumbs = $path . UELM_GlobalsUC::DIR_THUMBS . "/";
	...
	public static function getUrlHandle($url, $addonName = null){
		...
		$urlNew = UELM_HelperUC::URLtoAssetsRelative($url);
		...
		if($urlNew != $url){  //is inside assets
			...
			$urlNew = UELM_HelperUC::URLtoRelative($url);
			if($urlNew != $url)
				$urlNew = "uc_" . $urlNew;
			else
				$urlNew = $url;
	...
	public static function urlToSSLCheck($url){
		...
		if(UELM_GlobalsUC::$is_ssl == true)
			$url = UELM_UniteFunctionsUC::urlToSsl($url);
	...
	public static function downloadFileToCacheFolder($urlFile){
		...
		$filename = UELM_UniteFunctionsUC::getVal($info, "basename");
		if(empty($filename))
			UELM_UniteFunctionsUC::throwError("no file given");

		$ext = $info["extension"];

		if($ext != "zip")
			UELM_UniteFunctionsUC::throwError("wrong file given");

		$pathCache = UELM_GlobalsUC::$path_cache;
		UELM_UniteFunctionsUC::mkdirValidate($pathCache, "cache folder");

		$pathCacheImport = $pathCache . "import/";
		UELM_UniteFunctionsUC::mkdirValidate($pathCacheImport, "cache import folder");

		$filepath = $pathCacheImport . $filename;

		$content = UELM_UniteFunctionsUC::fileGetContents($urlFile);
		if(empty($content))
			UELM_UniteFunctionsUC::throwError("Can't dowonload file from url: $urlFile");
	...
	public static function getUrlAjax($action, $params = ""){
		...
		$url = UELM_UniteFunctionsUC::addUrlParams(UELM_GlobalsUC::$url_ajax_full, array(
			'action' => UELM_GlobalsUC::PLUGIN_NAME . '_ajax_action',
			'client_action' => $action,
			'nonce' => $nonce,
		));

		if(!empty($params))
			$url = UELM_UniteFunctionsUC::addUrlParams($url, $params);
	...
	public static function getViewUrl($viewName, $urlParams = "", $isBlankWindow = false, $isFront = false){
		...
		if($isFront == false)
			$link = UELM_UniteFunctionsUC::addUrlParams(UELM_GlobalsUC::$url_component_admin, $params);
		else
			$link = UELM_UniteFunctionsUC::addUrlParams(UELM_GlobalsUC::$url_component_client, $params);

		if($isBlankWindow == true)
			$link = UELM_UniteProviderFunctionsUC::convertUrlToBlankWindow($link);

		$link = UELM_UniteProviderFunctionsUC::applyFilters(UELM_UniteCreatorFilters::FILTER_MODIFY_URL_VIEW, $link, $viewName, $params, $isBlankWindow, $isFront);
	...
	public static function getViewUrl_Addons($type = ""){
		...
		if(empty($type))
			return self::getViewUrl(UELM_GlobalsUC::VIEW_ADDONS_LIST);

		$params = array();
		$type = urldecode($type);
		$params = "addontype=$type";

		$urlView = self::getViewUrl(UELM_GlobalsUC::VIEW_ADDONS_LIST, $params);
	...
	public static function getViewUrl_Default($params = ""){
		...
		return self::getViewUrl(UELM_GlobalsUC::$view_default, $params);
	}
	...
	public static function getViewUrl_TemplatesList($params = array(), $layoutType = null){
		...
		$url = self::getViewUrl(UELM_GlobalsUC::VIEW_TEMPLATES_LIST, $urlParams);

		$url = UELM_UniteProviderFunctionsUC::applyFilters(UELM_UniteCreatorFilters::FILTER_URL_TEMPLATES_LIST, $url, $params, $layoutType);
	...
	public static function getViewUrl_LayoutsList($params = array(), $layoutType = ""){
		...
		$url = self::getViewUrl(UELM_GlobalsUC::VIEW_LAYOUTS_LIST, $urlParams);

		$url = UELM_UniteProviderFunctionsUC::applyFilters(UELM_UniteCreatorFilters::FILTER_URL_LAYOUTS_LIST, $url, $params);
	...
	public static function getViewUrl_Layout($layoutID = null, $optParams = "", $layoutType = ""){
		...
		$url = self::getUrlViewObject(UELM_GlobalsUC::VIEW_LAYOUT, $layoutID, $optParams, true);
	...
	public static function getViewUrl_Template($templateID = null, $templateType = "", $optParams = ""){
		...
		UELM_UniteFunctionsUC::validateNotEmpty($templateType);
	...
	public static function getViewUrl_LayoutPreview($layoutID, $isBlankWindow = false, $addParams = "", $isFront = true){
		...
		if($layoutID == "[page]"){
			$urlPreviewTemplate = UELM_UniteProviderFunctionsUC::applyFilters("get_layout_preview_byid", null, $layoutID);
		...
		UELM_UniteFunctionsUC::validateNotEmpty($layoutID, "layout id");

		$urlPreview = null;
		$urlPreview = UELM_UniteProviderFunctionsUC::applyFilters("get_layout_preview_byid", $urlPreview, $layoutID);
		if(!empty($urlPreview))
			return ($urlPreview);
		...
		$url = self::getViewUrl(UELM_GlobalsUC::VIEW_LAYOUT_PREVIEW, $params, $isBlankWindow, $isFront);
	...
	public static function getUrlLayoutPreviewFront($layoutID, $addParams = null, $outputMode = null){
		...
		$url = UELM_HelperUC::getViewUrl_LayoutPreview($layoutID, true, $addParams, true);

		$url = UELM_UniteProviderFunctionsUC::applyFilters(UELM_UniteCreatorFilters::FILTER_MODIFY_URL_LAYOUT_PREVIEW_FRONT, $url, $layoutID, $addParams);
	...
	public static function getViewUrl_EditAddon($addonID, $params = "", $hash = ""){
		...
		return (self::getViewUrl(UELM_GlobalsUC::VIEW_EDIT_ADDON, $strParams));
	}
	...
	public static function getViewUrl_TestAddon($addonID, $optParams = ""){
		...
		return (self::getViewUrl(UELM_GlobalsUC::VIEW_TEST_ADDON, $params));
	}
	...
	public static function getViewUrl_AddonDefaults($addonID, $optParams = ""){
		...
		return (self::getViewUrl(UELM_GlobalsUC::VIEW_ADDON_DEFAULTS, $params));
	}
	...
	public static function getTitleFromUrl($url, $defaultTitle = "item"){
		...
		$info = pathinfo($url);
		$filename = UELM_UniteFunctionsUC::getVal($info, "filename");
	...
	private static function getPathFile($filename, $path, $defaultPath, $validateName, $ext = "php"){
		...
		UELM_UniteFunctionsUC::validateFilepath($filepath, $validateName);
	...
	public static function getPathTemplate($templateName, $path = null){
		...
		return self::getPathFile($templateName, $path, UELM_GlobalsUC::$pathTemplates, "Template");
	}
	...
	public static function getPathView($viewName, $path = null){
		...
		return self::getPathFile($viewName, $path, UELM_GlobalsUC::$pathViews, "View");
	}
	...
	public static function getPathViewObject($viewObjectName, $path = null){
		...
		return self::getPathFile($viewObjectName, $path, UELM_GlobalsUC::$pathViewsObjects, "View Object");
	}
	...
	public static function getPathSettings($settingsName, $path = null){
		...
		return self::getPathFile($settingsName, $path, UELM_GlobalsUC::$pathSettings, "Settings", "xml");
	}
	...
	public static function getPathTemplateProvider($templateName){
		...
		return self::getPathFile($templateName, UELM_GlobalsUC::$pathProviderTemplates, "", "Provider Template");
	}
	...
	public static function getPathViewProvider($viewName){
		...
		return self::getPathFile($viewName, UELM_GlobalsUC::$pathProviderViews, "", "Provider View");
	}
	...
	public static function getUrlFontAwesome($version = null){
		...
		if($version == "fa4")
			$url = UELM_GlobalsUC::$url_assets_libraries . "font-awsome/css/font-awesome.min.css";
		else    //fa5
			$url = UELM_GlobalsUC::$url_assets_libraries . "font-awesome6/fontawesome-all.min.css";
	...
	public static function includeUEAnimationStyles(){
		...
		self::addStyleAbsoluteUrl(UELM_GlobalsUC::$url_assets_libraries . "animations/ue_animations.css", "ue_animations");
	}
	...
	public static function putSmoothScrollIncludes($putJSInit = false){
		...
		$urlSmoothScroll = UELM_GlobalsUC::$url_assets_libraries . "smooth-scroll/smooth-scroll.min.js";
	...
	public static function addScript($scriptName, $handle = null, $folder = "js", $inFooter = false){
		...
		if($handle == null)
			$handle = UELM_GlobalsUC::PLUGIN_NAME . "-" . $scriptName;

		$url = UELM_GlobalsUC::$urlPlugin . $folder . "/" . $scriptName . ".js";

		UELM_UniteProviderFunctionsUC::addScript($handle, $url, $inFooter);
	}
	...
	public static function addScriptAbsoluteUrl($urlScript, $handle, $inFooter = false, $deps = array()){
		...
		UELM_UniteProviderFunctionsUC::addScript($handle, $urlScript, $inFooter, $deps);
	...
	public static function addScriptAbsoluteUrl_widget($urlScript, $handle, $inFooter = false){
		...
		if(UELM_GlobalsProviderUC::$isInsideEditor == true)
			...
			UELM_UniteProviderFunctionsUC::addScript($handle, $urlScript, $inFooter);
	...
	public static function addRemoteControlsScript(){
		...
		$urlFiltersJS = UELM_GlobalsUC::$url_assets_libraries . "remote/ue-remote-controls.js";
		UELM_HelperUC::addScriptAbsoluteUrl($urlFiltersJS, "ue_remote_controls");

		$isDebug = UELM_HelperUC::hasPermissionsFromQuery("ucremotedebug");
		
		//check if hide remote errors or not
		
		$isErrorsToAdmins = UELM_HelperProviderCoreUC_EL::getGeneralSetting("remote_error_admins");
		$isErrorsToAdmins = UELM_UniteFunctionsUC::strToBool($isErrorsToAdmins);
		
		$isHideErrors = false;
		
		if($isErrorsToAdmins == true){
			
			$hasPermissions = UELM_UniteFunctionsWPUC::isCurrentUserHasPermissions();
		...
		if($isDebug == true)
			UELM_HelperUC::putCustomScript("var ucRemoteDebugEnabled=true;", false, "remote_controls_debug");
		
		if($isHideErrors == true)
			UELM_HelperUC::putCustomScript("var g_ucRemoteHideErrors=true;", false, "remote_controls_hide_errors");
	...
	public static function putScriptHtml($url, $handle = null){
		...
		$htmlInclude = UELM_HelperHtmlUC::getHtmlJsInclude($url, $handle);
	...
	public static function putStyleHtml($url, $handle = null){
		...
		$htmlInclude = UELM_HelperHtmlUC::getHtmlCssInclude($url, $handle);
	...
	public static function addStyle($styleName, $handle = null, $folder = "css"){
		...
		if($handle == null)
			$handle = UELM_GlobalsUC::PLUGIN_NAME . "-" . $styleName;

		UELM_UniteProviderFunctionsUC::addStyle($handle, UELM_GlobalsUC::$urlPlugin . $folder . "/" . $styleName . ".css");
	}
	...
	public static function putCustomScript($script, $hardCoded = false, $putOnceHandle = null){
		...
		UELM_UniteProviderFunctionsUC::printCustomScript($script, $hardCoded, false, $putOnceHandle, true);
	}
	...
	public static function putInlineStyle($css){
		...
		UELM_UniteProviderFunctionsUC::printCustomStyle($css);
	}
	...
	public static function addStyleAbsoluteUrl($styleUrl, $handle, $deps = array()){
		...
		UELM_UniteProviderFunctionsUC::addStyle($handle, $styleUrl, $deps);
	}
	...
	public static function getHtmlErrorMessage($message, $trace = "", $prefix = null){
		...
		if(empty($prefix))
			$prefix = UELM_HelperUC::getText("addon_library") . " Error: ";
	...
	public static function getAssetsPath($objAddonType = null){
		...
		if(empty($objAddonType))
			return (UELM_GlobalsUC::$pathAssets);
	...
	public static function getAssetsUrl($objAddonType = null){
		...
		if(empty($objAddonType))
			return (UELM_GlobalsUC::$url_assets);
	...
	public static function validatePathUnderAssets($path, $objAddonType = null){
		...
		if(!$isUnderAssets)
			UELM_UniteFunctionsUC::throwError("The path should be under assets folder");
	}
	...
	public static function validateDBTablesExists(){
		...
		if($db->isTableExists(UELM_GlobalsUC::$table_categories . "," . UELM_GlobalsUC::$table_addons) == false)
			UELM_UniteFunctionsUC::throwError("Some DB table not exists");
	}
	...
	public static function isPathUnderAssetsPath($path, $objAddonType = null){
		...
		$isUnderAssets = UELM_UniteFunctionsUC::isPathUnderBase($path, $assetsPath);
	...
	public static function pathToAssetsRelative($path, $objAddonType = null){
		...
		$relativePath = UELM_UniteFunctionsUC::pathToRelative($path, $assetsPath);
	...
	public static function pathToAssetsAbsolute($path, $objAddonType = null){
		...
		$assetsPath = self::getAssetsPath($objAddonType);
		$path = UELM_UniteFunctionsUC::joinPaths($assetsPath, $path);
	...
	public static function isElementorEditMode(){
		...
		$argPost = UELM_UniteFunctionsUC::getPostGetVariable("post", "", UELM_UniteFunctionsUC::SANITIZE_KEY);
		$argAction = UELM_UniteFunctionsUC::getPostGetVariable("action", "", UELM_UniteFunctionsUC::SANITIZE_KEY);
	...
	public static function isAjaxAction($actionVar = "",$actionVar2 = ""){
		...
		$action = UELM_UniteFunctionsUC::getPostGetVariable("action", "", UELM_UniteFunctionsUC::SANITIZE_KEY);
		...
		$clientAction = UELM_UniteFunctionsUC::getPostGetVariable("client_action", "", UELM_UniteFunctionsUC::SANITIZE_KEY); 
	...
	public static function startBufferingWidgetsOutput(){
		...
		UELM_UniteCreatorOutput::$isBufferingCssActive = true;
	}
	...
	public static function outputWidgetsCssBuffer(){
		...
		$htmlCssIncludes = UELM_UniteCreatorOutput::$bufferCssIncludes;

		echo "\n";
		uelm_echo($htmlCssIncludes);

		$css = UELM_UniteCreatorOutput::$bufferBodyCss;
		if(!empty($css)){
			echo "\n<style type='text/css'> \n";
		...
		//clear the buffer
		UELM_UniteCreatorOutput::$bufferCssIncludes = "";
		UELM_UniteCreatorOutput::$bufferBodyCss = "";
	}
	...
	public static function outputTemplatePart($layoutType){
		...
		if(empty($objLayout)){
			...
			$message = esc_html__("Template part", "unlimited-elements") . ": " . $typeTitle . esc_html__(" not found. Please create one in template parts page", "unlimited-elements");
			$html = UELM_HelperHtmlUC::getErrorMessageHtml($message);
			uelm_echo($html);
	...
	public static function outputLayout($layoutID, $getHtml = false, $outputFullPage = false, $mode = null){
		...
		try{
			if(is_numeric($layoutID)){
				$layoutID = UELM_UniteProviderFunctionsUC::sanitizeVar($layoutID, UELM_UniteFunctionsUC::SANITIZE_ID);
			...
			$outputLayout = new UELM_UniteCreatorLayoutOutput();
			...
			if($getHtml == true){
				...
				UELM_HelperHtmlUC::outputExceptionBox($e, UELM_HelperUC::getText("addon_library") . " Error");
		}
	}
	...
	public static function getActivePluginVersions() {
		...
		if(defined("UELM_ENABLE_ELEMENTOR_SUPPORT"))
			$flagElementor = true;
		...
		$output = array();
		$output[UELM_GlobalsUC::VERSION_GUTENBERG] = $flagGutenberg;
		$output[UELM_GlobalsUC::VERSION_ELEMENTOR] = $flagElementor;
...
//init the operations
UELM_HelperUC::$operations = new UELM_UCOperations();
