...
class UELM_UniteCreatorTemplateEngineWork{
	...
	public function __construct(){
		$this->objParamsProcessor = new UELM_UniteCreatorParamsProcessor();
	}
	...
	private function validateFilterCallable($filter, $callable){	//Security Update 2
		...
		if($isClosure == false){
			...
			UELM_UniteFunctionsUC::throwError("Function <b>{$filter}</b> can execute only arrow functions. PHP functions like this: \"" . esc_html($callable) . "\" is forbidden.");
		}
		...
		if(is_string($callable) === true && in_array($callable, $forbiddenFunctions) === true)
			UELM_UniteFunctionsUC::throwError("Function \"" . $callable . "\" is forbidden for the \"" . $filter . "\" filter.");
	...
	public function checkPutSchemaItems($paramName){
		...
		$isEnable = UELM_UniteFunctionsUC::getVal($arrValues, $paramName."_enable");
		$isEnable = UELM_UniteFunctionsUC::strToBool($isEnable);
		
		if($isEnable == false)
			return(false);
		
		$schemaType = UELM_UniteFunctionsUC::getVal($arrValues, $paramName."_type");
		
		$schemaSelection = UELM_UniteFunctionsUC::getVal($arrValues, $paramName."_selection");
	...
	private function putSchemaItemsGlobal(){
		...
		$isEnable = UELM_UniteFunctionsUC::getVal($this->arrParams, $paramName."_enable");
		$isEnable = UELM_UniteFunctionsUC::strToBool($isEnable);
		
		if($isEnable == false)
			return(false);
		
		$schemaType = UELM_UniteFunctionsUC::getVal($this->arrParams, $paramName."_type");
		
		$schemaSelection = UELM_UniteFunctionsUC::getVal($this->arrParams, $paramName."_selection");
		...
		switch($type){
			case UELM_UniteCreatorAddon::ITEMS_TYPE_POST:
				
				$postParam = $this->addon->getParamByType(UELM_UniteCreatorDialogParam::PARAM_POSTS_LIST);
				
				$postListName = UELM_UniteFunctionsUC::getVal($postParam, "name");
				
				$arrSettings["post_list_name"] = $postListName;				
			break;
			case UELM_UniteCreatorAddon::ITEMS_TYPE_MULTISOURCE:
	...
	private function outputItem($index, $itemParams, $templateName, $sap, $newLine = true){
		...
		UELM_GlobalsProviderUC::$isUnderItem = true;
		...
		if($this->isItemsFromPosts === true){
			...
			UELM_GlobalsProviderUC::$isUnderRenderPostItem = true;

			//save post id
			$arrItem = UELM_UniteFunctionsUC::getVal($itemParams, "item");
			$postType = UELM_UniteFunctionsUC::getVal($arrItem, "object_type");
			$postID = UELM_UniteFunctionsUC::getVal($arrItem, "object_id");

			UELM_GlobalsProviderUC::$lastObjectID = $postID;

			//woo commerce global object product save
			if($postType == "product" && function_exists("wc_get_product")){
				global $product;
				$product = wc_get_product(UELM_GlobalsProviderUC::$lastObjectID);
			}

			//save post to allow dynamic tags inside the item
			$post = UELM_UniteFunctionsUC::getVal(UELM_GlobalsProviderUC::$arrFetchedPostsObjectsCache, $postID);
			...
			if(!empty($post)){
				...
				$originalPost = UELM_UniteFunctionsUC::getVal($GLOBALS, "post", null);
		...
		UELM_GlobalsProviderUC::$lastItemParams = $params;
		...
		if($this->isItemsFromPosts === true){
			UELM_GlobalsProviderUC::$isUnderRenderPostItem = false;
		...
		UELM_GlobalsProviderUC::$isUnderItem = false;
	}
	...
	private function putItemsWork($templateName, $sap = null, $numItem = null){
		...
		if($numItem !== null){
			$itemParams = UELM_UniteFunctionsUC::getVal($this->arrItems, $numItem);
	...
	public function putItemsJson($type = null){
		...
		//json encode
		$jsonItems = UELM_UniteFunctionsUC::jsonEncodeForClientSide($arrItems);
	...
	public function putAttributesJson($type = null, $key = null){
		...
		if($type == "clean")
			$arrAttr = UELM_UniteFunctionsUC::removeArrItemsByKeys($arrAttr, UELM_GlobalsProviderUC::$arrAttrConstantKeys);

		if(!empty($key))
			$arrAttr = UELM_UniteFunctionsUC::getVal($arrAttr, $key);

		$jsonAttr = UELM_UniteFunctionsUC::jsonEncodeForClientSide($arrAttr);
	...
	public function putFontOverride($name, $selector, $useID = false){
		...
		$arrFont = UELM_UniteFunctionsUC::getVal($arrFonts, $fontKey);

		if(empty($arrFont))
			return(false);

		$processor = new UELM_UniteCreatorParamsProcessor();
	...
	public function filterUCDate($dateStamp, $format = "", $formatDateFrom = "d/m/Y"){
		...
		if($format === "time_ago"){
			$strTimeAgo = UELM_UniteFunctionsUC::getTimeAgoString($dateStamp);
		...
		if($format === "time_ago_short"){
			...
			$strTimeAgo = UELM_UniteFunctionsUC::getTimeAgoString($dateStamp, "short");
		...
		$isTimeStamp = UELM_UniteFunctionsUC::isTimeStamp($stamp);
		...
		if($isTimeStamp == false){
			...
			$stamp = UELM_UniteFunctionsUC::date2Timestamp($stamp, $formatDateFrom);
	...
	public function putPostGetVar($varName, $default=""){
		...
		$varName = UELM_UniteProviderFunctionsUC::sanitizeVar($varName, UELM_UniteFunctionsUC::SANITIZE_KEY);

		$value = UELM_UniteFunctionsUC::getPostGetVariable($varName, $default , UELM_UniteFunctionsUC::SANITIZE_TEXT_FIELD);
	...
	public function showData(){
		...
		uelm_dmp("Params:");
		uelm_dmp($this->arrParams);
	
		uelm_dmp("Items:");
		uelm_dmp($this->arrItems);
	...
	public function getPostTags($postID){
		...
		if(empty($postID))
			UELM_UniteFunctionsUC::throwError("$errorPrefix - no postID argument found");

		$arrTerms = UELM_UniteFunctionsWPUC::getPostSingleTerms($postID, "post_tag");

		if(empty($arrTerms))
			return(array());

		$objParamsProcessor = new UELM_UniteCreatorParamsProcessor();
	...
	public function printVar($var){
		...
		$var = UELM_UniteFunctionsUC::modifyDataArrayForShow($var);
	...
	public function do_action($tag, $param = null, $param2 = null, $param3=null){
		...
		//add debug
		if($param === null)
			UELM_HelperUC::addDebug("running action: $tag");
		else
			UELM_HelperUC::addDebug("running action: $tag",array(
			"param"=>$param,
			"param2"=>$param2,
			"param3"=>$param3,
		));
	...
	public function apply_filters($tag, $value = null, $param1 = null, $param2=null){
		...
		UELM_UniteFunctionsUC::throwError("The apply_filters() function exists only in PRO version of the plugin");
	...
	public function getByPHPFunction($funName){
		...
		UELM_UniteFunctionsUC::throwError("The getByPHPFunction() function exists only in PRO version of the plugin. You can run any php function that return data and starting with 'get_' by it.");
	}
	...
	public function filterTruncate($value, $length = 100, $preserve = true, $separator = '...'){
		...
		$value = UELM_UniteFunctionsUC::truncateString($value, $length, $preserve, $separator);
	...
	public function getPostTerms($postID, $taxonomy, $addCustomFields = false, $type = "", $maxTerms = null){
		...
		uelm_dmp("no terms in this platform");
	...
	public function getWooChildProducts($productID, $getCustomFields = true, $getCategory = true){
		...
		$objWooIntegrate = UELM_UniteCreatorWooIntegrate::getInstance();
		$isActive = UELM_UniteCreatorWooIntegrate::isWooActive();
		...
		if($getCustomFields == true)
			$arrAdditions[UELM_GlobalsProviderUC::POST_ADDITION_CUSTOMFIELDS] = true;

		if($getCategory == true)
			$arrAdditions[UELM_GlobalsProviderUC::POST_ADDITION_CATEGORY] = true;

		$objProcessor = new UELM_UniteCreatorParamsProcessor();
	...
	public function getPostAuthor($authorID, $getMeta = false, $getAvatar = false){
		...
		$arrUserData = UELM_UniteFunctionsWPUC::getUserDataById($authorID, $getMeta, $getAvatar);
	...
	public function getUserData($username, $getMeta = false, $getAvatar = false){
		...
		$arrUserData = UELM_UniteFunctionsWPUC::getUserDataById($username, $getMeta, $getAvatar);
	...
	private function getAlternateTemplate($templateID){
		...
		$listingName = UELM_UniteFunctionsUC::getVal($this->arrParams, "listing_setting_name");
		
		if(empty($listingName))
			return(false);
		
		$altTemplateID = UELM_UniteFunctionsUC::getVal($this->arrParams, $listingName."_alt_templateid");
		...
		//check for sure
		$templateIDFromSettings = UELM_UniteFunctionsUC::getVal($this->arrParams, $listingName."_templateid");
	...
	private function getCurrentItemTemplateID($templateID, $altTemplateID, $item){
		...
		//both exists
		$itemParams = UELM_UniteFunctionsUC::getVal(UELM_GlobalsProviderUC::$lastItemParams, "item");
		
		$itemIndex = UELM_UniteFunctionsUC::getVal($itemParams, "item_index");
	...
	public function putDynamicLoopTemplate($item, $templateID){
		...
		$widgetID = UELM_UniteFunctionsUC::getVal($this->arrParams, "uc_id");
		...
		UELM_HelperProviderCoreUC_EL::putListingItemTemplate($item, $templateID, $widgetID);
	...
	public function filterWcPrice($price, $variationID = null){
		...
		if(empty(UELM_GlobalsProviderUC::$lastObjectID))
			return($newPrice);
	...
	public function filterJsonDecode($strJson){
		...
		$arrOutput = UELM_UniteFunctionsUC::jsonDecode($strJson);
	...
	public function getListingItemData($type = null, $defaultObjectID = null){
		...
		$data = UELM_UniteFunctionsWPUC::getQueriedObject($type, $defaultObjectID);

		$data = UELM_UniteFunctionsUC::convertStdClassToArray($data);
	...
	public function putPostImageAttributes($arrPost, $thumbName, $isPutPlaceholder = false, $urlPlaceholder = ""){
		...
		if(empty($arrPost))
			UELM_UniteFunctionsUC::throwError("No post found :(");
		...
		//uelm_dmp("put dummy placeholder");exit();

		if(!empty($arrPost[$thumbName])){

			$urlImage = $arrPost[$thumbName];
			$width = UELM_UniteFunctionsUC::getVal($arrPost, $thumbName."_width");
			$height = UELM_UniteFunctionsUC::getVal($arrPost, $thumbName."_height");
		...
		$isPutPlaceholder = UELM_UniteFunctionsUC::strToBool($isPutPlaceholder);
	...
	public function putElementorTemplate($templateID, $mode = null){
		...
		UELM_HelperProviderCoreUC_EL::putElementorTemplate($templateID,$mode);
	...
	public function ucfunc($type, $arg1 = null, $arg2= null, $arg3=null){
		switch($type){
			case "put_date_range":
				...
				$dateRange = UELM_HelperUC::$operations->getDateRangeString($arg1, $arg2, $arg3);
				uelm_echo($dateRange);

			break;
			case "get_general_setting":
				
				$value = UELM_HelperProviderCoreUC_EL::getGeneralSetting($arg1);

				return($value);
			break;
			case "run_code_once":

				$isRunOnce = UELM_HelperUC::isRunCodeOnce($arg1);
				return($isRunOnce);
			break;
			case "get_from_sql":

				$response = UELM_HelperUC::getFromSql($arg1,$arg2,$arg3);
			...
			case "get_loadmore_data":
				...
				$objPagination = new UELM_UniteCreatorElementorPagination();
				$strData = $objPagination->getLoadmoreData(UELM_GlobalsProviderUC::$isInsideEditor);

				return($strData);
			break;
			case "get_last_query_data":

				$arrData = UELM_HelperUC::$operations->getLastQueryData();
			...
			case "get_post_term":
				...
				$term = UELM_HelperProviderUC::getPostTermForTemplate($arg1, $arg2, $arg3);
				return($term);
			break;
			...
			case "put_unite_gallery_item":
				...
				$htmlItem = UELM_UniteCreatorUniteGallery::getUniteGalleryHtmlItem($arg1);
				uelm_echo($htmlItem);
			...
			case "get":
				...
				$var = UELM_UniteFunctionsUC::getVal(self::$arrSetVarsCache, $arg1);
			...
			case "get_woo_endpoint":
				...
				$arrEndpoints = UELM_UniteCreatorWooIntegrate::getWooEndpoint($arg1);
			...
			case "get_unitegallery_js":
				...
				$objUniteGallery = new UELM_UniteCreatorUniteGallery();
			...
			case "put_remote_parent_js":
				...
				UELM_HelperHtmlUC::putRemoteParentJS($arg1, $arg2);
			...
			case "get_post_custom_field":
				...
				$value = UELM_UniteFunctionsWPUC::getPostCustomField($postID, $fieldname);

				return($value);
			break;
			case "modify_text":
				
				$arg1 = UELM_HelperUC::$operations->modifyTextFromWidget($arg1);
			...
			case "get_term_image":
				...
				$arrImage = UELM_UniteFunctionsWPUC::getTermImage($arg1, $arg2);
			...
			case "get_term_custom_field":
				...
				$value = UELM_UniteFunctionsWPUC::getTermCustomField($termID, $fieldname);
			...
			case "get_post_image":
				...
				$arrImage = UELM_UniteFunctionsWPUC::getPostImage($arg1, $arg2);
			...
			case "put_term_meta_debug":
				...
				if(!empty($termID))
					UELM_HelperUC::$operations->putTermCustomFieldsDebug($termID);
			...
			case "put_terms_meta_debug":
				...
				UELM_HelperUC::$operations->putTermsCustomFieldsDebug($arrTerms);
			...
			case "put_post_content":
				...
				$content = UELM_HelperProviderCoreUC_EL::getPostContent($arg1, $arg2);
				uelm_echo($content);
			break;
			...
			case "put_hide_ids_css":
				...
				UELM_HelperHtmlUC::putHideIdsCss($arg1);

			break;
			case "get_posts":

				//$postType, $strCats, $strArgs
				$arrPosts = UELM_HelperUC::$operations->getPostsFromTwig($arg1,$arg2,$arg3);
			...
			case "put_entrance_animation_css":
				...
				UELM_UniteCreatorEntranceAnimations::putEntranceAnimationCss($this->arrParams, $arg1, $param);
			...
			case "put_entrance_animation_js":
				...
				UELM_UniteCreatorEntranceAnimations::putEntranceAnimationJs($this->arrParams, $arg1, $param);
			...
			case "get_current_user":
				...
				$userData = UELM_UniteFunctionsWPUC::getUserData($objUser,$arg2,$arg3);
			...
			case "get_url_ajax":
				...
				$urlBase = UELM_UniteFunctionsUC::getBaseUrl(UELM_GlobalsUC::$current_page_url);
			...
			case "put_docready_start":
				...
				UELM_HelperHtmlUC::putDocReadyStartJS($widgetID);
			...
			case "put_docready_end":
				...
				UELM_HelperHtmlUC::putDocReadyEndJS($widgetID);
			...
			case "get_nextprev_post_data":
				...
				$data = UELM_UniteFunctionsWPUC::getNextPrevPostData($arg1, $arg2);
			...
			case "render":		//render twig template
				...
				$html = $this->getRenderedHtml($arg1, UELM_GlobalsProviderUC::$isUnderItem);
				uelm_echo($html);
			...
			case "get_encoded_image":
				...
				$content = UELM_HelperUC::$operations->getLocalFileContentsByUrl($arg1);
			...
			case "put_post_terms_string":
				...
				if(empty($arg1))
					$arg1 = UELM_GlobalsProviderUC::$lastObjectID;

				$strTermsNames = UELM_UniteFunctionsWPUC::getPostTermsTitlesString($arg1, true);

				uelm_echo($strTermsNames);
			break;
			case "get_sort_filter_data":

				$sortFilterItems = UELM_UniteCreatorFiltersProcess::getSortFilterData($arg1, $this->arrParams);
			...
			case "get_breakpoints":
				...
				$arrBreakpoints = UELM_HelperProviderCoreUC_EL::getBreakpoints();

				uelm_dmp("breakpoints");
				uelm_dmp($arrBreakpoints);

			break;
			case "csv_to_json":

				$arrData = UELM_UniteFunctionsUC::maybeCsvDecode($arg1);

				if(empty($arrData))
					$arrData = array();

				$jsonData = UELM_UniteFunctionsUC::jsonEncodeForClientSide($arrData);

				uelm_echo($jsonData);
			break;
			case "validate_submit_button":

				$isInsideEditor = UELM_UniteFunctionsUC::getVal($this->arrParams, "uc_inside_editor");
				...
				if (empty($formErrors) === false) {
					...
					uelm_dmp("<span style='color:red;'>Form settings validation failed:<br />- $formErrors</span>");
				}
			break;
			case "get_variable":
				
				$getVarValue = UELM_UniteFunctionsUC::getGetVar($arg1,"",UELM_UniteFunctionsUC::SANITIZE_NOTHING);
				
				return($getVarValue);
			break;
			case "wpp_get_page_views":	//get post views using wordpress popular posts plugin
						
				$count = UELM_UniteCreatorPluginIntegrations::WPP_getPostViews($arg1);
				
				return($count);
			break;
			case "get_alphabet":

				require_once UELM_GlobalsUC::$pathFramework."alphabet_array.class.php";
				require_once UELM_GlobalsUC::$pathFramework."alphabet.class.php";
            ...
            case "get_breadcrumbs":
                ...
                $objBreadcrumbs = new UELM_UniteCreatorBreadcrumbs();
			...
			case "get_url_from_string":
				...
				$url = UELM_UniteFunctionsUC::getFirstUrlFromText($arg1);
			...
			default:
				...
				$type = UELM_UniteFunctionsUC::sanitizeAttr($type);

				uelm_dmp("<span style='color:red;'>ucfunc error: unknown action <b>'$type'</b>. Please check that the plugin is at latest version.</span>");
			break;
		}
	...
	public function putTestHTML($type = null, $data = null){
		...
		uelm_dmp("put some test html");
		dmP($type);

		//unset($data["current_post"]["content"]);
		//$post = UELM_UniteFunctionsUC::getVal($data, "")
		unset($data["content"]);
		uelm_dmp($data);
	...
	public function filterSanitizeHtml($html){
		...
		$html = UELM_UniteFunctionsUC::sanitize($html, UELM_UniteFunctionsUC::SANITIZE_HTML);
	...
	private function initTwig(){
		...
		if(empty($this->arrTemplates))
			UELM_UniteFunctionsUC::throwError("No templates found");

		if(class_exists("Twig\\Loader\\ArrayLoader") == false)
			UELM_UniteFunctionsUC::throwError("Twig template engine not loaded. Please check if it collides with some other plugin that also loading twig engine.");
		...
		if(class_exists("Twig\\Environment") == false){
			...
			UELM_UniteFunctionsUC::throwError($text);
		}
	...
	private function validateNotInited(){
		if(!empty($this->twig))
			UELM_UniteFunctionsUC::throwError("Can't add template or params when after rendered");
	}
	...
	private function validateInited(){
		...
		if($this->arrParams === null){
			UELM_UniteFunctionsUC::throwError("Please set the params");
		}
	...
	public function addTemplate($name, $html, $showError = true){
		...
		if($isExists == true){
			...
			UELM_UniteFunctionsUC::throwError("template with name: $name already exists");
		}
	...
	public function setParams($params){
		...
		//some small hack
		UELM_GlobalsProviderUC::$lastWidgetParams = $params;	
	...
	public function getRenderedHtml($name, $isInsideItems = false){
		...
		UELM_UniteFunctionsUC::validateNotEmpty($name);
		$this->validateInited();
		if(array_key_exists($name, $this->arrTemplates) == false)
			UELM_UniteFunctionsUC::throwError("Template with name: $name not exists");
		...
		if($isInsideItems == true)
			$params = UELM_GlobalsProviderUC::$lastItemParams;
