...
class UELM_UniteCreatorAPIIntegrations{
	...
	public function getTypes(){
		...
		if(UELM_GlobalsUnlimitedElements::$enableGoogleAPI === true){
		...
		if(UELM_GlobalsUnlimitedElements::$enableWeatherAPI === true)
			$types[self::TYPE_WEATHER_FORECAST] = "Weather Forecast";

		if(UELM_GlobalsUnlimitedElements::$enableCurrencyAPI === true)
	...
	public function getData($type, $params){
		...
		// add api keys
		$params[self::SETTINGS_OPEN_WEATHER_API_KEY] = UELM_HelperProviderCoreUC_EL::getGeneralSetting(self::SETTINGS_OPEN_WEATHER_API_KEY);
		$params[self::SETTINGS_EXCHANGE_RATE_API_KEY] = UELM_HelperProviderCoreUC_EL::getGeneralSetting(self::SETTINGS_EXCHANGE_RATE_API_KEY);
		...
		switch($type){
			...
			default:
				...
				if(UELM_UniteFunctionsUC::isMaxDebug()){
					UELM_UniteFunctionsUC::showTrace();
				}
				
				UELM_UniteFunctionsUC::throwError(__FUNCTION__ . " Error: API type \"$type\" is not implemented");
			break;
		}
	...
	public function getDataForMultisource($type, $params){
		...
		switch($type){
			case self::TYPE_CURRENCY_EXCHANGE:
				$data = UELM_UniteFunctionsUC::getVal($data, "rates_chosen");
			break;
			case self::TYPE_WEATHER_FORECAST:
				$data = UELM_UniteFunctionsUC::getVal($data, "daily");
			break;
			case self::TYPE_GOOGLE_REVIEWS:
				$data = UELM_UniteFunctionsUC::getVal($data, "reviews");
			break;
		}
	...
	public function addDataToParams($data, $name, $paramType = null){
		...
		$params = UELM_UniteFunctionsUC::getVal($data, $name, array());
		$params = UELM_UniteFunctionsUC::clearKeysFirstUnderscore($params);
				
		try{
			
			$apiType = UELM_UniteFunctionsUC::getVal($params, "api_type");
	...
	public function addServiceSettingsFields($settingsManager, $type, $name, $condition = null){
		...
		$fields = UELM_UniteFunctionsUC::getVal($fields, $type);
		...
		// add api type
		$params = array();
		$params["origtype"] = UELM_UniteCreatorDialogParam::PARAM_HIDDEN;

		$settingsManager->addHiddenInput($name . "_api_type", $type, "API Type", $params);
		
		// add the fields
		UELM_HelperProviderUC::addSettingsFields($settingsManager, $fields, $name, $condition, true);
	}
	...
	private function remapLegacyParams($params){
		...
		foreach($keysMap as $legacyKey => $key){
			$value = UELM_UniteFunctionsUC::getVal($params, $key);
			$legacyValue = UELM_UniteFunctionsUC::getVal($params, $legacyKey);
	...
	private function getParam($key, $fallback = null, $name = null){
		...
		$paramValue = UELM_UniteFunctionsUC::getVal($this->params, $key);
	...
	private function getRequiredParam($key, $label = null, $name = null){
		...
		UELM_UniteFunctionsUC::throwError(sprintf(__("%s is required.", "unlimited-elements"), $label));
	}
	...
	private function validateGoogleApiKey(){
		...
		if(empty($key) === true)
			UELM_UniteFunctionsUC::throwError(__("Google API key is missing.", "unlimited-elements"));
	}
	...
	private function validateGoogleCredentials(){
		...
		if($hasCredentials === false)
			UELM_UniteFunctionsUC::throwError(__("Google credentials are missing.", "unlimited-elements"));
	}
	...
	private function getCurrencyExchangeSettingsFields(){
		...
		$key = UELM_HelperProviderCoreUC_EL::getGeneralSetting(self::SETTINGS_EXCHANGE_RATE_API_KEY);
		...
		$fields = array_merge($fields, array(
			array(
				...
				"type" => UELM_UniteCreatorDialogParam::PARAM_TEXTFIELD,
				"text" => __("Currency Code", "unlimited-elements"),
				// translators: %s is page url
				"desc" => sprintf(__("Enter the three-letter <a href='%s' target='_blank'>currency code</a>.", "unlimited-elements"), "https://exchangerate-api.com/docs/supported-currencies"),
			),
			array(
				"id" => self::CURRENCY_EXCHANGE_FIELD_PRECISION,
				"type" => UELM_UniteCreatorDialogParam::PARAM_TEXTFIELD,
				"text" => __("Rate Precision", "unlimited-elements"),
				// translators: %1$d is a number, %2$d is a number, %3$d is a number
				"desc" => sprintf(__("Optional. You can specify the number of decimals for the rate: from %1\$d to %2\$d. The default value is %3\$d.", "unlimited-elements"), self::CURRENCY_EXCHANGE_MIN_PRECISION, self::CURRENCY_EXCHANGE_MAX_PRECISION, self::CURRENCY_EXCHANGE_DEFAULT_PRECISION),
			),
			array(
				"id" => self::CURRENCY_EXCHANGE_FIELD_INCLUDE_CURRENCIES,
				"type" => UELM_UniteCreatorDialogParam::PARAM_TEXTAREA,
				"text" => __("Include Currencies", "unlimited-elements"),
				"desc" => __("Optional. You can specify a comma separated list of currency codes to include, otherwise all currencies will be displayed.", "unlimited-elements"),
			),
			array(
				"id" => self::CURRENCY_EXCHANGE_FIELD_CACHE_TIME,
				"type" => UELM_UniteCreatorDialogParam::PARAM_TEXTFIELD,
				"text" => __("Cache Time", "unlimited-elements"),
				// translators: %d is a number
				"desc" => sprintf(__("Optional. You can specify the cache time of results in minutes. The default value is %d minutes.", "unlimited-elements"), self::CURRENCY_EXCHANGE_DEFAULT_CACHE_TIME),
	...
	private function getGoogleEventsSettingsFields(){
		...
		$fields = array_merge($fields, array(
			array(
				...
				"type" => UELM_UniteCreatorDialogParam::PARAM_TEXTFIELD,
				"text" => __("Calendar ID", "unlimited-elements"),
				"desc" => __("You can find the calendar ID on a calendar's \"Settings\" page under \"Integrate Calendar\".", "unlimited-elements"),
			),
			array(
				"id" => self::GOOGLE_EVENTS_FIELD_RANGE,
				"type" => UELM_UniteCreatorDialogParam::PARAM_DROPDOWN,
				"text" => __("Date Range", "unlimited-elements"),
				"options" => array(
					self::GOOGLE_EVENTS_RANGE_UPCOMING => __("Upcoming", "unlimited-elements"),
					self::GOOGLE_EVENTS_RANGE_TODAY => __("Today's", "unlimited-elements"),
					self::GOOGLE_EVENTS_RANGE_TOMORROW => __("Tomorrow's", "unlimited-elements"),
					self::GOOGLE_EVENTS_RANGE_WEEK => __("This week", "unlimited-elements"),
					self::GOOGLE_EVENTS_RANGE_MONTH => __("This month", "unlimited-elements"),
				),
			...
			array(
				...
				"type" => UELM_UniteCreatorDialogParam::PARAM_DROPDOWN,
				"text" => __("Order By", "unlimited-elements"),
				"options" => array(
					self::GOOGLE_EVENTS_ORDER_DATE_DESC => __("Date (newest)", "unlimited-elements"),
					self::GOOGLE_EVENTS_ORDER_DATE_ASC => __("Date (oldest)", "unlimited-elements"),
				),
			...
			array(
				...
				"type" => UELM_UniteCreatorDialogParam::PARAM_TEXTFIELD,
				"text" => __("Events Limit", "unlimited-elements"),
				// translators: %d is a number
				"desc" => sprintf(__("Optional. You can specify the maximum number of events: from 1 to 2500. The default value is %d.", "unlimited-elements"), self::GOOGLE_EVENTS_DEFAULT_LIMIT),
				"default" => self::GOOGLE_EVENTS_DEFAULT_LIMIT,
			),
			array(
				"id" => self::GOOGLE_EVENTS_FIELD_TIMEZONE,
				"type" => UELM_UniteCreatorDialogParam::PARAM_TEXTFIELD,
				"text" => __("Timezone", "unlimited-elements"),
				"desc" => __("Example: Europe/Rome, UTC, -06:30, +08:45. Leave empty for timezone set in wp settings", "unlimited-elements"),
			),
			array(
				...
				"text" => __("Cache Time", "unlimited-elements"),
				// translators: %d is a number
				"desc" => sprintf(__("Optional. You can specify the cache time of results in minutes. The default value is %d minutes.", "unlimited-elements"), self::GOOGLE_EVENTS_DEFAULT_CACHE_TIME),
	...
	private function getYoutubePlaylistSettingsFields(){
		...
		$fields = array_merge($fields, array(
			array(
				...
				"type" => UELM_UniteCreatorDialogParam::PARAM_TEXTFIELD,
				"text" => __("Playlist ID", "unlimited-elements"),
				// translators: %1$s is page url, %2$s is page url
				"desc" => sprintf(__("You can find the playlist ID in a YouTube URL: <br />— %1\$s<br />— %2\$s", "unlimited-elements"), "https://youtube.com/playlist?list=<b>[YOUR_PLAYLIST_ID]</b>", "https://youtube.com/watch?v=aBC-123xYz&list=<b>[YOUR_PLAYLIST_ID]</b>"),
			),
			array(
				...
				"text" => __("Order By", "unlimited-elements"),
				"options" => array(
					self::YOUTUBE_PLAYLIST_ORDER_DEFAULT => __("Default", "unlimited-elements"),
					self::YOUTUBE_PLAYLIST_ORDER_DATE_ADDED_DESC => __("Date added (newest)", "unlimited-elements"),
					self::YOUTUBE_PLAYLIST_ORDER_DATE_ADDED_ASC => __("Date added (oldest)", "unlimited-elements"),
					self::YOUTUBE_PLAYLIST_ORDER_DATE_ADDED_RANDOM => __("Date added (random)", "unlimited-elements"),
					self::YOUTUBE_PLAYLIST_ORDER_DATE_PUBLISHED_DESC => __("Date published (newest)", "unlimited-elements"),
					self::YOUTUBE_PLAYLIST_ORDER_DATE_PUBLISHED_ASC => __("Date published (oldest)", "unlimited-elements"),
					self::YOUTUBE_PLAYLIST_ORDER_DATE_PUBLISHED_RANDOM => __("Date published (random)", "unlimited-elements"),
				),
			...
			array(
				...
				"type" => UELM_UniteCreatorDialogParam::PARAM_TEXTFIELD,
				"text" => __("Videos Limit", "unlimited-elements"),
				// translators: %d is a number
				"desc" => sprintf(__("Optional. You can specify the maximum number of videos: from 1 to 50. The default value is %d.", "unlimited-elements"), self::YOUTUBE_PLAYLIST_DEFAULT_LIMIT),
			...
			array(
				...
				"text" => __("Cache Time", "unlimited-elements"),
				// translators: %d is a number
				"desc" => sprintf(__("Optional. You can specify the cache time of results in minutes. The default value is %d minutes.", "unlimited-elements"), self::YOUTUBE_PLAYLIST_DEFAULT_CACHE_TIME),
	...
	private function getCurrencyExchangeData(){
		...
		if(empty($includeCurrencies) === true)
			$filteredData = UELM_UniteFunctionsUC::assocToArray($data);

		foreach($includeCurrencies as $code){
			$rate = UELM_UniteFunctionsUC::getVal($data, $code);
		...
		$baseRate = UELM_UniteFunctionsUC::getVal($data, $baseCode);

		$data = UELM_UniteFunctionsUC::assocToArray($data);
		$filteredDataJson = UELM_UniteFunctionsUC::jsonEncodeForClientSide($filteredData);
	...
	private function getSerpAPIKey(){
		...
		if(UELM_GlobalsUnlimitedElements::$enableSerpAPI == false)
			return(null);
		
		$serpKey = UELM_HelperProviderCoreUC_EL::getGeneralSetting("serpapi_key");
	...
	private function getGoogleReviewsData(){
		...
		try{
			...
			if($this->googleReviewsShowDebug == true){
				...
				echo UELM_HelperHtmlUC::getDebugWarningMessageHtml($message);
			} 
		...
		//add api source
			...
			if($isSerp == true){
				...
				$data["api_search_link_for_debug"] = UELM_UniteFunctionsUC::getVal($arrMetaData, "json_endpoint");
				$data["api_search_html_link_for_debug"] = UELM_UniteFunctionsUC::getVal($arrMetaData, "prettify_html_file");
			...
			if($this->googleReviewsShowDebug == true){
				...
				echo UELM_HelperHtmlUC::getErrorMessageHtml($message,"",true);
			}
	...
	private function getGoogleReviewsSettingsFields($isSingle = false){
		...
		$fields[] = array(
				...
				"type" => UELM_UniteCreatorDialogParam::PARAM_TEXTFIELD,
				"text" => __("Place ID", "unlimited-elements"),
				// translators: %s is page url
				"desc" => sprintf(__("You can find the place ID by using <a href='%s' target='_blank'>Place ID Finder</a>.", "unlimited-elements"), "https://developers.google.com/maps/documentation/javascript/examples/places-placeid-finder"),
		...
		if($isSerpEnabled == false)
			$text = sprintf(__("To get more then 5 reviews, enter %s key in general settings", "unlimited-elements"), "<a href='https://serpapi.com' target='_blank'>serpapi.com</a>");
		else
			$text = sprintf(__("Fetching google reviews using %s service.", "unlimited-elements"), "<a href='https://serpapi.com' target='_blank'>serpapi.com</a>");
		
		//if there is no option - no need for text
		if(UELM_GlobalsUnlimitedElements::$enableSerpAPI == true){
			
			$fields[] = array(
					"id" => self::GOOGLE_REVIEWS_FIELD_API_TEXT,
					"type" => UELM_UniteCreatorDialogParam::PARAM_STATIC_TEXT,
					"text" => $text,
			);
		}
		...
		if($isSerpEnabled == false){
			...
			$fields[] = array(
					...
					"text" => __("Cache Time", "unlimited-elements"),
					// translators: %d is a number
					"desc" => sprintf(__("Optional. You can specify the cache time of results in minutes. The default value is %d minutes.", "unlimited-elements"), self::GOOGLE_REVIEWS_DEFAULT_CACHE_TIME),
			...
			$fields[] = array(
				...
				"type" => UELM_UniteCreatorDialogParam::PARAM_DROPDOWN,
				"text" => __("Cache Time", "unlimited-elements"),
				"desc" => __("Select how often the reviews should be refreshed.", "unlimited-elements"),
				"options" => array(
					"day" => __("Once a day", "unlimited-elements"),
					"week" => __("Once a week", "unlimited-elements"),
					"month" => __("Once a month", "unlimited-elements"),
				),
				"default" => "week"
			);
		...
		$fields[] = array(
				...
				"type" => UELM_UniteCreatorDialogParam::PARAM_TEXTFIELD,
				"text" => __("Language Code", "unlimited-elements"),
				"desc" => __("Optional. Specify a language code. Example: de and google will translate the review.", "unlimited-elements"),
				"default" => "",
			);
		...
		if($isSerpEnabled == true){
			...
			$fields[] = array(
				...
				"type" => UELM_UniteCreatorDialogParam::PARAM_DROPDOWN,
				"text" => __("Sort By", "unlimited-elements"),
				"options" => array(
	...
	public function getGoogleSheetsSettingsFields(){
		...
		$fields = array_merge($fields, array(
			array(
				...
				"type" => UELM_UniteCreatorDialogParam::PARAM_TEXTFIELD,
				"text" => __("Spreadsheet ID", "unlimited-elements"),
				// translators: %s is a string
				"desc" => sprintf(__("You can find the spreadsheet ID in a Google Sheets URL: %s", "unlimited-elements"), "https://docs.google.com/spreadsheets/d/<b>[YOUR_SPREADSHEET_ID]</b>/edit#gid=0"),
			),
			array(
				"id" => self::GOOGLE_SHEETS_FIELD_SHEET_ID,
				"type" => UELM_UniteCreatorDialogParam::PARAM_TEXTFIELD,
				"text" => __("Sheet ID", "unlimited-elements"),
				// translators: %s is page url
				"desc" => sprintf(__("Optional. You can find the sheet ID in a Google Sheets URL: %s", "unlimited-elements"), "https://docs.google.com/spreadsheets/d/aBC-123_xYz/edit#gid=<b>[YOUR_SHEET_ID]</b>"),
			),
			array(
				...
				"text" => __("Cache Time", "unlimited-elements"),
				// translators: %d is a number
				"desc" => sprintf(__("Optional. You can specify the cache time of results in minutes. The default value is %d minutes.", "unlimited-elements"), self::GOOGLE_SHEETS_DEFAULT_CACHE_TIME),
				"default" => self::GOOGLE_SHEETS_DEFAULT_CACHE_TIME,
			),
			array(
				"id" => self::GOOGLE_SHEETS_EXCLUDE_FIRST_ROW,
				"type" => UELM_UniteCreatorDialogParam::PARAM_RADIOBOOLEAN,
				"text" => __("Exclude First Row", "unlimited-elements"),
				// translators: %s is a string
				"desc" => __("You can exclude first row from Google Sheet Table", "unlimited-elements"),
			),
		));
	...
	private function getWeatherForecastSettingsFields(){
		...
		$key = UELM_HelperProviderCoreUC_EL::getGeneralSetting(self::SETTINGS_OPEN_WEATHER_API_KEY);
		...
		$fields = array_merge($fields, array(
			array(
				...
				"type" => UELM_UniteCreatorDialogParam::PARAM_TEXTFIELD,
				"text" => __("Country Code", "unlimited-elements"),
				// translators: %s is page url
				"desc" => sprintf(__("Specify the two-letter <a href='%s' target='_blank'>country code</a>.", "unlimited-elements"), "https://en.wikipedia.org/wiki/ISO_3166-2#Current_codes"),
				"default" => "GB",
			),
			array(
				"id" => self::WEATHER_FORECAST_FIELD_CITY,
				"type" => UELM_UniteCreatorDialogParam::PARAM_TEXTFIELD,
				"text" => __("City Name", "unlimited-elements"),
				"default" => "London",
			),
			array(
				"id" => self::WEATHER_FORECAST_FIELD_UNITS,
				"type" => UELM_UniteCreatorDialogParam::PARAM_DROPDOWN,
				"text" => __("Units", "unlimited-elements"),
				"options" => array(
					self::WEATHER_FORECAST_UNITS_METRIC => __("Metric", "unlimited-elements"),
					self::WEATHER_FORECAST_UNITS_IMPERIAL => __("Imperial", "unlimited-elements"),
				),
			...
			array(
				...
				"text" => __("Language Code", "unlimited-elements"),
				"default" => "",
				"desc" => __("2 digits of language code. for example: 'fr' (french) for the list of all codes: <a href='https://en.wikipedia.org/wiki/List_of_ISO_639_language_codes'>Press Here</a>. Leave empty for auto select. ", "unlimited-elements"),
			),
	...
	private function getWeatherForecastData(){
		...
		$currentForecast = UELM_UniteFunctionsUC::getVal($forecasts, "current");
		$hourlyForecasts = UELM_UniteFunctionsUC::getVal($forecasts, "hourly");
		$dailyForecasts = UELM_UniteFunctionsUC::getVal($forecasts, "daily");
	...
	private function addEmptyApiKeyField($fields, $key, $id, $name){
		...
		if(empty($key) === true){
			$fields[] = array(
				...
				"text" => sprintf(__("%s key is missing. Please add the key in the \"General Settings > Integrations\".", "unlimited-elements"), $name),
			);
		}
	...
	private function addGoogleEmptyCredentialsField($fields, $id){
		...
		if($hasCredentials === false){
			$fields[] = array(
				"id" => $id,
				"type" => UELM_UniteCreatorDialogParam::PARAM_STATIC_TEXT,
				"text" => __("Google credentials are missing. Please connect to Google or add an API key in the \"General Settings > Integrations\".", "unlimited-elements"),
			);
		}
	...
	private function outputGoogleReviewsRefreshButton(){
		...
		$html .= __('Manual Refresh Reviews', 'unlimited-elements');
		$html .= '</button>';
