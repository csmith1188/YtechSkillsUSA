...
class UELM_UniteCreatorRSS{
    ...
    public function __construct(){
    	...
    	if(self::$showDebug == false){
    		$rssDebug = UELM_HelperUC::hasPermissionsFromQuery("ucrssdebug");
    ...
    public function getRssFields($name = null) {
        ...
        $fields = array(
            array(
                ...
                "id" => "rss_url",
                "text" => __("RSS URL", "unlimited-elements"),
                "desc" => __("Enter some RSS service url. Example: https://wired.com/feed/rss", "unlimited-elements"),
            ...
            array(
                ...
                "options" => array(
                    ...
                    "today" => __("Today", "unlimited-elements"),
                    "yesterday" => __("Yesterday", "unlimited-elements"),
                    "last_2_days" => __("Last 2 Days", "unlimited-elements"),
                    "last_3_days" => __("Last 3 Days", "unlimited-elements")
                ),
                "default" => "all"
            ),
            array(
            ...
            array(
                ...
                "text" => "",
                "desc" => __("Specify the <a target='_blank' href='https://www.php.net/manual/en/datetime.format.php'>date format</a>, default is 'd/m/Y, H:i'", "unlimited-elements"),
            ...
            array(
                "type" => UELM_UniteCreatorDialogParam::PARAM_TEXTFIELD,
                "id" => "rss_items_limit",
                "text" => __("Items Limit", "unlimited-elements"),
                "desc" => __("Optional. You can specify the maximum number of items: from 1 to 50., Use 0 for all", "unlimited-elements"),
                "placeholder" => "0",
                "default" => "0"
            ),
        );
	...
	public function getRssFeedData($data, $arrValues, $name, $showDebug = false){
		...
		if(empty($arrValues)){
			...
			if($showDebug)
				uelm_dmp("no data found");
			
			return($data);
		}
			
		$rss_url = UELM_UniteFunctionsUC::getVal($arrValues, $name.'_rss_url');

		if(empty($rss_url)){
			if($showDebug)
				uelm_dmp("no url found for rss");
		...
		if($showDebug){
			uelm_dmp("Get rss data from: $rss_url");			
		}
		try{
	        
			$rssContent = UELM_HelperUC::$operations->getUrlContents($rss_url, $getUrlDebug, true);
				        
		}catch(Exception $e){
			
            if($showDebug){
                uelm_dmp("Failed to fetch rss: ".$e->getMessage());
            }
        ...
        //uelm_dmp($response);exit();
        
        if(empty($rssContent)) {
            if($showDebug)
                uelm_dmp("no content found for rss");

            return($data);
        }
		
        $arrData = UELM_UniteFunctionsUC::maybeXmlDecode($rssContent);
		
        if(empty($arrData)) {
            if($showDebug)
                uelm_dmp('no data found for rss');
        ...
        if($showDebug == true && is_array($arrData)) {
            ...
            uelm_dmp("Original rss data");
			            
			UELM_HelperHtmlUC::putHtmlDataDebugBox($arrData);						
        }

        if(is_array($arrData) == false){

            if($showDebug == true){
                uelm_dmp("No RSS data found. The input is: ");
                echo "<div style='background-color:lightgray'>";
                uelm_dmp(htmlspecialchars($rssContent));
                echo "</div>";
            }
        ...
        if(!empty($arrData)) {
            (bool) $showDateFormated = UELM_UniteFunctionsUC::getVal($arrValues, $name."_rss_show_date_formatted");
            $dateFormat = null;
            if ($showDateFormated) {
                $dateFormat = UELM_UniteFunctionsUC::getVal($arrValues, $name."_rss_date_format");
                if (empty($dateFormat)) {
            ...
            // trim by date limits
            (bool) $showFilterByDate = UELM_UniteFunctionsUC::getVal($arrValues, $name."_rss_show_filter_by_date");
            if($showFilterByDate && !empty($arrData)) {
            	
                $filterByDateOption = UELM_UniteFunctionsUC::getVal($arrValues, $name . "_rss_filter_by_date_option");
            ...
            // trim by items limit
            (int) $dataItemsLimit = UELM_UniteFunctionsUC::getVal($arrValues, $name."_rss_items_limit");
		...
		if ($from_time > 0 && $to_time > 0) {
			...
			foreach ($arrData as $key => $value){
				...
				if (array_key_exists('publish_date', $value)) {
					...
					$timestamp = UELM_UniteFunctionsUC::date2Timestamp($dateString);
	...
	private function simplifyRssDataArray($arrRss, $dateFormat = null){
        ...
        $firstRssElement = UELM_UniteFunctionsUC::getArrFirstValue($arrRss);
			...
			if (is_array($value)) {
				...
				if (is_numeric($key)) {
					...
					if (!empty($dateFormat) && ($newKey == 'publish_date' || $newKey == 'published')) {
						$date_time = UELM_UniteFunctionsUC::date2Timestamp($value);
	...
	private function createDate($arrRss, $dateFormat) {
		...
		foreach ($arrRss as $key => $value) {
			if (is_array($value)) {
				...
				$timestamp = UELM_UniteFunctionsUC::date2Timestamp($value);
				if (!empty($timestamp)) {
    ...
    public function filterByTypeAndCatId($array, $type, $catIdValue) {
        ...
        foreach ($array as $key => $subArray) {
            ...
            if ($subArray[UELM_GlobalsUC::ATTR_CATID] != $catIdValue) {
                continue;
            }
     ...
     */
    public function getRssFeedKeys(UELM_UniteCreatorAddonWork $addon, $data){
    	
        $params = $addon->getParams();
		
        $keys = array();
		
        $rssFeedArr = UELM_UniteFunctionsUC::getVal($data, "rss_feed");
        if (empty($rssFeedArr)) {
            return;
        }

        $firstRssElement = UELM_UniteFunctionsUC::getArrFirstValue($rssFeedArr);
        ...
        $rssKeyArr = $this->filterByTypeAndCatId($params, UELM_UniteCreatorDialogParam::PARAM_TEXTFIELD, $catValue[UELM_GlobalsUC::ATTR_CATID]);
        
        $isAutoDetect = UELM_UniteFunctionsUC::getVal($data, "auto_detect_keys");
        $isAutoDetect = UELM_UniteFunctionsUC::strToBool($isAutoDetect);
        
        $isShowDebug = UELM_UniteFunctionsUC::getVal($data, "show_debug");
        $isShowDebug = UELM_UniteFunctionsUC::strToBool($isShowDebug);
		
        if ($isShowDebug) {
            uelm_dmp('--- Fields Keys ---');
        }
        ...
        foreach ($rssKeyArr as $keyValue) {
            ...
            if ($isAutoDetect == false) {
                $defaultKey = UELM_UniteFunctionsUC::getVal($keyValue, 'value');
            }
            ...
            if ($isShowDebug == true) {
            	...
            	$title = UELM_UniteFunctionsUC::getVal($keyValue, "title");
            	$key = UELM_UniteFunctionsUC::getVal($keys, $keyName);
            	
            	$autoText = $isAutoKey?" <span style='color:grey;'>[auto detect]</span>":"";
            	
                if(!array_key_exists($keyName, $keys)){
                    uelm_dmp("{$title}: <span style='color:darkred;'>not detected, please select a custom one</span>");
                }
                 else if (array_key_exists($keys[$keyName], $firstRssElement))
                    uelm_dmp("{$title}: <b>$key</b> {$autoText}");
                 else 
                    uelm_dmp("{$title}: <span style='color:darkred;'> key not found, please select another one</span>");
        ...
        if ($isShowDebug) {
            uelm_dmp('--- first RSS item structure ---');
            $firstRssShow = UELM_UniteFunctionsUC::modifyDataArrayForShow($firstRssElement);
            uelm_dmp($firstRssShow);
        }
    ...
    private function hasImageKey($rssItem) {
        ...
        $possibleImageKeys = UELM_UniteFunctionsUC::getVal($this->rssAutoDetectKeys, "image_key");    
