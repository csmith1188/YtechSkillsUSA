...
class UELM_UniteCreatorBrowserWork extends UELM_HtmlOutputBaseUC{
	...
	public function __construct(){
		...
		$this->webAPI = new UELM_UniteCreatorWebAPI();
		$this->textBuy = esc_html__("Buy PRO", "unlimited-elements");
		$this->textHoverProAddon = esc_html__("This addon is available<br>for Blox PRO users only.", "unlimited-elements");
		$this->urlBuy = UELM_GlobalsUC::$url_buy_platform;
		
		self::$serial++;
		$this->prefix = UELM_UniteFunctionsUC::getRandomString(5).self::$serial;
	...
	public function initAddonType($addonType){
		...
		if(is_array($addonType)){
			...
			UELM_UniteFunctionsUC::validateNotEmpty($addonType,"multiple addon types");
			
			
			$this->isMultipleAddonTypes = true;
			foreach($addonType as $type){
				
				$objAddonType = UELM_UniteCreatorAddonType::getAddonTypeObject($type);
			...
			$this->objAddonType = UELM_UniteCreatorAddonType::getAddonTypeObject($addonType);
		}
	...
	private function getIcon($name){
		...
		$icon = UELM_UniteFontManagerUC::getIcon($name, $this->arrIcons);
	...
	private function getHtmlCatalogHeader(){
		...
		$textBlox = esc_html__("Blox", "unlimited-elements");
		$textShowAll = esc_html__("Show All", "unlimited-elements");
		$textInstalled = esc_html__("Installed", "unlimited-elements");
		$textFree = esc_html__("Free", "unlimited-elements");
		$textPro = esc_html__("Pro", "unlimited-elements");
		
		$textBuy = $this->textBuy;
		
		$textAlreadyBought = esc_html__("Already bought Blox PRO?", "unlimited-elements");
		$textTheProductActive = esc_html__("The product is Active!", "unlimited-elements");
		$textDeactivate = esc_html__("Deactivate", "unlimited-elements");
		$textCheckUpdate = esc_html__("Check Catalog Update", "unlimited-elements");
		$textClear = esc_html__("Clear", "unlimited-elements");
	...
	private function getHtmlTabs($arrCats){
		...
		foreach($arrCats as $catTitle=>$cat){
			...
			if($this->isPages == false)
				$arrItems = UELM_UniteFunctionsUC::getVal($cat, "addons");
			else
				$arrItems = $cat;
			...
			if(empty($catTitle))
				$catTitle = UELM_UniteFunctionsUC::getVal($cat, "title");
			
			if(empty($catTitle)){
				$id = UELM_UniteFunctionsUC::getVal($cat, "id");
				if(empty($id))
					$id = $counter;
				
				$catTitle = esc_html__("Category", "unlimited-elements")." {$id}";
			}
		...
		$htmlTitleCategories = esc_html__("Categories", "unlimited-elements");
		if(!empty($totalItems))
	...
	private function getHtmlContent_cats($arrCats){
		...
		foreach($arrCats as $catTitle => $cat){
			...
			$title = UELM_UniteFunctionsUC::getVal($cat, "title");
			...
			if(empty($arrItems)){
				...
				if($this->isPages == false)
					$html .= esc_html__("No addons in this category", "unlimited-elements");
				else 
					$html .= esc_html__("No pages in this category", "unlimited-elements");
				
			}
			else{
				
				if(is_array($arrItems) == false)
					UELM_UniteFunctionsUC::throwError("The cat addons array should be array");
	...
	public function isWebAddonFree($addon){
		...
		$isFree = UELM_UniteFunctionsUC::getVal($addon, "isfree");
	...
	private function getAddonUrlPreview($addon){
		...
		$isWeb = UELM_UniteFunctionsUC::getVal($addon, "isweb");
		$previewPage = UELM_UniteFunctionsUC::getVal($addon, "preview");
		
		if(!empty($previewPage) && $isWeb == true){
			$urlPreview = UELM_GlobalsUC::URL_PREVIEW_WIDGETS.$previewPage;	
			return($urlPreview);
		}
		...
		$addonName = UELM_UniteFunctionsUC::getVal($addon, "name");
	...
	public function getCatalogAddonStateData($state, $isPage = false, $urlPreview = null, $addon = array(),$options = array()){
		...
		$isWebCatalogMode = UELM_UniteFunctionsUC::getVal($options, "web_catalog_mode");
		$isWebCatalogMode = UELM_UniteFunctionsUC::strToBool($isWebCatalogMode);
		...
		$textItem = __("widget","unlimited-elements");
		...
		$actionPreview = __("Preview", "unlimited-elements");
        ...
        switch($state){
        	case self::STATE_FREE:
        		$label = 'free';
        		$labelText = __('Free',"unlimited-elements");
        		$hoverText = __("This ","unlimited-elements"). $textItemHigh .__(" Is Free","unlimited-elements")."<br>".__("To use it click install","unlimited-elements");
        		$hoverIcon = '<i class="'.$iconCloudDownload.'" aria-hidden="true"></i>';
        		$action = "Install";
        		
        		if(UELM_GlobalsUC::$isProductActive){
        			$labelText = __('Web',"unlimited-elements");
        			$hoverText = __("You can install this ","unlimited-elements").$textItem ."<br>".__("To use it click install","unlimited-elements");
        		}
        	...
        	case self::STATE_PRO:
        		...
        		$label = 'pro';
        		$labelText = __('Pro',"unlimited-elements");
        ...
        if($isPage == true){
	        ...
	        if(UELM_GlobalsUC::$isProductActive && $isWebCatalogMode == true){
	        	$showState = false;
	        }
        ...
        if($showState == true){
        	...
        	$addStateClass = "";
        	if(UELM_UniteCreatorManagerAddons::$stateLabelCounter > 0)
        	$addStateClass = "uc-state-label".UELM_UniteCreatorManagerAddons::$stateLabelCounter;
        	
			$htmlState = "<div class='uc-state-label uc-state-{$label} $addStateClass'>
				<div class='uc-state-label-text'>{$labelText}</div>
			</div>";			
        	
        	UELM_UniteCreatorManagerAddons::$stateLabelCounter++;
		...
		if($showAdditions == true){
			...
			$textInstalling = esc_html__("Installing", "unlimited-elements");
	...
	private function getHtmlItem($arrItem){
		...
		if($this->isFromWebOnly == true)
			...
			$isFromWeb = UELM_UniteFunctionsUC::getVal($arrItem, "isweb");
			$isFromWeb = UELM_UniteFunctionsUC::strToBool($isFromWeb);
		}
		...
		*/
		...
		$name = UELM_UniteFunctionsUC::getVal($arrItem,"name");		
		$alias = UELM_UniteFunctionsUC::getVal($arrItem, "alias");
		$cat = UELM_UniteFunctionsUC::getVal($arrItem, "cat");
		
		
		if(!empty($alias) && $this->isPages == false && $isFromWeb == false)
			$name = $alias;	
		
		$name = UELM_UniteFunctionsUC::sanitizeAttr($name);
		
		$title = UELM_UniteFunctionsUC::getVal($arrItem, "title");
		$title = UELM_UniteFunctionsUC::sanitizeAttr($title);
		...
		$urlPreviewImage = UELM_UniteFunctionsUC::getVal($arrItem, $paramImage);
		$urlPreviewImage = UELM_UniteFunctionsUC::sanitizeAttr($urlPreviewImage);
		
		$id = UELM_UniteFunctionsUC::getVal($arrItem, "id");
		...
		if($this->isPages == true){
			...
			$urlItemPreview = UELM_UniteFunctionsUC::getVal($arrItem, "url");
		}
	...
	protected function getHtmlEmptyItem(){
		...
		$state = self::STATE_INSTALLED;
		$title = esc_html__("Not Selected", "unlimited-elements");
	...
	private function removeEmptyCatalogCats($arrCats){
		...
		foreach($arrCats as $key=>$cat){
			...
			$addons = UELM_UniteFunctionsUC::getVal($cat, "addons");
			if(empty($addons))
				unset($arrCats[$key]);			
		}
	...
	* get catalog html
	...
	public function putScripts(){
		...
		UELM_UniteCreatorAdmin::onAddScriptsBrowser();
	}
	...
	public function putBrowser($putMode = true){
		...
		if(empty($this->objAddonType))
			$this->objAddonType = UELM_UniteCreatorAddonType::getAddonTypeObject(UELM_GlobalsUC::ADDON_TYPE_REGULAR_ADDON);
	...
	public function putScriptsAndBrowser($getHTML = false){
		...
		try{
			...
			$htmlError = UELM_HelperUC::getHtmlErrorMessage($message, $trace);
	...
	private function putCatalogUpdateDialog(){
		...
		?>
			...
			<div id="uc_dialog_catalog_update" title="<?php esc_html_e("Check And Update Catalog", "unlimited-elements")?>" class="unite-inputs" style="display:none">
				<div class="unite-dialog-inside">
					
					<span id="uc_dialog_catalog_update_loader" class="loader_text">
						<?php esc_html_e("Checking Update", "unlimited-elements")?>...
					</span>
