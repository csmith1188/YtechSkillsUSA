...
class UELM_UniteCreatorParamsProcessorWork{
	...
	private function validateInited(){
		...
		if(empty($this->addon))
			UELM_UniteFunctionsUC::throwError("The params processor is not inited");
	...
	private function validateProcessTypeInited(){
		...
		if(empty($this->processType))
			UELM_UniteFunctionsUC::throwError("The process type is not inited");
	...
	public static function validateProcessType($type){
		UELM_UniteFunctionsUC::validateValueInArray($type, "process type",array(
	...
	protected function convertValueByType($value, $type, $param){
		...
		if(is_string($value)){
			...
			switch($type){
				case "uc_image":
				case "uc_mp3":
				case UELM_UniteCreatorDialogParam::PARAM_FILE:
					$value = UELM_HelperUC::URLtoFull($value);
					break;
			}
		...
		switch($type){
			case UELM_UniteCreatorDialogParam::PARAM_NUMBER:		//modify number field
				
				if(is_array($value)){
					
					$defaultValue = UELM_UniteFunctionsUC::getVal($param, "default_value");
					$value = UELM_UniteFunctionsUC::getVal($value, "size", $defaultValue);
		...
		if($addonType == "elementor")
			$value = UELM_HelperProviderCoreUC_EL::processParamValueByType($value, $type, $param);
	...
	private function checkModifyParamOptions_manual($options, $phpFilter){
		...
		switch($phpFilter){
			case "ue_sort_filter":
				...
				$optionsSortBy = UELM_UniteFunctionsWPUC::getArrSortBy(true, true);
	...
	private function checkModifyParamOptions($param){
		...
		if(is_string($value) === true && in_array($value, $options) === false)
			$value = UELM_UniteFunctionsUC::getArrFirstValue($options);
	...
	* return if it's output process type
	...
	protected function getProcessedParamsValue_responsive($data, $param){
		...
		$valueTablet = UELM_UniteFunctionsUC::getVal($param, "value_tablet", $defaultValueTablet);
		$valueMobile = UELM_UniteFunctionsUC::getVal($param, "value_mobile",$defaultValueMobile);
	...
	private function getProcessedParamsValue_radioBoolean($data, $param, $value){
		...
		$valueTrue = UELM_UniteFunctionsUC::getVal($param, "true_value");

		if(empty($valueTrue))
			return($data);

		$value = UELM_UniteFunctionsUC::getVal($param, "false_value");
	...
	public function processFont($value, $arrFont, $isReturnCss = false, $cssSelector = null, $fontsKey=""){
		...
		if($isReturnCss == true){
			...
			self::$counter++;
			$spanClass = "uc-style-".self::$counter.UELM_UniteFunctionsUC::getRandomString(10, true);
			...
			switch($styleName){
				case "font-family":
					...
					if(empty($arrGoogleFonts)){
						$arrFontsPanelData = UELM_HelperUC::getFontPanelData();
						$arrGoogleFonts = $arrFontsPanelData["arrGoogleFonts"];
					}

					if(isset($arrGoogleFonts[$styleValue])){
						$urlGoogleFont = UELM_HelperHtmlUC::getGoogleFontUrl($arrGoogleFonts[$styleValue]);
						...
						if(!empty($this->addon)){
							...
							$handle = UELM_HelperUC::getUrlHandle($urlGoogleFont);
							UELM_HelperUC::addStyleAbsoluteUrl($urlGoogleFont, $handle);
						}
				...
				break;
				case "font-size":
					$arrStyle[$styleName] = UELM_UniteFunctionsUC::normalizeSize($styleValue);
				break;
				case "font-size-mobile":
				case "mobile-size":
				case "font-size-tablet":

					$styleValue = UELM_UniteFunctionsUC::normalizeSize($styleValue);
					...
					$cssMobileSize = UELM_HelperHtmlUC::wrapCssMobile($cssMobileSize, $isTablet);
				...
				default:
					UELM_UniteFunctionsUC::throwError("Wrong font style: $styleName");
				break;
			}
		}
		...
		if($isReturnCss == true){
			$css = UELM_UniteFunctionsUC::arrStyleToStrInlineCss($arrStyle, $addStyles, false);
		...
		if(!empty($arrStyle) || !empty($addStyles))
			$style = UELM_UniteFunctionsUC::arrStyleToStrInlineCss($arrStyle, $addStyles);
		...
		if($this->processType == self::PROCESS_TYPE_OUTPUT_BACK){
			...
			//UELM_UniteFunctionsUC::showTrace();exit();
		}
	...
	private function processFonts($arrValues, $type, $itemIndex=null){
		...
		switch($type){
			...
			break;
			default:
				UELM_UniteFunctionsUC::throwError("Wrong fonts type: $type");
			break;
		}
			...
			if(is_array($value)){
				...
				foreach($value as $itemIndex => $item){
					...
					foreach($item as $itemKey => $itemValue){
						...
						$arrFont = UELM_UniteFunctionsUC::getVal($arrFonts, $fontsKey);
	...
	public function isFontsPanelEnabled(){
		...
		foreach($arrParams as $param){
			...
			switch($type){
				...
				case UELM_UniteCreatorDialogParam::PARAM_FONT_OVERRIDE:
	...
	private function getParamsNamesForFonts($paramsType){
		...
		switch($paramsType){
			...
			break;
			default:
				UELM_UniteFunctionsUC::throwError("Wrong params type: $paramsType");
			break;
		}
		...
		foreach($arrParams as $param){
			...
			if($paramsType == "items"){
				...
				$title = esc_html__("Items", "unlimited-elements")." => ".$title;
			}

			$fontEditable = UELM_UniteFunctionsUC::getVal($param, "font_editable");
			$fontEditable = UELM_UniteFunctionsUC::strToBool($fontEditable);
	...
	public function getPostData($postID, $arrPostAdditions = null){
		uelm_dmp("getPostData: function for override");exit();
	}
	...
	private function getProcessedParamsValue_post($data, $value, $param, $processType){
		...
		$name = UELM_UniteFunctionsUC::getVal($param, "name");
		$arrPostAdditions = UELM_UniteFunctionsUC::getVal($param, "post_additions");
		...
		switch($processType){
			case self::PROCESS_TYPE_CONFIG:		//get additional post title
				...
				/*
				$postTitle = UELM_UniteProviderFunctionsUC::getPostTitleByID($postID);
				$data[$name] = $postID;
	...
	private function addImageAttributes_getImageKey($paramName, $name, $param, $data){
		...
		$chosenImageSize = UELM_UniteFunctionsUC::getVal($param, $paramName);
	...
	private function addImageAttributes($data, $name, $param){
		...
		$addImageSizes = UELM_UniteFunctionsUC::getVal($param, "add_image_sizes");
		$addImageSizes = UELM_UniteFunctionsUC::strToBool($addImageSizes);
		...
		$url = UELM_UniteFunctionsUC::getVal($data, $imageKey);
		$width = UELM_UniteFunctionsUC::getVal($data, $imageKey."_width");
		$height = UELM_UniteFunctionsUC::getVal($data, $imageKey."_height");
		...
		$alt = UELM_UniteFunctionsUC::getVal($data, "{$name}_alt");
		...
		if($addImageSizes == true){
			...
			$imageSize = UELM_UniteFunctionsUC::getVal($param, "value_size","full");
	...
	private function getImageJsonDefaultUrl($param){
		...
		$defaultValue = UELM_UniteFunctionsUC::getVal($param, "default_value_json");
	...
	protected function getProcessedParamsValue_image($data, $value, $param){
		...
		$name = UELM_UniteFunctionsUC::getVal($param, "name");
		$mediaType = UELM_UniteFunctionsUC::getVal($param, "media_type");
		...
		//sanitize the url
		if(!empty($imageUrl))
			$imageUrl = UELM_UniteFunctionsUC::sanitize($imageUrl, UELM_UniteFunctionsUC::SANITIZE_URL);
		
		$data[$name] = $imageUrl;

		$sizeFilters = UELM_UniteFunctionsUC::getVal($param, "size_filters");
		$data = $this->addOtherImageThumbs($data, $name, $imageId, $sizeFilters);

		$isNoImageData = UELM_UniteFunctionsUC::getVal($param, "no_image_data");
		$isNoImageData = UELM_UniteFunctionsUC::strToBool($isNoImageData);

		if($isNoImageData !== true)
			$data = $this->addOtherImageData($data, $name, $imageId);

		$isNoAttributes = UELM_UniteFunctionsUC::getVal($param, "no_attributes");
		$isNoAttributes = UELM_UniteFunctionsUC::strToBool($isNoAttributes);
	...
	protected function getProcessedParamsValue_file($data, $value, $param){
		...
		if(is_array($value) === true){
			$fileId = UELM_UniteFunctionsUC::getVal($value, "id");
			$fileUrl = UELM_UniteFunctionsUC::getVal($value, "url");
		}else{
			if(is_numeric($value) === true)
				$fileId = $value;
			else
		...
		if(empty($fileId) === false)
			...
			$fileUrl = UELM_HelperUC::URLtoFull($fileUrl);

		//sanitize the url
		if(!empty($fileUrl))
			$fileUrl = UELM_UniteFunctionsUC::sanitize($fileUrl, UELM_UniteFunctionsUC::SANITIZE_URL);
	...
	protected function getProcessedParamsValue_icon($data, $value, $param, $processType){
		...
		if(is_array($value) == true){
			...
			$library = UELM_UniteFunctionsUC::getVal($value, "library");

			if(isset($value["value"]))
				$value = UELM_UniteFunctionsUC::getVal($value, "value");

			if($library == "svg"){

				$value = UELM_UniteFunctionsUC::getVal($value, "url");	//in case of svg
				$isSVG = true;
				...
				$putAs = UELM_UniteFunctionsUC::getVal($param, "put_svg_as");

				if($putAs == "svg")
					$svgContent = UELM_HelperUC::getFileContentByUrl($value, "svg");
			}
		...
		$iconsType = UELM_UniteFunctionsUC::getVal($param, "icons_type");
		if(empty($iconsType) || $iconsType == "fa"){
			$value = UELM_UniteFontManagerUC::fa_convertIcon($value);
			$data[$name] = $value;
		}
	...
	public function getInstagramData($value, $name, $param){
		...
		try{
			...
			if(is_array($value)){
				...
				$valueMaxItems = UELM_UniteFunctionsUC::getVal($value, $name."_num_items");

				if(empty($valueMaxItems) && is_numeric($valueMaxItems) == false)
					$valueMaxItems = null;

				$value = UELM_UniteFunctionsUC::getVal($param, $name);
			}

			if(empty($value))
				$value = UELM_UniteCreatorSettingsWork::INSTAGRAM_DEFAULT_VALUE;

			$maxItems = UELM_UniteFunctionsUC::getVal($param, "max_items");
			...
			$data["error"] = __("Instagram Gallery Error: ","unlimited-elements").$message;
	...
	private function getGoogleMapOutput($value, $name, $param){
		...
		$objView = new UELM_UniteCreatorMappickerView();
	...
	private function getItemsVariablesProcessed($arrItem, $index, $numItems){
		...
		$objVarOutput = new UELM_UniteCreatorVariablesOutput();
		$objVarOutput->init($this->arrMainParamsValuesCache);

		foreach($arrVars as $var){
			$name = UELM_UniteFunctionsUC::getVal($var, "name");
			UELM_UniteFunctionsUC::validateNotEmpty($name, "variable name");
	...
	private function getMainVariablesProcessed($arrParams){
		...
		//get variables
		$objVariablesOutput = new UELM_UniteCreatorVariablesOutput();
	...
	protected function modifyParamsBySpecialAddonBehaviour($arrParams){
		...
		foreach($arrParams as $param){
			...
			$filterSource = UELM_UniteFunctionsUC::getVal($param, "value");
			if($filterSource === "")
				$filterSource = UELM_UniteFunctionsUC::getVal($param, "default_value");
		...
		foreach($arrParams as $param){
			...
			if($isAuthors == true){
				if($type == UELM_UniteCreatorDialogParam::PARAM_POST_TERMS || $name == "taxonomy")
					continue;
			}else{
				if($type == UELM_UniteCreatorDialogParam::PARAM_USERS || $name == "authors")
					continue;
			}
    ...
    protected function modifyDataBySpecialAddonBehaviour($data){
        ...
        $cssId = UELM_UniteFunctionsUC::getVal($data, 'advanced_css_id', '');
        if ($cssId !== '') {
	...
	public function processParamsForOutput($arrParams){
		...
		if(is_array($arrParams) == false)
			UELM_UniteFunctionsUC::throwError("objParams should be array");
	...
	private function getHtmlMenuItem($item, $showSubmenu = true, $htmlBase="ul", $param=array()){
		...
		$link = UELM_UniteFunctionsUC::getVal($item, "link");
		$title = UELM_UniteFunctionsUC::getVal($item, "title");
		$alias = UELM_UniteFunctionsUC::getVal($item, "alias");
		$isActive = UELM_UniteFunctionsUC::getVal($item, "active");
		$isCurrent = UELM_UniteFunctionsUC::getVal($item, "current");

		$arrSubmenu = UELM_UniteFunctionsUC::getVal($item, "submenu");

		$itemClass = UELM_UniteFunctionsUC::getVal($param, "menu_item_class");
		$itemClass = trim($itemClass);

		//get active class
		$activeClass = UELM_UniteFunctionsUC::getVal($param, "menu_item_active_class");
		$activeClass = trim($activeClass);
		if(empty($activeClass))
			$activeClass = "uc-menuitem-active";

		//get current class
		$currentClass = UELM_UniteFunctionsUC::getVal($param, "menu_item_current_class");
		...
		$wrapSubmenuItem =  UELM_UniteFunctionsUC::getVal($param, "menu_wrap_submenu_item");

		$isWrapSubmenu = false;
		if($wrapSubmenuItem == "wrap"){
			$isWrapSubmenu = true;
			$submenuWrapperClass = UELM_UniteFunctionsUC::getVal($param, "menu_submenu_wrapper_class");
		...
		if($showSubmenu == true && !empty($arrSubmenu)){
			...
			$submenuClass = UELM_UniteFunctionsUC::getVal($param, "menu_submenu_class");
	...
	private function getMenuData($value, $name, $param, $processType){
		...
		$messageEmpty = esc_html__("No menu selected", "unlimited-elements");
		...
		$htmlBase = UELM_UniteFunctionsUC::getVal($param, "menu_html_base","ul");
		$menuClass = UELM_UniteFunctionsUC::getVal($param,"menu_class");

		$menuType = UELM_UniteFunctionsUC::getVal($value, "{$name}_menutype");
		$showSubmenu = UELM_UniteFunctionsUC::getVal($value, "{$name}_show_submenu");
		$showSubmenu = UELM_UniteFunctionsUC::strToBool($showSubmenu);

		$arrItems = UELM_UniteProviderFunctionsUC::getMenuItems($menuType, $showSubmenu);

		if(empty($arrItems)){
			$message = esc_html__("No items in ","unlimited-elements").$menuType.esc_html__(" menu","unlimited-elements");
			return($message);
		}
	...
	private function getLinkData($data, $value, $name, $param, $processType){
		...
		$url = UELM_UniteFunctionsUC::sanitize($url, UELM_UniteFunctionsUC::SANITIZE_URL);
		
		$isExternal = UELM_UniteFunctionsUC::getVal($value, "is_external");
		$noFollow = UELM_UniteFunctionsUC::getVal($value, "nofollow");

		$customAttributes = UELM_UniteFunctionsUC::getVal($value, "custom_attributes");
	...
	protected function getSliderData_work($data, $value, $name, $valueOnly = false){
		...
		$unit = UELM_UniteFunctionsUC::getVal($value, "unit");
	...
	protected function getDateTimeData($data, $value, $name, $param, $processType){
		...
		if(is_numeric($value) && UELM_UniteFunctionsUC::detectDateFormat($value) == ""){
		...
		$stamp = UELM_UniteFunctionsUC::date2Timestamp($value);
	...
	protected function outputHoverAnimationsStyles($value, $name, $param, $processType){
		...
		if(strpos($value, UELM_GlobalsUnlimitedElements::PREFIX_ANIMATION_CLASS) === 0)
			UELM_HelperUC::includeUEAnimationStyles();
		else
			UELM_HelperProviderCoreUC_EL::includeHoverAnimationsStyles($value);
	}
	...
	private function getSpecialParamsData($data, $value, $name, $param, $processType){
		...
		switch($type){
			case "dynamic_popup":
				...
				$arrValues = UELM_UniteFunctionsUC::getVal($data, $name);

				$linkType = UELM_UniteFunctionsUC::getVal($arrValues, $name."_link_type");
				...
				if($linkType == "meta"){
					...
					$linkTypeMetaField = UELM_UniteFunctionsUC::getVal($arrValues, $name."_meta_field");
			...
			case "ucform_conditions":
				...
				$objForm = new UELM_UniteCreatorForm();
			...
			case "weather_api":
			case "reviews":
				$data = UELM_UniteCreatorAPIIntegrations::getInstance()->addDataToParams($data, $name, $type);
            break;
            case "rss_feed":
                
            	$arrValues = UELM_UniteFunctionsUC::getVal($param, "value");
            	            	
            	$objRss = new UELM_UniteCreatorRSS();
            	
            	$addonValues = $this->addon->getOriginalValues();
				
            	$showDebug = UELM_UniteFunctionsUC::getVal($addonValues,"show_debug");
            ...
            case "repeater":
            	...
            	$debugData = UELM_UniteFunctionsUC::getVal($arrValues, "{$name}_repeater_debug_data");
            	$debugData = UELM_UniteFunctionsUC::strToBool($debugData);
            	            	
            	$debugMeta = UELM_UniteFunctionsUC::getVal($arrValues, "{$name}_repeater_debug_meta");
            	$debugMeta = UELM_UniteFunctionsUC::strToBool($debugMeta);
            	
            	$arrRepeaterItems = UELM_HelperProviderUC::getRepeaterItems($arrValues, $name, $debugData, $debugMeta);
	...
	protected function maybeSanitizeLink($data, $name, $value){
		...
		//if it's a link
		$value = UELM_UniteFunctionsUC::sanitize($value, UELM_UniteFunctionsUC::SANITIZE_URL);
	...
	public function sortParamsBeforeProcess($param1, $param2){
		...
		$type1 = UELM_UniteFunctionsUC::getVal($param1, "type");
		$type2 = UELM_UniteFunctionsUC::getVal($param2, "type");

		if($type1 == UELM_UniteCreatorDialogParam::PARAM_SPECIAL)
			return(-1);

		if($type2 == UELM_UniteCreatorDialogParam::PARAM_SPECIAL)
			return(1);
	...
	public function getProcessedMainParamsValues($processType){
		...
		if($this->isOutputProcessType($processType) === true){
			$arrParams = UELM_UniteProviderFunctionsUC::applyFilters(UELM_UniteCreatorFilters::FILTER_MODIFY_ADDON_OUTPUT_PARAMS, $arrParams, $this->addon);
	...
	public function getProcessedItemsData_getImageSize($processType = null){
		...
		$paramsSpecial = $this->addon->getParams(UELM_UniteCreatorDialogParam::PARAM_SPECIAL);
		...
		foreach($paramsSpecial as $param){
			...
			if(is_array($value)){
				$value = UELM_UniteFunctionsUC::getVal($value, $name."_size");
			}

			$destParamName = UELM_UniteFunctionsUC::getVal($param, "image_size_param_name");
	...
	public function getProcessedItemsData_modifyImageItem($arrItemParams, $arrImageSizes){
		...
		$defaultSize = UELM_UniteFunctionsUC::getVal($arrImageSizes, "_default_");

		foreach($arrItemParams as $index => $param){

			$type = UELM_UniteFunctionsUC::getVal($param, "type");

			if($type != UELM_UniteCreatorDialogParam::PARAM_IMAGE)
				continue;

			$name = UELM_UniteFunctionsUC::getVal($param, "name");

			$size = UELM_UniteFunctionsUC::getVal($arrImageSizes, $name);
	...
	public function getProcessedItemsData($arrItems, $processType, $forTemplate = true, $filterType = null){
		...
		if($specialType === UELM_UniteCreatorAddon::ITEMS_TYPE_IMAGE)
			return $arrItems;
		...
		foreach($arrItems as $index => $arrItemValues){
			...
			$elementorID = UELM_UniteFunctionsUC::getVal($arrItemValues, "_id");

			// get gutenberg item id
			if(empty($elementorID))
				$elementorID = UELM_UniteFunctionsUC::getVal($arrItemValues, "_generated_id");

			//if not found - generate one
			if(empty($elementorID))
				$elementorID = UELM_UniteFunctionsUC::getRandomString(5);
			...
			switch($itemsType){
				case UELM_UniteCreatorAddon::ITEMS_TYPE_IMAGE:
					...
					$urlThumb = UELM_UniteFunctionsUC::getVal($item, "image_thumb_large");

					if(empty($urlThumb))
						$urlThumb = $urlImage;

					$item["thumb"] = $urlThumb;
					$item["image_id"] = UELM_UniteFunctionsUC::getVal($arrItemValues, "image_id");
					$item["raw_caption"] = UELM_UniteFunctionsUC::getVal($arrItemValues, "raw_caption");
					$item["raw_title"] = UELM_UniteFunctionsUC::getVal($arrItemValues, "raw_title");
				break;
			}
