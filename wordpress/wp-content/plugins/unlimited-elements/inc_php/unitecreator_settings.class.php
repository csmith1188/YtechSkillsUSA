...
class UELM_UniteCreatorSettingsWork extends UELM_UniteSettingsAdvancedUC{
	...
	public function getSettingsCreatorFormat(){
		...
		foreach($this->arrSettings as $setting){
			...
			$settingName = UELM_UniteFunctionsUC::getVal($setting, "name");

			$param = array();
			$origType = UELM_UniteFunctionsUC::getVal($setting, "origtype");
			$function = UELM_UniteFunctionsUC::getVal($setting, "function");

			UELM_UniteFunctionsUC::validateNotEmpty($origType, "settings original type for: $settingName");

			$param["type"] = $origType;
			$param["title"] = UELM_UniteFunctionsUC::getVal($setting, "text");
			$param["name"] = UELM_UniteFunctionsUC::getVal($setting, "name");
			$param["description"] = UELM_UniteFunctionsUC::getVal($setting, "description");
			$param["default_value"] = UELM_UniteFunctionsUC::getVal($setting, "default_value");
			$param["placeholder"] = UELM_UniteFunctionsUC::getVal($setting, "placeholder");
			...
			foreach($arrKeys as $key){
				...
				$value = UELM_UniteFunctionsUC::getVal($setting, $key);
			...
			$classAdd = UELM_UniteFunctionsUC::getVal($setting, UELM_UniteSettingsUC::PARAM_CLASSADD);
			if(!empty($classAdd))
				$param[UELM_UniteSettingsUC::PARAM_CLASSADD] = $classAdd;

			$addParams = UELM_UniteFunctionsUC::getVal($setting, UELM_UniteSettingsUC::PARAM_ADDPARAMS);
			if(!empty($addParams))
				$param[UELM_UniteSettingsUC::PARAM_ADDPARAMS] = $addParams;

			$isMultiple = UELM_UniteFunctionsUC::getVal($setting, "is_multiple");	//for dropdown
			if(!empty($isMultiple))
				$param["is_multiple"] = true;

			$elementorCondition = UELM_UniteFunctionsUC::getVal($setting, "elementor_condition");	//for dropdown
			if(!empty($elementorCondition))
				$param["elementor_condition"] = $elementorCondition;

			$addDynamic = UELM_UniteFunctionsUC::getVal($setting, "add_dynamic");
			$addDynamic = UELM_UniteFunctionsUC::strToBool($addDynamic);

			if($addDynamic)
				$param["add_dynamic"] = true;


			$labelBlock = UELM_UniteFunctionsUC::getVal($setting, "label_block");	//label block
			if(!empty($labelBlock))
	...
	*/
	public function getMultipleCreatorParams($param){
		...
		if(!empty($this->arrSettings))
			UELM_UniteFunctionsUC::throwError("the settings should be empty for this operation");
	...
	public function getArrUCSettingTypes(){
		...
		$arrTypes = array(
			"uc_textfield",
			UELM_UniteCreatorDialogParam::PARAM_NUMBER,
			"uc_textarea",
			"uc_editor",
			UELM_UniteCreatorDialogParam::PARAM_RADIOBOOLEAN,
			"uc_checkbox",
			"uc_dropdown",
			"uc_colorpicker",
			"uc_image",
			UELM_UniteCreatorDialogParam::PARAM_FILE,
			"uc_mp3",
			"uc_icon",
			UELM_UniteCreatorDialogParam::PARAM_ICON_LIBRARY,
			UELM_UniteCreatorDialogParam::PARAM_SHAPE,
			UELM_UniteCreatorDialogParam::PARAM_HR,
			UELM_UniteCreatorDialogParam::PARAM_HEADING,
			"uc_font_override",
			UELM_UniteCreatorDialogParam::PARAM_POST,
			UELM_UniteCreatorDialogParam::PARAM_POSTS_LIST,
			"uc_statictext",
			UELM_UniteCreatorDialogParam::PARAM_MENU
		);
	...
	protected function addSettingsProvider($type, $name,$value,$title,$extra ){
		...
		uelm_dmp("function for override: addSettingsProvider ");
		exit();
	...
	protected function addPostTermsPicker($name,$value,$title,$extra){
		...
		uelm_dmp("addPostsListPicker - function for override");
		exit();
	}
	...
	protected function addListingPicker($name,$value,$title,$extra){
		...
		uelm_dmp("addListingPicker - function for override");
		exit();
	}
	...
	protected function addWooCatsPicker($name,$value,$title,$extra){
		...
		uelm_dmp("addWooCatsPicker - function for override");
		exit();
	}
	...
	protected function addUsersPicker($name,$value,$title,$extra){
		...
		uelm_dmp("addUsersPicker - function for override");
		exit();
	}
	...
	protected function addTemplatePicker($name, $value, $title, $extra){
		...
		uelm_dmp("addTemplatePicker - function for override");
		exit();
	}
	...
	protected function addBackgroundSettings($name, $value, $title, $param, $extra){
		...
		uelm_dmp("addBackgroundSettings - function for override");
		exit();
	}
	...
	protected function addMenuPicker($name, $value, $title, $extra){
		...
		uelm_dmp("addMenuPicker - function for override");
		exit();
	}
	...
	protected function addInstagramSelector($name,$value,$title,$extra){
		...
		$defaultMaxItems = UELM_UniteFunctionsUC::getVal($extra, "max_items");
		if(is_numeric($defaultMaxItems) == false || $defaultMaxItems < 1)
			$defaultMaxItems = 12;

		$objServies = new UELM_UniteServicesUC();
		$objServies->includeInstagramAPI();

		$accessData = UELM_HelperInstaUC::getInstagramSavedAccessData();
		$accessToken = UELM_UniteFunctionsUC::getVal($accessData, "access_token");
		$username = UELM_UniteFunctionsUC::getVal($accessData, "username");
		...
		if(!empty($accessToken)){
			...
			$text = __("Get data from instagram user: ", "unlimited-elements");
			...
			$linkSettings = UELM_HelperUC::getViewUrl(UELM_GlobalsUnlimitedElements::VIEW_SETTINGS_ELEMENTOR);
			$htmlLink = UELM_HelperHtmlUC::getHtmlLink($linkSettings, __("General Settings","unlimited-elements"),"","",true);

			/*
			$text = __("Please connect to your instagram account from ", "unlimited-elements");
			$text .= $htmlLink;

			$this->addStaticText($text, $name, $params);
			*/

			$description = esc_html__("Type instagram user (@username). This method is deprecated. Please connect with your instagram user from general settings", "unlimited-elements");
			...
			$this->addTextBox($name, $value ,esc_html__("Instagram User", "unlimited-elements"), $params);
		...
		$desciption = __("Number of instagram items. Leave empty for default number ( %d ) set by the widget", "unlimited-elements");
		$desciption = sprintf($desciption, $defaultMaxItems);
		$params["description"] = $desciption;

		$this->addTextBox($name."_num_items", $defaultMaxItems ,esc_html__("Number Of Items", "unlimited-elements"), $params);
	...
	public function addItemsPanelRepeater($addon, $source){
		...
		if($itemsType == UELM_UniteCreatorAddon::ITEMS_TYPE_IMAGE){
	...
	public function addItemsPanel($addon, $source = null){
		...
		$objManager = new UELM_UniteCreatorManagerInline();
	...
	public function addTabIfNeeded($name){
		...
		if(empty($tabs) === true){
			$tabs = array(
				"name" => "tabs_" . UELM_UniteFunctionsUC::getRandomString(5),
				"items" => array(),
			);
		}

		// add/update the tab
		$value = UELM_UniteFunctionsUC::getVal($tabs["items"], $name);
		if(empty($value) === true){

			$value = "tab_" . UELM_UniteFunctionsUC::getRandomString(5);
	 ...
	 * set current addon
	 */
	public function setCurrentAddon(UELM_UniteCreatorAddon $addon){
	...
	private function checkParamsSource($arrParams){
		...
		$source = UELM_UniteFunctionsUC::getVal($arrParams, "source");

		if($source == "addon"){

			if(empty($this->currentAddon))
				UELM_UniteFunctionsUC::throwError("You must set current addon before init settings for addon related image select option");
	...
	public function addImageBaseSettings(){
		...
		$this->addTextArea("description", "", esc_html__("Description", "unlimited-elements"),$extra);

		/*
		$extra = array("origtype"=>"uc_radioboolean");
		$this->addRadioBoolean("enable_link", esc_html__("Enable Link", "unlimited-elements"),false, "Yes","No",$extra);

		$extra = array("class"=>"unite-input-link", "origtype"=>"uc_textfield");
		$this->addTextBox("link", "", esc_html__("Link", "unlimited-elements"),$extra);
		*/
	...
	private function addItemsImageSizeSetting($name, $param){
		...
		$title = UELM_UniteFunctionsUC::getVal($param, "title");
	...
	private function addSpecialParam($name, $param){
		...
		$condition = UELM_HelperProviderCoreUC_EL::paramToElementorCondition($param);
		...
		switch($attributeType){
			...
			case "entrance_animation":
				...
				UELM_UniteCreatorEntranceAnimations::addSettings($this, $name, $param);
			...
			case "dynamic_popup":
				...
				$arrOptions = array();
				$arrOptions["post"] = __("Post Link","unlimited-elements");
				$arrOptions["popup"] = __("Dynamic Post Popup","unlimited-elements");
				$arrOptions["empty"] = __("Disable Link","unlimited-elements");
				$arrOptions["meta"] = __("Link From Meta Field","unlimited-elements");
				...
				$text = __("This option works with \"Dynamic Post Popup\" widget. Please put it to the page.", "unlimited-elements");
				...
				$this->addTextBox($name."_meta_field","", __("Meta Field Name","unlimited-elements"), $params);

			break;
			case "contact_form7":

				//add not exists heading.
				$isInstalled = UELM_UniteCreatorPluginIntegrations::isContactFrom7Installed();
				...
				if($isInstalled == false){
					...
					$text = __("The Contact Form 7 Plugin is not installed", "unlimited-elements");
				...
				$arrForms = UELM_UniteCreatorPluginIntegrations::getArrContactForm7();

				$default = UELM_UniteFunctionsUC::getArrFirstValue($arrForms);
			...
			case "ucform_conditions":
				...
				$params = array();
				$params["elementor_condition"] = $condition;
				$params["origtype"] = UELM_UniteCreatorDialogParam::PARAM_REPEATER;
				$params["hide_label"] = true;
				$params["title_field"] = "{{{operator}}} {{{field_name}}} {{{condition}}} {{{field_value}}}";

				$settingsItems = UELM_UniteCreatorForm::getConditionsRepeaterSettings();
			...
			case "sort_filter_fields":
				...
				$description = UELM_UniteFunctionsUC::getVal($param, "description");

				if(!empty($description))
					$params["description"] = $description;

				$settingsItems = UELM_HelperProviderUC::getSortFilterRepeaterFields();
				
				$settingsValues = UELM_HelperProviderUC::getSortFilterDefaultValues();
			...
			break;
			case "currency_api":
				UELM_UniteCreatorAPIIntegrations::getInstance()->addServiceSettingsFields($this, UELM_UniteCreatorAPIIntegrations::TYPE_CURRENCY_EXCHANGE, $name, $condition);
			break;
			case "weather_api":
				UELM_UniteCreatorAPIIntegrations::getInstance()->addServiceSettingsFields($this, UELM_UniteCreatorAPIIntegrations::TYPE_WEATHER_FORECAST, $name, $condition);
			break;
            case "rss_feed":
                
            	$objRss = new UELM_UniteCreatorRSS();
            	
            	$fields = $objRss->getRssFields($name);
                
                UELM_HelperProviderUC::addSettingsFields($this, $fields, $name, $condition);
            break;
            case "repeater":
            	
            	UELM_HelperProviderUC::addRepeaterSettings($this, $name,"",true, true);
			...
			case "reviews":
				...
				UELM_UniteCreatorAPIIntegrations::getInstance()->addServiceSettingsFields($this, UELM_UniteCreatorAPIIntegrations::TYPE_GOOGLE_REVIEWS, $name, $condition);
				
			break;
			default:
				UELM_UniteFunctionsUC::throwError("Add special param error: wrong attribute type: $attributeType, please check that the plugin version is up to date");
			break;
		}
	...
	protected function modifyBeforeAdd_categories($setting, $addonType, $addNew = false){
		...
		if($addNew == true)
			$arrCats["new"] = "[". esc_html__("New Category", "unlimited-elements")."]";

		$fistValue = UELM_UniteFunctionsUC::getFirstNotEmptyKey($arrCats);
	...
	protected function modifyBeforeAdd($setting, $modifyType){
		...
		switch($modifyType){
			...
			case "library_sections_categories":
				...
				$setting = $this->modifyBeforeAdd_categories($setting, UELM_GlobalsUC::ADDON_TYPE_LAYOUT_SECTION, true);
	...
	public function addByCreatorParam_handleConditions($param, $isForSap = false){
		...
		$attribute2 = UELM_UniteFunctionsUC::getVal($param, "condition_attribute2");
		$operator2 = UELM_UniteFunctionsUC::getVal($param, "condition_operator2");
		$value2 = UELM_UniteFunctionsUC::getVal($param, "condition_value2");
    ...
    public function addControl_byElementorConditions($nameChild, $arrConditions){
    	...
    	if(is_array($arrConditions) == false)
    		UELM_UniteFunctionsUC::throwError("The elementor conditions should be array");
	...
	private function addControls_byElementorConditions(){
		...
		foreach($this->arrSettings as $setting){
			...
			$elementorCondition	 = UELM_UniteFunctionsUC::getVal($setting, "elementor_condition");
	...
	private function addFreeVersionInsideNotification(){
    	...
    	$text = UELM_GlobalsUnlimitedElements::$insideNotificationText;
		
    	$urlBuy = UELM_GlobalsUnlimitedElements::$insideNotificationUrl;
    	
		$text = str_replace( "[url_buy]", $urlBuy, $text );
	    
		$html = "<div class='uc-settings-cta-notification'>{$text}</div>";
		
		UELM_GlobalsUnlimitedElements::$insideNotificationText;
	...
	private function checkAddImageSizes($paramImage){
		...
		$isAddSizes = UELM_UniteFunctionsUC::getVal($paramImage, "add_image_sizes");
    	...
    	if($type == UELM_UniteCreatorDialogParam::PARAM_POSTS_LIST){
	    	$paramTitle = $title .= " ".__("Image Size","unlimited-elements");
	    	$paramName = $name .= "_imagesize";
    	}else{
	    	$paramTitle = $title .= " ".__("Size","unlimited-elements");
    	...
    	$newParam["type"] = UELM_UniteCreatorDialogParam::PARAM_DROPDOWN;
	...
	public function addByCreatorParam($param, $inputValue = null){
		...
		$arrReadySetting = UELM_UniteFunctionsUC::getVal($param, "uc_setting");

		if(!empty($arrReadySetting)){
			$classAdd = UELM_UniteFunctionsUC::getVal($arrReadySetting, UELM_UniteSettingsUC::PARAM_CLASSADD);

			$arrReadySetting[UELM_UniteSettingsUC::PARAM_CLASSADD] = $classAdd;
		...
		$value = UELM_UniteFunctionsUC::getVal($param, "value", $defaultValue);
		$type = UELM_UniteFunctionsUC::getVal($param, "type");
		$title = UELM_UniteFunctionsUC::getVal($param, "title");
		$name = UELM_UniteFunctionsUC::getVal($param, "name");
		$description = UELM_UniteFunctionsUC::getVal($param, "description");
		$placeholder = UELM_UniteFunctionsUC::getVal($param, "placeholder");
		$tabName = UELM_UniteFunctionsUC::getVal($param, "tabname");
		$labelBlock = UELM_UniteFunctionsUC::getVal($param, "label_block");
		$labelBlock = UELM_UniteFunctionsUC::strToBool($labelBlock);
		$isResponsive = UELM_UniteFunctionsUC::getVal($param, "is_responsive");
		$isResponsive = UELM_UniteFunctionsUC::strToBool($isResponsive);
		$unit = UELM_UniteFunctionsUC::getVal($param, "unit");

		if($unit == "other")
			$unit = UELM_UniteFunctionsUC::getVal($param, "unit_custom");
		...
		foreach($this->arrAddAttributes as $attributeName){
			$attributeValue = UELM_UniteFunctionsUC::getVal($param, $attributeName);
		...
		switch ($type){
			case UELM_UniteCreatorDialogParam::PARAM_EDITOR:
			...
			break;
			case UELM_UniteCreatorDialogParam::PARAM_LINK:
				$this->addLink($name, $value, $title, $extra);
			break;
			case UELM_UniteCreatorDialogParam::PARAM_NUMBER:
				$extra["class"] = UELM_UniteCreatorSettingsOutput::INPUT_CLASS_NUMBER;

				$values = array(
					"desktop" => $value,
					"tablet" => UELM_UniteFunctionsUC::getVal($param, "default_value_tablet"),
					"mobile" => UELM_UniteFunctionsUC::getVal($param, "default_value_mobile"),
				);
				...
				foreach($responsive as $device => $suffix){
					$value = UELM_UniteFunctionsUC::getVal($values, $device);
			...
			break;
			case UELM_UniteCreatorDialogParam::PARAM_CHECKBOX:
				$textNear = UELM_UniteFunctionsUC::getVal($param, "text_near");
				$isChecked = UELM_UniteFunctionsUC::getVal($param, "is_checked");
				$isChecked = UELM_UniteFunctionsUC::strToBool($isChecked);
			...
			break;
			case UELM_UniteCreatorDialogParam::PARAM_MULTIPLE_SELECT:
				$options = UELM_UniteFunctionsUC::getVal($param, "options");

				$this->addMultiSelect($name, $options, $title, $value, $extra);
			break;
			case UELM_UniteCreatorDialogParam::PARAM_TERM_SELECT:
				$extra["post_select"] = true;
				$extra["post_select_type"] = "term";

				$this->addMultiSelect($name, array(), $title, $value, $extra);
			break;
			case UELM_UniteCreatorDialogParam::PARAM_POST_SELECT:
				$extra["post_select"] = true;
				$extra["post_select_type"] = "post";

				$this->addMultiSelect($name, array(), $title, $value, $extra);
			break;
			case UELM_UniteCreatorDialogParam::PARAM_COLORPICKER:
				$this->addColorPicker($name, $value, $title, $extra);
			break;
			case UELM_UniteCreatorDialogParam::PARAM_ADDONPICKER:
				$extra["addontype"] = UELM_UniteFunctionsUC::getVal($param, "addon_type");
			...
			case "uc_statictext":
				...
				$isUpdateValue = false;
			break;
			case UELM_UniteCreatorDialogParam::PARAM_GALLERY:
				$this->addGallery($name, $defaultValue, $title, $extra);
			break;
			case UELM_UniteCreatorDialogParam::PARAM_ICON:
				$this->addIconPicker($name,$value,$title,$extra);
			break;
			case UELM_UniteCreatorDialogParam::PARAM_ICON_LIBRARY:
				$extra["enable_svg"] = UELM_UniteFunctionsUC::getVal($param, "enable_svg");

				$this->addIconPicker($name,$value,$title,$extra);
			break;
			case UELM_UniteCreatorDialogParam::PARAM_SHAPE:
				$this->addShapePicker($name,$value,$title,$extra);
			break;
			case UELM_UniteCreatorDialogParam::PARAM_MAP:
			...
			break;
			case UELM_UniteCreatorDialogParam::PARAM_INSTAGRAM:
				$extra["max_items"] = UELM_UniteFunctionsUC::getVal($param, "max_items");

				$this->addInstagramSelector($name, $value, $title, $extra);
			break;
			case UELM_UniteCreatorDialogParam::PARAM_POST:
				$this->addPostPicker($name,$value,$title,$extra);
			break;
			case UELM_UniteCreatorDialogParam::PARAM_POSTS_LIST:
				$extra["for_woocommerce_products"] = UELM_UniteFunctionsUC::getVal($param, "for_woocommerce_products");
				$extra["default_max_posts"] = UELM_UniteFunctionsUC::getVal($param, "default_max_posts");

				$this->addPostsListPicker($name,$value,$title,$extra);
			break;
			case UELM_UniteCreatorDialogParam::PARAM_POST_TERMS:
				$extra["for_woocommerce"] = UELM_UniteFunctionsUC::getVal($param, "for_woocommerce");
				$extra["filter_type"] = UELM_UniteFunctionsUC::getVal($param, "filter_type");
			...
			break;
			case UELM_UniteCreatorDialogParam::PARAM_DATASET:
			...
			case UELM_UniteCreatorDialogParam::PARAM_MENU:
				$useFor = UELM_UniteFunctionsUC::getVal($param, "usefor");
			...
			case UELM_UniteCreatorDialogParam::PARAM_BORDER:
				...
				$typeParams = array_merge($extra, array(
					"selector" => $selector,
					"selector_value" => UELM_HelperHtmlUC::getCSSSelectorValueByParam($type, "style"),
				));
				...
				$widthParams = array_merge($extra, array(
					...
					"selector" => $selector,
					"selector_value" => UELM_HelperHtmlUC::getCSSSelectorValueByParam($type, "width"),
				...
				foreach($responsive as $device => $suffix){
					...
					$addValue["unit"] = UELM_UniteFunctionsUC::getVal($param, "width_{$device}_unit");
				...
				$colorParams = array_merge($extra, array(
					"elementor_condition" => $typeCondition,
					"selector" => $selector,
					"selector_value" => UELM_HelperHtmlUC::getCSSSelectorValueByParam($type, "color"),
				));
			...
			case UELM_UniteCreatorDialogParam::PARAM_BORDER_DIMENTIONS:
				$extra["selector_value"] = UELM_HelperHtmlUC::getCSSSelectorValueByParam($type);

				$withoutUnits = UELM_UniteFunctionsUC::getVal($param, "no_units");
				$withoutUnits = UELM_UniteFunctionsUC::strToBool($withoutUnits);
				...
				foreach($responsive as $device => $suffix){
					$addValue = array();
					$addValue["top"] = UELM_UniteFunctionsUC::getVal($param, "{$device}_top");
					$addValue["bottom"] = UELM_UniteFunctionsUC::getVal($param, "{$device}_bottom");
					$addValue["left"] = UELM_UniteFunctionsUC::getVal($param, "{$device}_left");
					$addValue["right"] = UELM_UniteFunctionsUC::getVal($param, "{$device}_right");
					$addValue["unit"] = UELM_UniteFunctionsUC::getVal($param, "{$device}_unit");
					$addValue["is_linked"] = UELM_UniteFunctionsUC::getVal($param, "{$device}_is_linked", true);
			...
			case UELM_UniteCreatorDialogParam::PARAM_SLIDER:
				$extra["min"] = UELM_UniteFunctionsUC::getVal($param, "min");
				$extra["max"] = UELM_UniteFunctionsUC::getVal($param, "max");
				$extra["step"] = UELM_UniteFunctionsUC::getVal($param, "step");
				$extra["units"] = UELM_UniteFunctionsUC::getVal($param, "units");
				...
				foreach($responsive as $device => $suffix){
					$value = UELM_UniteFunctionsUC::getVal($param, "default_value{$suffix}");
			...
			break;
			case UELM_UniteCreatorDialogParam::PARAM_HOVER_ANIMATIONS:
				$animations = UELM_HelperProviderCoreUC_EL::getHoverAnimations();
				$prefix = UELM_GlobalsUnlimitedElements::PREFIX_ANIMATION_CLASS;
				$items = array("" => __("None", "unlimited-elements"));

				foreach($animations as $key => $value){
					$items[$prefix . $key] = $value;
				}

				$value = UELM_UniteFunctionsUC::getVal($param, "default_value");
			...
			break;
			case UELM_UniteCreatorDialogParam::PARAM_DATETIME:
			...
			default:
				...
				if($isAdded == false)
					UELM_UniteFunctionsUC::throwError("initByCreatorParams error: Wrong setting type: $type");
			break;
		}
    ...
    private function sortParamsByCats($arrCats, $params){
    	...
    	foreach($params as $param){
    		...
    		unset($param[UELM_GlobalsUC::ATTR_CATID]);
    		...
    		if(array_key_exists($catID, $arrOutput) == false){
    			...
    			//set category title
    			$catTitle = __("General", "unlimited-elements");
				
    			if($catID != "cat_general_general"){
    				$sectionCounter++;
    				$catTitle = __("Section ","unlimited-elements") . $sectionCounter;
    			}
	...
	private function addEditWidgetButton(){
    	...
    	if(class_exists("UELM_UniteProviderAdminUC") == false)
    		return(false);

    	if(UELM_UniteProviderAdminUC::$isUserHasCapability == false)
    		return(false);
		
    	$addonID =  $this->currentAddon->getID();
		
    	$urlEditAddon = UELM_HelperUC::getViewUrl_EditAddon($addonID, "", "tab=uc_tablink_html");
    	...
    	$this->addButton("html_button_gotoaddon", __("Edit Block HTML","unlimited-elements"), self::PARAM_NOTEXT, $arrParams);
	...
	public function addAdvancedSection(){
		...
		$this->addSap(esc_html__("Advanced", "unlimited-elements"), "advanced");

		$params = array();
		$params["description"] = __("Show widget data for debugging purposes. Please turn off this option when you releasing the widget.", "unlimited-elements");

		$this->addRadioBoolean("show_widget_debug_data", __("Show Widget Data For Debug", "unlimited-elements"), false, "Yes", "No", $params);

		$isItemsEnabled = $this->currentAddon->isHasItems();
		$hasPostsList = $this->currentAddon->isParamTypeExists(UELM_UniteCreatorDialogParam::PARAM_POSTS_LIST);

		//--------- debug type options ---------
		$debugTypeOptions = array();
		$debugTypeOptions["default"] = __('Default', 'unlimited-elements');

		if($hasPostsList == true)
			$isItemsEnabled = true;

		if($isItemsEnabled == true)
			$debugTypeOptions["items_only"] = __('Items Only', 'unlimited-elements');

		if($hasPostsList == true){
			$debugTypeOptions["post_titles"] = __('Posts Titles', 'unlimited-elements');
			$debugTypeOptions["post_meta"] = __('Posts Titles and Meta', 'unlimited-elements');
		}

		$debugTypeOptions["current_post_data"] = __('Current Post Data', 'unlimited-elements');
		$debugTypeOptions["settings_values"] = __('Show Settings Values', 'unlimited-elements');
		...
		if($hasDebugType == true){
			...
			$this->addSelect("widget_debug_data_type", $debugTypeOptions,
				__("Debug Data Type", "unlimited-elements"), "default", $params);
		}

		$this->addControl("show_widget_debug_data", "widget_debug_data_type", "show", "true");
		
		if(self::$addEditWidgetHTML === null){
				self::$addEditWidgetHTML = UELM_HelperProviderCoreUC_EL::getGeneralSetting("show_edit_html_button");
				self::$addEditWidgetHTML = UELM_UniteFunctionsUC::strToBool(self::$addEditWidgetHTML);
		}
	...
	public function addGutenbergEditorBackgroundSection(){
		...
		$this->addSap(esc_html__("Background", "unlimited-elements"), "uc_background");
		
		$params = array();
		$params["description"] = __("Choose the background location.", "unlimited-elements");

		$arrItems = array(
			'back'  => __( 'In Background', 'unlimited-elements' ),
			'front' => __( 'In Foregroud', 'unlimited-elements' )
			/*
			'body' => __( 'Site Body Background', 'unlimited-elements' ),
			'body_front' => __( 'Site Body Foreground', 'unlimited-elements' ),
			'layout' => __( 'Layout Background', 'unlimited-elements' ),
			'layout_front' => __( 'Layout Foreground', 'unlimited-elements' )
			*/
		);
		
		$arrItems = array_flip($arrItems);
		
		$this->addSelect("background_location", $arrItems, __( 'Background Location', 'unlimited-elements' ), "back", $params);
	...
	private function addImageSizesParam($paramImage){
    	...
    	$param = UELM_HelperProviderUC::getImageSizesParamFromPostListParam($paramImage);
	...
	private function markGeneralTab($arrParamsWithCats){
		...
		foreach($arrParamsWithCats as $catID => &$cat){
			...
			$tab = UELM_UniteFunctionsUC::getVal($cat, "tab");
	...
	public function initByCreatorParams($arrParams, $arrCats = array()){
		...
		foreach($arrParamsWithCats as $catID => $arrCat){
			...
			$tab = UELM_UniteFunctionsUC::getVal($arrCat, "tab");
			$arrParams = UELM_UniteFunctionsUC::getVal($arrCat, "params");
			
			$isGeneralCategory = UELM_UniteFunctionsUC::getVal($arrCat, "is_general");
			...
			foreach($arrParams as $param){
				...
				switch($type) {
					...
					case UELM_UniteCreatorDialogParam::PARAM_POSTS_LIST:
						...
						$showImageSizes = UELM_UniteFunctionsUC::getVal($postsListParam, "show_image_sizes");
			...
			if($isGeneralCategory == true && UELM_GlobalsUC::$isProVersion == false){
		...
		if($addPagination == true){
			...
			if(!empty($postsListParam))
				$this->addPaginationAndFilteringSection($postsListParam);
	          	...
	          	$enablePagination = UELM_UniteFunctionsUC::getVal($listingParam, "enable_pagination");
	          	$enablePagination = UELM_UniteFunctionsUC::strToBool($enablePagination);
	
	          	$enableFiltering = UELM_UniteFunctionsUC::getVal($listingParam, "enable_ajax");
	          	$enableFiltering = UELM_UniteFunctionsUC::strToBool($enableFiltering);
		...
		if(UELM_GlobalsUnlimitedElements::$enableSchema == true){
	...
	private function addPostsQuerySection($param){
		...
		$forWooCommerce = UELM_UniteFunctionsUC::getVal($param, "for_woocommerce_products");
		$forWooCommerce = UELM_UniteFunctionsUC::strToBool($forWooCommerce);

		if($forWooCommerce === true)
			$label = esc_html__("Products Query", "unlimited-elements");
		else
			$label = esc_html__("Posts Query", "unlimited-elements");
	...
	private function addMultisourceSections($listingParam){
		...
		$name = UELM_UniteFunctionsUC::getVal($listingParam, "name");
		$useFor = UELM_UniteFunctionsUC::getVal($listingParam, "use_for");
		...
		if($isForGallery === true)
			$textPrefix = __("Gallery ", "unlimited-elements");

		//------ posts -------------
		$label = $textPrefix . __("Posts Query", "unlimited-elements");
		...
		$postParam["type"] = UELM_UniteCreatorDialogParam::PARAM_POSTS_LIST;
		...
		if($isForItems === true){
			//------ terms -------------
			$label = $textPrefix . __("Terms Query", "unlimited-elements");
			...
			$termParam["type"] = UELM_UniteCreatorDialogParam::PARAM_POST_TERMS;
			$termParam["name"] = $name . "_terms";

			$this->addByCreatorParam($termParam);

			//------ users -------------
			$label = $textPrefix . __("Users Query", "unlimited-elements");
			...
			$usersParam["type"] = UELM_UniteCreatorDialogParam::PARAM_USERS;
			$usersParam["name"] = $name . "_users";

			$this->addByCreatorParam($usersParam);

			//------ menu -------------
			$label = $textPrefix . __("Menu Query", "unlimited-elements");
			...
			$menuParam["type"] = UELM_UniteCreatorDialogParam::PARAM_MENU;
			...
			//------ gallery -------------
			$label = $textPrefix . __("Select Items Images", "unlimited-elements");
			...
			$galleryParam["type"] = UELM_UniteCreatorDialogParam::PARAM_GALLERY;
			$galleryParam["name"] = $name . "_gallery";
			$galleryParam["default_value"] = UELM_HelperProviderUC::getArrDynamicGalleryDefaults();
		...
		if($isWooActive === true){
			$label = $textPrefix . __("Products Query", "unlimited-elements");
