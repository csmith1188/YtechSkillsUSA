...
class UELM_UniteCreatorUniteGallery{
		...
		private function validateGalleryType($type){
			...
			switch($type){
				...
				default:
					...
					UELM_UniteFunctionsUC::throwError("Wrong gallery type: $type");
		...
		protected function getParam($name, $validateMode = null){
			...
			if(array_key_exists($name, $this->arrParams)){
				...
				$value = UELM_UniteFunctionsUC::getVal($this->arrOriginalParams, $name);				
			}
			
			switch ($validateMode) {
				case self::VALIDATE_EXISTS:
					if (array_key_exists($name, $arrParams) == false)
						UELM_UniteFunctionsUC::throwError("The param: {$name} don't exists");
				break;
				case self::VALIDATE_NUMERIC:
					
					if (is_numeric($value) == false)
						UELM_UniteFunctionsUC::throwError("The param: {$name} is not numeric ($value)");
				break;
				case self::VALIDATE_SIZE:
					if(strpos($value, "%") === false && is_numeric($value) == false)
						UELM_UniteFunctionsUC::throwError("The param: {$name} is not size");
				break;
		...
		private function buildJsParam($paramName, $validate = null, $type = null, $replaceParamName = null){
			...
			if(array_key_exists($paramName, $this->arrJsParamsAssoc))
				UELM_UniteFunctionsUC::throwError("Unable to biuld js param: <b>$paramName</b> already exists");
		...
		private function buildThemeRelatedJSParams($arr){
			...
			switch($this->galleryType){
				...
				default:
					...
					UELM_UniteFunctionsUC::throwError("buildThemeRelatedJSParams: no options for this type yet: ".$this->galleryType);
	...
	private function modifyParams(){
		...
		$galleryTheme = UELM_UniteFunctionsUC::getVal($this->arrParams, "gallery_theme");
	...
	private function setParams($data, $addon){
		...
		$this->galleryType = UELM_UniteFunctionsUC::getVal($data, "gallery_theme");
		...
		$this->arrParams = UELM_UniteFunctionsUC::getDiffArrItems($arrOriginalParams, $arrDefaultValues, $arrMustKeys);
	 ...
	 */
	public function getUniteGalleryJsSettings($data, UELM_UniteCreatorAddon $addon){
		...
		public static function getUniteGalleryHtmlItem($item){
			...
			$thumb = UELM_UniteFunctionsUC::getVal($item, "thumb");
			$description = UELM_UniteFunctionsUC::getVal($item, "description");
			...
			$title = UELM_UniteFunctionsUC::sanitize($title, UELM_UniteFunctionsUC::SANITIZE_ATTR);
			$description = UELM_UniteFunctionsUC::sanitize($description, UELM_UniteFunctionsUC::SANITIZE_ATTR);
			
			$image = UELM_UniteFunctionsUC::sanitize($image, UELM_UniteFunctionsUC::SANITIZE_URL);
			$thumb = UELM_UniteFunctionsUC::sanitize($thumb, UELM_UniteFunctionsUC::SANITIZE_URL);
			...
			switch($type){
				...
				case "youtube":
					$videoID = UELM_UniteFunctionsUC::getVal($item, "videoid");
					$videoID = UELM_UniteFunctionsUC::sanitize($videoID, UELM_UniteFunctionsUC::SANITIZE_YOUTUBE);
										
				break;
				case "vimeo":
					$videoID = UELM_UniteFunctionsUC::getVal($item, "videoid");
					$videoID = UELM_UniteFunctionsUC::sanitize($videoID, UELM_UniteFunctionsUC::SANITIZE_VIMEO);
										
				break;
				case "wistia":
										
					$videoID = UELM_UniteFunctionsUC::getVal($item, "videoid");
					$videoID = UELM_UniteFunctionsUC::sanitize($videoID, UELM_UniteFunctionsUC::SANITIZE_WISTIA);
					
				break;
				case "html5video":
					
					$urlMp4 = UELM_UniteFunctionsUC::getVal($item, "url_mp4");
					
					$urlMp4 = UELM_UniteFunctionsUC::sanitize($urlMp4, UELM_UniteFunctionsUC::SANITIZE_URL);
				...
				case "iframe":
					...
					$urlVideo = UELM_UniteFunctionsUC::getVal($item, "url_video");
										
					$urlVideo = UELM_UniteFunctionsUC::sanitize($urlVideo, UELM_UniteFunctionsUC::SANITIZE_URL);
