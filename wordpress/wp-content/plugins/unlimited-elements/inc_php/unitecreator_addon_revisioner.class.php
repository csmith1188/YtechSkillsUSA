...
class UELM_UniteCreatorAddonRevisioner{

	const FOLDER_NAME = 'uelm_revisions';
	...
	public function downloadAddonRevision($addon, $revisionId){
		...
		UELM_UniteFunctionsUC::downloadFile($filePath, $fileName);
	}
	...
	public function deleteAddonRevisions($addon){
		...
		UELM_UniteFunctionsUC::deleteDir($folderPath);
	}
	...
	private static function registerHooks(){
		...
		UELM_UniteProviderFunctionsUC::addAction(UELM_UniteCreatorFilters::ACTION_ADDON_AFTER_UPDATE, array($instance, 'createAddonRevision'));
		UELM_UniteProviderFunctionsUC::addAction(UELM_UniteCreatorFilters::ACTION_ADDON_AFTER_DELETE, array($instance, 'deleteAddonRevisions'));
	}
	...
	private function getFolderPath($addon){
		...
		$path = UELM_GlobalsUC::$path_images
			. self::FOLDER_NAME . '/'
			. $id . '/';
	...
	private function purgeRevisions($addon){
		...
		UELM_UniteFunctionsUC::deleteListOfFiles($deleteFiles);
	}
	...
	private function createRevision($addon){
		...
		$revisionsPath = UELM_GlobalsUC::$path_images . self::FOLDER_NAME . "/";

		UELM_UniteFunctionsUC::checkCreateDir($revisionsPath);
		...
		UELM_UniteFunctionsUC::checkCreateDir($folderPath);
