...
class UELM_UniteCreatorSchema {
	...
	private function getArrSchemas(){
		...
		foreach($arrSchemas as $schama){
			...
			$type = UELM_UniteFunctionsUC::getVal($schama, "type");
			
			$name = strtolower($type);
			
			$title = UELM_UniteFunctionsUC::getVal($schama, "title"); 
	...
	private function getSchemaOptionsByName($name){
		...
		$arrSchema = UELM_UniteFunctionsUC::getVal($arrSchemas, $name);
	...
	private function convertWidgetItems($arrItems, $paramName){
		...
		foreach($arrItems as $item){
			...
			$arrItem = UELM_UniteFunctionsUC::getVal($item, "item");
			
			$arrItem = UELM_UniteFunctionsUC::getVal($arrItem, $paramName);
	...
	private function putSchemaByPost($schemaType, $arrItems, $arrSettings){
		...
		$postListName = UELM_UniteFunctionsUC::getVal($arrSettings, "post_list_name");
	...
	public function putSchemaItems($schemaType, $arrItems, $paramsItems, $arrSettings){
		...
		$title = UELM_UniteFunctionsUC::getVal($arrSettings, "title");
		...
		$showDebug = UELM_UniteFunctionsUC::getVal($arrSettings, "debug");
	...
	public function showAddonSchema(){
		...
		if(self::$showDebug == false){
			self::$showDebug = UELM_HelperUC::hasPermissionsFromQuery("ucschemadebug");
		}
	...
	private function sanitizeItemValue($value, $role){
		...
		switch($role){
			...
			case self::ROLE_IMAGE:
				if($value == UELM_GlobalsUC::$url_no_image_placeholder)
					$value = "";
			case self::ROLE_LINK:
	...
	private function getItemExtraFieldValue($fieldName, $postID, $fieldNameType){
		...
		if($fieldNameType == "meta"){
			...
			$metaValue = UELM_UniteFunctionsWPUC::getPostCustomField($postID, $metaKey);
		...
		if($fieldNameType == "term"){
			...
			$termName = UELM_UniteFunctionsWPUC::getPostTerm_firstTermName($postID, $taxonomu);
	...
	private function getItemSchemaContent($item, $arrFieldsByRoles, $itemType){
		...
		switch($itemType){
			case UELM_UniteCreatorAddon::ITEMS_TYPE_POST:
				...
				$postID = UELM_UniteFunctionsUC::getVal($item, "id");
			break;
			case UELM_UniteCreatorAddon::ITEMS_TYPE_MULTISOURCE:
				
				$itemSource = UELM_UniteFunctionsUC::getVal($item, "item_source");
				if($itemSource == "posts")
					$postID = UELM_UniteFunctionsUC::getVal($item, "object_id");
		...
		foreach($arrFieldsByRoles as $role => $fieldName){
			...
			if(!empty($extraFieldType))		//take the value from the extra field
				...
				$value = UELM_UniteFunctionsUC::getVal($item, $fieldName);
	...
	private function getParamNamesByTypes($params, $type){
		...
		foreach($params as $param){
			...
			$fieldType = UELM_UniteFunctionsUC::getVal($param, "type");
			
			//content fields check
			
			if($type == self::CONTENT_FIELDS){
				
				$typeFound = in_array($fieldType, array(UELM_UniteCreatorDialogParam::PARAM_TEXTAREA, 
					  							   UELM_UniteCreatorDialogParam::PARAM_EDITOR) 
				);
	...
	private function getFieldsByRoles($params){
		...
		$arrTextParams = $this->getParamNamesByTypes($params, UELM_UniteCreatorDialogParam::PARAM_TEXTFIELD);
		
		if(isset($arrTextParams["title"]))
			$roleTitle = "title";
		else
			$roleTitle = UELM_UniteFunctionsUC::getFirstNotEmptyKey($arrTextParams);
		...
		if(empty($roleDescription)){
			...
			if(isset($arrContentParams["description"]))
				...
				$roleDescription = UELM_UniteFunctionsUC::getFirstNotEmptyKey($arrContentParams);	//get first key
		...
		if(!empty($arrContentParams))
			$roleContent = UELM_UniteFunctionsUC::getFirstNotEmptyKey($arrContentParams);
		...
		$arrLinkParams = $this->getParamNamesByTypes($params, UELM_UniteCreatorDialogParam::PARAM_LINK);
		
		if(!empty($arrLinkParams))
			$roleLink = UELM_UniteFunctionsUC::getFirstNotEmptyKey($arrLinkParams);
		
		//guess image
		
		$arrImageParams = $this->getParamNamesByTypes($params, UELM_UniteCreatorDialogParam::PARAM_IMAGE);
		
		if(isset($arrImageParams["image"]))
			$roleImage = "image";
		
		if(!empty($arrImageParams))
			$roleImage = UELM_UniteFunctionsUC::getFirstNotEmptyKey($arrImageParams);
	...
	private function getFieldsByRoles_checkMetaTermKeys($arrFieldsByRoles, $fieldMap, $roleKey, $arrSettings){
        ...
        $fieldKey = UELM_UniteFunctionsUC::getVal($arrSettings, "extrafieldkey_" . $roleKey);
        ...
        switch($fieldMap){
        	case "meta":	// check that meta field is valid
        		...
        		$isValid = UELM_UniteFunctionsUC::isMetaKeyValid($fieldKey);
        		
        		if($isValid == false){
        			$fieldKey = UELM_UniteFunctionsUC::sanitize($fieldKey, UELM_UniteFunctionsUC::SANITIZE_HTML);
        			UELM_UniteFunctionsUC::throwError("The meta name: $fieldKey is not valid");
        		}
        			        		
        	break;
        	case "term":	//check that taxonomy is valid
        		$isValid = UELM_UniteFunctionsUC::isTaxonomyNameValid($fieldKey);
				
        		if($isValid == false){
        			$fieldKey = UELM_UniteFunctionsUC::sanitize($fieldKey, UELM_UniteFunctionsUC::SANITIZE_HTML);
        			UELM_UniteFunctionsUC::throwError("The taxonomy name: $fieldKey is not valid");
        		}
	...
	private function getFieldsByRolesFinal($paramsItems, $arrSettings) {
		...
		$itemsType = UELM_UniteFunctionsUC::getVal($arrSettings, "item_type");
	    ...
	    $isMappingEnabled = UELM_UniteFunctionsUC::getVal($arrSettings, "enable_mapping");
	    $isMappingEnabled = UELM_UniteFunctionsUC::strToBool($isMappingEnabled);
	    ...
	    foreach ($arrManualRoles as $roleKey) {
	        ...
	        $manualValue = UELM_UniteFunctionsUC::getVal($arrSettings, "fieldmap_" . $roleKey);
	    ...
	    foreach($arrExtraFields as $roleKey){
	        ...
	        $fieldMap = UELM_UniteFunctionsUC::getVal($arrSettings, "fieldmap_" . $roleKey);
 ...
 *
 * @param UELM_UniteCreatorDialogSettings $objSettings
 * @param string $name
...
private function addFieldsMappingSettings($objSettings, $name, $paramsItems, $isPost) {
    ...
    $arrParam = array();
    $arrParam["origtype"] = UELM_UniteCreatorDialogParam::PARAM_RADIOBOOLEAN;
    $arrParam["elementor_condition"] = array($name . "_enable" => "true");
    $arrParam["description"] = __("Enable manual fields mapping by roles. The post related fields are relevant only if the content from posts", "unlimited-elements");
	
    $objSettings->addRadioBoolean(
        "{$name}_enable_mapping",
        __("Enable Fields Mapping", "unlimited-elements"),
        false,
        "Yes",
        "No",
        $arrParam
    ...
    $arrExtraFieldOptions["nothing"] = __("[Select Content]", "unlimited-elements");
    $arrExtraFieldOptions["meta"] = __("Post Meta Field", "unlimited-elements");
    $arrExtraFieldOptions["term"] = __("Post Term", "unlimited-elements");
    
    $arrFieldOptions = array();
    $arrFieldOptions[self::ROLE_AUTO] = __("[Auto Detect]", "unlimited-elements");
    
    if($isPost == true){
    	
 		$arrFieldOptions['title']   = __("Post Title", "unlimited-elements");
        $arrFieldOptions['intro'] = __("Post Intro", "unlimited-elements");
        $arrFieldOptions['intro_full'] = __("Post Intro Full", "unlimited-elements");
        $arrFieldOptions['content'] = __("Post Content", "unlimited-elements");
                
    }else{
    	
	    foreach ($paramsItems as $param) {
	        $paramName = UELM_UniteFunctionsUC::getVal($param, "name");
	        $paramTitle = UELM_UniteFunctionsUC::getVal($param, "title");
	        $paramType = UELM_UniteFunctionsUC::getVal($param, "type");
	
	        if (empty($paramName))
	            continue;
	
	        $isTextType = ($paramType === UELM_UniteCreatorDialogParam::PARAM_TEXTFIELD);
	        $isContentType = in_array($paramType, array(
	            UELM_UniteCreatorDialogParam::PARAM_TEXTAREA,
	            UELM_UniteCreatorDialogParam::PARAM_EDITOR
	        ));
    ...
    $arrFieldOptions['meta'] = __("Post Meta Field", "unlimited-elements");
    $arrFieldOptions['term'] = __("Post Term", "unlimited-elements");
    ...
    $arrRoles = array(
        self::ROLE_TITLE => __("Title Field", "unlimited-elements"),
        self::ROLE_DESCRIPTION => __("Description Field", "unlimited-elements"),
        self::ROLE_HEADING => __("Heading Field", "unlimited-elements"),
        self::ROLE_CONTENT => __("Content Field", "unlimited-elements"),
        self::ROLE_EXTRA_FIELD1 => __("Extra Field 1", "unlimited-elements"),
        self::ROLE_EXTRA_FIELD2 => __("Extra Field 2", "unlimited-elements"),
        self::ROLE_EXTRA_FIELD3 => __("Extra Field 3", "unlimited-elements"),
        self::ROLE_EXTRA_FIELD4 => __("Extra Field 4", "unlimited-elements")
    );
    ...
    foreach ($arrRoles as $roleKey => $roleLabel) {
        ...
        $arrParam = array();
        $arrParam["origtype"] = UELM_UniteCreatorDialogParam::PARAM_DROPDOWN;
        ...
        $arrParam = array();
        $arrParam["origtype"] = UELM_UniteCreatorDialogParam::PARAM_TEXTFIELD;
        ...
        $objSettings->addTextBox("{$name}_extrafieldkey_{$roleKey}", "", esc_html__("Field Name", "unlimited-elements"),$arrParam);
	...
	public function addSchemaSettings(&$objSettings, $name, $param){
		...
		$arrParam = array();
		$arrParam["origtype"] = UELM_UniteCreatorDialogParam::PARAM_RADIOBOOLEAN;
		$arrParam["description"] = UELM_UniteFunctionsUC::getVal($param, "description");
		
		$objSettings->addRadioBoolean($name."_enable", $param["title"],false,"Yes","No",$arrParam);
		
		
		if(UELM_GlobalsUnlimitedElements::$enableCustomSchema == true){
			...
			$arrOptions = array(
				__("From List","unlimited-elements") => "list",
				__("Custom","unlimited-elements") => "custom",
			);
			
			$objSettings->addSelect($name."_selection",$arrOptions, __("Schema Source","unlimited-elements") , "list", $arrParam);
		
			//------- custom textarea
			
			$arrParam = array();
			$arrParam["origtype"] = UELM_UniteCreatorDialogParam::PARAM_TEXTAREA;
			$arrParam["elementor_condition"] = array($name."_enable"=>"true",$name."_selection"=>"custom");
			
			$descripiton = __("Paste any JSON schema from ","unlimited-elements");
			$descripiton .= "<a href='https://schema.org/Person' target='_blank'>schema.org</a>";
			$descripiton .= __(" site","unlimited-elements");
			
			$descripiton .= __("<br>Posible Placeholders Are: %title%, %description%, %heading%, %link%, %content%, %image%, %field1%, %field2%, %field3%, %field4% ","unlimited-elements");
			
			$arrParam["description"] = $descripiton;
			
			$objSettings->addTextArea($name."_custom", "", __("Custom JSON Schema","unlimited-elements") , $arrParam);
		...
		foreach($arrSchemas as $schema){
			...
			$schemaName = UELM_UniteFunctionsUC::getVal($schema, "name");
			$schemaTitle = UELM_UniteFunctionsUC::getVal($schema, "title");
		...
		if(UELM_GlobalsUnlimitedElements::$enableCustomSchema == false)
			$arrParam["elementor_condition"] = array($name."_enable"=>"true");
		
		
		$arrOptions = array_flip($arrOptions);
		
		$title = __('Schema Type',"unlimited-elements");
		...
		$arrParam["description"] = __('Use to describe the action, like how to tie a shoes',"unlimited-elements");
		$arrParam["label_block"] = true;
		
		$title = __('Schema Main Title',"unlimited-elements");
		...
		$arrParam = array();
		$arrParam["origtype"] = UELM_UniteCreatorDialogParam::PARAM_HR;
		$arrParam["elementor_condition"] = array($name."_enable"=>"true");
		...
		if(empty($this->objAddon))
			UELM_UniteFunctionsUC::throwError("No addon found, please set addon for the schema object");
		
		$itemsType = $this->objAddon->getItemsType();
		
		$isPost = ($itemsType == UELM_UniteCreatorAddon::ITEMS_TYPE_POST);
		...
		$arrParam["origtype"] = UELM_UniteCreatorDialogParam::PARAM_RADIOBOOLEAN;
		$arrParam["elementor_condition"] = array($name."_enable"=>"true");
		$arrParam["description"] = __('Show schema debug in front end footer',"unlimited-elements");
		
		$title = __('Show Schema Debug',"unlimited-elements");
		...
		$arrParam = array();
		$arrParam["origtype"] = UELM_UniteCreatorDialogParam::PARAM_STATIC_TEXT;
		$arrParam["elementor_condition"] = array($name."_enable"=>"true");
		
		$text = __('To show post meta fields and terms turn on debug option in Advanced Section',"unlimited-elements");
	...
	public function addSchemaMultipleSettings(&$objSettings){
		...
		$param = array();
		$param["title"] = __('Enable Schema',"unlimited-elements");
		$param["description"] = "";
	...
	private function showDebugMessage(){
		...
		$message = __('Schema Debug: This widget will generate schema debug at the footer',"unlimited-elements");

		$html = UELM_HelperHtmlUC::getDebugWarningMessageHtml($message);
	...
	private function showDebugSchema($schemaType, $arrSchema){
		...
		uelm_dmp("Schema Output Debug: $schemaType");
		
		uelm_dmp("The Fields Mapping and Content");
		
		UELM_HelperHtmlUC::putHtmlDataDebugBox($this->debugFields);
		
		uelm_dmp("The Schema Output");
		
		UELM_HelperHtmlUC::putHtmlDataDebugBox($arrSchema);
	}
	...
	private function getAllSchemaItemsContent($data, $schemaType){
		...
		$arrAddonsData = UELM_UniteFunctionsUC::getVal($data, "addons");
		...
		foreach($arrAddonsData as $addonData){
			...
			$items = UELM_UniteFunctionsUC::getVal($addonData, "items");
			$params = UELM_UniteFunctionsUC::getVal($addonData, "params");
			$settings = UELM_UniteFunctionsUC::getVal($addonData, "settings");
			...
			if($schemaType == "custom"){
				...
				$schemaContent = UELM_UniteFunctionsUC::getVal($settings, "custom");
				
				$schemaContent = $this->normalizeCustomSchemaTemplate($schemaContent);
			}

			$itemType = UELM_UniteFunctionsUC::getVal($settings, "item_type");
			...
			if(self::$showDebug == true){
				...
				$arrParamsAssoc = UELM_UniteFunctionsUC::arrayToAssoc($params, "name", "type");
	...
	private function replacePlaceholders_values($value, $item){
		...
		foreach($arrRoles as $role){
			...
			$content = UELM_UniteFunctionsUC::getVal($item, $role);
		...
		if (preg_match('/%[a-zA-Z0-9_]+%/', $value) == true){
			...
			if(UELM_HelperUC::isRunCodeOnce("uc_schema_replace_placeholders") == true){
				
				$message = "Custom Schema Error: Unknown Placeholder: ".$value." Please change for a known placeholder from the list.";
				
				$htmlError = UELM_HelperHtmlUC::getErrorMessageHtml($message,"",true);
				uelm_dmp($htmlError);
	...
	private function schemaCustomItem($item){
		...
		$schemaJson = UELM_UniteFunctionsUC::getVal($item, "schema_custom_json");
		...
		try{
			...
			$arrSchema = UELM_UniteFunctionsUC::jsonDecodeValidate($schemaJson);
