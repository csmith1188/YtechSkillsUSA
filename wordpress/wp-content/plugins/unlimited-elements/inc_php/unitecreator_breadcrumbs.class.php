...
class UELM_UniteCreatorBreadcrumbs {
    ...
    public function getBreadcrumbItems($params) {
		...
		$isDebug = UELM_HelperUC::hasPermissionsFromQuery("ucbreadcrumbsdebug");
		if($isDebug == true)
			self::$showDebug = true;
		
			
    	if(self::$showDebug == true)
    		uelm_dmp("Breadcrumbs Debug");
    ...
    private function getCategoryType($category){
    	...
    	$taxonomyName = UELM_UniteFunctionsWPUC::getTermTaxonomyName($category);
    ...
    private function getBreadcrumbs_page() {
        ...
        foreach($ancestors as $ancestor_id) {
            $items[] = array(
            	...
            	'type' => UELM_UniteFunctionsWPUC::getPostTypeTitleByPost($ancestor_id)
            );
        }
        ...
        $items[] = array(
            ...
            'url' => '',
            'type' => UELM_UniteFunctionsWPUC::getPostTypeTitle()
        );
    ...
    private function addBlogPage($items){
      ...
      $items[] = array(
		...
		'type' => UELM_UniteFunctionsWPUC::getPostTypeTitle($posts_page->post_type)
	  );
