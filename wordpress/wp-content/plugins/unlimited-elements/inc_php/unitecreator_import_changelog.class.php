...
class UELM_UniteCreatorImportExportChangelog{
	...
	public function exportChangelog($typeExport) {
		...
		$changelogTable = UELM_UniteFunctionsWPUC::prefixDBTable(UELM_GlobalsUC::TABLE_CHANGELOG_NAME);
		$addonsTable = UELM_GlobalsUC::$table_addons;
		...
		if($typeExport == 'export-file')
			UELM_UniteFunctionsUC::downloadJson($filename, $content);
	...
	public function importChangelog() {
		...
		if ($hasPermissions == false || $isNonceValid == false) {
			...
			$this->processImportJsonAction_error("Security check failed or insufficient permissions.", "unlimited-elements");
		...
		if (empty($_FILES['json_file']['tmp_name']) || $_FILES['json_file']['type'] !== 'application/json') {
			...
			$this->processImportJsonAction_error(__("Invalid file or upload error.", "unlimited-elements"));

			return(false);
		}

		$jsonData = UELM_UniteFunctionsUC::fileGetContents($_FILES['json_file']['tmp_name']);

		$decodedData = json_decode($jsonData, true);

		if(empty($decodedData))
			$this->processImportJsonAction_error(__("Invalid JSON data found", "unlimited-elements"));

		$success = $this->processRecordChangelogAction($decodedData);

		if($success == false)
			$this->processImportJsonAction_error(__("Import processing failed", "unlimited-elements"));


		//if all ok, show success data

		set_transient("uelm_changelog_import_success", __("Change Log imported successfully!", "unlimited-elements"), 30);
	...
	public function processRecordChangelogAction($data){
		...
		if($isChangelogImportDisabled){
			echo esc_attr(__( "The import operation disabled in the general settings.", "unlimited-elements" ));
			exit;
		}
		...
		$addons = $objAddons->getArrAddonsShort('', array(), UELM_GlobalsUC::ADDON_TYPE_ELEMENTOR);
		...
		foreach ($data as $item) {
			...
			$addon = UELM_UniteFunctionsUC::getVal($arrAddonsAssoc, $item['addon_name']);
	...
	private function processImportJsonAction_error($errorMessage){
		...
		$keyError = "uelm_changelog_import_error";
		$urlViewImport = UELM_HelperUC::getViewUrl(UELM_GlobalsUnlimitedElements::VIEW_CHANGELOG_IMPORT);
