 ...
 *
 */
	class UELM_HelperHtmlUC extends UELM_UniteHelperBaseUC{
		...
		public static function getHTMLSelect($arr,$default="",$htmlParams="",$assoc = false, $addData = null, $addDataText = null){
			...
			if($addData == "not_chosen"){
				...
				if(empty($itemText))
					$itemText = "[".esc_html__("not chosen", "unlimited-elements")."]";
		...
		public static function getTableAddonsRow($addonID, $title){
			...
			$editLink = UELM_HelperUC::getViewUrl_EditAddon($addonID);
			...
			$html.= "	<td>\n";
			$html.= " 	  <a href='{$editLink}' class='unite-button-secondary float_left mleft_15'>". esc_html__("Edit","unlimited-elements") . "</a>\n";
			$html.= "		<a href='javascript:void(0)' data-addonid='{$addonID}' class='uc-button-delete unite-button-secondary float_left mleft_15'>".esc_html__("Delete","unlimited-elements")."</a>";
			$html.= "		<span class='loader_text uc-loader-delete mleft_10' style='display:none'>" . esc_html__("Deleting", "unlimited-elements") . "</span>";
			$html.= "		<a href='javascript:void(0)' data-addonid='{$addonID}' class='uc-button-duplicate unite-button-secondary float_left mleft_15'>" . esc_html__("Duplicate","unlimited-elements")."</a>\n";
			$html.= "		<span class='loader_text uc-loader-duplicate mleft_10' style='display:none'>" . esc_html__("Duplicating", "unlimited-elements") . "</span>";
			$html.= "		<a href='javascript:void(0)' data-addonid='{$addonID}' data-title='{$htmlTitle}' class='uc-button-savelibrary unite-button-secondary float_left mleft_15'>" . esc_html__("Save To Library","unlimited-elements")."</a>\n";
			$html.= "		<span class='loader_text uc-loader-save mleft_10' style='display:none'>" . esc_html__("Saving to library", "unlimited-elements") . "</span>";
			$html.= "	</td>\n";
			$html.= "	</tr>\n";
		...
		public static function getGlobalJsOutput(){
			...
			$jsArrayText = UELM_UniteFunctionsUC::phpArrayToJsArrayText(UELM_GlobalsUC::$arrClientSideText,"				");

			//prepare assets path
			$pathAssets = UELM_HelperUC::pathToRelative(UELM_GlobalsUC::$pathAssets, false);
			$pathAssets = urlencode($pathAssets);

			//check catalog
			$objWebAPI = new UELM_UniteCreatorWebAPI();
			$isNeedCheckCatalog = $objWebAPI->isTimeToCheckCatalog();

			$arrGeneralSettings = array();
			$arrGeneralSettings["color_picker_type"] = UELM_GlobalsUC::$colorPickerType;

			$arrGeneralSettings = UELM_UniteProviderFunctionsUC::applyFilters(UELM_UniteCreatorFilters::FILTER_CLIENTSIDE_GENERAL_SETTINGS, $arrGeneralSettings);

			//uelm_dmp($arrGeneralSettings);exit();

			$strGeneralSettings = UELM_UniteFunctionsUC::jsonEncodeForClientSide($arrGeneralSettings);

			$js = "";
			$js .= self::TAB2.'var g_pluginNameUC = "'.UELM_GlobalsUC::PLUGIN_NAME.'";'.self::BR;
			$js .= self::TAB2.'var g_pathAssetsUC = decodeURIComponent("'.$pathAssets.'");'.self::BR;
			$js .= self::TAB2.'var g_urlAjaxActionsUC = "'.UELM_GlobalsUC::$url_ajax.'";'.self::BR;
			$js .= self::TAB2.'var g_urlViewBaseUC = "'.UELM_GlobalsUC::$url_component_admin.'";'.self::BR;
			$js .= self::TAB2.'var g_urlViewBaseNowindowUC = "'.UELM_GlobalsUC::$url_component_admin_nowindow.'";'.self::BR;
			$js .= self::TAB2.'var g_urlBaseUC = "'.UELM_GlobalsUC::$url_base.'";'.self::BR;
			$js .= self::TAB2.'var g_urlAssetsUC = "'.UELM_GlobalsUC::$url_assets.'";'.self::BR;
			...
			if(UELM_GlobalsUC::$enableWebCatalog == true){
				...
				$js .= self::TAB2.'var g_ucCatalogAddonType="'.UELM_GlobalsUC::$defaultAddonType.'";'.self::BR;
			}

			//output icons
			$jsonFaIcons = UELM_UniteFontManagerUC::fa_getJsonIcons();
			$js .= self::TAB2.'var g_ucFaIcons = '.$jsonFaIcons.';'.self::BR;

			//output elementor icons
			$jsonElementorIcons = UELM_UniteFontManagerUC::elementor_getJsonIcons();
			$js .= self::TAB2.'var g_ucElIcons = '.$jsonElementorIcons.';'.self::BR;


			//get nonce
			if(method_exists("UELM_UniteProviderFunctionsUC", "getNonce"))
				$js .= self::TAB2 . "var g_ucNonce='".UELM_UniteProviderFunctionsUC::getNonce()."';";
		...
		public static function getVersionText(){
			...
			$filepath = UELM_GlobalsUC::$pathPlugin . "changelog.txt";
			
			if(file_exists($filepath) == false)
				UELM_UniteFunctionsUC::throwError("file: $filepath not exists");
		...
		public static function getHtmlCustomScripts($arrScripts){
			...
			if(is_array($arrScripts) == false){
				UELM_UniteFunctionsUC::throwError("arrScripts should be array");
			}
		...
		public static function putHtmlDataDebugBox($data){
			...
			if(is_array($data))
				$data = UELM_UniteFunctionsUC::modifyDataArrayForShow($data);
		...
		public static function putAddonTypesBrowserDialogs($filterType = null, $objLayoutType = null){
			...
			if(!empty($filterType)){
				...
				$arrTypes = array(
					UELM_GlobalsUC::ADDON_TYPE_REGULAR_ADDON,
					UELM_GlobalsUC::ADDON_TYPE_SHAPE_DEVIDER,
					UELM_GlobalsUC::ADDON_TYPE_LAYOUT_SECTION,
					UELM_GlobalsUC::ADDON_TYPE_SHAPES,
					UELM_GlobalsUC::ADDON_TYPE_BGADDON,
				);


			foreach($arrTypes as $addontype){

				if($addontype == UELM_GlobalsUC::ADDON_TYPE_REGULAR_ADDON && !empty($arrMultipleTypesRegular))
		...
		public static function putGlobalsHtmlOutput(){
			...
			if(method_exists("UELM_UniteProviderFunctionsUC", "putMasterHTML"))
				UELM_UniteProviderFunctionsUC::putMasterHTML()
			?>
		...
		public static function putDialogControlFieldsNotice(){
			?>
				...
				<div class="unite-inputs-label unite-italic">
					* <?php esc_html_e("only dropdown and radio boolean field types are used for conditional inputs", "unlimited-elements")?>.
				</div>
		...
		public static function putPluginVersionHtml(){
			...
			$objPlugins = new UELM_UniteCreatorPlugins();
			...
			foreach($arrPlugins as $plugin){
				...
				$name = UELM_UniteFunctionsUC::getVal($plugin, "name");
				$title = UELM_UniteFunctionsUC::getVal($plugin, "title");
				$version = UELM_UniteFunctionsUC::getVal($plugin, "version");
				$silentMode = UELM_UniteFunctionsUC::getVal($plugin, "silent_mode");
				$silentMode = UELM_UniteFunctionsUC::strToBool($silentMode);
		...
		public static function outputError(Error $e){
			...
			uelm_dmp("PHP Error Occured!!!");

			uelm_dmp("<b>$message </b>");
			uelm_dmp("in file: <b>$file</b> (line: <b>$line</b>)");
			uelm_dmp("trace: ");
			uelm_dmp($trace);
		...
		public static function outputException($e, $prefix="", $forceTrace = false){
			...
			if(empty($prefix))
				$prefix = UELM_GlobalsUnlimitedElements::$pluginTitleCurrent." Error: ";
			...
			if(UELM_GlobalsUC::$SHOW_TRACE == true || $forceTrace === true)
				uelm_dmp($trace);
			else
		...
		public static function outputExceptionBox($e, $prefix=""){
			...
			$showTrace = UELM_GlobalsUC::$SHOW_TRACE_FRONT;
			if(UELM_UniteProviderFunctionsUC::isAdmin() == true)
				$showTrace = UELM_GlobalsUC::$SHOW_TRACE;
		...
		public static function putHtmlSettings($filename, $formID, $arrValues = array(), $pathSettings = null){
			...
			if($pathSettings === null)
				$pathSettings = UELM_GlobalsUC::$pathSettings;

			$filepathSettings = $pathSettings."{$filename}.xml";

			UELM_UniteFunctionsUC::validateFilepath($filepathSettings, "settings file - {$filename}.xml");

			$settings = new UELM_UniteSettingsAdvancedUC();
		...
		public static function outputMemoryUsageLog(){
			$arrLog = UELM_HelperUC::getLastMemoryUsage();
			...
			$date = UELM_UniteFunctionsUC::timestamp2DateTime($timestamp);

			$urlPage = UELM_UniteFunctionsUC::getVal($arrLog[0], "current_page");
		...
		public static function putHtmlAdminNotices(){
			...
			$arrNotices = UELM_HelperUC::getAdminNotices();
			if(empty($arrNotices))
				return(false);
		...
		public static function putInternalAdminNotices(){
			...
			if(strpos(UELM_GlobalsUC::URL_API, "http://localhost") !== false)
			...
			switch($type){
				case UELM_UniteCreatorDialogParam::PARAM_BACKGROUND:
					switch($subtype){
						...
						default:
							UELM_UniteFunctionsUC::throwError("Param \"$type\" subtype \"$subtype\" is not implemented.");
					}
				...
				default:
					UELM_UniteFunctionsUC::throwError("Param \"$type\" is not implemented.");
			}
		}
		...
		public static function putHtmlConditions($type){
			...
			?>
				...
				<label for="<?php echo esc_attr($checkboxID)?>" class="unite-inputs-label-inline-free">
						<?php esc_html_e("Enable Condition", "unlimited-elements")?>:
				</label>
				...
				<div class="uc-dialog-conditions-content">
					...
					<div class="uc-dialog-conditions-empty">
						...
						<?php esc_attr_e("No parent attribute (dropdown, checkbox or radio) exists in this category", "unlimited-elements")?>
					</div>

					<div class="uc-dialog-conditions-inputs">

						<label class="unite-inputs-label">
							<?php esc_attr_e("Show When", "unlimited-elements")?>:
						</label>
							...
							<tr>
								...
								<td>
									<select class="uc-dialog-condition-operator" name="condition_operator">
										<option value="equal"><?php esc_attr_e("Equal","unlimited-elements")?></option>
										<option value="not_equal"><?php esc_attr_e("Not Equal","unlimited-elements")?></option>
									</select>
								</td>
								<td>
									<div class="uc-dialog-condition-value-wrapper">
										...
										<a href="javascript:void(0)" class="uc-dialog-link-addcondition" title="<?php esc_attr_e("Add Condition", "unlimited-elements") ?>">+</a>
									</div>
								</td>
							</tr>
...
<?php
		...
		public static function putJSFunc_isElementInViewport($checkRunOnce = true){
			...
			if($checkRunOnce == true){
				...
				$isRunOnce = UELM_HelperUC::isRunCodeOnce("js_isElementInViewport");
	 ...
	 */
    function uelm_IsElementInViewport(objElement) {
      ...
      return (elementBottom > viewportTop && elementTop < viewportBottom);
		...
		public static function putDocReadyStartJS($widgetID){
			...
			if(UELM_GlobalsProviderUC::$isGutenbergOutput == true):
			?>
