...
class UELM_UniteCreatorLibraryWork{
	...
	private function loadLibrary_getIncludes($xmlIncludes){
		...
		foreach($objIncludes as $objInclude){
			...
			$handle = (string)UELM_UniteFunctionsUC::getVal($attribs, "handle");
			$local = (string)UELM_UniteFunctionsUC::getVal($attribs, "local");
			$remote = (string)UELM_UniteFunctionsUC::getVal($attribs, "remote");
			$condition = (string)UELM_UniteFunctionsUC::getVal($attribs, "condition");
			
			if(empty($local) && empty($remote))
				UELM_UniteFunctionsUC::throwError("Include: $handle must have some url");
		
			if(!empty($local)){
				$local = trim($local);
				$urlInclude = UELM_GlobalsUC::$url_assets_libraries.$local;
	...
	private function loadLibrary(){
		...
		$filepathLibrary = UELM_GlobalsUC::$pathSettings."library.xml";
		UELM_UniteFunctionsUC::validateFilepath($filepathLibrary);
		
		$arrLibrary = array();
			
		$obj = UELM_UniteFunctionsUC::loadXMLFile($filepathLibrary);
		...
		foreach($items as $objItem){
			...
			$name = (string)UELM_UniteFunctionsUC::getVal($attribs, "name");
			
			$arrItem = array();
			$arrItem["name"] = $name;
			$arrItem["title"] = (string)UELM_UniteFunctionsUC::getVal($attribs, "title");
			
			UELM_UniteFunctionsUC::validateNotEmpty($name, "item name");
			UELM_UniteFunctionsUC::validateNotEmpty($arrItem["title"], "item title");
	...
	public function getLibraryIncludes($name){
		...
		$urlBase = UELM_GlobalsUC::$url_assets_libraries;
