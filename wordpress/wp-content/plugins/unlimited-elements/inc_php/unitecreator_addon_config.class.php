...
class UELM_UniteCreatorAddonConfig extends UELM_HtmlOutputBaseUC{
	...
	private function valdiateStartAddon(){
		...
		if($this->startWithAddon == false)
			UELM_UniteFunctionsUC::throwError("No start addon found");
	...
	public function getHtmlFrame($putMode = false){
		...
		$title = esc_html__("Addon Title", "unlimited-elements");
		$this->valdiateStartAddon();
		
		$addHtml = "";
		$title = $this->startAddon->getTitle(true);
		$title .= " - ".esc_html__("Config", "unlimited-elements");
		...
		$strOptions = UELM_UniteFunctionsUC::jsonEncodeForHtmlData($options,"options");
	 ...
	 * set start addon
	 */
	public function setStartAddon(UELM_UniteCreatorAddon $objAddon){
		$this->startWithAddon = true;
		
		UELM_UniteFunctionsUC::validateNotEmpty($objAddon, "addon");
