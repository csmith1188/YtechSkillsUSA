...
class UELM_UniteCreatorAddonType{
	...
	private function initDistinctTypeName(){
		...
		if($this->isLayout == true)
			$this->typeNameDistinct = UELM_GlobalsUC::ADDON_TYPE_REGULAR_LAYOUT;
		else
			$this->typeNameDistinct = UELM_GlobalsUC::ADDON_TYPE_REGULAR_ADDON;
	...
	protected function initInternal(){
		...
		if(empty($this->typeNameCategory) && $this->isLayout == true)
			$this->typeNameCategory = UELM_GlobalsUC::ADDON_TYPE_REGULAR_LAYOUT;
	...
	public static function getAddonTypeObject($type, $isLayout = false){
		...
		if(empty($type)){
			...
			if($isLayout == true)
				$type = UELM_GlobalsUC::ADDON_TYPE_REGULAR_LAYOUT;
			else
				$type = UELM_GlobalsUC::ADDON_TYPE_REGULAR_ADDON;
		}
		
		
		$cacheName = $type;
		if(empty($type))
			$cacheName = UELM_GlobalsUC::ADDON_TYPE_REGULAR_ADDON;
		...
		switch($type){
			case UELM_GlobalsUC::ADDON_TYPE_SHAPE_DEVIDER:
				$objType = new UELM_UniteCreatorAddonType_Shape_Divider();
			break;
			case UELM_GlobalsUC::ADDON_TYPE_SHAPES:
				$objType = new UELM_UniteCreatorAddonType_Shape();
			break;
			case "elementor":	//special type
			case "vc":	//special type
				$objType = new UELM_UniteCreatorAddonType($type);
			break;
			case UELM_GlobalsUC::ADDON_TYPE_REGULAR_ADDON:
				$objType = new UELM_UniteCreatorAddonType();
			break;
			case UELM_GlobalsUC::ADDON_TYPE_REGULAR_LAYOUT:
				$objType = new UELM_UniteCreatorAddonType_Layout();
			break;
			case UELM_GlobalsUC::ADDON_TYPE_LAYOUT_SECTION:
				$objType = new UELM_UniteCreatorAddonType_Layout_Section();
			break;
			case UELM_GlobalsUC::ADDON_TYPE_LAYOUT_GENERAL:
				$objType = new UELM_UniteCreatorAddonType_Layout_General();
			break;
			case UELM_GlobalsUC::ADDON_TYPE_BGADDON:
				$objType = new UELM_UniteCreatorAddonType_BGAddon();
			break;
			default:
				UELM_UniteFunctionsUC::throwError("wrong addon type: ".$type);
			break;
		}
	...
	public static function getAddonTypesForAddonPicker(){
		...
		$arrTypeNames = array();
		$arrTypeNames[] = UELM_GlobalsUC::ADDON_TYPE_REGULAR_ADDON;
		$arrTypeNames[] = UELM_GlobalsUC::ADDON_TYPE_SHAPES;
