...
class UELM_OpenWeatherAPIForecast extends UELM_OpenWeatherAPIModel{
	...
	public function getPressure(){
		...
		$pressure = sprintf(__("%s hPa", "unlimited-elements"), $pressure);
	...
	private function getWeatherArrayAttribute($key){
		...
		$weather = UELM_UniteFunctionsUC::getVal($weather, 0, array()); // the first weather condition is primary
		$value = UELM_UniteFunctionsUC::getVal($weather, $key);
	...
	private function getTemperature($key, $fallback = null){
		...
		$temperature = UELM_UniteFunctionsUC::getVal($temperature, $key, $fallback);
		$temperature = $this->formatTemperature($temperature);
	...
	private function formatPercentage($value){
		// translators: %s is a string
		return sprintf(__("%s%%", "unlimited-elements"), $value);
	}
	...
	private function formatPrecipitation($value){
		...
		if(is_array($value))
			$value = UELM_UniteFunctionsUC::getArrFirstValue($value);

		if(is_array($value))
			$value = 0;

		// translators: %s is a string
		return sprintf(__("%s mm", "unlimited-elements"), $value);
	}
	...
	private function formatSpeed($value){
		...
		switch($this->getUnits()){
			case self::UNITS_IMPERIAL:
				// translators: %s is a string
				return sprintf(__("%s mph", "unlimited-elements"), $value);
			default:
				// translators: %s is a string
				return sprintf(__("%s m/s", "unlimited-elements"), $value);
		}
	}
	...
	private function formatTemperature($value){
		...
		//		return sprintf(__("%s°C", "unlimited-elements"), $value);
		//	case self::UNITS_IMPERIAL:
		//		return sprintf(__("%s°F", "unlimited-elements"), $value);
		//	default:
		//		return sprintf(__("%sK", "unlimited-elements"), $value);
		//}
	}
