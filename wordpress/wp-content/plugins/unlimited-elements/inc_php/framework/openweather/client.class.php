...
class UELM_OpenWeatherAPIClient{
	...
	public function getForecasts($country, $city, $units = UELM_OpenWeatherAPIForecast::UNITS_STANDARD, $locale = null){
		...
		$params = array(
			"latitude" => UELM_UniteFunctionsUC::getVal($response, "lat"),
			"longitude" => UELM_UniteFunctionsUC::getVal($response, "lon"),
			"timezone" => UELM_UniteFunctionsUC::getVal($response, "timezone"),
			"timezone_offset" => UELM_UniteFunctionsUC::getVal($response, "timezone_offset"),
			"units" => $units,
			"locale" => $locale
		);
		
		$current = UELM_UniteFunctionsUC::getVal($response, "current", array());
		$current = UELM_OpenWeatherAPIForecastCurrent::transform($current, $params);
		
		
		$hourly = UELM_UniteFunctionsUC::getVal($response, "hourly", array());
		$hourly = UELM_OpenWeatherAPIForecastHourly::transformAll($hourly, $params);

		$daily = UELM_UniteFunctionsUC::getVal($response, "daily", array());
		$daily = UELM_OpenWeatherAPIForecastDaily::transformAll($daily, $params);

		$alerts = UELM_UniteFunctionsUC::getVal($response, "alerts", array());
	...
	private function get($url, $params = array()){
		...
		return $this->request(UELM_HttpRequest::METHOD_GET, $url, $params);
	}
	...
	private function request($method, $url, $params = array()){
		...
		$query = ($method === UELM_HttpRequest::METHOD_GET) ? $params : array();
		$body = ($method !== UELM_HttpRequest::METHOD_GET) ? $params : array();
	...
	private function throwError($message){
		...
		UELM_UniteFunctionsUC::throwError("OpenWeather API Error: $message");
	}
