...
class UELM_UniteSettingsOutputSidebarUC extends UELM_UniteCreatorSettingsOutput{
	...
	private function drawResponsivePicker($selectedType){
		...
		$devices = array(
			...
			"mobile" => array(
				"title" => __("Mobile", "unlimited-elements"),
	...
	protected function drawSettingRow($setting, $mode = ""){
		...
		$descErrors = UELM_UniteFunctionsUC::validateHTML($description);
		...
		$attribsText = UELM_UniteFunctionsUC::getVal($setting, "attrib_text");
		if(empty($attribsText) && empty($text))
			$toDrawText = false;

		$labelBlock = UELM_UniteFunctionsUC::getVal($setting, "label_block");
		...
		$responsiveId = UELM_UniteFunctionsUC::getVal($setting, "responsive_id");
		$responsiveType = UELM_UniteFunctionsUC::getVal($setting, "responsive_type");

		if($isResponsive === true)
			$addAttr .= " data-responsive-id=\"$responsiveId\" data-responsive-type=\"$responsiveType\"";
		
		$tabsId = UELM_UniteFunctionsUC::getVal($setting, "tabs_id");
		$tabsValue = UELM_UniteFunctionsUC::getVal($setting, "tabs_value");
	...
	protected function drawTextRow($setting){
		...
		$id = UELM_UniteFunctionsUC::getVal($setting, "id");
		$label = UELM_UniteFunctionsUC::getVal($setting, "label");
		$text = UELM_UniteFunctionsUC::getVal($setting, "text");
		$classAdd = UELM_UniteFunctionsUC::getVal($setting, UELM_UniteSettingsUC::PARAM_CLASSADD);
		$isHeading = UELM_UniteFunctionsUC::getVal($setting, "is_heading");
		$isHeading = UELM_UniteFunctionsUC::strToBool($isHeading);
		...
		<li id="<?php echo esc_attr($id) ?>_row" <?php 
			...
			<span class="unite-settings-static-text<?php echo esc_attr($classAdd); ?>">
				<?php if($type == UELM_UniteCreatorSettings::TYPE_HTML):?>
				<?php uelm_echo($text) ?>
				<?php else:?>
				<?php echo esc_html($text); ?>
				<?php endif?>				
	...
	protected function drawSapBefore($sap, $key){
		...
		$name = UELM_UniteFunctionsUC::getVal($sap, "name");
		$text = UELM_UniteFunctionsUC::getVal($sap, "text");
		//$classIcon = UELM_UniteFunctionsUC::getVal($sap, "icon");
		$classIcon = null; // disable icon for now
		$isHidden = UELM_UniteFunctionsUC::getVal($sap, "hidden");
		$isHidden = UELM_UniteFunctionsUC::strToBool($isHidden);

		if(empty($tab) === true)
			$tab = UELM_UniteSettingsUC::TAB_CONTENT;

		if(empty($name) === true)
			$name = "unnamed_" . UELM_UniteFunctionsUC::getRandomString();
