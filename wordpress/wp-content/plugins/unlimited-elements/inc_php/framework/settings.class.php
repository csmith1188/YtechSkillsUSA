 ...
 */
	class UELM_UniteSettingsUC{
		...
		protected $idPrefix;
		protected $defaultText = "Enter Format String";
	    ...
	    public function __construct(){
	    	...
	    	$suffix = time()."_".UELM_UniteFunctionsUC::getRandomString(5)."_";
		...
		private function modifySettingsData($arrSettings){
			...
			foreach($arrSettings as $key=>$content){
				switch(getType($content)){
					...
					case "object":
					case "array":
						$content = UELM_UniteFunctionsUC::convertStdClassToArray($content);
					break;
				}
		...
		private function getIndexByName($name, $throwError = true){
			...
			if(!empty($this->arrIndex)){
				...
				if(array_key_exists($name, $this->arrIndex) == false){
					...
					if($throwError == true)
						UELM_UniteFunctionsUC::throwError("getIndexByName: setting with name: $name not found");
					else 
						return(null);
				}
			...
			if($throwError == true)
				UELM_UniteFunctionsUC::throwError("Setting with name: $name don't exists");
			else
				return(null);
		...
		public function getArrSettingsBySap($sapName){
			...
			if($sapKey === null)
				UELM_UniteFunctionsUC::throwError("sap: $sapName not found");
		...
		public function getArrTextFromAllSettings(){
			...
			foreach($this->arrSettings as $setting){
				...
				$desc = UELM_UniteFunctionsUC::getVal($setting, "description");
		...
		public function getSettingValue($name,$default=""){
			...
			$value = UELM_UniteFunctionsUC::getVal($setting, "value",$default);
		...
		public function addSap($text, $name, $tab = null, $params = array()){
			...
			if(is_array($name))
				UELM_UniteFunctionsUC::throwError("addSap function: sap name can't be array: $text");
			
			if(is_numeric($tab) || $tab === true)
				UELM_UniteFunctionsUC::throwError("The tab should not be numeric");
			
			if(empty($tab))
				$tab = self::TAB_CONTENT;

			if(empty($text))
				UELM_UniteFunctionsUC::throwError("sap $name must have a text");

			if(empty($name))
				$name = "uc_sap_".UELM_UniteFunctionsUC::getRandomString();

			$opened = UELM_UniteFunctionsUC::getVal($params, "opened");
		...
		public function getSap($sapKey){
			...
			$arrSap = UELM_UniteFunctionsUC::getVal($this->arrSaps, $sapKey);

			if(empty($arrSap))
				UELM_UniteFunctionsUC::throwError("sap with key: $sapKey not found");
		...
		public function getSapByName($sapName){
			...
			if(isset($this->arrSapIndex[$sapName]) == false)
				UELM_UniteFunctionsUC::throwError("sap with name: $sapName not found");

			$index = UELM_UniteFunctionsUC::getVal($this->arrSapIndex, $sapName);

			$arrSap = UELM_UniteFunctionsUC::getVal($this->arrSaps, $index);

			if(empty($arrSap))
				UELM_UniteFunctionsUC::throwError("sap with key: $index not found");
		...
		protected function getSapKeyByName($name){
			...
			$index = UELM_UniteFunctionsUC::getVal($this->arrSapIndex, $name);
	...
	public function addFile($name, $defaultValue = "", $text = "", $arrParams = array()){
		...
		public function addHr($name="",$params=array()){
			...
			if($name != "")
			  ...
			  $this->HRIdCounter++;
			  $itemName = "hr_".UELM_UniteFunctionsUC::getRandomString();
		...
		protected function addSettingGlobalParams($setting){
			...
			$settingType = UELM_UniteFunctionsUC::getVal($setting, "type");
			foreach($this->arrGlobalParams as $param){

				$types = UELM_UniteFunctionsUC::getVal($param, "types");
		...
		private function addSettingOriginalType($setting){
			...
			$originalType = UELM_UniteFunctionsUC::getVal($setting, "origtype");

			if(empty($originalType) === false)
				return $setting;

			$typesMap = array(
				self::TYPE_HIDDEN => UELM_UniteCreatorDialogParam::PARAM_HIDDEN,
				self::TYPE_HR => UELM_UniteCreatorDialogParam::PARAM_HR,
				self::TYPE_RANGE => UELM_UniteCreatorDialogParam::PARAM_SLIDER,
				self::TYPE_REPEATER => UELM_UniteCreatorDialogParam::PARAM_REPEATER,
				self::TYPE_SELECT => UELM_UniteCreatorDialogParam::PARAM_DROPDOWN,
				self::TYPE_STATIC_TEXT => UELM_UniteCreatorDialogParam::PARAM_STATIC_TEXT,
				self::TYPE_SWITCHER => UELM_UniteCreatorDialogParam::PARAM_RADIOBOOLEAN,
				self::TYPE_TEXT => UELM_UniteCreatorDialogParam::PARAM_TEXTFIELD,
			);

			$type = UELM_UniteFunctionsUC::getVal($setting, "type");

			$setting["origtype"] = UELM_UniteFunctionsUC::getVal($typesMap, $type);
		...
		protected function add($name, $defaultValue = "", $text = "", $type = self::TYPE_TEXT, $arrParams = array()){
			...
			$modifyType = UELM_UniteFunctionsUC::getVal($setting, "modifytype");
			if(!empty($modifyType)){
		...
		public function addSettingByArray($setting){
			...
			if(is_array($setting) == false)
				UELM_UniteFunctionsUC::throwError("addSettingByArray: Empty setting given");

			$name = UELM_UniteFunctionsUC::getVal($setting, "name");

			UELM_UniteFunctionsUC::validateNotEmpty($name, "setting name");
		...
		public function addControl($control_item_name, $controlled_item_name, $control_type, $value, $isSap = false){
			...
			UELM_UniteFunctionsUC::validateNotEmpty($control_item_name,"control parent");
			UELM_UniteFunctionsUC::validateNotEmpty($controlled_item_name,"control child");
			UELM_UniteFunctionsUC::validateNotEmpty($control_type,"control type");
		...
		private function getControlAction($parentName, $arrControl, $showDebug = false){
			...
			$value = UELM_UniteFunctionsUC::getVal($arrControl, "value");
			...
			if($showDebug == true){
				uelm_dmp("compare type: $type");
				uelm_dmp("compare value: $value");
				uelm_dmp("parent value: $parentValue");
					
				$isEqual = $this->isControlValuesEqual($parentValue, $value);
				
				uelm_dmp("is equal: $isEqual");
			}
		...
		public function setSettingsStateByControls(){
			...
			foreach($this->arrControlChildren as $childName => $arrParents){
				...
				foreach($arrParents as $parentName){
					...
					$isSap = UELM_UniteFunctionsUC::getVal($arrControl, "forsap");
					$isSap = UELM_UniteFunctionsUC::strToBool($isSap);
					...
					if($debug == true){
						uelm_dmp("parent exists");
						uelm_dmp($isParentExists);
					}
					...
					if($isParentExists == false){
						...
						if($isSap == true){
							...
							$tabTitle = UELM_UniteFunctionsUC::getVal($sap, "text");
							
							$childName .= " - ".$tabTitle;
						}
						
						UELM_UniteFunctionsUC::throwError("Control attribute (<b>$parentName</b>) not exists in $childType: <b>$childName</b>");
			...
			if($debug == true){
				uelm_dmp("the settings");
				uelm_dmp($this->arrSettings);
				exit();
			}
		...
		public function setPairedSettings(){
			...
			foreach($this->arrSettings as $setting){
				$addSettingName = UELM_UniteFunctionsUC::getVal($setting, self::PARAM_ADDFIELD);
		...
		private function isLoadConditionMet($loadCondition){
			...
			switch($loadCondition){
				case "elementor_enabled":
					...
					return (UELM_GlobalsUnlimitedElements::$enableElementorSupport == true);
				break;
				case "gutenberg_enabled":
					
					return (UELM_GlobalsUnlimitedElements::$enableGutenbergSupport == true);
				break;
				default:
					return(true);
				break;
		...
		public function loadXMLFile($filepath, $loadedSettingsType = null){
			...
			$obj = UELM_UniteFunctionsUC::loadXMLFile($filepath);

			if(empty($obj))
				UELM_UniteFunctionsUC::throwError("Wrong xml file format: $filepath");
			...
			foreach($fieldsets as $fieldset){
				...
				$sapName = (string)UELM_UniteFunctionsUC::getVal($attribs, "name");
				$sapLabel = (string)UELM_UniteFunctionsUC::getVal($attribs, "label");
				$sapIcon = (string)UELM_UniteFunctionsUC::getVal($attribs, "icon");
				$loadFrom = (string)UELM_UniteFunctionsUC::getVal($attribs, "loadfrom");
				$loadCondition = (string)UELM_UniteFunctionsUC::getVal($attribs, "load_condition");
				$loadParam = (string)UELM_UniteFunctionsUC::getVal($attribs, "loadparam");
				$loadedSettingsType = (string)UELM_UniteFunctionsUC::getVal($attribs, "loadtype");
				$nodraw = (string)UELM_UniteFunctionsUC::getVal($attribs, "nodraw");
				$visibility = (string)UELM_UniteFunctionsUC::getVal($attribs, "visibility");

				$isForceShow = false;

				if($visibility == "dev" && UELM_GlobalsUC::$inDev == false && $isForceShow == false)
					continue;

				$sapParams = array();
				if(!empty($nodraw)){
					$sapParams["nodraw"] = UELM_UniteFunctionsUC::strToBool($nodraw);
				}
				
				UELM_UniteFunctionsUC::validateNotEmpty($sapName,"name in fieldset attributes: $sapLabel");
				...
				UELM_UniteFunctionsUC::validateNotEmpty($sapLabel,"sapLabel");
				...
				$fields = UELM_UniteFunctionsUC::getVal($fieldset, "field");
				...
				foreach($fields as $field){
					...
					$fieldType = (string)UELM_UniteFunctionsUC::getVal($attribs, "type");
					$fieldName = (string)UELM_UniteFunctionsUC::getVal($attribs, "name");
					$fieldLabel = (string)UELM_UniteFunctionsUC::getVal($attribs, "label");
					$fieldDefaultValue = (string)UELM_UniteFunctionsUC::getVal($attribs, "default");
					...
					UELM_UniteFunctionsUC::validateNotEmpty($fieldType,"type");

					//validate name
					if($fieldType != self::TYPE_HR && $fieldType != self::TYPE_CONTROL &&
						$fieldType != "bulk_control_start" && $fieldType != "bulk_control_end")
						UELM_UniteFunctionsUC::validateNotEmpty($fieldName,"name");
					switch ($fieldType){
						case self::TYPE_CHECKBOX:

							$fieldDefaultValue = UELM_UniteFunctionsUC::strToBool($fieldDefaultValue);
						...
						case self::TYPE_CONTROL:
							$parent = UELM_UniteFunctionsUC::getVal($arrParams, "parent");
							$child =  UELM_UniteFunctionsUC::getVal($arrParams, "child");
							$ctype =  UELM_UniteFunctionsUC::getVal($arrParams, "ctype");
							$value =  UELM_UniteFunctionsUC::getVal($arrParams, "value");
						...
						break;
						default:
							UELM_UniteFunctionsUC::throwError("wrong type: $fieldType");
						break;
					}
		...
		public function addFromXmlFile($filepathXML){
			...
			$objNewSettings = new UELM_UniteSettingsUC();
		...
		private function getOptionsFromXMLField($field, $fieldName){
			...
			$options = UELM_UniteFunctionsUC::getVal($arrField, "option");
			...
			foreach($options as $option){
				...
				if(gettype($option) == "string")
					UELM_UniteFunctionsUC::throwError("Wrong options type: ".$option." in field: $fieldName");

				$attribs = $option->attributes();

				$optionValue = (string)UELM_UniteFunctionsUC::getVal($attribs, "value");
				$optionText = (string)UELM_UniteFunctionsUC::getVal($attribs, "text");

				//validate options:
				UELM_UniteFunctionsUC::validateNotEmpty($optionText,"option text");
		 ...
		 */
		public function mergeSettings(UELM_UniteSettingsUC $settings){
			...
			foreach ($arrSapsNew as $key => $sap) {
				$sapName = UELM_UniteFunctionsUC::getVal($sap, "name");
			...
			foreach ($arrSettingsNew as $setting) {
				...
				$sapOld = UELM_UniteFunctionsUC::getVal($setting, "sap");
				if (array_key_exists($sapOld, $arrNewSapKeys) == false) {
					UELM_UniteFunctionsUC::throwError("sap {$sapOld} should be exists in sap keys array");
				}
		...
		private function addExternalSettings($filename, $loadParam = null, $loadType = null){
			$filepathSettings = UELM_GlobalsUnlimitedElements::$pathPluginSettings."{$filename}.xml";

			if(file_exists($filepathSettings) == false)
				$filepathSettings = UELM_GlobalsUC::$pathSettings."{$filename}.xml";
			
			if(file_exists($filepathSettings) == false)
				UELM_UniteFunctionsUC::throwError("The file: {$filename}.xml don't found in settings folder");
		...
		public function updateArrSettingByName($name,$setting){
			...
			foreach($this->arrSettings as $key => $settingExisting){
				$settingName = UELM_UniteFunctionsUC::getVal($settingExisting,"name");
				if($settingName == $name){
		...
		public function modifyValueByDatatypeFunc($value, $datatype){
			...
			switch($datatype){
				...
				case self::DATATYPE_LINK:
					...
					$link = UELM_UniteFunctionsUC::getHrefFromHtml($value);
					if(!empty($link))
						$value = $link;
		...
		public function setStoredValues($arrValues){
			...
			foreach($this->arrSettings as $key=>$setting){
				...
				$datatype = UELM_UniteFunctionsUC::getVal($setting, "datatype");
