...
if ( ! defined( 'ABSPATH' ) ) exit;
	...
	class UELM_UniteCreatorDB{
		...
		public function __construct(){
			...
			$this->pdb = new UELM_UniteProviderDBUC();
		...
		private function throwError($message,$code=-1){
			UELM_UniteFunctionsUC::throwError($message,$code);
		}
		...
		public function delete($table,$where){
			...
			UELM_UniteFunctionsUC::validateNotEmpty($table,"table name");
			UELM_UniteFunctionsUC::validateNotEmpty($where,"where");
		...
		public function update($tableName,$arrData,$where){
			...
			UELM_UniteFunctionsUC::validateNotEmpty($tableName,"table cannot be empty");
			UELM_UniteFunctionsUC::validateNotEmpty($where,"where cannot be empty");
			UELM_UniteFunctionsUC::validateNotEmpty($arrData,"data cannot be empty");
		...
		public function updateRecordsOrdering($table, $arrIDs){
			...
			//get items assoc
			$arrRecords = UELM_UniteFunctionsUC::arrayToAssoc($arrRecords,"id");
			
			$order = 0;
			foreach($arrIDs as $recordID){
				$order++;
				
				$arrRecord = UELM_UniteFunctionsUC::getVal($arrRecords, $recordID);
		...
		public function fetchPage($tableName, $pagingOptions, $where="", $orderField="", $groupByField="", $sqlAddon=""){
			...
			$page = UELM_UniteFunctionsUC::getVal($pagingOptions, "page");
			$rowsInPage = UELM_UniteFunctionsUC::getVal($pagingOptions, "inpage");
			
			
			//valdiate and sanitize
			UELM_UniteFunctionsUC::validateNumeric($page);
			UELM_UniteFunctionsUC::validateNumeric($rowsInPage);
			UELM_UniteFunctionsUC::validateNotEmpty($rowsInPage);
			if($page < 1)
				$page = 1;
		...
		private function initDBTableTitles(){
			...
			$arrTitles = array();
			$arrTitles[UELM_GlobalsUC::$table_addons] = esc_html__("Addon", "unlimited-elements");
			$arrTitles[UELM_GlobalsUC::$table_categories] = esc_html__("Category", "unlimited-elements");
			$arrTitles[UELM_GlobalsUC::$table_layouts] = esc_html__("Page", "unlimited-elements");
		...
		public function fetchSingle($tableName,$where="",$orderField="",$groupByField="",$sqlAddon=""){
			...
			if(is_array($tableName)){
				...
				$tableName = UELM_UniteFunctionsUC::getVal($arguments, "tableName");
				$where = UELM_UniteFunctionsUC::getVal($arguments, "where");
				$orderField = UELM_UniteFunctionsUC::getVal($arguments, "orderField");
				$groupByField = UELM_UniteFunctionsUC::getVal($arguments, "groupByField");
				$sqlAddon = UELM_UniteFunctionsUC::getVal($arguments, "sqlAddon");
				$errorEmpty = UELM_UniteFunctionsUC::getVal($arguments, "errorEmpty");
			}
			...
			if(empty($errorEmpty)){
				...
				$tableTitle = UELM_UniteFunctionsUC::getVal(self::$arrTableTitles, $tableName, __("Record", "unlimited-elements"));
				
				$errorEmpty = $tableTitle." ".__("not found", "unlimited-elements");
			}
		...
		public function getSqlAddonType($addonType, $dbfield = "addontype"){
			...
			if($addonType == UELM_GlobalsUC::ADDON_TYPE_REGULAR_ADDON || $addonType == UELM_GlobalsUC::ADDON_TYPE_LAYOUT_GENERAL)
				$addonType = null;
