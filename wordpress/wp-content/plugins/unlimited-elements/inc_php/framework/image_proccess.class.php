...
if ( ! defined( 'ABSPATH' ) ) exit;
	...
	class UELM_UniteImageViewUC{
		...
		public function __construct(){
			...
			$this->jpg_quality = UELM_GlobalsUC::DEFAULT_JPG_QUALITY;
		...
		private function getThumbFilename(){
			...
			$dirname = UELM_UniteFunctionsUC::getVal($info, "dirname");
			if($dirname == ".")
				$dirname = null;
			...
			switch($this->effect){
				...
				case self::EFFECT_BRIGHTNESS:
					if(defined("IMG_FILTER_BRIGHTNESS")){
						...
						UELM_UniteFunctionsUC::validateNumeric($this->effect_arg1,"'ea1' argument");
	...
	private function effect_Blur(&$gdimg, $radius=0.5) {
		...
		public function makeThumb($filepathImage, $pathThumbs, $maxWidth=-1, $maxHeight=-1, $type=""){
			...
			//validate input
			UELM_UniteFunctionsUC::validateFilepath($filepathImage, "image not found");
			
			UELM_UniteFunctionsUC::validateDir($pathThumbs, "Thumbs folder don't exists.");
			...
			//get filename
			$info = UELM_UniteFunctionsUC::getPathInfo($filepathImage);
			$filename = UELM_UniteFunctionsUC::getVal($info, "basename");
			
			UELM_UniteFunctionsUC::validateNotEmpty($filename, "image filename not given");
		...
		public function strPngToStrJpg($strPng, $quality = 70){
			...
			if(empty($image))
				UELM_UniteFunctionsUC::throwError("can't convert image");
