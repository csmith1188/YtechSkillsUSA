...
class UELM_InstaObjItemUC{
	...
	private function validateInited(){
		...
		if($this->isInited == false)
			UELM_UniteFunctionsUC::throwError("The item is not inited");
	...
	public function getNumCommentsText(){
		...
		$numComments = UELM_HelperInstaUC::convertNumberToText($this->numComments);
		return($numComments);
	}
	...
	public function getNumLikesText(){
		...
		$numLikes = UELM_HelperInstaUC::convertNumberToText($this->numLikes);
		return($numLikes);
	...
	public function getNumVideoViewsText(){
		...
		$numViews = UELM_HelperInstaUC::convertNumberToText($this->videoViews);
	...
	public function getTimePassedText(){
		...
		$timeSinse = UELM_HelperInstaUC::getTimeSince($this->createdDateStamp);
	...
	private function getVal($field){
		...
		$value = UELM_UniteFunctionsUC::getVal($this->item, $field);
	...
	private function parseComments(){
		...
		$this->numComments = UELM_UniteFunctionsUC::getVal($comments, "count");
		
		$commentsData = UELM_UniteFunctionsUC::getVal($comments, "data");
		...
		foreach($commentsData as $comment){
			...
			$objComment = new UELM_InstaObjCommentUC();
	...
	private function parseLikes(){
		...
		$numLikes = UELM_UniteFunctionsUC::getVal($likes, "count");
		if(empty($numLikes))
			$numLikes = 0;
		...
		$likesData = UELM_UniteFunctionsUC::getVal($likes, "data");
	...
	private function parseUser(){
		...
		$this->itemUser = new UELM_InstaObjUserUC();
	...
	private function parseCaption(){
		...
		$this->objCaption = new UELM_InstaObjCommentUC();
	...
	public function init($item){
		...
		$this->locationName = UELM_UniteFunctionsUC::getVal($arrLocation, "name");
		...
		$this->canViewComments = UELM_UniteFunctionsUC::strToBool($canViewComments);
		...
		$this->createdDateText = UELM_HelperInstaUC::stampToDate($this->createdDateStamp);
		...
		switch($this->type){
			...
			break;
			default:
				UELM_UniteFunctionsUC::throwError("Wrong item type: $this->type");
			break;
		}
		...
		if(!empty($this->item))
			UELM_UniteFunctionsUC::throwError("There something else need to be parsed");
	...
	private function initNew_isVideo($item){
		...
		$isVideo = UELM_UniteFunctionsUC::getVal($item, "is_video");
		$isVideo = UELM_UniteFunctionsUC::strToBool($isVideo);
		
		$mediaType = UELM_UniteFunctionsUC::getVal($item, "media_type");
		...
		if(isset($item["video_url"])){
			...
			$this->arrVideos[self::VIDEO_STANDART]["url"] = UELM_UniteFunctionsUC::getVal($item, "video_url");
		}
		
		$this->videoViews = UELM_UniteFunctionsUC::getVal($item, "video_view_count");
	...
	public function initNewAPI($item){
		...
		$this->id = UELM_UniteFunctionsUC::getVal($item, "id");
		$this->code = UELM_UniteFunctionsUC::getVal($item, "code");
		if(empty($this->code))
			$this->code = $this->code = UELM_UniteFunctionsUC::getVal($item, "shortcode");
		
		$commentsDisabled = UELM_UniteFunctionsUC::getVal($item, "comments_disabled");
		$commentsDisabled = UELM_UniteFunctionsUC::strToBool($commentsDisabled);
		
		$this->canViewComments = !$commentsDisabled;
				
		$this->createdDateStamp = UELM_UniteFunctionsUC::getVal($item, "taken_at_timestamp");
				
		if(!empty($this->createdDateStamp))
			$this->createdDateText = UELM_HelperInstaUC::stampToDate($this->createdDateStamp);
		
		$this->captionText = UELM_UniteFunctionsUC::getVal($item, "caption");
		...
		$urlImageNormal = UELM_UniteFunctionsUC::getVal($item, "display_url");
		
		$this->arrImages[self::IMAGE_LOW]["url"] = UELM_UniteFunctionsUC::getVal($item, "thumbnail_src");
		$this->arrImages[self::IMAGE_STANDARD]["url"] = $urlImageNormal;
		
		$arrLikes = UELM_UniteFunctionsUC::getVal($item, "edge_liked_by");
		if(empty($arrLikes))
			$arrLikes = UELM_UniteFunctionsUC::getVal($item, "edge_media_preview_like");
		
		$this->numLikes = UELM_UniteFunctionsUC::getVal($arrLikes, "count"); 
		 		
		//init owner
		$ownerID = UELM_UniteFunctionsUC::getVal($item, "owner");
		if(!empty($ownerID))
			$this->itemUserID = $ownerID;
				
		//get comments
		$arrComments = UELM_UniteFunctionsUC::getVal($item, "comments");
		if(empty($arrComments)){
			$arrComments = UELM_UniteFunctionsUC::getVal($item, "edge_media_to_comment");
		}			
			
		$this->numComments = UELM_UniteFunctionsUC::getVal($arrComments, "count");
		
		$commentsNodes = UELM_UniteFunctionsUC::getVal($arrComments, "nodes");
		if(empty($commentsNodes))
			$commentsNodes = UELM_UniteFunctionsUC::getVal($arrComments, "edges");
		
		if(!empty($commentsNodes)){
			
			$this->objComments = new UELM_InstaObjCommentsUC();
	...
	public function initOfficialAPI($item){
		...
		$urlImage = UELM_UniteFunctionsUC::getVal($item, "media_url");
		
		if($this->type == self::TYPE_VIDEO){
			
			$url = $this->arrVideos[self::VIDEO_STANDART]["url"] = $urlImage;
			
			$urlImage = UELM_UniteFunctionsUC::getVal($item, "thumbnail_url");
			
			$this->urlVideo = UELM_UniteFunctionsUC::getVal($item, "media_url");
		}
		...
		$this->link = UELM_UniteFunctionsUC::getVal($item, "permalink");
		
		$time = UELM_UniteFunctionsUC::getVal($item, "timestamp");
