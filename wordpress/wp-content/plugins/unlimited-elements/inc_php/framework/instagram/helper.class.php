...
class UELM_HelperInstaUC{

	const RENEW_DELAY_SIX_HOURS = 21600;
	const KEY_RENEW_BLOCKED = "uelm_instagram_renew_blocked";
	...
	public static function getInstagramSavedAccessData(){
		...
		$arrData = array();
		$arrData["access_token"] = UELM_UniteFunctionsUC::getVal($settings, "instagram_access_token");
		$arrData["user_id"] = UELM_UniteFunctionsUC::getVal($settings, "instagram_user_id");
		$arrData["username"] = UELM_UniteFunctionsUC::getVal($settings, "instagram_username");
		$arrData["expires"] = UELM_UniteFunctionsUC::getVal($settings, "instagram_expires");
	...
	public static function isAccessTokenExists(){
		...
		$token = UELM_UniteFunctionsUC::getVal($arrData, "access_token");
	...
	public static function redirectToGeneralSettings(){
		...
		$urlRedirect = UELM_HelperUC::getViewUrl(UELM_GlobalsUnlimitedElements::VIEW_SETTINGS_ELEMENTOR,"#tab=instagram");
		UELM_UniteFunctionsUC::redirectToUrl($urlRedirect);
	...
	public static function saveInstagramConnectDataAjax($data, $noUser = false, $redirect = true){
		...
		$userID = UELM_UniteFunctionsUC::getVal($data, "user_id");
		$username = UELM_UniteFunctionsUC::getVal($data, "username");
		$expiresIn = UELM_UniteFunctionsUC::getVal($data, "expires");
		
		self::validateInstance($username);
		
		UELM_UniteFunctionsUC::validateNumeric($expiresIn, "expires in ");

		$expiresAt = time()+$expiresIn;

		$expireDate = UELM_UniteFunctionsUC::timestamp2Date($expiresAt);

		UELM_UniteFunctionsUC::validateNotEmpty($accessToken,"instagram access token");

		if($noUser == false){
			UELM_UniteFunctionsUC::validateNotEmpty($userID,"instagram user id");
			UELM_UniteFunctionsUC::validateNotEmpty($userID,"instagram username");
		}
		...
		UELM_HelperUC::$operations->updateUnlimitedElementsGeneralSettings($arrUpdate);
	...
	public static function getHTMLExpires($expiresAt){
		...
		if($expireDays < 0){
			...
			<span class='unite-color-red'>
				"<?php 
				esc_attr_e("The token has expired ","unlimited-elements");
				echo esc_attr($expireDays);
				esc_attr_e(" ago","unlimited-elements");
				?>"
			</span>
			<?php
		} else {
			esc_attr_e("The token will expire in ","unlimited-elements");
			echo esc_attr($expireDays);
			esc_attr_e(" days. Don't worry, it should auto renew.","unlimited-elements");
		}

		//add renew link
		if($expireDays < 60) {
			$linkRenew = UELM_HelperUC::getUrlAjax("renew_instagram_access_token");
			?>
			<a href="<?php echo esc_url($linkRenew);?>"><?php esc_attr_e("renew access token","unlimited-elements")?></a>
			<?php
		}
	}
	...
	public static function putConnectWithInstagramButton(){
		...
		$urlAuthorize = UELM_InstagramAPIOfficialUC::URL_AUTHORIZE;
		$clientID = UELM_InstagramAPIOfficialUC::APP_CLIENT_ID;
		$urlConnect = UELM_InstagramAPIOfficialUC::URL_APP_CONNECT;

		$urlReturn = UELM_HelperUC::getUrlAjax("save_instagram_connect_data");

		$urlReturn = UELM_UniteFunctionsUC::encodeContent($urlReturn);
		...
		$buttonText = __("Connect With Instagram", "unlimited-elements");

		//put access data as well
		$data = self::getInstagramSavedAccessData();
		$accessToken = UELM_UniteFunctionsUC::getVal($data, "access_token");

		$expiresAt = UELM_UniteFunctionsUC::getVal($data, "expires");
		...
		if(!empty($accessToken)){
			...
			$urlTestView = UELM_HelperUC::getViewUrl("instagram-test");
			$linkTest = UELM_HelperHtmlUC::getHtmlLink($urlTestView, "Test Instagram Data");
			
			$text = __("The instagram access token are already set up", "unlimited-elements");
			
			if(!empty($username)){
				$username = esc_html($username);
				$text .= __(" for user: ", "unlimited-elements")."<b>$username</b>";
			}
			...
			<div id="uc_instagram_reconnect_message" class="instagram-reconnect-message">
				...
				<a id="uc_button_delete_insta_data" href="javascript:void(0)" class="unite-button-secondary"> 
					<?php esc_attr_e("Clear Access Data","unlimited-elements")?>
				</a>
				<br>
				&nbsp;<a href="<?php echo esc_url($urlTestView);?>"><?php esc_attr_e('Test Instagram Data',"unlimited-elements")?></a>
			</div>
			...
			if(UELM_GlobalsUnlimitedElements::$enableInstagramErrorMessage == true){
				?>
	...
	public static function renewAccessToken(){
		...
		$objAPI = new UELM_InstagramAPIOfficialUC();
		$response = $objAPI->renewToken($accessToken);

		$data = array();
		$data["access_token"] = UELM_UniteFunctionsUC::getVal($response, "access_token");
		$data["expires"] = UELM_UniteFunctionsUC::getVal($response, "expires_in");
	...
	public static function checkRenewAccessToken(){
		...
		$expires = UELM_UniteFunctionsUC::getVal($accessData, "expires");
		...
		//$strTime = UELM_UniteFunctionsUC::timestamp2DateTime($expires);
	...
	public static function checkRenewAccessToken_onceInAWhile(){
		...
		$value = UELM_UniteProviderFunctionsUC::getTransient(self::KEY_RENEW_BLOCKED);

		if(!empty($value))
			return(false);

		UELM_UniteProviderFunctionsUC::setTransient(self::KEY_RENEW_BLOCKED, true, self::RENEW_DELAY_SIX_HOURS);
	...
	public static function validateInstance($user, $instance="user"){
		...
		UELM_UniteFunctionsUC::validateNotEmpty($user,"instagram $instance");
		
		if(preg_match('/^[a-zA-Z0-9._]+$/', $user) == false)
			UELM_UniteFunctionsUC::throwError("The instagram $instance is incorrect");
