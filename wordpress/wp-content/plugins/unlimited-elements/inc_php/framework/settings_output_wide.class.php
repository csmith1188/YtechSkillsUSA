...
if ( ! defined( 'ABSPATH' ) ) exit;
	...
	class UELM_UniteSettingsOutputWideUC extends UELM_UniteCreatorSettingsOutput{
		...
		protected function drawSettingRow($setting, $mode = ""){
			...
			//set cellstyle:
			$cellStyle = "";
			if(isset($setting[UELM_UniteSettingsUC::PARAM_CELLSTYLE])){
				$cellStyle .= $setting[UELM_UniteSettingsUC::PARAM_CELLSTYLE];
			}
			...
			$addField = UELM_UniteFunctionsUC::getVal($setting, UELM_UniteSettingsUC::PARAM_ADDFIELD);
			$addFields = array();
			if(!empty($addField)){
			...
			if(!empty($addFields)):
				...
				foreach($addFields as $addFieldName){
					...
					UELM_UniteFunctionsUC::validateNotEmpty($addSetting,"AddSetting {$addFieldName}");
					$addSettings[] = $addSetting;
					
					$addSettingText = UELM_UniteFunctionsUC::getVal($addSetting,"text","");
