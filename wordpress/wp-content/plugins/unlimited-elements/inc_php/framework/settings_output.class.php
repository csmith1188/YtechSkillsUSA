...
class UELM_UniteSettingsOutputUCWork extends UELM_HtmlOutputBaseUC{
	 ...
	 */
	public function init(UELM_UniteSettingsUC $settings){

		if($this->isParent == false)
			UELM_UniteFunctionsUC::throwError("The output class must be parent of some other class.");

		$this->settings = new UELM_UniteSettingsUC();
	...
	protected function validateInited(){
		if(empty($this->settings))
			UELM_UniteFunctionsUC::throwError("The output class not inited. Please call init() function with some settings class");
	}
	...
	public function setShowSaps($show = true, $type = null){
		...
		switch($type){
			...
			break;
			default:
				UELM_UniteFunctionsUC::throwError("Wrong saps type: $type ");
			break;
		}
	...
	protected function getDefaultAddHtml($setting, $implodeArray = false){
		...
		$isResponsive = UELM_UniteFunctionsUC::getVal($setting, "is_responsive");
		$isResponsive = UELM_UniteFunctionsUC::strToBool($isResponsive);
		
		if($isResponsive == true){
			$repsonsiveID = UELM_UniteFunctionsUC::getVal($setting, "responsive_id");
			$repsonsiveType = UELM_UniteFunctionsUC::getVal($setting, "responsive_type");
	...
	protected function drawSettingRow_getText($setting){
		...
		//modify text:
		$text = UELM_UniteFunctionsUC::getVal($setting, "text", "");
	...
	protected function drawSettingRow_getTextStyle($setting){
		...
		//set text style:
		$textStyle = UELM_UniteFunctionsUC::getVal($setting, UELM_UniteSettingsUC::PARAM_TEXTSTYLE);
	...
	protected function drawSettingRow_getRowHiddenClass($setting){
		...
		$type = UELM_UniteFunctionsUC::getVal($setting, "type");
		$isHidden = UELM_UniteFunctionsUC::getVal($setting, "hidden");
		$isHidden = UELM_UniteFunctionsUC::strToBool($isHidden);

		if($this->showSaps === true && $this->sapsType === self::SAPS_TYPE_INLINE){
			$sap = UELM_UniteFunctionsUC::getVal($setting, "sap");
			$sap = (int)$sap;
		...
		if($isHidden === true) {
			if($type === UELM_UniteSettingsUC::TYPE_HIDDEN) {
	...
	protected function drawSettingRow_getRowClass($setting, $basClass = ""){
		...
		$showin = UELM_UniteFunctionsUC::getVal($setting, "showin");
		if(!empty($showin)){
			if(!empty($class))
				$class .= " ";
	...
	*/
	...
	protected function getInputClassAttr($setting, $defaultClass="", $addClassParam="", $wrapClass = true){
		...
		$class = UELM_UniteFunctionsUC::getVal($setting, "class", $defaultClass);
		...
		$isTransparent = UELM_UniteFunctionsUC::getVal($setting, UELM_UniteSettingsUC::PARAM_MODE_TRANSPARENT);
	...
	protected function drawLinkInput($setting){
		...
		$urlValue = UELM_UniteFunctionsUC::getVal($value, "url");

		$externalId = "$id-external";
		$externalChecked = UELM_UniteFunctionsUC::getVal($value, "is_external");
		$externalChecked = $externalChecked === "on";

		$nofollowId = "$id-nofollow";
		$nofollowChecked = UELM_UniteFunctionsUC::getVal($value, "nofollow");
		$nofollowChecked = $nofollowChecked === "on";

		$attributesId = "$id-attributes";
		$attributesValue = UELM_UniteFunctionsUC::getVal($value, "custom_attributes");
		...
		<div class="unite-setting-link-wrapper">
			<div class="unite-setting-link-field">
				<input
					...
					placeholder="<?php esc_attr_e("Search or enter URL", "unlimited-elements"); ?>"
				...
				<button
					...
					type="button"
					title="<?php esc_attr_e("Options", "unlimited-elements"); ?>"
				>
				...
				<div class="unite-setting-link-option">
					<div class="unite-setting-link-checkbox">
						...
						<label for="<?php echo esc_attr($externalId); ?>">
							<?php esc_html_e("Open in new window", "unlimited-elements"); ?>
						</label>
					</div>
					<div class="unite-setting-link-checkbox">
						<input
						...
						<label for="<?php echo esc_attr($nofollowId); ?>">
							<?php esc_html_e("Add nofollow", "unlimited-elements"); ?>
						</label>
					</div>
				</div>
				<div class="unite-setting-link-option">
					<div class="unite-setting-text-wrapper">
						<div class="unite-setting-text">
							<?php esc_html_e("Custom attributes", "unlimited-elements"); ?>
						</div>
					</div>
					<input
					...
					<div class="unite-setting-helper">
						<?php esc_html_e("Set custom attributes for the link element. Separate attribute keys from values using the | (pipe) character. Separate key-value pairs with a comma.", "unlimited-elements"); ?>
					</div>
				</div>
			</div>
	...
	protected function modifyImageSetting($setting){
		...
		$urlBase = UELM_UniteFunctionsUC::getVal($setting, "url_base", null);
		...
		foreach($keys as $key){
			...
			if(empty($value) === false && is_numeric($value) === false)
				$value = UELM_HelperUC::URLtoFull($value, $urlBase);
	...
	protected function drawImageInput($setting){
		...
		$name = UELM_UniteFunctionsUC::getVal($setting, "name");
		$value = UELM_UniteFunctionsUC::getVal($setting, "value");
		$title = UELM_UniteFunctionsUC::getVal($setting, "text");
		$source = UELM_UniteFunctionsUC::getVal($setting, "source");
		$error = UELM_UniteFunctionsUC::getVal($setting, "error");
		...
		$imageId = UELM_UniteFunctionsUC::getVal($value, "id");
		$imageUrl = UELM_UniteFunctionsUC::getVal($value, "url");
		$imageSize = UELM_UniteFunctionsUC::getVal($value, "size", "full");

		if(empty($imageId) === false)
			$imageUrl = UELM_UniteProviderFunctionsUC::getImageUrlFromImageID($imageId, $imageSize);
		else
			$imageUrl = UELM_HelperUC::URLtoFull($imageUrl);
		...
		$urlTitle = sprintf(__("%s URL", "unlimited-elements"), $title);
		$urlName = UELM_UniteFunctionsUC::getVal($setting, "url_name");
        $urlValue = $imageUrl;

		$sizes = UELM_UniteFunctionsWPUC::getArrThumbSizes();
		$sizeName = UELM_UniteFunctionsUC::getVal($setting, "size_name");
		// translators: %s is a string
		$sizeTitle = sprintf(__("%s Size", "unlimited-elements"), $title);
		$sizeValue = $imageSize;
	...
	protected function drawMp3Input($setting){
		...
		?>
			<div class="unite-setting-mp3">
				...
				?> />
				<a href="javascript:void(0)" class="unite-button-secondary unite-button-choose"><?php esc_html_e("Choose", "unlimited-elements")?></a>
			</div>
		<?php
	}
	...
	protected function drawIconPickerInput($setting){
		...
		$name = UELM_UniteFunctionsUC::getVal($setting, "name");
		$value = UELM_UniteFunctionsUC::getVal($setting, "value");
		$iconsType = UELM_UniteFunctionsUC::getVal($setting, "icons_type");
		$enableSvg = UELM_UniteFunctionsUC::getVal($setting, "enable_svg");
		$enableSvg = UELM_UniteFunctionsUC::strToBool($enableSvg);

		if(empty($iconsType) === true || $iconsType === "fa"){
			$setting["value"] = UELM_UniteFontManagerUC::fa_convertIcon($value);
			$value = $setting["value"];
		}
		...
		<div class="unite-iconpicker">
			<div class="unite-setting-buttons-group">
				<div class="unite-iconpicker-button unite-setting-button uc-tip" title="<?php esc_attr_e("None", "unlimited-elements"); ?>" data-action="none">
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 12 12">
						<path d="m2.111 9.889 7.778-7.778" />
						<path d="M6 11.5a5.5 5.5 0 1 0 0-11 5.5 5.5 0 0 0 0 11Z" />
					</svg>
				</div>
				<div class="unite-iconpicker-button unite-setting-button uc-tip" title="<?php esc_attr_e("Upload SVG", "unlimited-elements"); ?>" data-action="upload">
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 12 12">
				...
				<div class="unite-iconpicker-button unite-setting-button uc-tip" title="<?php esc_attr_e("Icon Library", "unlimited-elements"); ?>" data-action="library">
	...
	protected function drawAddonPickerInput($setting){
		...
		$addonType = UELM_UniteFunctionsUC::getVal($setting, "addontype");
		$objAddonType = UELM_UniteCreatorAddonType::getAddonTypeObject($addonType);
		if(empty($addonType))
			$addonType = UELM_GlobalsUC::ADDON_TYPE_REGULAR_ADDON;
		...
		$textSelect = __("Select ", "unlimited-elements").$objAddonType->textSingle;
		...
		//add data holder
		$addDataHolder = UELM_UniteFunctionsUC::getVal($setting, "add_data_holder");
		$addDataHolder = UELM_UniteFunctionsUC::strToBool($addDataHolder);

		$addClearButton = UELM_UniteFunctionsUC::getVal($setting, "add_clear_button");
		$addClearButton = UELM_UniteFunctionsUC::strToBool($addClearButton);

		$addConfigureButton = UELM_UniteFunctionsUC::getVal($setting, "add_configure_button");
		$addConfigureButton = UELM_UniteFunctionsUC::strToBool($addConfigureButton);

		$configureButtonAction = UELM_UniteFunctionsUC::getVal($setting, "configure_button_action");
		...
		?>
				...
				<input type="hidden" id="<?php echo esc_attr($setting["id"])?>_data" name="<?php echo esc_attr($setting["name"])?>_data" value="" />
					...
					<a href="javascript:void(0)" class="unite-button-secondary uc-action-button" data-action="clear" ><?php esc_html_e("Clear", "unlimited-elements")?></a>
					<?php endif?>

					<?php if($addConfigureButton == true):?>
					<a href="javascript:void(0)" class="unite-button-secondary uc-action-button" data-action="configure" data-configureaction="<?php echo esc_attr($configureButtonAction)?>" ><?php esc_html_e("Configure", "unlimited-elements")?></a>
					<?php endif?>
	...
	private function getGroupSelectorAddAttr($setting){
		...
		$selectorName = UELM_UniteFunctionsUC::getVal($setting, "group_selector");

		if(empty($selectorName))
			return null;

		$selectorName = is_array($selectorName) === false ? [$selectorName] : $selectorName;
		$selectorName = UELM_UniteFunctionsUC::jsonEncodeForHtmlData($selectorName);
	...
	private function getSelectorsAddAttr($setting){
		...
		$selector = UELM_UniteFunctionsUC::getVal($setting, "selector");
		$selector1 = UELM_UniteFunctionsUC::getVal($setting, "selector1");
		...
		$strAttr = UELM_UniteFunctionsUC::jsonEncodeForHtmlData($arrData);
	...
	protected function drawGallerySetting($setting){
		...
		>
			<div
				...
				data-text-none="<?php esc_attr_e("No images selected", "unlimited-elements"); ?>"
				data-text-one="<?php 
				// translators: %d is a number
				esc_attr_e("%d image selected", "unlimited-elements"); 
				?>"
				data-text-default="<?php 
				// translators: %d is a number
				esc_attr_e("%d images selected", "unlimited-elements"); 
				?>"
			>
				<?php esc_html_e("No images selected", "unlimited-elements"); ?>
			</div>
			<div class="unite-setting-gallery-content">
				<div class="unite-setting-gallery-empty">
					<button class="unite-button-primary unite-setting-gallery-add">
						<?php esc_html_e("Add Images", "unlimited-elements"); ?>
					</button>
				</div>
				...
				<div class="unite-setting-gallery-actions">
					<button
						...
						data-text="<?php esc_attr_e("Are you sure you want to clear the gallery?", "unlimited-elements"); ?>"
					>
						<?php esc_html_e("Clear Gallery", "unlimited-elements"); ?>
					</button>
					<button class="unite-setting-gallery-action unite-setting-gallery-edit">
						<?php esc_html_e("Edit Gallery", "unlimited-elements"); ?>
					</button>
				</div>
			</div>
		</div>
	...
	protected function drawMapPickerInput($setting){
		...
		$dialogTitle = esc_html__("Select Map","unlimited-elements");

		$filepathPickerObject = UELM_GlobalsUC::$pathViewsObjects."mappicker_view.class.php";
		require_once $filepathPickerObject;

		$objPicker = new UELM_UniteCreatorMappickerView();
		$objPicker->setData($value);

		$strMapData = UELM_UniteFunctionsUC::jsonEncodeForHtmlData($value, "mapdata");
	...
	protected function drawPostPickerInput($setting){
		uelm_dmp("drawPostPickerInput: function for override");
		exit();
	}
	...
	protected function drawModulePickerInput($setting){
		uelm_dmp("drawModulePickerInput: function for override");
		exit();
	}
	...
	protected function drawColorPickerInput($setting){
		...
		$pickerType = UELM_GlobalsUC::$colorPickerType;
		...
		if($pickerType == "farbtastic"){
			...
			$rgb = UELM_UniteFunctionsUC::html2rgb($bgcolor);
			$bw = UELM_UniteFunctionsUC::yiq($rgb[0],$rgb[1],$rgb[2]);
	...
	protected function drawEditorInput($setting){
		...
		uelm_dmp("provider settings output - function to override");
		exit();
	}
	...
	protected function drawFontsPanel($setting){
		...
		uelm_dmp("draw fonts panel - function for override");
		exit();
	}
	...
	protected function drawItemsPanel($setting){
		...
		uelm_dmp("draw items panel - function for override");
		exit();
	}
	...
	protected function drawButtonsGroupSetting($setting){
		...
		$deselectable = UELM_UniteFunctionsUC::getVal($setting, "deselectable");
		$deselectable = UELM_UniteFunctionsUC::strToBool($deselectable);
	...
	protected function drawGroupSelectorSetting($setting){
		...
		$id = UELM_UniteFunctionsUC::getVal($setting, "id");
		$name = UELM_UniteFunctionsUC::getVal($setting, "name");
		$selectorReplace = UELM_UniteFunctionsUC::getVal($setting, "selector_replace");
	...
	protected function drawInputs($setting){
		...
		switch($setting["type"]){
			case UELM_UniteSettingsUC::TYPE_TEXT:
				$this->drawTextInput($setting);
			break;
			case UELM_UniteSettingsUC::TYPE_DATETIME:
				$this->drawDateTimeInput($setting);
			break;
			case UELM_UniteSettingsUC::TYPE_COLOR:
				$this->drawColorPickerInput($setting);
			break;
			case UELM_UniteSettingsUC::TYPE_SELECT:
				$this->drawSelectInput($setting);
			break;
			case UELM_UniteSettingsUC::TYPE_MULTISELECT:
				$this->drawMultiSelectInput($setting);
			break;
			case UELM_UniteSettingsUC::TYPE_CHECKBOX:
				$this->drawCheckboxInput($setting);
			break;
			case UELM_UniteSettingsUC::TYPE_RADIO:
				$this->drawRadioInput($setting);
			break;
			case UELM_UniteSettingsUC::TYPE_TEXTAREA:
				$this->drawTextAreaInput($setting);
			break;
			case UELM_UniteSettingsUC::TYPE_LINK:
				$this->drawLinkInput($setting);
			break;
			case UELM_UniteSettingsUC::TYPE_IMAGE:
				$this->drawImageInput($setting);
			break;
			case UELM_UniteSettingsUC::TYPE_MP3:
				$this->drawMp3Input($setting);
			break;
			case UELM_UniteSettingsUC::TYPE_ICON:
				$this->drawIconPickerInput($setting);
			break;
			case UELM_UniteSettingsUC::TYPE_ADDON:
				$this->drawAddonPickerInput($setting);
			break;
			case UELM_UniteSettingsUC::TYPE_MAP:
				$this->drawMapPickerInput($setting);
			break;
			case UELM_UniteSettingsUC::TYPE_POST:
				$this->drawPostPickerInput($setting);
			break;
			case UELM_UniteSettingsUC::TYPE_EDITOR:
				$this->drawEditorInput($setting);
			break;
			case UELM_UniteCreatorSettings::TYPE_FONT_PANEL:
				$this->drawFontsPanel($setting);
			break;
			case UELM_UniteCreatorSettings::TYPE_ITEMS:
				$this->drawItemsPanel($setting);
			break;
			case UELM_UniteCreatorSettings::TYPE_BUTTON:
				$this->drawButtonInput($setting);
			break;
			case UELM_UniteCreatorSettings::TYPE_RANGE:
				$this->drawRangeSliderInput($setting);
			break;
			case UELM_UniteCreatorSettings::TYPE_HIDDEN:
				$this->drawHiddenInput($setting);
			break;
			case UELM_UniteCreatorSettings::TYPE_REPEATER:
			...
			break;
			case UELM_UniteSettingsUC::TYPE_SWITCHER:
				$this->drawSwitcherSetting($setting);
			break;
			case UELM_UniteCreatorSettings::TYPE_DIMENTIONS:
				$this->drawDimentionsSetting($setting);
			break;
			case UELM_UniteCreatorSettings::TYPE_GALLERY:
				$this->drawGallerySetting($setting);
			break;
			case UELM_UniteCreatorSettings::TYPE_BUTTONS_GROUP:
				$this->drawButtonsGroupSetting($setting);
			break;
			case UELM_UniteCreatorSettings::TYPE_TABS:
				$this->drawTabsSetting($setting);
			break;
			case UELM_UniteCreatorSettings::TYPE_GROUP_SELECTOR:
				$this->drawGroupSelectorSetting($setting);
			break;
			case UELM_UniteSettingsUC::TYPE_CUSTOM:
				if(method_exists($this,"drawCustomInputs") == false){
					UELM_UniteFunctionsUC::throwError("Method don't exists: drawCustomInputs, please override the class");
				}
	...
	protected function drawRangeSliderInput($setting){
		...
		$name = UELM_UniteFunctionsUC::getVal($setting, "name");
		$defaultValue = UELM_UniteFunctionsUC::getVal($setting, "default_value");
		$value = UELM_UniteFunctionsUC::getVal($setting, "value", $defaultValue);
		$min = UELM_UniteFunctionsUC::getVal($setting, "min", 0);
		$max = UELM_UniteFunctionsUC::getVal($setting, "max", 0);
		$step = UELM_UniteFunctionsUC::getVal($setting, "step", 1);
		$units = UELM_UniteFunctionsUC::getVal($setting, "units");
		$unit = empty($units) === false ? reset($units) : "px";
		$showSlider = UELM_UniteFunctionsUC::getVal($setting, "show_slider", true);
		$showSlider = UELM_UniteFunctionsUC::strToBool($showSlider);
	...
	protected function drawRepeaterInput($setting){
		...
		$itemsValues = UELM_UniteFunctionsUC::getVal($setting, "items_values");
		$strData = UELM_UniteFunctionsUC::jsonEncodeForHtmlData($itemsValues, "itemvalues");
		$addItemText = UELM_UniteFunctionsUC::getVal($setting, "add_button_text");

		if(empty($addItemText))
			$addItemText = esc_html__("Add Item", "unlimited-elements");

		//get empty text
		$emptyText = UELM_UniteFunctionsUC::getVal($setting, "empty_text");

		if(empty($emptyText))
			$emptyText = esc_html__("No items found.", "unlimited-elements");

		$objSettingsItems = UELM_UniteFunctionsUC::getVal($setting, "settings_items");

		UELM_UniteFunctionsUC::validateNotEmpty($objSettingsItems, "settings items");
		...
		if($this->isSidebar === true){
			$output = new UELM_UniteSettingsOutputSidebarUC();
		...
		//get item title
		$itemTitle = UELM_UniteFunctionsUC::getVal($setting, "item_title");

		if(empty($itemTitle))
			$itemTitle = esc_html__("Item", "unlimited-elements");

		$itemTitle = htmlspecialchars($itemTitle);

		//delete button text
		$deleteButtonText = UELM_UniteFunctionsUC::getVal($setting, "delete_button_text");

		if(empty($deleteButtonText))
			$deleteButtonText = esc_html__("Delete", "unlimited-elements");

		$duplicateButtonText = UELM_UniteFunctionsUC::getVal($setting, "duplicate_button_text");

		if(empty($duplicateButtonText))
			$duplicateButtonText = esc_html__("Duplicate", "unlimited-elements");
	...
	protected function drawTextInput($setting){
		...
		$name = UELM_UniteFunctionsUC::getVal($setting, "name");
		$value = UELM_UniteFunctionsUC::getVal($setting, "value");
		$placeholder = UELM_UniteFunctionsUC::getVal($setting, "placeholder");
		$style = UELM_UniteFunctionsUC::getVal($setting, "style");
		$disabled = UELM_UniteFunctionsUC::getVal($setting, "disabled");
		$readonly = UELM_UniteFunctionsUC::getVal($setting, "readonly");
		$unit = UELM_UniteFunctionsUC::getVal($setting, "unit");
		$units = UELM_UniteFunctionsUC::getVal($setting, "units");
		$step = UELM_UniteFunctionsUC::getVal($setting, "step");
		$typeNumber = UELM_UniteFunctionsUC::getVal($setting, "type_number");
		$typeNumber = UELM_UniteFunctionsUC::strToBool($typeNumber);
		$typePassword = UELM_UniteFunctionsUC::getVal($setting, "ispassword");
		$typePassword = UELM_UniteFunctionsUC::strToBool($typePassword);
	...
	protected function drawButtonInput($setting){
		...
		$gotoView = UELM_UniteFunctionsUC::getVal($setting, "gotoview");

		if(!empty($gotoView))
			$href = UELM_HelperUC::getViewUrl($gotoView);

		$url = UELM_UniteFunctionsUC::getVal($setting, "url");

		if(!empty($url))
			$href = $url;

		$isNewWindow = UELM_UniteFunctionsUC::getVal($setting, "newwindow");
		?>
	...
	protected function drawTextAreaInput($setting){
		...
		$rows = UELM_UniteFunctionsUC::getVal($setting, "rows");
		if(!empty($rows))
			$rows = "rows='$rows'";

		$cols = UELM_UniteFunctionsUC::getVal($setting, "cols");
		if(!empty($cols))
			$cols = "cols='$cols'";
	...
	protected function drawRadioInput($setting){
		...
		$defaultValue = UELM_UniteFunctionsUC::getVal($setting, "default_value");
		$disabled = UELM_UniteFunctionsUC::getVal($setting, "disabled");
		$disabled = UELM_UniteFunctionsUC::strToBool($disabled);
	...
	protected function drawCheckboxInput($setting){
		...
		if($value == true)
			...
			$textNear = UELM_UniteFunctionsUC::getVal($setting, "text_near");
		...
		if(!empty($textNear)){
			...
			if($this->showDescAsTips == true){
				$description = UELM_UniteFunctionsUC::getVal($setting, "description");
		...
		$defaultValue = UELM_UniteFunctionsUC::getVal($setting, "default_value");
		$defaultValue = UELM_UniteFunctionsUC::strToBool($defaultValue);
	...
	protected function drawSelectInput($setting){
		...
		$isPostSelect = UELM_UniteFunctionsUC::getVal($setting, "post_select");
		$isPostSelect = UELM_UniteFunctionsUC::strToBool($isPostSelect);
		...
		$disabled = UELM_UniteFunctionsUC::getVal($setting, "disabled");
		$disabled = UELM_UniteFunctionsUC::strToBool($disabled);
		$items = UELM_UniteFunctionsUC::getVal($setting, "items", array());
		$items = empty($items) === false ? $items: array();
		$args = UELM_UniteFunctionsUC::getVal($setting, "args");
	...
	protected function drawInputAdditions($setting,$showDescription = true){
		...
		if($showDescription === false)
			$description = "";
		$unit = UELM_UniteFunctionsUC::getVal($setting, "unit");
		$required = UELM_UniteFunctionsUC::getVal($setting, "required");
		$addHtml = UELM_UniteFunctionsUC::getVal($setting, UELM_UniteSettingsUC::PARAM_ADDTEXT);
	...
	* set form id
	...
	public function setFormID($formID){
		...
		if(isset(self::$arrIDs[$formID]))
			UELM_UniteFunctionsUC::throwError("Can't output settings with the same ID: $formID");

		self::$arrIDs[$formID] = true;

		UELM_UniteFunctionsUC::validateNotEmpty($formID, "formID");
	...
	private function drawSubSettings($setting){
		...
		if($this->isSidebar === false){
			uelm_dmp("the attribute will be available in elementor");
		...
		>
			<button
				...
				type="button"
				title="<?php esc_attr_e("Reset", "unlimited-elements"); ?>"
			>
				<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 12 12">
	...
	private function drawSubSettingsDialogs(){
		...
		foreach($this->subSettingsDialogs as $type){
			...
			<div
				...
				data-id="<?php echo esc_attr($type, "unlimited-elements"); ?>"
			>
	...
	public function drawWrapperStart(){
		...
		UELM_UniteFunctionsUC::validateNotEmpty($this->settingsMainClass, "settings main class not found, please use wide, inline or sidebar output");
		...
		if(!empty($this->addCss)) {
			UELM_UniteProviderFunctionsUC::printCustomStyle($this->addCss, true);
		}
	...
	public function draw($formID, $drawForm = false){
		...
		if(empty($this->settings))
			UELM_UniteFunctionsUC::throwError("No settings are inited. Please init the settings in output class");
		...
		if($drawForm == true){
			...
			if(empty($formID))
				UELM_UniteFunctionsUC::throwError("The form ID can't be empty. you must provide it");
	...
	*/
	...
	protected function drawSapsTabs(){
		...
		<div class="unite-settings-tabs">
			...
			<?php foreach($arrSaps as $key=>$sap){
				$text = $sap["text"];
				UELM_UniteFunctionsUC::validateNotEmpty($text,"sap $key text");
	...
	private function drawSettingsRowByType($setting, $mode){
		switch($setting["type"]){
			case UELM_UniteSettingsUC::TYPE_HR:
				$this->drawHrRow($setting);
			break;
			case UELM_UniteSettingsUC::TYPE_STATIC_TEXT:
			case UELM_UniteSettingsUC::TYPE_HTML:
	...
	private function drawSettings_settings($filterSapID = null, $mode = null, $arrSettings = null){
		...
		foreach($arrSettings as $setting){
			if(isset($setting[UELM_UniteSettingsUC::PARAM_NODRAW])) {
                continue;
            }
			if($filterSapID !== null){
				$sapID = UELM_UniteFunctionsUC::getVal($setting, "sap");
	...
	private function drawSettings_saps_accordion_tabs(){
		...
		<div class="unite-settings-accordion-saps-tabs">
			<a href="javascript:void(0)" class="unite-settings-tab unite-active" data-id="content"><?php esc_attr_e("Content","unlimited-elements"); ?></a>
			<a href="javascript:void(0)" class="unite-settings-tab" data-id="style"><?php esc_attr_e("Style","unlimited-elements"); ?></a>
			<a href="javascript:void(0)" class="unite-settings-tab" data-id="advanced"><?php esc_attr_e("Advanced","unlimited-elements"); ?></a>
		</div>
		<?php
	}
	...
	private function drawSettings_saps($filterSapID = null, $mode=null){
        ...
        foreach($arrSaps as $key=>$sap){
            ...
            $tab = UELM_UniteFunctionsUC::getVal($sap, "tab");

            if($tab == UELM_UniteSettingsUC::TAB_STYLE)
                $isHasStyleTab = true;
        }
        ...
        foreach($arrSaps as $key=>$sap):
            ...
            $arrSettings = UELM_UniteFunctionsUC::getVal($sap, "settings");

            $nodraw = UELM_UniteFunctionsUC::getVal($sap, "nodraw");
            if($nodraw === true)
                continue;
	...
	public function drawSettings($filterSapID = null){
		...
		//set special mode
		$mode = "";
		if(count($arrSettings) == 1 && $arrSettings[0]["type"] == UELM_UniteSettingsUC::TYPE_EDITOR)
				$mode = "single_editor";
