...
class UELM_UniteZipUC{
	...
	public function makeZip($srcPath, $zipFilepath, $additionPaths = array()){
		...
		if(!is_dir($srcPath))
			UELM_UniteFunctionsUC::throwError("The path: '$srcPath' don't exists, can't zip");
		...
		if($this->isZipArchiveExists === true){
			...
			if($success === false)
				UELM_UniteFunctionsUC::throwError("Can't create zip file: $zipFilepath");
		}else{
			$this->contents = array();
			$this->ctrldir = array();
		}
		...
		if(gettype($additionPaths) != "array")
			UELM_UniteFunctionsUC::throwError("Wrong additional paths variable.");

		//add additional paths
		if(!empty($additionPaths))
			foreach($additionPaths as $path){
				if(!is_dir($path))
					UELM_UniteFunctionsUC::throwError("Path: $path not found, can't zip");
	...
	public function extract($src, $dest){
		...
		if($this->isZipArchiveExists() === true){
			...
			UELM_UniteFunctionsUC::throwError("Can't extract zip: $filename");
		}
	...
	private function addItem($basePath, $path){
		...
		if(is_dir($path)){    //directory
			...
			if(!file_exists($path))
				UELM_UniteFunctionsUC::throwError("filepath: '$path' don't exists, can't zip");
	...
	private function checkCreateDir($filepath){
		...
		UELM_UniteFunctionsUC::mkdir($dir);
		if(is_dir($dir) == false)
			UELM_UniteFunctionsUC::throwError("Can't create directory: {$dir} maybe zip file is brocken");
	}
	...
	private function writeFile($str, $filepath){
		...
		UELM_UniteFunctionsUC::filePutContents($filepath, $str);

		if(UELM_UniteFunctionsUC::fileExists($filepath) == false)
			UELM_UniteFunctionsUC::throwError("can't write file: $filepath");
	}
		...
		if($result !== true){
			switch($result){
				case ZipArchive::ER_NOZIP:
					UELM_UniteFunctionsUC::throwError('not a zip archive');
				case ZipArchive::ER_INCONS :
					UELM_UniteFunctionsUC::throwError('consistency check failed');
				case ZipArchive::ER_CRC :
					UELM_UniteFunctionsUC::throwError('checksum failed');
				default:
					UELM_UniteFunctionsUC::throwError('error ' . $result);
			}
		}
	...
	private function addFileToCustomZip($path, $rel_path){
		...
		if(is_file($path) == false)
			UELM_UniteFunctionsUC::throwError("can't add to zip file: $path");

		$content = UELM_UniteFunctionsUC::fileGetContents($path);
		$time = filemtime($path);
	...
	private function _createZIPFile(array &$contents, array &$ctrlDir, $path){
		...
		UELM_UniteFunctionsUC::writeFile($buffer, $path);
	...
	private function extract_custom($src, $dest){
		...
		if(!extension_loaded('zlib'))
			UELM_UniteFunctionsUC::throwError('Zlib not supported, please enable in php.ini');

		$this->_data = UELM_UniteFunctionsUC::fileGetContents($src);
		if(!$this->_data)
			UELM_UniteFunctionsUC::throwError('Get ZIP Data failed');

		$success = $this->extract_custom_readZipInfo($this->_data);
		if(!$success)
			UELM_UniteFunctionsUC::throwError('Get ZIP Information failed');
		...
		for($i = 0, $n = count($this->_metadata); $i < $n; $i++){
			...
			if($lastPathCharacter !== '/' && $lastPathCharacter !== '\\'){
				...
				$destFilepath = UELM_UniteFunctionsUC::cleanPath($dest . '/' . $this->_metadata[$i]['name']);
	...
	private function extract_custom_readZipInfo(&$data){
		...
		do{
			if($dataLength < $fhStart + 31){
				UELM_UniteFunctionsUC::throwError('Invalid Zip Data');
			}
