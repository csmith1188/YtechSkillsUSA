...
if ( ! defined( 'ABSPATH' ) ) exit;
	...
	class UELM_UniteSettingsAdvancedUC extends UELM_UniteSettingsUC{
		...
		public function addRadioBoolean($name, $text, $defaultValue = true, $trueText = "Yes", $falseText = "No", $params = array()){
			...
			$trueValue = UELM_UniteFunctionsUC::getVal($params, "true_value", true);
			$falseValue = UELM_UniteFunctionsUC::getVal($params, "false_value", false);
		...
		public function updateSelectToAlignHor($name, $default = null){
			...
			$arrItems = array(
					...
					"center"=>__("Center", "unlimited-elements"),
		...
		public function updateSelectToAlignVert($name, $default = null){
			...
			$arrItems = array(
					...
					"middle"=>__("Middle", "unlimited-elements"),
		...
		protected function addContentSelectorField($option, $name, $arrDefaultValues){
			uelm_dmp("addContentSelectorFields: function for override");
			exit();
		}
		...
		public function addContentSelector($name, $defaultValue = "",$text = "",$arrParams = array()){
			...
			if(is_array($defaultValue))
			    $radioValue = UELM_UniteFunctionsUC::getVal($defaultValue, $name);
			...
			foreach($arrOptions as $text=>$option){
				...
				switch($option){
					case "custom":
						...
						$params = array(
								UELM_UniteSettingsUC::PARAM_CLASSADD=>"unite-content-content");

						if(is_string($defaultValue))
						    $value = $defaultValue;
						else
						  $value = UELM_UniteFunctionsUC::getVal($defaultValue, $name."_content");

						$this->addEditor($name."_content", $value, __("&nbsp;Custom Content","unlimited-elements"), $params);
