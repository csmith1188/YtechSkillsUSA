 ...
 *
 */
class UELM_UniteTableUC extends UELM_HtmlOutputBaseUC{
	...
	private function validatePaging(){
		...
		if($this->isPaging == false)
			UELM_UniteFunctionsUC::throwError("The paging should be available");
	...
	private function validateTopActions(){
		...
		if($this->isInsideTopActions == false)
			UELM_UniteFunctionsUC::throwError("The top actions form should be started");
	...
	private function getPageFromGet(){
		...
		$page = UELM_UniteFunctionsUC::getGetVar(self::GET_FIELD_PAGE,1,UELM_UniteFunctionsUC::SANITIZE_ID);
		$page = (int)$page;
	...
	private function getInPageFromGet(){
		$inpage = UELM_UniteFunctionsUC::getGetVar(self::GET_FIELD_INPAGE, $this->defaultInPage, UELM_UniteFunctionsUC::SANITIZE_ID);
		$inpage = (int)$inpage;
	...
	private function getCategoryFromGet(){
		...
		$cat = UELM_UniteFunctionsUC::getGetVar(self::GET_FIELD_CATEGORY, "", UELM_UniteFunctionsUC::SANITIZE_KEY);
	...
	private function getSearchValueFromGet(){
		...
		$search = UELM_UniteFunctionsUC::getGetVar(self::GET_FIELD_SEARCH, "", UELM_UniteFunctionsUC::SANITIZE_TEXT_FIELD);
	...
	private function getOrderValueFromGet(){
		...
		$order = UELM_UniteFunctionsUC::getGetVar(self::GET_FIELD_ORDER, "", UELM_UniteFunctionsUC::SANITIZE_TEXT_FIELD);
	...
	private function getArrFieldsValuesFromGet($fieldNames, $exceptField = null){
		...
		foreach($fieldNames as $name){
			...
			$fieldValue = UELM_UniteFunctionsUC::getGetVar($name, "", UELM_UniteFunctionsUC::SANITIZE_TEXT_FIELD);
			if(!empty($fieldValue))
	...
	private function getUrlPage($page = null,$exceptField=null){
		...
		$urlPage = UELM_UniteFunctionsUC::addUrlParams($this->baseUrl, $arrGetFields);
	...
	public function setPagingData($baseURl, $data){
		...
		$this->total = UELM_UniteFunctionsUC::getVal($data, "total");
		$this->page = UELM_UniteFunctionsUC::getVal($data, "page");
		$this->inPage = UELM_UniteFunctionsUC::getVal($data, "inpage");
		$this->numPages = UELM_UniteFunctionsUC::getVal($data, "num_pages");
	
		UELM_UniteFunctionsUC::validateNotEmpty($this->inPage, "in page");
		if($this->total > 0){
			UELM_UniteFunctionsUC::validateNotEmpty($this->page, "page");
			UELM_UniteFunctionsUC::validateNotEmpty($this->numPages, "num pages");
		}
	...
	private function arrFieldsToHtmlHiddenInputs($arrGetFields){
		...
		foreach($arrGetFields as $name=>$value)
			$html .= self::TAB3.UELM_HelperHtmlUC::getHiddenInputField($name, $value).self::BR;
	...
	public function getHTMLInpageSelect(){
		...
		$htmlSelect = UELM_HelperHtmlUC::getHTMLSelect($arrNumbers, $inpage, "name='{$fieldInpage}' class='unite-tableitems-selectrecords' onchange='this.form.submit()'");
	...
	public function putFilterCategory(){
		...
		$htmlSelect = UELM_HelperHtmlUC::getHTMLSelect($arrCats, $cat, "name='{$fieldCat}' class='unite-tableitems-category' onchange='this.form.submit()'", true);
		$htmlGetFields = $this->getHtmlHiddenInputs($fieldCat);
		
		$html = "<span class='uc-table-top-filter-title'>".esc_html__("Filter Category", "unlimited-elements")."</span>";
	...
	private function getPaginationHtml(){
		...
		if($total_pages > 0 && $total_pages != 1 && $current_page <= $total_pages){ //verify total pages and current page number
			...
			if($current_page > 1 && $isShowExtras == true){
				...
				$titleFirst = esc_html__("First", "unlimited-elements");
				$titlePrev = esc_html__("Previous", "unlimited-elements");
			...
			if($current_page < $total_pages && $isShowExtras == true){
				...
				$titleNext = esc_html__("Next Page", "unlimited-elements");
				$titleLast = esc_html__("Last Page", "unlimited-elements");
	...
	public function putSearchForm($buttonText = "", $clearText = "", $putByDefault = true){
		...
		if(empty($buttonText))
			$buttonText = esc_html__("Search", "unlimited-elements");
		
		if(empty($clearText))
			$clearText = esc_html__("Clear", "unlimited-elements");
	...
	public function putTableOrderHeader($name, $text){
		...
		$link = UELM_UniteFunctionsUC::addUrlParams($link, self::GET_FIELD_ORDER."=".$orderForLink);
