...
class UELM_UniteFunctionsUC{
	...
	public static function showObjData($obj){
		...
		uelm_dmp("Vars: ");
		uelm_dmp(get_object_vars($obj));

		uelm_dmp("Methods:");
		uelm_dmp(get_class_methods($obj));
	...
	public static function getPostGetVariable($name, $initVar = "", $sanitizeType = ""){
		...
		$var = UELM_UniteProviderFunctionsUC::sanitizeVar($var, $sanitizeType);
	...
	public static function filterArrayByKeyValue($arr,$key,$val){
		...
		foreach($arr as $item){
			...
			$itemVal = UELM_UniteFunctionsUC::getVal($item, $key);
	...
	public static function convertStdClassToArray($d){
		...
		if (is_array($d)){
			...
			return array_map(array("UELM_UniteFunctionsUC","convertStdClassToArray"), $d);
		} else {
			return $d;
		}
	...
	public static function getDiffArrItems($arr, $arrDefault, $arrMustKeys = array()){
		...
		if(!empty($arrMustKeys))
			$arrMustKeys = UELM_UniteFunctionsUC::arrayToAssoc($arrMustKeys);
	...
	public static function assocToArrayKeyValue($assoc, $keyName, $valueName, $firstItem = null){
		...
		foreach($assoc as $item){
			if(!array_key_exists($keyName, $item))
				UELM_UniteFunctionsUC::throwError("field: $keyName not found in array");

			if(!array_key_exists($valueName, $item))
				UELM_UniteFunctionsUC::throwError("field: $valueName not found in array");
	...
	public static function trimArrayItems($arr){
		if(gettype($arr) != "array")
			UELM_UniteFunctionsUC::throwError("trimArrayItems error: The type must be array");
	...
	public static function insertToAssocArray($arrItems, $arrNewItems){
		...
		foreach($arrNewItems as $item){
			$insertAfter = UELM_UniteFunctionsUC::getVal($item, "insert_after");
	...
	public static function modifyDataArrayForShow($arrData, $convertSingleArray = false) {
	    ...
	    foreach ($arrData as $key => $value) {
	        ...
	        if (is_string($value) && strlen($value) > $truncateSize) {
	            $value = UELM_UniteFunctionsUC::truncateString($value, $truncateSize);
	        }
	...
	public static function truncateString($value, $length = 100, $preserve = true, $separator = '...', $charset="utf-8"){
		...
		$arrErrors = UELM_UniteFunctionsUC::validateHTML($value);
	...
	public static function arrayToXML($array, $rootName, $xml = null){
		...
		if($xml === null){
			...
			if($strXML === false)
				UELM_UniteFunctionsUC::throwError("Wrong xml output");
	...
	public static function maybeUnserialize($str){
		...
		if(!is_array($arrOutput))
			UELM_UniteFunctionsUC::throwError("The unserialized string should be alwayas array type");
	...
	public static function getArrSanitizeTypes(){
		...
		$arrSanitize = array();
		$arrSanitize[self::SANITIZE_ID] = __("Sanitize ID", "unlimited-elements");
		$arrSanitize[self::SANITIZE_KEY] = __("Sanitize KEY", "unlimited-elements");
		$arrSanitize[self::SANITIZE_TEXT_FIELD] = __("Sanitize Text Field", "unlimited-elements");
		$arrSanitize[self::SANITIZE_NOTHING] = __("No Sanitize (not recomended)", "unlimited-elements");
	...
	public static function utf8EncodeRecursive($data){
		...
		if (is_string($data)) {
	        ...
	        return array_map(array("UELM_UniteFunctionsUC", 'utf8EncodeRecursive'), $data);
	...
	public static function getBaseUrl($url, $stripPagination = false){
		...
		$scheme = UELM_UniteFunctionsUC::getVal($arrUrl, "scheme","http");
		$host = UELM_UniteFunctionsUC::getVal($arrUrl, "host");
		$path = UELM_UniteFunctionsUC::getVal($arrUrl, "path");
	...
	public static function urlDecode($url) {
	    ...
	    $decoded = preg_replace_callback('/&#x([a-fA-F0-9]+);/i', array("UELM_UniteFunctionsUC","hexEntityDecode"), $decoded);
	...
	public static function validateObjectMethod($object, $strMethod, $objectName){
		...
		if(method_exists($object, "initByID") == false)
			UELM_UniteFunctionsUC::throwError("Object: $objectName don't have method $strMethod");
	...
	public static function validateEmail($email, $fieldName="email"){
		...
		self::throwError(sprintf(__("The %s is not valid", "unlimited-elements"), $fieldName));
	...
	public static function sanitize($str, $type){
		...
		switch($type){
			...
			case self::SANITIZE_NOTHING:
				$str = UELM_UniteProviderFunctionsUC::sanitizeVar($str, $type);
			break;
	...
	*/
	...
	public static function mkdirValidate($path, $dirName = ""){
		...
		if(self::isDir($path) == false){
			self::mkdir($path);
			if(!self::isDir($path))
				UELM_UniteFunctionsUC::throwError("$dirName path: {$path} could not be created. Please check your permissions");
		}
	...
	public static function writeFile($str, $filepath){
		...
		if(is_array($str))
			UELM_UniteFunctionsUC::throwError("write file should accept only string in file: ". $filepath);


		$res = self::filePutContents($filepath, $str);
		
		if($res === false)
			UELM_UniteFunctionsUC::throwError("File $filepath could not been created. Check folder permissions");
	}
	...
	public static function getSessionVar($name, $base){
		...
		if(empty($base))
			UELM_UniteFunctionsUC::throwError("Can't get session var without the base");

		if(!isset($_SESSION))
			return("");

		$arrBase = UELM_UniteFunctionsUC::getVal($_SESSION, $base);

		if(empty($arrBase))
			return("");

		$value = UELM_UniteFunctionsUC::getVal($_SESSION, $name);
	...
	public static function setSessionVar($name, $value, $base){
		...
		if(empty($base))
			UELM_UniteFunctionsUC::throwError("Can't set session var without the base");
	...
	public static function getTimeAgoString($time_stamp, $textFormat="long"){
		...
		$textHours = __('hours',"unlimited-elements"); 
		$textHour = __('hour',"unlimited-elements");
		
		$textMunites = __('minutes',"unlimited-elements");
		$textMunite = __('minute',"unlimited-elements");
		
		$textYears = __('years',"unlimited-elements");
		$textYear = __('year',"unlimited-elements");
		
		$textWeeks = __('weeks',"unlimited-elements");
		$textWeek = __('week',"unlimited-elements");
		
		
		if($textFormat == "short"){
			
			$textHours = __('h',"unlimited-elements"); 
			$textHour = __('h',"unlimited-elements");
			$textMunites = __('min',"unlimited-elements");
			$textMunite = __('min',"unlimited-elements");
		}
		...
		if ($time_difference >= 60 * 60 * 24 * 30.4368499)
			return self::getTimeAgoStringUnit($time_stamp, 60 * 60 * 24 * 30.4368499,__('months',"unlimited-elements"),__('month',"unlimited-elements"));
		...
		if ($time_difference >= 60 * 60 * 24)
			return self::getTimeAgoStringUnit($time_stamp, 60 * 60 * 24, __('days',"unlimited-elements"),__('day',"unlimited-elements"));
	...
	private static function getTimeAgoStringUnit($time_stamp, $divisor, $strUnit, $strUnitSingle){
		...
		if($time_units == 1)
			$output = $time_units . " ".$strUnitSingle." ". __("ago","unlimited-elements");
		else
			$output = $time_units . " ".$strUnit." ". __("ago","unlimited-elements");
	...
	public static function loadXMLFile($filepath){
		...
		if(file_exists($filepath) == false)
			UELM_UniteFunctionsUC::throwError("xml file not found: ".$filepath);
		...
		if(empty($obj)){
			...
			if(empty($xmlString))
				UELM_UniteFunctionsUC::throwError("xml load: No content found in: ".$filepath);
		...
		if(empty($obj)){
			...
			UELM_UniteFunctionsUC::throwError($errorsHTML);
		}
	...
	public static function isEnoughtPHPMemory($mbReserve = 32){
		...
		/*
		uelm_dmp(number_format($available));
		uelm_dmp(number_format($used));
		uelm_dmp("-----------");
		*/
	...
	public function downloadImage($filepath, $filename, $mimeType = ""){
		...
		if($mimeType == ""){
			$info = UELM_UniteFunctionsUC::getPathInfo($filepath);
			$ext = $info["extension"];
			$mimeType = "image/$ext";
		}
	...
	public static function downloadFile($filepath, $filename = null){
		...
		UELM_UniteFunctionsUC::validateFilepath($filepath,"export file");
